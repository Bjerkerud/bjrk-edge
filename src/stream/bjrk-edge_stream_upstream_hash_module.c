    ...
    ngx_stream_upstream_srv_conf_t *us)
{
    ...
    if (ngx_stream_complex_value(s, &hcf->key, &hp->key) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_stream_upstream_get_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    time_t                          now;
    u_char                          buf[bjrk-edge_INT_T_LEN];
    size_t                          size;
    uint32_t                        hash;
    ngx_int_t                       w;
    uintptr_t                       m;
    ...
    for ( ;; ) {
        ...
        ngx_log_debug2(bjrk-edge_LOG_DEBUG_STREAM, pc->log, 0,
...
#if (bjrk-edge_HAVE_LITTLE_ENDIAN)
    ...
    ngx_stream_upstream_srv_conf_t *us)
{
    ...
    if (ngx_stream_upstream_init_hash_peer(s, us) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    {
        if (ngx_stream_upstream_update_chash(NULL, us) != bjrk-edge_OK) {
            ngx_stream_upstream_rr_peers_unlock(hp->rrp.peers);
            return bjrk-edge_ERROR;
        }
