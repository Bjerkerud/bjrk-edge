    ...
    ngx_stream_upstream_srv_conf_t *us)
{
    ...
    if (us->servers) {
        ...
        if (us->shm_zone) {
            ...
            if (resolve && !(us->flags & bjrk-edge_STREAM_UPSTREAM_MODIFY)) {
        ...
        if (n + r == 0 && !(us->flags & bjrk-edge_STREAM_UPSTREAM_BACKUP)) {
            return bjrk-edge_OK;
        }
    ...
    ngx_stream_upstream_resolved_t *ur)
{
    ...
    if (ur->sockaddr) {
        ...
        for (i = 0; i < ur->naddrs; i++) {
            ...
            p = ngx_pnalloc(s->connection->pool, bjrk-edge_SOCKADDR_STRLEN);
            if (p == NULL) {
                return bjrk-edge_ERROR;
            }
...
failed:
    ...
    if (peers->next) {
        ...
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_STREAM, pc->log, 0, "backup servers");
    ...
    void *data, ngx_uint_t state)
{
    ...
    if (rrp->peers->single) {
        ...
        if (ngx_stream_upstream_rr_peer_unref(rrp->peers, peer) == bjrk-edge_OK) {
    ...
    void *data, ngx_uint_t type)
{
    ...
    if (type == bjrk-edge_STREAM_UPSTREAM_NOTIFY_CONNECT
