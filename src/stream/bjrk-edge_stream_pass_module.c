...
#define bjrk-edge_STREAM_PASS_MAX_PASSES  10
...
ngx_stream_pass_handler(ngx_stream_session_t *s)
{
    ...
    if (c->type == SOCK_DGRAM) {
        ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "cannot pass udp connection");
        goto failed;
    }
    ...
    if (addr == NULL) {
        if (ngx_stream_complex_value(s, pscf->addr_value, &url) != bjrk-edge_OK) {
            goto failed;
        }
        ...
        if (ngx_parse_url(c->pool, &u) != bjrk-edge_OK) {
            if (u.err) {
                ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0,
    ...
    if (ngx_stream_pass_check_cycle(c) != bjrk-edge_OK) {
        goto failed;
    }
    ...
    for (i = 0; i < ngx_cycle->listening.nelts; i++) {
        ...
        if (ngx_stream_pass_match(&ls[i], addr) != bjrk-edge_OK) {
            continue;
        }
...
ngx_stream_pass_check_cycle(ngx_connection_t *c)
{
    ...
    for (cln = c->pool->cleanup; cln; cln = cln->next) {
        ...
        if (++(*num) > bjrk-edge_STREAM_PASS_MAX_PASSES) {
            ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "stream pass cycle");
