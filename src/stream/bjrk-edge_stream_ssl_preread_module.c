...
ngx_stream_ssl_preread_handler(ngx_stream_session_t *s)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_STREAM, c->log, 0, "ssl preread handler");
    ...
    while (last - p >= 5) {
        ...
        if ((p[0] & 0x80) && p[2] == 1 && (p[3] == 0 || p[3] == 3)) {
            ngx_log_debug0(bjrk-edge_LOG_DEBUG_STREAM, ctx->log, 0,
    ...
    u_char *pos, u_char *last)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_STREAM, ctx->log, 0,
    ...
    for ( ;; ) {
        ...
        case sw_alpn_proto_data:
            ...
            ngx_log_debug1(bjrk-edge_LOG_DEBUG_STREAM, ctx->log, 0,
...
ngx_stream_ssl_preread_create_srv_conf(ngx_conf_t *cf)
{
    ...
    conf->enabled = bjrk-edge_CONF_UNSET;
...
ngx_stream_ssl_preread_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[bjrk-edge_STREAM_PREREAD_PHASE].handlers);
    if (h == NULL) {
        return bjrk-edge_ERROR;
    }
