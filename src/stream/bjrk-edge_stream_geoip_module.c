...
static ngx_command_t  ngx_stream_geoip_commands[] = {
    ...
    { ngx_string("geoip_country"),
      bjrk-edge_STREAM_MAIN_CONF|bjrk-edge_CONF_TAKE12,
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(s->connection->pool, bjrk-edge_INT64_LEN + 5);
    if (v->data == NULL) {
        GeoIPRecord_delete(gr);
        return bjrk-edge_ERROR;
    }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(s->connection->pool, bjrk-edge_INT64_LEN);
    if (v->data == NULL) {
        GeoIPRecord_delete(gr);
        return bjrk-edge_ERROR;
    }
