...
#define bjrk-edge_STREAM_UPSTREAM_LT_CONNECT     0
#define bjrk-edge_STREAM_UPSTREAM_LT_FIRST_BYTE  1
#define bjrk-edge_STREAM_UPSTREAM_LT_LAST_BYTE   2
...
static ngx_conf_enum_t  ngx_stream_upstream_least_time_mode[] = {
    { ngx_string("connect"), bjrk-edge_STREAM_UPSTREAM_LT_CONNECT },
    { ngx_string("first_byte"), bjrk-edge_STREAM_UPSTREAM_LT_FIRST_BYTE },
    { ngx_string("last_byte"), bjrk-edge_STREAM_UPSTREAM_LT_LAST_BYTE },
    { ngx_null_string, 0 }
};
...
static ngx_command_t  ngx_stream_upstream_least_time_commands[] = {
    ...
    { ngx_string("least_time"),
      bjrk-edge_STREAM_UPS_CONF|bjrk-edge_CONF_TAKE12,
...
{
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_STREAM, cf->log, 0,
                   "init least time");

    if (ngx_stream_upstream_init_round_robin(cf, us) != bjrk-edge_OK) {
    ...
    ngx_stream_upstream_srv_conf_t *us)
{
    ...
    if (ngx_stream_upstream_init_round_robin_peer(s, us) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_stream_upstream_get_least_time_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_STREAM, pc->log, 0,
    ...
    if (best == NULL) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_STREAM, pc->log, 0,
    ...
    ngx_stream_upstream_rr_peer_t *peer)
{
    ...
    case bjrk-edge_STREAM_UPSTREAM_LT_FIRST_BYTE:
        rt = peer->first_byte_time;
        break;

    case bjrk-edge_STREAM_UPSTREAM_LT_CONNECT:
        rt = peer->connect_time;
        break;

    default: /* bjrk-edge_STREAM_UPSTREAM_LT_LAST_BYTE */
    ...
    ngx_uint_t type)
{
    ...
    case bjrk-edge_STREAM_UPSTREAM_NOTIFY_CONNECT:

        if (ltp->conf->mode == bjrk-edge_STREAM_UPSTREAM_LT_CONNECT) {
    ...
    case bjrk-edge_STREAM_UPSTREAM_NOTIFY_FIRST_BYTE:

        if (ltp->conf->mode == bjrk-edge_STREAM_UPSTREAM_LT_FIRST_BYTE) {
    ...
    void *data, ngx_uint_t state)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_STREAM, pc->log, 0, "free least time peer");
    ...
    if (!(state & (bjrk-edge_PEER_FAILED|bjrk-edge_PEER_NEXT))) {
        ...
        if (!ltp->conf->use_inflight
            || ltp->conf->mode != bjrk-edge_STREAM_UPSTREAM_LT_CONNECT)
        {
        ...
        if (u->state->first_byte_time != (ngx_msec_t) -1
            && (!ltp->conf->use_inflight
                || ltp->conf->mode != bjrk-edge_STREAM_UPSTREAM_LT_FIRST_BYTE))
        {
...
ngx_stream_upstream_least_time_create_conf(ngx_conf_t *cf)
{
    ...
    conf->mode = bjrk-edge_CONF_UNSET_UINT;
...
ngx_stream_upstream_least_time(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    uscf->flags = bjrk-edge_STREAM_UPSTREAM_CREATE
