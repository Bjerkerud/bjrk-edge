    ...
    ngx_stream_complex_value_t *val, size_t default_value)
{
    ...
    if (ngx_stream_complex_value(s, val, &value) != bjrk-edge_OK) {
        return default_value;
    }
...
ngx_stream_script_compile(ngx_stream_script_compile_t *sc)
{
    ...
    if (ngx_stream_script_init_arrays(sc) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    for (i = 0; i < sc->source->len; /* void */ ) {
        ...
        if (sc->source->data[i] == '$') {
            ...
            if (sc->source->data[i] >= '1' && sc->source->data[i] <= '9') {
                ...
                if (ngx_stream_script_add_capture_code(sc, n) != bjrk-edge_OK) {
                    return bjrk-edge_ERROR;
                }
            ...
            if (ngx_stream_script_add_var_code(sc, &name) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
...
ngx_stream_script_done(ngx_stream_script_compile_t *sc)
{
    ...
    if (sc->zero) {
        ...
        if (ngx_stream_script_add_copy_code(sc, &zero, 0) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    if (sc->conf_prefix || sc->root_prefix) {
        if (ngx_stream_script_add_full_name_code(sc) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_stream_script_check_length(ngx_stream_script_engine_t *e, size_t len)
{
    ...
    if (e->end - e->pos < (ssize_t) len) {
        ngx_log_error(bjrk-edge_LOG_ALERT, e->session->connection->log, 0,
                      "no buffer space in script copy");
        e->ip = ngx_stream_script_exit;
        e->status = bjrk-edge_STREAM_INTERNAL_SERVER_ERROR;
        return bjrk-edge_ERROR;
    }
...
ngx_stream_script_copy_code(ngx_stream_script_engine_t *e)
{
    ...
    if (!e->skip) {
        ...
        if (ngx_stream_script_check_length(e, code->len) != bjrk-edge_OK) {
            return;
        }
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_STREAM, e->session->connection->log, 0,
...
ngx_stream_script_copy_var_code(ngx_stream_script_engine_t *e)
{
    ...
    if (!e->skip) {
        ...
        if (value && !value->not_found) {
            ...
            if (ngx_stream_script_check_length(e, value->len) != bjrk-edge_OK) {
                return;
            }
            ...
            ngx_log_debug2(bjrk-edge_LOG_DEBUG_STREAM,
...
ngx_stream_script_copy_capture_code(ngx_stream_script_engine_t *e)
{
    ...
    if (n < s->ncaptures) {
        ...
        if (ngx_stream_script_check_length(e, len) != bjrk-edge_OK) {
            return;
        }
...
ngx_stream_script_full_name_code(ngx_stream_script_engine_t *e)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_STREAM, e->session->connection->log, 0,
