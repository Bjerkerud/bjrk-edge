...
#define bjrk-edge_STREAM_LIMIT_CONN_PASSED            1
#define bjrk-edge_STREAM_LIMIT_CONN_REJECTED          2
#define bjrk-edge_STREAM_LIMIT_CONN_REJECTED_DRY_RUN  3
...
static ngx_command_t  ngx_stream_limit_conn_commands[] = {
    ...
    { ngx_string("limit_conn_zone"),
      bjrk-edge_STREAM_MAIN_CONF|bjrk-edge_CONF_TAKE2,
...
static ngx_stream_variable_t  ngx_stream_limit_conn_vars[] = {
    ...
    { ngx_string("limit_conn_status"), NULL,
      ngx_stream_limit_conn_status_variable, 0, bjrk-edge_STREAM_VAR_NOCACHEABLE, 0 },
...
ngx_stream_limit_conn_handler(ngx_stream_session_t *s)
{
    ...
    for (i = 0; i < lccf->limits.nelts; i++) {
        ...
        if (ngx_stream_complex_value(s, &ctx->key, &key) != bjrk-edge_OK) {
        ...
        s->limit_conn_status = bjrk-edge_STREAM_LIMIT_CONN_PASSED;
        ...
        if (node == NULL) {
            ...
            if (node == NULL) {
                ...
                if (lccf->dry_run) {
                    s->limit_conn_status =
                                        bjrk-edge_STREAM_LIMIT_CONN_REJECTED_DRY_RUN;
                    return bjrk-edge_DECLINED;
                }

                s->limit_conn_status = bjrk-edge_STREAM_LIMIT_CONN_REJECTED;

                return bjrk-edge_STREAM_SERVICE_UNAVAILABLE;
            }
...
ngx_stream_limit_conn_cleanup(void *data)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_STREAM, lccln->shm_zone->shm.log, 0,
...
ngx_stream_limit_conn_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[bjrk-edge_STREAM_PREACCESS_PHASE].handlers);
    if (h == NULL) {
        return bjrk-edge_ERROR;
    }
