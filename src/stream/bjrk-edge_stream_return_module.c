...
ngx_stream_return_handler(ngx_stream_session_t *s)
{
    ...
    if (ngx_stream_complex_value(s, &rscf->text, &text) != bjrk-edge_OK) {
...
ngx_stream_return_write_handler(ngx_event_t *ev)
{
    ...
    if (ngx_stream_top_filter(s, ctx->out, 1) == bjrk-edge_ERROR) {
        ngx_stream_finalize_session(s, bjrk-edge_STREAM_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    if (ngx_handle_write_event(ev, 0) != bjrk-edge_OK) {
        ngx_stream_finalize_session(s, bjrk-edge_STREAM_INTERNAL_SERVER_ERROR);
        return;
    }
