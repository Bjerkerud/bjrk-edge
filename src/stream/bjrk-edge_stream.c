...
ngx_stream_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_stream_conf_ctx_t **) conf = ctx;
    ...
    ngx_stream_max_module = ngx_count_modules(cf->cycle, bjrk-edge_STREAM_MODULE);
    ...
    cf->module_type = bjrk-edge_STREAM_MODULE;
    cf->cmd_type = bjrk-edge_STREAM_MAIN_CONF;
    ...
    if (ngx_stream_init_phases(cf, cmcf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    if (ngx_stream_variables_init_vars(cf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }

    *cf = pcf;

    if (ngx_stream_init_phase_handlers(cf, cmcf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }

    /* optimize the lists of ports, addresses and server names */

    if (ngx_stream_optimize_servers(cf, cmcf, cmcf->ports) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
...
{
    if (ngx_array_init(&cmcf->phases[bjrk-edge_STREAM_POST_ACCEPT_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_STREAM_PREACCESS_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_stream_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_STREAM_ACCESS_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_stream_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_STREAM_SSL_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_stream_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_STREAM_PREREAD_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_stream_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_STREAM_LOG_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_stream_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    ngx_stream_core_main_conf_t *cmcf)
{
    ...
    for (i = 0; i < bjrk-edge_STREAM_LOG_PHASE; i++) {
    ...
    for (i = 0; i < bjrk-edge_STREAM_LOG_PHASE; i++) {
        ...
        case bjrk-edge_STREAM_PREREAD_PHASE:
            checker = ngx_stream_core_preread_phase;
            break;

        case bjrk-edge_STREAM_CONTENT_PHASE:
    ...
    ngx_stream_conf_port_t *port, ngx_stream_listen_opt_t *lsopt)
{
    ...
    for (i = 0; i < port->addrs.nelts; i++) {
        ...
        if (ngx_stream_add_server(cf, cscf, &addr[i]) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    ...
    ngx_array_t *ports)
{
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        for (a = 0; a < port[p].addrs.nelts; a++) {
            ...
            {
                if (ngx_stream_server_names(cf, cmcf, &addr[a]) != bjrk-edge_OK) {
                    return bjrk-edge_ERROR;
                }
            }
        }

        if (ngx_stream_init_listening(cf, &port[p]) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
#if (bjrk-edge_HAVE_INET6)
        case AF_INET6:
            if (ngx_stream_add_addrs6(cf, stport, addr) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
            break;
#endif
        default: /* AF_INET */
            if (ngx_stream_add_addrs(cf, stport, addr) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
            break;
        }
