...
static ngx_command_t  ngx_stream_map_commands[] = {
    ...
    { ngx_string("map"),
      bjrk-edge_STREAM_MAIN_CONF|bjrk-edge_CONF_BLOCK|bjrk-edge_CONF_TAKE2,
    ...
    uintptr_t data)
{
    ...
    if (ngx_stream_complex_value(s, &map->value, &val) != bjrk-edge_OK) {
    ...
    if (!value->valid) {
        ...
        if (ngx_stream_complex_value(s, cv, &str) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
