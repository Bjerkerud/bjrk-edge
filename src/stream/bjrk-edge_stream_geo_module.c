...
static ngx_command_t  ngx_stream_geo_commands[] = {
    ...
    { ngx_string("geo"),
      bjrk-edge_STREAM_MAIN_CONF|bjrk-edge_CONF_BLOCK|bjrk-edge_CONF_TAKE12,
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_stream_geo_addr(s, ctx, &addr) != bjrk-edge_OK) {
    ...
    ngx_addr_t *addr)
{
    ...
    if (ngx_parse_addr(s->connection->pool, addr, v->data, v->len) == bjrk-edge_OK) {
        return bjrk-edge_OK;
    }
...
ngx_stream_geo_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    var = ngx_stream_add_variable(cf, &name, bjrk-edge_STREAM_VAR_CHANGEABLE);
    if (var == NULL) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    *cf = save;
    ...
    if (ctx.no_cacheable) {
        var->flags |= bjrk-edge_STREAM_VAR_NOCACHEABLE;
    }
    ...
    ngx_str_t *value)
{
    ...
    if (ngx_stream_geo_cidr_value(cf, net, &cidr) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
