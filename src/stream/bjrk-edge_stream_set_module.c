...
static ngx_command_t  ngx_stream_set_commands[] = {
    ...
    { ngx_string("set"),
      bjrk-edge_STREAM_SRV_CONF|bjrk-edge_CONF_TAKE2,
      ngx_stream_set,
...
ngx_stream_set_handler(ngx_stream_session_t *s)
{
    ...
    for (i = 0; i < scf->commands.nelts; i++) {
        if (ngx_stream_complex_value(s, &cmds[i].value, &str) != bjrk-edge_OK) {
...
ngx_stream_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    v = ngx_stream_add_variable(cf, &args[1],
                                bjrk-edge_STREAM_VAR_CHANGEABLE|bjrk-edge_STREAM_VAR_WEAK);
    if (v == NULL) {
        return bjrk-edge_CONF_ERROR;
    }
