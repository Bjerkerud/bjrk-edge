...
static ngx_command_t  ngx_stream_core_commands[] = {
    ...
    { ngx_string("variables_hash_max_size"),
      bjrk-edge_STREAM_MAIN_CONF|bjrk-edge_CONF_TAKE1,
    ...
    { ngx_string("server"),
      bjrk-edge_STREAM_MAIN_CONF|bjrk-edge_CONF_BLOCK|bjrk-edge_CONF_NOARGS,
    ...
    { ngx_string("listen"),
      bjrk-edge_STREAM_SRV_CONF|bjrk-edge_CONF_1MORE,
    ...
    ngx_stream_phase_handler_t *ph)
{
    ...
    if (rc == bjrk-edge_ERROR) {
        rc = bjrk-edge_STREAM_INTERNAL_SERVER_ERROR;
    }
...
ngx_stream_preread_can_peek(ngx_connection_t *c)
{
    ...
    if ((ngx_event_flags & bjrk-edge_USE_CLEAR_EVENT) == 0) {
        return 0;
    }
...
ngx_stream_preread_peek(ngx_stream_session_t *s, ngx_stream_phase_handler_t *ph)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_STREAM, c->log, 0, "stream recv(): %z", n);
    ...
    if (c->buffer->last == c->buffer->end) {
        ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "preread buffer full");
        return bjrk-edge_STREAM_BAD_REQUEST;
    }
    ...
    ngx_stream_phase_handler_t *ph)
{
    ...
    if (c->type == SOCK_STREAM
        && cscf->tcp_nodelay
        && ngx_tcp_nodelay(c) != bjrk-edge_OK)
    {
        ngx_stream_finalize_session(s, bjrk-edge_STREAM_INTERNAL_SERVER_ERROR);
        return bjrk-edge_OK;
    }
...
ngx_stream_core_create_srv_conf(ngx_conf_t *cf)
{
    ...
    cscf->resolver_timeout = bjrk-edge_CONF_UNSET_MSEC;
    cscf->proxy_protocol_timeout = bjrk-edge_CONF_UNSET_MSEC;
    cscf->tcp_nodelay = bjrk-edge_CONF_UNSET;
    cscf->preread_buffer_size = bjrk-edge_CONF_UNSET_SIZE;
    cscf->preread_timeout = bjrk-edge_CONF_UNSET_MSEC;
...
ngx_stream_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        if (cf->cycle->modules[m]->type != bjrk-edge_STREAM_MODULE) {
            continue;
        }
    ...
    *cscfp = cscf;
    ...
    pcf = *cf;
    cf->ctx = ctx;
    cf->cmd_type = bjrk-edge_STREAM_SRV_CONF;
...
ngx_stream_core_listen(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (n = 0; n < u.naddrs; n++) {
        ...
        if (ngx_stream_add_listen(cf, cscf, &lsopt) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }
