...
static ngx_stream_variable_t  ngx_stream_core_variables[] = {
    ...
    { ngx_string("proxy_protocol_tlv_"), NULL,
      ...
      0, bjrk-edge_STREAM_VAR_PREFIX, 0 },
    ...
    { ngx_string("session_time"), NULL, ngx_stream_variable_session_time,
      0, bjrk-edge_STREAM_VAR_NOCACHEABLE, 0 },
...
ngx_stream_add_variable(ngx_conf_t *cf, ngx_str_t *name, ngx_uint_t flags)
{
    ...
    if (flags & bjrk-edge_STREAM_VAR_PREFIX) {
    ...
    for (i = 0; i < cmcf->variables_keys->keys.nelts; i++) {
        ...
        if (!(v->flags & bjrk-edge_STREAM_VAR_CHANGEABLE)) {
            ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
                               "the duplicate \"%V\" variable", name);
            return NULL;
        }

        if (!(flags & bjrk-edge_STREAM_VAR_WEAK)) {
            v->flags &= ~bjrk-edge_STREAM_VAR_WEAK;
        }
...
ngx_stream_get_indexed_variable(ngx_stream_session_t *s, ngx_uint_t index)
{
        ...
        == bjrk-edge_OK)
    {
        ...
        if (v[index].flags & bjrk-edge_STREAM_VAR_NOCACHEABLE) {
    ...
    ngx_uint_t key)
{
    ...
    if (v) {
        if (v->flags & bjrk-edge_STREAM_VAR_INDEXED) {
        ...
        if (vv && v->get_handler(s, vv, v->data) == bjrk-edge_OK) {
            ngx_stream_variable_depth++;
            return vv;
        }
    ...
    if (n != cmcf->prefix_variables.nelts) {
        if (v[n].get_handler(s, vv, (uintptr_t) name) == bjrk-edge_OK) {
            return vv;
        }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_connection_local_sockaddr(s->connection, &str, 0) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_connection_local_sockaddr(s->connection, NULL, 0) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(s->connection->pool, bjrk-edge_OFF_T_LEN);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(s->connection->pool, bjrk-edge_TIME_T_LEN + 4);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
...
ngx_stream_variable_status(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    v->data = ngx_pnalloc(s->connection->pool, bjrk-edge_INT_T_LEN);
    if (v->data == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(s->connection->pool, bjrk-edge_ATOMIC_T_LEN);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(s->connection->pool, bjrk-edge_INT64_LEN);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
...
ngx_stream_regex_compile(ngx_conf_t *cf, ngx_regex_compile_t *rc)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        v = ngx_stream_add_variable(cf, &name, bjrk-edge_STREAM_VAR_CHANGEABLE);
        if (v == NULL) {
            return NULL;
        }
    ...
    ngx_str_t *str)
{
    ...
    for (i = 0; i < re->nvariables; i++) {
        ...
        ngx_log_debug2(bjrk-edge_LOG_DEBUG_STREAM, s->connection->log, 0,
...
ngx_stream_variables_init_vars(ngx_conf_t *cf)
{
    ...
    for (i = 0; i < cmcf->variables.nelts; i++) {
        ...
        for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
                   ...
                   == 0)
            {
                ...
                av->flags |= bjrk-edge_STREAM_VAR_INDEXED;
                v[i].flags = av->flags;

                av->index = i;

                if (av->get_handler == NULL
                    || (av->flags & bjrk-edge_STREAM_VAR_WEAK))
                {
                    break;
                }
    ...
    for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
        ...
        if (av->flags & bjrk-edge_STREAM_VAR_NOHASH) {
            key[n].key.data = NULL;
        }
    }
