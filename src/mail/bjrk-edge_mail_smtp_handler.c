...
ngx_mail_smtp_init_session(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_resolve_addr(ctx) != bjrk-edge_OK) {
        ngx_mail_close_connection(c);
    }
}
...
ngx_mail_smtp_resolve_addr_handler(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->state) {
        ...
        if (ctx->state == bjrk-edge_RESOLVE_NXDOMAIN) {
            s->host = smtp_unavailable;
    ...
    if (ngx_mail_smtp_validate_host(&ctx->name) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0,
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0,
...
ngx_mail_smtp_block_reading(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "smtp reading blocked");
...
ngx_mail_smtp_invalid_pipelining(ngx_event_t *rev)
{
    ...
    if (rev->timedout) {
        ...
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "delay greeting");
        ...
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "invalid pipelining");

        if (s->buffer == NULL) {
            if (ngx_mail_smtp_create_buffer(s, c) != bjrk-edge_OK) {
                return;
            }
        }
...
ngx_mail_smtp_create_buffer(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_array_init(&s->args, c->pool, 2, sizeof(ngx_str_t)) == bjrk-edge_ERROR) {
...
ngx_mail_smtp_auth_state(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "smtp auth state");
    ...
    if (s->out.len) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "smtp send handler busy");
        s->blocked = 1;
    ...
    if (rc == bjrk-edge_OK) {
        ...
        case ngx_smtp_start:
            ...
            case bjrk-edge_SMTP_HELO:
            case bjrk-edge_SMTP_EHLO:
                rc = ngx_mail_smtp_helo(s, c);
                break;

            case bjrk-edge_SMTP_AUTH:
                rc = ngx_mail_smtp_auth(s, c);
                break;

            case bjrk-edge_SMTP_QUIT:
                s->quit = 1;
                ngx_str_set(&s->out, smtp_bye);
                break;

            case bjrk-edge_SMTP_MAIL:
                rc = ngx_mail_smtp_mail(s, c);
                break;

            case bjrk-edge_SMTP_RCPT:
                rc = ngx_mail_smtp_rcpt(s, c);
                break;

            case bjrk-edge_SMTP_RSET:
                rc = ngx_mail_smtp_rset(s, c);
                break;

            case bjrk-edge_SMTP_NOOP:
                break;

            case bjrk-edge_SMTP_STARTTLS:
                rc = ngx_mail_smtp_starttls(s, c);
                ngx_str_set(&s->out, smtp_starttls);
                break;

            default:
                rc = bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
                break;
            }
    ...
    case bjrk-edge_MAIL_PARSE_INVALID_COMMAND:
        s->mail_state = ngx_smtp_start;
        s->state = 0;
...
ngx_mail_smtp_helo(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (s->command == bjrk-edge_SMTP_HELO) {
        ...
        if (c->ssl == NULL) {
            ...
            if (sslcf->starttls == bjrk-edge_MAIL_STARTTLS_ON) {
                s->out = sscf->starttls_capability;
                return bjrk-edge_OK;
            }

            if (sslcf->starttls == bjrk-edge_MAIL_STARTTLS_ONLY) {
...
ngx_mail_smtp_auth(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_mail_starttls_only(s, c)) {
        return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
    }
#endif
    ...
    case bjrk-edge_MAIL_AUTH_LOGIN:
    ...
    case bjrk-edge_MAIL_AUTH_LOGIN_USERNAME:
    ...
    case bjrk-edge_MAIL_AUTH_PLAIN:
    ...
    case bjrk-edge_MAIL_AUTH_CRAM_MD5:

        if (!(sscf->auth_methods & bjrk-edge_MAIL_AUTH_CRAM_MD5_ENABLED)) {
            return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
        }
        ...
        if (s->salt.data == NULL) {
            ...
            if (ngx_mail_salt(s, c, cscf) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        }

        if (ngx_mail_auth_cram_md5_salt(s, c, "334 ", 4) == bjrk-edge_OK) {
    ...
    case bjrk-edge_MAIL_AUTH_EXTERNAL:

        if (!(sscf->auth_methods & bjrk-edge_MAIL_AUTH_EXTERNAL_ENABLED)) {
            return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
        }
...
ngx_mail_smtp_mail(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (!(sscf->auth_methods & bjrk-edge_MAIL_AUTH_NONE_ENABLED)) {
...
ngx_mail_smtp_rcpt(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    s->auth_method = bjrk-edge_MAIL_AUTH_NONE;
    ...
    char *err)
{
    ...
    ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, err, &cmd);
}
