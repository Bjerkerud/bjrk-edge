...
ngx_mail_init_connection(ngx_connection_t *c)
{
    ...
    s->signature = bjrk-edge_MAIL_MODULE;
    ...
    len = ngx_sock_ntop(c->sockaddr, c->socklen, text, bjrk-edge_SOCKADDR_STRLEN, 1);

    ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "*%uA client %*s connected to %V",
...
{
    u_char                    *p, buf[bjrk-edge_PROXY_PROTOCOL_MAX_HEADER];
    size_t                     size;
    ssize_t                    n;
    ngx_err_t                  err;
    ngx_connection_t          *c;
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0,
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "recv(): %z", n);
    ...
    if (ngx_mail_realip_handler(s) != bjrk-edge_OK) {
        ngx_mail_close_connection(c);
        return;
    }
...
ngx_mail_ssl_handshake_handler(ngx_connection_t *c)
{
    ...
    if (c->ssl->handshaked) {
        ...
        if (ngx_mail_verify_cert(s, c) != bjrk-edge_OK) {
            return;
        }
...
ngx_mail_auth_plain(ngx_mail_session_t *s, ngx_connection_t *c, ngx_uint_t n)
{
    ...
    if (ngx_decode_base64(&plain, &arg[n]) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
    ...
    ngx_uint_t n)
{
    ...
    if (ngx_decode_base64(&login, &arg[n]) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
...
ngx_mail_auth_login_password(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_decode_base64(&passwd, &arg[0]) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
            "client sent invalid base64 encoding in AUTH LOGIN command");
        return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
    }
...
ngx_mail_auth_cram_md5(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_decode_base64(&login, &arg[0]) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
    ...
    ngx_uint_t n)
{
    ...
    if (ngx_decode_base64(&external, &arg[n]) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
    ...
    s->auth_method = bjrk-edge_MAIL_AUTH_EXTERNAL;
...
ngx_mail_read_command(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (rc == bjrk-edge_MAIL_PARSE_INVALID_COMMAND) {
