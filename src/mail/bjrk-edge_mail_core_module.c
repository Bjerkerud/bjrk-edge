...
static ngx_command_t  ngx_mail_core_commands[] = {
    ...
    { ngx_string("server"),
      bjrk-edge_MAIL_MAIN_CONF|bjrk-edge_CONF_BLOCK|bjrk-edge_CONF_NOARGS,
    ...
    { ngx_string("listen"),
      bjrk-edge_MAIL_SRV_CONF|bjrk-edge_CONF_1MORE,
    ...
    { ngx_string("protocol"),
      bjrk-edge_MAIL_SRV_CONF|bjrk-edge_CONF_TAKE1,
...
ngx_mail_core_create_srv_conf(ngx_conf_t *cf)
{
    ...
    cscf->timeout = bjrk-edge_CONF_UNSET_MSEC;
    cscf->resolver_timeout = bjrk-edge_CONF_UNSET_MSEC;

    cscf->max_errors = bjrk-edge_CONF_UNSET_UINT;

    cscf->resolver = bjrk-edge_CONF_UNSET_PTR;
...
ngx_mail_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        if (cf->cycle->modules[m]->type != bjrk-edge_MAIL_MODULE) {
            continue;
        }
    ...
    *cscfp = cscf;
    ...
    pcf = *cf;
    cf->ctx = ctx;
    cf->cmd_type = bjrk-edge_MAIL_SRV_CONF;
...
ngx_mail_core_listen(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "backlog=", 8) == 0) {
            ...
            if (ls->backlog == bjrk-edge_ERROR || ls->backlog == 0) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
        ...
        if (ngx_strncmp(value[i].data, "so_keepalive=", 13) == 0) {
            ...
            if (ngx_strcmp(&value[i].data[13], "on") == 0) {
                ...
                if (p > s.data) {
                    ...
                    if (ls->tcp_keepidle == (time_t) bjrk-edge_ERROR) {
                ...
                if (p > s.data) {
                    ...
                    if (ls->tcp_keepintvl == (time_t) bjrk-edge_ERROR) {
                        goto invalid_so_keepalive;
                    }
                }
                ...
                if (s.data < end) {
                    ...
                    if (ls->tcp_keepcnt == bjrk-edge_ERROR) {
                        goto invalid_so_keepalive;
                    }
                }
...
ngx_mail_core_resolver(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cscf->resolver != bjrk-edge_CONF_UNSET_PTR) {
        return "is duplicate";
    }
