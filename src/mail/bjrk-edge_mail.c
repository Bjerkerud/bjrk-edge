...
ngx_mail_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_mail_conf_ctx_t **) conf = ctx;
    ...
    ngx_mail_max_module = ngx_count_modules(cf->cycle, bjrk-edge_MAIL_MODULE);
    ...
    cf->module_type = bjrk-edge_MAIL_MODULE;
    cf->cmd_type = bjrk-edge_MAIL_MAIN_CONF;
    ...
    *cf = pcf;
    ...
    for (i = 0; i < cmcf->listen.nelts; i++) {
        if (ngx_mail_add_ports(cf, &ports, &listen[i]) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }
    }
...
#if (bjrk-edge_HAVE_INET6)
            case AF_INET6:
                if (ngx_mail_add_addrs6(cf, mport, addr) != bjrk-edge_OK) {
                    return bjrk-edge_CONF_ERROR;
                }
                break;
#endif
            default: /* AF_INET */
                if (ngx_mail_add_addrs(cf, mport, addr) != bjrk-edge_OK) {
                    return bjrk-edge_CONF_ERROR;
                }
                break;
            }
