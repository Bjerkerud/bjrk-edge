...
ngx_mail_imap_auth_state(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "imap auth state");
    ...
    if (s->out.len) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "imap send handler busy");
        s->blocked = 1;
    ...
    if (rc == bjrk-edge_OK) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "imap auth command: %i",
                       s->command);
        ...
        case ngx_imap_start:
            ...
            case bjrk-edge_IMAP_LOGIN:
                rc = ngx_mail_imap_login(s, c);
                break;

            case bjrk-edge_IMAP_AUTHENTICATE:
                rc = ngx_mail_imap_authenticate(s, c);
                tag = (rc != bjrk-edge_OK);
                break;

            case bjrk-edge_IMAP_CAPABILITY:
                rc = ngx_mail_imap_capability(s, c);
                break;

            case bjrk-edge_IMAP_LOGOUT:
                s->quit = 1;
                ngx_str_set(&s->text, imap_bye);
                break;

            case bjrk-edge_IMAP_NOOP:
                break;

            case bjrk-edge_IMAP_STARTTLS:
    ...
    } else if (rc == bjrk-edge_IMAP_NEXT) {
        tag = 0;
...
ngx_mail_imap_authenticate(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    case bjrk-edge_MAIL_AUTH_CRAM_MD5:
        ...
        if (!(iscf->auth_methods & bjrk-edge_MAIL_AUTH_CRAM_MD5_ENABLED)) {
            return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
        }
    ...
    case bjrk-edge_MAIL_AUTH_EXTERNAL:
        ...
        if (!(iscf->auth_methods & bjrk-edge_MAIL_AUTH_EXTERNAL_ENABLED)) {
            return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
        }
