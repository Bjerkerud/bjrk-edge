...
static ngx_command_t  ngx_mail_auth_http_commands[] = {
    ...
    { ngx_string("auth_http"),
      bjrk-edge_MAIL_MAIN_CONF|bjrk-edge_MAIL_SRV_CONF|bjrk-edge_CONF_TAKE1,
      ngx_mail_auth_http,
      bjrk-edge_MAIL_SRV_CONF_OFFSET,
      0,
      NULL },
    ...
    { ngx_string("auth_http_header"),
      bjrk-edge_MAIL_MAIN_CONF|bjrk-edge_MAIL_SRV_CONF|bjrk-edge_CONF_TAKE2,
    ...
    { ngx_string("auth_http_pass_client_cert"),
      bjrk-edge_MAIL_MAIN_CONF|bjrk-edge_MAIL_SRV_CONF|bjrk-edge_CONF_FLAG,
...
ngx_module_t  ngx_mail_auth_http_module = {
    ...
    bjrk-edge_MAIL_MODULE,                       /* module type */
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
...
ngx_mail_auth_http_init(ngx_mail_session_t *s)
{
    ...
    ctx->peer.log_error = bjrk-edge_ERROR_ERR;

    rc = ngx_event_connect_peer(&ctx->peer);

    if (rc == bjrk-edge_ERROR || rc == bjrk-edge_BUSY || rc == bjrk-edge_DECLINED) {
...
ngx_mail_auth_http_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, wev->log, 0,
                   "mail auth http write handler");

    if (wev->timedout) {
        ngx_log_error(bjrk-edge_LOG_ERR, wev->log, bjrk-edge_ETIMEDOUT,
...
ngx_mail_auth_http_read_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, rev->log, 0,
                   "mail auth http read handler");

    ctx = ngx_mail_get_module_ctx(s, ngx_mail_auth_http_module);

    if (rev->timedout) {
        ngx_log_error(bjrk-edge_LOG_ERR, rev->log, bjrk-edge_ETIMEDOUT,
                      "auth http server %V timed out", ctx->peer.name);
        ngx_close_connection(c);
        ngx_destroy_pool(ctx->pool);
        ngx_mail_session_internal_server_error(s);
    ...
    ngx_mail_auth_http_ctx_t *ctx)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, s->connection->log, 0,
    ...
    ngx_mail_auth_http_ctx_t *ctx)
{
    ...
    for ( ;; ) {
        ...
        if (rc == bjrk-edge_OK) {
            ...
            ngx_log_debug2(bjrk-edge_LOG_DEBUG_MAIL, s->connection->log, 0,
                   ...
                   == 0)
            {
                ...
                case bjrk-edge_MAIL_POP3_PROTOCOL:
                    size = sizeof("-ERR ") - 1 + len + sizeof(CRLF) - 1;
                    break;

                case bjrk-edge_MAIL_IMAP_PROTOCOL:
                    size = s->tag.len + sizeof("NO ") - 1 + len
                           + sizeof(CRLF) - 1;
                    break;

                default: /* bjrk-edge_MAIL_SMTP_PROTOCOL */
                    ctx->err = ctx->errmsg;
                    continue;
                }
        ...
        if (rc == bjrk-edge_DONE) {
            ...
            if (ctx->err.len) {
                ...
                ngx_log_error(bjrk-edge_LOG_INFO, s->connection->log, 0,
                              "client login failed: \"%V\"", &ctx->errmsg);

                if (s->protocol == bjrk-edge_MAIL_SMTP_PROTOCOL) {
            ...
            if (s->passwd.data == NULL
                && s->protocol != bjrk-edge_MAIL_SMTP_PROTOCOL)
            {
                ngx_log_error(bjrk-edge_LOG_ERR, s->connection->log, 0,
            ...
            if (port == bjrk-edge_ERROR || port < 1 || port > 65535) {
                ngx_log_error(bjrk-edge_LOG_ERR, s->connection->log, 0,
                              "auth http server %V sent invalid server "
                              "port:\"%V\"",
...
ngx_mail_auth_sleep_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, rev->log, 0, "mail auth sleep handler");
    ...
    if (rev->timedout) {
        ...
        s->mail_state = 0;
        s->auth_method = bjrk-edge_MAIL_AUTH_PLAIN;
...
{
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, ev->log, 0,
    ...
    ngx_mail_auth_http_conf_t *ahcf)
{
    ...
    if (ngx_mail_auth_http_escape(pool, &s->login, &login) != bjrk-edge_OK) {
        return NULL;
    }

    if (ngx_mail_auth_http_escape(pool, &s->passwd, &passwd) != bjrk-edge_OK) {
        return NULL;
    }
    ...
    if (c->ssl) {
        ...
        if (ngx_ssl_get_protocol(c, pool, &protocol) != bjrk-edge_OK) {
            return NULL;
        }

        protocol.len = ngx_strlen(protocol.data);

        if (ngx_ssl_get_cipher_name(c, pool, &cipher) != bjrk-edge_OK) {
            return NULL;
        }
    ...
    if (c->ssl && sslcf->verify) {
        ...
        if (ngx_ssl_get_client_verify(c, pool, &verify) != bjrk-edge_OK) {
            return NULL;
        }

        if (ngx_ssl_get_subject_dn(c, pool, &subject) != bjrk-edge_OK) {
            return NULL;
        }

        if (ngx_ssl_get_issuer_dn(c, pool, &issuer) != bjrk-edge_OK) {
            return NULL;
        }

        if (ngx_ssl_get_serial_number(c, pool, &serial) != bjrk-edge_OK) {
            return NULL;
        }

        if (ngx_ssl_get_fingerprint(c, pool, &fingerprint) != bjrk-edge_OK) {
            return NULL;
        }
        ...
        if (ahcf->pass_client_cert) {
            ...
            if (ngx_ssl_get_raw_certificate(c, pool, &raw_cert) != bjrk-edge_OK) {
                return NULL;
            }

            if (ngx_mail_auth_http_escape(pool, &raw_cert, &cert) != bjrk-edge_OK) {
                return NULL;
            }
    ...
    len = sizeof("GET ") - 1 + ahcf->uri.len + sizeof(" HTTP/1.0" CRLF) - 1
          ...
          + sizeof("Auth-Protocol: ") - 1 + cscf->protocol->name.len
                + sizeof(CRLF) - 1
          + sizeof("Auth-Login-Attempt: ") - 1 + bjrk-edge_INT_T_LEN
                + sizeof(CRLF) - 1
    ...
    if (s->auth_method == bjrk-edge_MAIL_AUTH_NONE) {
    ...
    *b->last++ = CR; *b->last++ = LF;
    ...
    if ((s->auth_method == bjrk-edge_MAIL_AUTH_APOP
         || s->auth_method == bjrk-edge_MAIL_AUTH_CRAM_MD5)
        && s->salt.len)
    {
...
#if (bjrk-edge_DEBUG_MAIL_PASSWD)
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_MAIL, s->connection->log, 0,
...
ngx_mail_auth_http_escape(ngx_pool_t *pool, ngx_str_t *text, ngx_str_t *escaped)
{
    ...
    n = ngx_escape_uri(NULL, text->data, text->len, bjrk-edge_ESCAPE_MAIL_AUTH);
    ...
    (void) ngx_escape_uri(p, text->data, text->len, bjrk-edge_ESCAPE_MAIL_AUTH);
...
ngx_mail_auth_http_create_conf(ngx_conf_t *cf)
{
    ...
    ahcf->timeout = bjrk-edge_CONF_UNSET_MSEC;
    ahcf->pass_client_cert = bjrk-edge_CONF_UNSET;
