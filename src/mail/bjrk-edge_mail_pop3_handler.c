...
ngx_mail_pop3_init_session(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (pscf->auth_methods
        & (bjrk-edge_MAIL_AUTH_APOP_ENABLED|bjrk-edge_MAIL_AUTH_CRAM_MD5_ENABLED))
    {
...
ngx_mail_pop3_auth_state(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "pop3 auth state");
    ...
    if (s->out.len) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_MAIL, c->log, 0, "pop3 send handler busy");
        s->blocked = 1;
    ...
    if (rc == bjrk-edge_OK) {
        ...
        case ngx_pop3_start:
            ...
            case bjrk-edge_POP3_USER:
                rc = ngx_mail_pop3_user(s, c);
                break;

            case bjrk-edge_POP3_CAPA:
                rc = ngx_mail_pop3_capa(s, c, 1);
                break;

            case bjrk-edge_POP3_APOP:
                rc = ngx_mail_pop3_apop(s, c);
                break;

            case bjrk-edge_POP3_AUTH:
                rc = ngx_mail_pop3_auth(s, c);
                break;

            case bjrk-edge_POP3_QUIT:
                s->quit = 1;
                break;

            case bjrk-edge_POP3_NOOP:
                break;

            case bjrk-edge_POP3_STLS:
...
ngx_mail_pop3_apop(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (!(pscf->auth_methods & bjrk-edge_MAIL_AUTH_APOP_ENABLED)) {
        return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
    }
    ...
    s->auth_method = bjrk-edge_MAIL_AUTH_APOP;
...
ngx_mail_pop3_auth(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    case bjrk-edge_MAIL_AUTH_CRAM_MD5:
        ...
        if (!(pscf->auth_methods & bjrk-edge_MAIL_AUTH_CRAM_MD5_ENABLED)) {
            return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
        }

        if (ngx_mail_auth_cram_md5_salt(s, c, "+ ", 2) == bjrk-edge_OK) {
    ...
    case bjrk-edge_MAIL_AUTH_EXTERNAL:
        ...
        if (!(pscf->auth_methods & bjrk-edge_MAIL_AUTH_EXTERNAL_ENABLED)) {
            return bjrk-edge_MAIL_PARSE_INVALID_COMMAND;
        }
