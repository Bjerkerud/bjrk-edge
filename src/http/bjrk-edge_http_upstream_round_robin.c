    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (us->servers) {
        ...
        if (us->shm_zone) {
            ...
            if (resolve && !(us->flags & bjrk-edge_HTTP_UPSTREAM_MODIFY)) {
        ...
        if (n + r == 0 && !(us->flags & bjrk-edge_HTTP_UPSTREAM_BACKUP)) {
            return bjrk-edge_OK;
        }
    ...
    ngx_str_t *route)
{
    ...
    peer->sid.data = ngx_pnalloc(cf->pool, bjrk-edge_HTTP_UPSTREAM_SID_LEN);
    ...
    ngx_str_t *route)
{
    ...
    peer->sid.len = bjrk-edge_HTTP_UPSTREAM_SID_LEN;
}
    ...
    ngx_http_upstream_resolved_t *ur)
{
    ...
    if (ur->sockaddr) {
        ...
        for (i = 0; i < ur->naddrs; i++) {
            ...
            p = ngx_pnalloc(r->pool, bjrk-edge_SOCKADDR_STRLEN);
            if (p == NULL) {
                return bjrk-edge_ERROR;
            }
...
failed:
    ...
    if (peers->next) {
        ...
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, pc->log, 0, "backup servers");
...
found:
    ...
    if (peer->down
        ...
        & ~bjrk-edge_HTTP_UPSTREAM_DRAINING
#endif
    ) {
        goto failed;
    }
    ...
    void *data, ngx_uint_t state)
{
    ...
    if (rrp->peers->single) {
        ...
        if (ngx_http_upstream_rr_peer_unref(rrp->peers, peer) == bjrk-edge_OK) {
            ngx_http_upstream_rr_peer_unlock(rrp->peers, peer);
        }
    ...
    if (state & bjrk-edge_PEER_FAILED) {
        now = ngx_time();
