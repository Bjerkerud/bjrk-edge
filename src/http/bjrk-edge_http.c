...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    ngx_http_max_module = ngx_count_modules(cf->cycle, bjrk-edge_HTTP_MODULE);
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        ...
        if (module->preconfiguration) {
            if (module->preconfiguration(cf) != bjrk-edge_OK) {
                return bjrk-edge_CONF_ERROR;
            }
        }
    }
    ...
    cf->module_type = bjrk-edge_HTTP_MODULE;
    cf->cmd_type = bjrk-edge_HTTP_MAIN_CONF;
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        if (ngx_http_init_locations(cf, cscfp[s], clcf) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }

        if (ngx_http_init_static_location_trees(cf, clcf) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }
    }


    if (ngx_http_init_phases(cf, cmcf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }

    if (ngx_http_init_headers_in_hash(cf, cmcf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        ...
        if (module->postconfiguration) {
            if (module->postconfiguration(cf) != bjrk-edge_OK) {
                return bjrk-edge_CONF_ERROR;
            }
        }
    }

    if (ngx_http_variables_init_vars(cf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    *cf = pcf;
    ...
    if (ngx_http_init_phase_handlers(cf, cmcf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    if (ngx_http_optimize_servers(cf, cmcf, cmcf->ports) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
...
{
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_POST_READ_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_SERVER_REWRITE_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_REWRITE_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_PREACCESS_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_ACCESS_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_PRECONTENT_PHASE].handlers,
                       cf->pool, 2, sizeof(ngx_http_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_CONTENT_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[bjrk-edge_HTTP_LOG_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
...
ngx_http_init_phase_handlers(ngx_conf_t *cf, ngx_http_core_main_conf_t *cmcf)
{
    ...
    find_config_index = 0;
    use_rewrite = cmcf->phases[bjrk-edge_HTTP_REWRITE_PHASE].handlers.nelts ? 1 : 0;
    use_access = cmcf->phases[bjrk-edge_HTTP_ACCESS_PHASE].handlers.nelts ? 1 : 0;

    n = 1                  /* find config phase */
        + use_rewrite      /* post rewrite phase */
        + use_access;      /* post access phase */

    for (i = 0; i < bjrk-edge_HTTP_LOG_PHASE; i++) {
    ...
    for (i = 0; i < bjrk-edge_HTTP_LOG_PHASE; i++) {
        ...
        case bjrk-edge_HTTP_SERVER_REWRITE_PHASE:
        ...
        case bjrk-edge_HTTP_FIND_CONFIG_PHASE:
            find_config_index = n;
        ...
        case bjrk-edge_HTTP_REWRITE_PHASE:
        ...
        case bjrk-edge_HTTP_POST_REWRITE_PHASE:
            if (use_rewrite) {
        ...
        case bjrk-edge_HTTP_ACCESS_PHASE:
            checker = ngx_http_core_access_phase;
            n++;
            break;

        case bjrk-edge_HTTP_POST_ACCESS_PHASE:
            if (use_access) {
        ...
        case bjrk-edge_HTTP_CONTENT_PHASE:
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
         ...
         q = ngx_queue_next(q))
    {
        ...
        if (ngx_http_init_locations(cf, NULL, clcf) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
    ...
    if (ngx_http_join_exact_locations(cf, locations) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_core_loc_conf_t *clcf)
{
    ...
    if (ngx_http_escape_location_name(cf, clcf) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_escape_location_name(ngx_conf_t *cf, ngx_http_core_loc_conf_t *clcf)
{
    ...
    if (escape) {
        ...
        ngx_escape_uri(p, clcf->name.data, clcf->name.len, bjrk-edge_ESCAPE_URI);
    ...
    ngx_http_conf_port_t *port, ngx_http_listen_opt_t *lsopt)
{
    ...
    for (i = 0; i < port->addrs.nelts; i++) {
        ...
        if (ngx_http_add_server(cf, cscf, &addr[i]) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
...
#if (bjrk-edge_HTTP_V2 && bjrk-edge_HTTP_SSL                                              \
    ...
    ngx_array_t *ports)
{
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        for (a = 0; a < port[p].addrs.nelts; a++) {
            ...
            {
                if (ngx_http_server_names(cf, cmcf, &addr[a]) != bjrk-edge_OK) {
                    return bjrk-edge_ERROR;
                }
            }
        }

        if (ngx_http_init_listening(cf, &port[p]) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
    ...
    ngx_http_conf_addr_t *addr)
{
    ...
    ha.temp_pool = ngx_create_pool(bjrk-edge_DEFAULT_POOL_SIZE, cf->log);
    ...
    if (ngx_hash_keys_array_init(&ha, bjrk-edge_HASH_LARGE) != bjrk-edge_OK) {
        goto failed;
    }
    ...
    if (ha.keys.nelts) {
        ...
        if (ngx_hash_init(&hash, ha.keys.elts, ha.keys.nelts) != bjrk-edge_OK) {
            goto failed;
        }
    }
...
#if (bjrk-edge_HAVE_INET6)
        case AF_INET6:
            if (ngx_http_add_addrs6(cf, hport, addr) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
            break;
#endif
        default: /* AF_INET */
            if (ngx_http_add_addrs(cf, hport, addr) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
            break;
        }
    ...
    ngx_str_t *default_types)
{
    ...
    if (*keys) {
        ...
        if (ngx_hash_init(&hash, (*keys)->elts, (*keys)->nelts) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }
