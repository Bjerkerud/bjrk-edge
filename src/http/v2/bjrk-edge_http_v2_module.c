...
ngx_http_v2_create_main_conf(ngx_conf_t *cf)
{
    ...
    h2mcf->recv_buffer_size = bjrk-edge_CONF_UNSET_SIZE;
...
ngx_http_v2_create_srv_conf(ngx_conf_t *cf)
{
    ...
    h2scf->enable = bjrk-edge_CONF_UNSET;

    h2scf->pool_size = bjrk-edge_CONF_UNSET_SIZE;

    h2scf->concurrent_streams = bjrk-edge_CONF_UNSET_UINT;

    h2scf->preread_size = bjrk-edge_CONF_UNSET_SIZE;

    h2scf->streams_index_mask = bjrk-edge_CONF_UNSET_UINT;
...
ngx_http_v2_create_loc_conf(ngx_conf_t *cf)
{
    ...
    h2lcf->chunk_size = bjrk-edge_CONF_UNSET_SIZE;
...
ngx_http_v2_recv_buffer_size(ngx_conf_t *cf, void *post, void *data)
{
    ...
    if (*sp <= bjrk-edge_HTTP_V2_STATE_BUFFER_SIZE) {
...
ngx_http_v2_preread_size(ngx_conf_t *cf, void *post, void *data)
{
    ...
    if (*sp > bjrk-edge_HTTP_V2_MAX_WINDOW) {
        ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
...
ngx_http_v2_chunk_size(ngx_conf_t *cf, void *post, void *data)
{
    ...
    if (*sp > bjrk-edge_HTTP_V2_MAX_FRAME_SIZE) {
        *sp = bjrk-edge_HTTP_V2_MAX_FRAME_SIZE;
    }
