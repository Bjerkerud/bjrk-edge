...
#define bjrk-edge_HTTP_V2_TABLE_SIZE  4096
...
#define bjrk-edge_HTTP_V2_STATIC_TABLE_ENTRIES                                      \
    ...
    ngx_uint_t name_only)
{
    ...
    if (index < bjrk-edge_HTTP_V2_STATIC_TABLE_ENTRIES) {
        h2c->state.header = ngx_http_v2_static_table[index];
        return bjrk-edge_OK;
    }

    index -= bjrk-edge_HTTP_V2_STATIC_TABLE_ENTRIES;
    ...
    if (index < h2c->hpack.added - h2c->hpack.deleted) {
        ...
        rest = h2c->hpack.storage + bjrk-edge_HTTP_V2_TABLE_SIZE - entry->name.data;
        ...
        *p = '\0';
        ...
        rest = h2c->hpack.storage + bjrk-edge_HTTP_V2_TABLE_SIZE - entry->value.data;
    ...
    ngx_http_v2_header_t *header)
{
    ...
    if (h2c->hpack.entries == NULL) {
        ...
        h2c->hpack.size = bjrk-edge_HTTP_V2_TABLE_SIZE;
        h2c->hpack.free = bjrk-edge_HTTP_V2_TABLE_SIZE;
    ...
    avail = h2c->hpack.storage + bjrk-edge_HTTP_V2_TABLE_SIZE - h2c->hpack.pos;
    ...
    if (avail >= header->name.len) {
        ...
        avail = bjrk-edge_HTTP_V2_TABLE_SIZE;
    }
...
ngx_http_v2_table_size(ngx_http_v2_connection_t *h2c, size_t size)
{
    ...
    if (size > bjrk-edge_HTTP_V2_TABLE_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
