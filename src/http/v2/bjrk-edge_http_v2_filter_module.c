...
#define bjrk-edge_HTTP_V2_NO_TRAILERS           (ngx_http_v2_out_frame_t *) -1
...
ngx_http_v2_header_filter(ngx_http_request_t *r)
{
    ...
    case bjrk-edge_HTTP_OK:
        status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_200_INDEX);
        break;

    case bjrk-edge_HTTP_NO_CONTENT:
        r->header_only = 1;
        ...
        status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_204_INDEX);
        break;

    case bjrk-edge_HTTP_PARTIAL_CONTENT:
        status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_206_INDEX);
        break;

    case bjrk-edge_HTTP_NOT_MODIFIED:
        r->header_only = 1;
        status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_304_INDEX);
        break;
    ...
    default:
        ...
        case bjrk-edge_HTTP_BAD_REQUEST:
            status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_400_INDEX);
            break;

        case bjrk-edge_HTTP_NOT_FOUND:
            status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_404_INDEX);
            break;

        case bjrk-edge_HTTP_INTERNAL_SERVER_ERROR:
            status = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_STATUS_500_INDEX);
            break;
    ...
    if (r->headers_out.server == NULL) {
        ...
        if (clcf->server_tokens == bjrk-edge_HTTP_SERVER_TOKENS_ON) {
            len += 1 + nginx_ver_len;

        } else if (clcf->server_tokens == bjrk-edge_HTTP_SERVER_TOKENS_BUILD) {
    ...
    if (r->headers_out.content_type.len) {
        ...
        if (r->headers_out.content_type.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
            ngx_log_error(bjrk-edge_LOG_CRIT, fc->log, 0,
                          "too long response header value: "
                          "\"Content-Type: %V\"", &r->headers_out.content_type);
            return bjrk-edge_ERROR;
        }

        len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + r->headers_out.content_type.len;
    ...
    {
        len += 1 + ngx_http_v2_integer_octets(bjrk-edge_OFF_T_LEN) + bjrk-edge_OFF_T_LEN;
    }
    ...
    if (r->headers_out.location && r->headers_out.location->value.len) {
        ...
        if (r->headers_out.location->value.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
            ngx_log_error(bjrk-edge_LOG_CRIT, fc->log, 0,
            ...
            && clcf->absolute_redirect)
        {
            if (clcf->server_name_in_redirect) {
                ...
                host.len = bjrk-edge_SOCKADDR_STRLEN;
                host.data = addr;

                if (ngx_connection_local_sockaddr(fc, &host, 0) != bjrk-edge_OK) {
                    return bjrk-edge_ERROR;
                }
            }
        ...
        len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + r->headers_out.location->value.len;
    }
    ...
    if (status) {
        ...
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_STATUS_INDEX);
        *pos++ = bjrk-edge_HTTP_V2_ENCODE_RAW | 3;
    ...
    if (r->headers_out.server == NULL) {
        ...
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_SERVER_INDEX);
    ...
    if (r->headers_out.date == NULL) {
        ...
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_DATE_INDEX);
        pos = ngx_http_v2_write_value(pos, ngx_cached_http_time.data,
                                      ngx_cached_http_time.len, tmp);
    }

    if (r->headers_out.content_type.len) {
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_CONTENT_TYPE_INDEX);
        ...
        && r->headers_out.content_length_n >= 0)
    {
        ...
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_CONTENT_LENGTH_INDEX);

        p = pos;
        pos = ngx_sprintf(pos + 1, "%O", r->headers_out.content_length_n);
        *p = bjrk-edge_HTTP_V2_ENCODE_RAW | (u_char) (pos - p - 1);
    }

    if (r->headers_out.last_modified == NULL
        && r->headers_out.last_modified_time != -1)
    {
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_LAST_MODIFIED_INDEX);

        ngx_http_time(pos, r->headers_out.last_modified_time);
        len = sizeof("Wed, 31 Dec 1986 18:00:00 GMT") - 1;

        ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP, fc->log, 0,
    ...
    if (r->headers_out.location && r->headers_out.location->value.len) {
        ...
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_LOCATION_INDEX);
    ...
    if (r->gzip_vary) {
        ...
        *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_VARY_INDEX);
...
#if (bjrk-edge_DEBUG)
        if (fc->log->log_level & bjrk-edge_LOG_DEBUG_HTTP) {
            ngx_strlow(tmp, header[i].key.data, header[i].key.len);

            ngx_log_debug3(bjrk-edge_LOG_DEBUG_HTTP, fc->log, 0,
    ...
    if (ngx_http_v2_init_stream(r) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_v2_early_hints_filter(ngx_http_request_t *r)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, fc->log, 0,
                   "http2 output header: \":status: %03ui\"",
                   (ngx_uint_t) bjrk-edge_HTTP_EARLY_HINTS);

    *pos++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_STATUS_INDEX);
    *pos++ = bjrk-edge_HTTP_V2_ENCODE_RAW | 3;
    pos = ngx_sprintf(pos, "%03ui", (ngx_uint_t) bjrk-edge_HTTP_EARLY_HINTS);
    ...
    u_char *end, ngx_uint_t fin, ngx_uint_t flush)
{
    ...
    type = bjrk-edge_HTTP_V2_HEADERS_FRAME;
    flags = fin ? bjrk-edge_HTTP_V2_END_STREAM_FLAG : bjrk-edge_HTTP_V2_NO_FLAG;
    frame_size = stream->connection->frame_size;

    for ( ;; ) {
        if (rest <= frame_size) {
            frame_size = rest;
            flags |= bjrk-edge_HTTP_V2_END_HEADERS_FLAG;
        }

        b = ngx_create_temp_buf(r->pool, bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE);
        if (b == NULL) {
            return NULL;
        }
        ...
        *ll = cl;
        ...
        if (rest) {
            frame->length += bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE;

            type = bjrk-edge_HTTP_V2_CONTINUATION_FRAME;
            flags = bjrk-edge_HTTP_V2_NO_FLAG;
            continue;
        }
...
ngx_http_v2_create_trailers_frame(ngx_http_request_t *r)
{
    ...
    if (len == 0) {
        return bjrk-edge_HTTP_V2_NO_TRAILERS;
    }
...
ngx_http_v2_send_chain(ngx_connection_t *fc, ngx_chain_t *in, off_t limit)
{
    ...
    if (in == NULL || stream->out_closed) {
        ...
        if (size) {
            ngx_log_error(bjrk-edge_LOG_ERR, fc->log, 0,
                          "output on closed stream");
            return bjrk-edge_CHAIN_ERROR;
        }

        if (ngx_http_v2_filter_send(fc, stream) == bjrk-edge_ERROR) {
            return bjrk-edge_CHAIN_ERROR;
        }
    ...
    if (size && ngx_http_v2_flow_control(h2c, stream) == bjrk-edge_DECLINED) {

        if (ngx_http_v2_filter_send(fc, stream) == bjrk-edge_ERROR) {
            return bjrk-edge_CHAIN_ERROR;
        }

        if (ngx_http_v2_flow_control(h2c, stream) == bjrk-edge_DECLINED) {
            fc->write->active = 1;
            fc->write->ready = 0;
            return in;
        }
    ...
    trailers = bjrk-edge_HTTP_V2_NO_TRAILERS;
    ...
    for ( ;; ) {
        ...
        if (cl->buf->last_buf) {
            ...
            if (trailers != bjrk-edge_HTTP_V2_NO_TRAILERS) {
                cl->buf->last_buf = 0;
            }
        }
    ...
    if (in && ngx_http_v2_flow_control(h2c, stream) == bjrk-edge_DECLINED) {
        fc->write->active = 1;
        fc->write->ready = 0;
    }
    ...
    size_t len, ngx_chain_t *first, ngx_chain_t *last)
{
    ...
    flags = last->buf->last_buf ? bjrk-edge_HTTP_V2_END_STREAM_FLAG : 0;

    ngx_log_debug4(bjrk-edge_LOG_DEBUG_HTTP, stream->request->connection->log, 0,
    ...
    if (buf->start == NULL) {
        buf->start = ngx_palloc(stream->request->pool,
                                bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE);
        if (buf->start == NULL) {
            return NULL;
        }

        buf->end = buf->start + bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE;
        buf->last = buf->end;
...
ngx_http_v2_filter_send(ngx_connection_t *fc, ngx_http_v2_stream_t *stream)
{
    ...
    if (stream->queued == 0 && !c->buffered) {
        fc->buffered &= ~bjrk-edge_HTTP_V2_BUFFERED;
        return bjrk-edge_OK;
    }

    stream->blocked = 1;

    if (ngx_http_v2_send_output_queue(stream->connection) == bjrk-edge_ERROR) {
        fc->error = 1;
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_v2_out_frame_t *frame)
{
    ...
    stream->request->header_size += bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE
                                    + frame->length;
    ...
    ngx_http_v2_out_frame_t *frame)
{
    ...
    r->connection->sent += bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE + frame->length;
