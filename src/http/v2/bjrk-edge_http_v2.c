...
#include <bjrk-edge_http_v2_module.h>


/* errors */
#define bjrk-edge_HTTP_V2_NO_ERROR                     0x0
#define bjrk-edge_HTTP_V2_PROTOCOL_ERROR               0x1
#define bjrk-edge_HTTP_V2_INTERNAL_ERROR               0x2
#define bjrk-edge_HTTP_V2_FLOW_CTRL_ERROR              0x3
#define bjrk-edge_HTTP_V2_SETTINGS_TIMEOUT             0x4
#define bjrk-edge_HTTP_V2_STREAM_CLOSED                0x5
#define bjrk-edge_HTTP_V2_SIZE_ERROR                   0x6
#define bjrk-edge_HTTP_V2_REFUSED_STREAM               0x7
#define bjrk-edge_HTTP_V2_CANCEL                       0x8
#define bjrk-edge_HTTP_V2_COMP_ERROR                   0x9
#define bjrk-edge_HTTP_V2_CONNECT_ERROR                0xa
#define bjrk-edge_HTTP_V2_ENHANCE_YOUR_CALM            0xb
#define bjrk-edge_HTTP_V2_INADEQUATE_SECURITY          0xc
#define bjrk-edge_HTTP_V2_HTTP_1_1_REQUIRED            0xd

/* frame sizes */
#define bjrk-edge_HTTP_V2_SETTINGS_ACK_SIZE            0
#define bjrk-edge_HTTP_V2_RST_STREAM_SIZE              4
#define bjrk-edge_HTTP_V2_PRIORITY_SIZE                5
#define bjrk-edge_HTTP_V2_PING_SIZE                    8
#define bjrk-edge_HTTP_V2_GOAWAY_SIZE                  8
#define bjrk-edge_HTTP_V2_WINDOW_UPDATE_SIZE           4

#define bjrk-edge_HTTP_V2_SETTINGS_PARAM_SIZE          6

/* settings fields */
#define bjrk-edge_HTTP_V2_HEADER_TABLE_SIZE_SETTING    0x1
#define bjrk-edge_HTTP_V2_ENABLE_PUSH_SETTING          0x2
#define bjrk-edge_HTTP_V2_MAX_STREAMS_SETTING          0x3
#define bjrk-edge_HTTP_V2_INIT_WINDOW_SIZE_SETTING     0x4
#define bjrk-edge_HTTP_V2_MAX_FRAME_SIZE_SETTING       0x5

#define bjrk-edge_HTTP_V2_FRAME_BUFFER_SIZE            24

#define bjrk-edge_HTTP_V2_ROOT                         (void *) -1
...
static ngx_http_v2_handler_pt ngx_http_v2_frame_states[] = {
    ngx_http_v2_state_data,               /* bjrk-edge_HTTP_V2_DATA_FRAME */
    ngx_http_v2_state_headers,            /* bjrk-edge_HTTP_V2_HEADERS_FRAME */
    ngx_http_v2_state_priority,           /* bjrk-edge_HTTP_V2_PRIORITY_FRAME */
    ngx_http_v2_state_rst_stream,         /* bjrk-edge_HTTP_V2_RST_STREAM_FRAME */
    ngx_http_v2_state_settings,           /* bjrk-edge_HTTP_V2_SETTINGS_FRAME */
    ngx_http_v2_state_push_promise,       /* bjrk-edge_HTTP_V2_PUSH_PROMISE_FRAME */
    ngx_http_v2_state_ping,               /* bjrk-edge_HTTP_V2_PING_FRAME */
    ngx_http_v2_state_goaway,             /* bjrk-edge_HTTP_V2_GOAWAY_FRAME */
    ngx_http_v2_state_window_update,      /* bjrk-edge_HTTP_V2_WINDOW_UPDATE_FRAME */
    ngx_http_v2_state_continuation        /* bjrk-edge_HTTP_V2_CONTINUATION_FRAME */
};

#define bjrk-edge_HTTP_V2_FRAME_STATES                                              \
...
ngx_http_v2_init(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "init http2 connection");
    ...
    h2c->send_window = bjrk-edge_HTTP_V2_DEFAULT_WINDOW;
    h2c->recv_window = bjrk-edge_HTTP_V2_MAX_WINDOW;

    h2c->init_window = bjrk-edge_HTTP_V2_DEFAULT_WINDOW;

    h2c->frame_size = bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE;
    ...
    if (ngx_http_v2_send_settings(h2c) == bjrk-edge_ERROR) {
        ngx_http_close_connection(c);
        return;
    }

    if (ngx_http_v2_send_window_update(h2c, 0, bjrk-edge_HTTP_V2_MAX_WINDOW
                                               - bjrk-edge_HTTP_V2_DEFAULT_WINDOW)
        == bjrk-edge_ERROR)
    {
        ngx_http_close_connection(c);
        return;
    ...
    if (ngx_exiting) {
        ngx_http_v2_finalize_connection(h2c, bjrk-edge_HTTP_V2_NO_ERROR);
        return;
    }
...
ngx_http_v2_read_handler(ngx_event_t *rev)
{
    ...
    if (rev->timedout) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, bjrk-edge_ETIMEDOUT, "client timed out");
        ngx_http_v2_finalize_connection(h2c, bjrk-edge_HTTP_V2_PROTOCOL_ERROR);
        return;
    }

    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http2 read handler");
    ...
    if (c->close) {
        ...
        if (!h2c->goaway) {
            ...
            if (ngx_http_v2_send_goaway(h2c, bjrk-edge_HTTP_V2_NO_ERROR)
                == bjrk-edge_ERROR)
            {
                ngx_http_v2_finalize_connection(h2c, 0);
                return;
            ...
            if (ngx_http_v2_send_output_queue(h2c) == bjrk-edge_ERROR) {
                ngx_http_v2_finalize_connection(h2c, 0);
                return;
            }
        }
    ...
    available = h2mcf->recv_buffer_size - bjrk-edge_HTTP_V2_STATE_BUFFER_SIZE;
    ...
    do {
        ...
        if (h2c->total_bytes / 8 > h2c->payload_bytes + 1048576) {
            ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "http2 flood detected");
            ngx_http_v2_finalize_connection(h2c, bjrk-edge_HTTP_V2_NO_ERROR);
            return;
        }
    ...
    if (ngx_handle_read_event(rev, 0) != bjrk-edge_OK) {
        ngx_http_v2_finalize_connection(h2c, bjrk-edge_HTTP_V2_INTERNAL_ERROR);
        return;
    }

    if (h2c->last_out && ngx_http_v2_send_output_queue(h2c) == bjrk-edge_ERROR) {
        ngx_http_v2_finalize_connection(h2c, 0);
        return;
    }
...
ngx_http_v2_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http2 write handler");
...
ngx_http_v2_send_output_queue(ngx_http_v2_connection_t *h2c)
{
    ...
    for (frame = h2c->last_out; frame; frame = fn) {
        ...
        ngx_log_debug4(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
    ...
    if (ngx_handle_write_event(wev, clcf->send_lowat) != bjrk-edge_OK) {
        goto error;
    }
    ...
    if (tcp_nodelay && clcf->tcp_nodelay && ngx_tcp_nodelay(c) != bjrk-edge_OK) {
        goto error;
    }
    ...
    for ( /* void */ ; out; out = fn) {
        ...
        if (out->handler(h2c, out) != bjrk-edge_OK) {
            out->blocked = 1;
            break;
        }
...
ngx_http_v2_lingering_close(ngx_connection_t *c)
{
    ...
    if (clcf->lingering_close == bjrk-edge_HTTP_LINGERING_OFF) {
        ngx_http_close_connection(c);
        return;
    }
    ...
    if (wev->active && (ngx_event_flags & bjrk-edge_USE_LEVEL_EVENT)) {
        if (ngx_del_event(wev, bjrk-edge_WRITE_EVENT, 0) != bjrk-edge_OK) {
            ngx_http_close_connection(c);
            return;
        }
    }

    if (ngx_shutdown_socket(c->fd, bjrk-edge_WRITE_SHUTDOWN) == -1) {
...
ngx_http_v2_lingering_close_handler(ngx_event_t *rev)
{
    ...
    ngx_http_core_loc_conf_t  *clcf;
    ngx_http_v2_connection_t  *h2c;
    u_char                     buffer[bjrk-edge_HTTP_LINGERING_BUFFER_SIZE];
    ...
    do {
        n = c->recv(c, buffer, bjrk-edge_HTTP_LINGERING_BUFFER_SIZE);

        ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "lingering read: %z", n);
...
ngx_http_v2_state_preface(ngx_http_v2_connection_t *h2c, u_char *pos,
    u_char *end)
{
    static const u_char preface[] = bjrk-edge_HTTP_V2_PREFACE_START;
    ...
    if (ngx_memcmp(pos, preface, sizeof(preface) - 1) != 0) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
                      "invalid connection preface");

        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_PROTOCOL_ERROR);
    }
...
ngx_http_v2_state_preface_end(ngx_http_v2_connection_t *h2c, u_char *pos,
    u_char *end)
{
    static const u_char preface[] = bjrk-edge_HTTP_V2_PREFACE_END;
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, h2c->connection->log, 0,
...
ngx_http_v2_state_head(ngx_http_v2_connection_t *h2c, u_char *pos, u_char *end)
{
    ...
    if (end - pos < bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE) {
    ...
    pos += bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE;

    type = ngx_http_v2_parse_type(head);

    ngx_log_debug4(bjrk-edge_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                   "http2 frame type:%ui f:%Xd l:%uz sid:%ui",
                   type, h2c->state.flags, h2c->state.length, h2c->state.sid);

    if (type >= bjrk-edge_HTTP_V2_FRAME_STATES) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
...
ngx_http_v2_state_data(ngx_http_v2_connection_t *h2c, u_char *pos, u_char *end)
{
    ...
    if (h2c->state.flags & bjrk-edge_HTTP_V2_PADDED_FLAG) {
        ...
        if (h2c->state.length == 0) {
            ...
            return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_SIZE_ERROR);
        }
        ...
        if (h2c->state.padding >= size) {
            ...
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_PROTOCOL_ERROR);
        }
    ...
    if (size > h2c->recv_window) {
        ...
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_FLOW_CTRL_ERROR);
    }

    h2c->recv_window -= size;

    if (h2c->recv_window < bjrk-edge_HTTP_V2_MAX_WINDOW / 4) {
        ...
        {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
        }
    ...
    if (size > stream->recv_window) {
        ...
        if (ngx_http_v2_terminate_stream(h2c, stream,
                                         bjrk-edge_HTTP_V2_FLOW_CTRL_ERROR)
            == bjrk-edge_ERROR)
        {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
    ...
    if (stream->no_flow_control
        && stream->recv_window < bjrk-edge_HTTP_V2_MAX_WINDOW / 4)
    {
        if (ngx_http_v2_send_window_update(h2c, node->id,
                                           bjrk-edge_HTTP_V2_MAX_WINDOW
                                           - stream->recv_window)
            == bjrk-edge_ERROR)
        {
            return ngx_http_v2_connection_error(h2c,
        ...
        stream->recv_window = bjrk-edge_HTTP_V2_MAX_WINDOW;
    }
    ...
    if (stream->in_closed) {
        ...
        if (ngx_http_v2_terminate_stream(h2c, stream,
                                         bjrk-edge_HTTP_V2_STREAM_CLOSED)
            == bjrk-edge_ERROR)
        {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
    ...
    u_char *end)
{
    ...
    if (size >= h2c->state.length) {
        size = h2c->state.length;
        stream->in_closed = h2c->state.flags & bjrk-edge_HTTP_V2_END_STREAM_FLAG;
    }
    ...
    if (r->request_body) {
        ...
        if (rc != bjrk-edge_OK && rc != bjrk-edge_AGAIN) {
            stream->skip_data = 1;
        ...
        if (size > (size_t) (buf->end - buf->last)) {
            ngx_log_error(bjrk-edge_LOG_ALERT, h2c->connection->log, 0,
    ...
    u_char *end)
{
    ...
    padded = h2c->state.flags & bjrk-edge_HTTP_V2_PADDED_FLAG;
    priority = h2c->state.flags & bjrk-edge_HTTP_V2_PRIORITY_FLAG;
    ...
    depend = 0;
    excl = 0;
    weight = bjrk-edge_HTTP_V2_DEFAULT_WEIGHT;
    ...
    if (h2c->state.pool == NULL) {
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_INTERNAL_ERROR);
    }
    ...
    if (h2c->processing >= h2scf->concurrent_streams) {
        ...
        status = bjrk-edge_HTTP_V2_REFUSED_STREAM;
        goto rst_stream;
    }
    ...
    if (!h2c->settings_ack
        && !(h2c->state.flags & bjrk-edge_HTTP_V2_END_STREAM_FLAG)
        && h2scf->preread_size < bjrk-edge_HTTP_V2_DEFAULT_WINDOW)
    {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
           ...
           > clcf->keepalive_time)
    {
        ...
        if (ngx_http_v2_send_goaway(h2c, bjrk-edge_HTTP_V2_NO_ERROR) == bjrk-edge_ERROR) {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
        }
    }
...
rst_stream:
    ...
    if (h2c->refused_streams++ > ngx_max(h2scf->concurrent_streams, 100)) {
        ...
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_NO_ERROR);
    }

    if (ngx_http_v2_send_rst_stream(h2c, h2c->state.sid, status) != bjrk-edge_OK) {
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_INTERNAL_ERROR);
    }
    ...
    u_char *end)
{
    ...
    if (!(h2c->state.flags & bjrk-edge_HTTP_V2_END_HEADERS_FLAG)
        && h2c->state.length < bjrk-edge_HTTP_V2_INT_OCTETS)
    {
    ...
    if (value < 0) {
        if (value == bjrk-edge_AGAIN) {
            return ngx_http_v2_state_headers_save(h2c, pos, end,
                                               ngx_http_v2_state_header_block);
        }

        if (value == bjrk-edge_DECLINED) {
            ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
                          "client sent header block with too long %s value",
                          size_update ? "size update" : "header index");

            return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_COMP_ERROR);
        }
    ...
    if (indexed) {
        if (ngx_http_v2_get_indexed_header(h2c, value, 0) != bjrk-edge_OK) {
            return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_COMP_ERROR);
        }
    ...
    if (size_update) {
        if (ngx_http_v2_table_size(h2c, value) != bjrk-edge_OK) {
            return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_COMP_ERROR);
        }
    ...
    } else if (ngx_http_v2_get_indexed_header(h2c, value, 1) != bjrk-edge_OK) {
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_COMP_ERROR);
    }
    ...
    u_char *end)
{
    ...
    if (len < 0) {
        if (len == bjrk-edge_AGAIN) {
            return ngx_http_v2_state_headers_save(h2c, pos, end,
                                                  ngx_http_v2_state_field_len);
        }

        if (len == bjrk-edge_DECLINED) {
            ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP, h2c->connection->log, 0,
    ...
    if ((size_t) len > cscf->large_client_header_buffers.size) {
        ...
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_ENHANCE_YOUR_CALM);
    }
    ...
    u_char *end)
{
    ...
    if (h2c->state.index) {
        if (ngx_http_v2_add_header(h2c, header) != bjrk-edge_OK) {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
        }
    ...
    if (ngx_http_v2_validate_header(r, header) != bjrk-edge_OK) {
        ngx_http_finalize_request(r, bjrk-edge_HTTP_BAD_REQUEST);
        goto error;
    }
    ...
    if (ngx_http_v2_construct_request_line(r) != bjrk-edge_OK) {
        goto error;
    }
    ...
    {
        if (ngx_http_v2_cookie(r, header) != bjrk-edge_OK) {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
        }
        ...
        if (r->headers_in.count++ >= cscf->max_headers) {
            ...
            ngx_http_finalize_request(r, bjrk-edge_HTTP_REQUEST_HEADER_TOO_LARGE);
            goto error;
        }
        ...
        if (hh && hh->handler(r, h, hh->offset) != bjrk-edge_OK) {
            goto error;
        }
    }
    ...
    u_char *end, ngx_http_v2_handler_pt handler)
{
    ...
    if ((size_t) (end - pos) < len + bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE) {
    ...
    if (ngx_http_v2_parse_type(head) != bjrk-edge_HTTP_V2_CONTINUATION_FRAME) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    u_char *end)
{
    ...
    if (h2c->state.length != bjrk-edge_HTTP_V2_PRIORITY_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    if (end - pos < bjrk-edge_HTTP_V2_PRIORITY_SIZE) {
        return ngx_http_v2_state_save(h2c, pos, end,
    ...
    pos += bjrk-edge_HTTP_V2_PRIORITY_SIZE;
    ...
    u_char *end)
{
    ...
    if (h2c->state.length != bjrk-edge_HTTP_V2_RST_STREAM_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    if (end - pos < bjrk-edge_HTTP_V2_RST_STREAM_SIZE) {
        return ngx_http_v2_state_save(h2c, pos, end,
    ...
    pos += bjrk-edge_HTTP_V2_RST_STREAM_SIZE;
    ...
    case bjrk-edge_HTTP_V2_CANCEL:
        ngx_log_error(bjrk-edge_LOG_INFO, fc->log, 0,
                      "client canceled stream %ui", h2c->state.sid);
        break;

    case bjrk-edge_HTTP_V2_INTERNAL_ERROR:
        ngx_log_error(bjrk-edge_LOG_INFO, fc->log, 0,
    ...
    u_char *end)
{
    ...
    if (h2c->state.flags == bjrk-edge_HTTP_V2_ACK_FLAG) {
    ...
    if (h2c->state.length % bjrk-edge_HTTP_V2_SETTINGS_PARAM_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    u_char *end)
{
    ...
    while (h2c->state.length) {
        if (end - pos < bjrk-edge_HTTP_V2_SETTINGS_PARAM_SIZE) {
            return ngx_http_v2_state_save(h2c, pos, end,
                                          ngx_http_v2_state_settings_params);
        }

        h2c->state.length -= bjrk-edge_HTTP_V2_SETTINGS_PARAM_SIZE;
        ...
        case bjrk-edge_HTTP_V2_INIT_WINDOW_SIZE_SETTING:

            if (value > bjrk-edge_HTTP_V2_MAX_WINDOW) {
                ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
        ...
        case bjrk-edge_HTTP_V2_MAX_FRAME_SIZE_SETTING:

            if (value > bjrk-edge_HTTP_V2_MAX_FRAME_SIZE
                || value < bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE)
            {
                ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
                              "client sent SETTINGS frame with incorrect "
        ...
        case bjrk-edge_HTTP_V2_ENABLE_PUSH_SETTING:
        ...
        case bjrk-edge_HTTP_V2_HEADER_TABLE_SIZE_SETTING:
        ...
        pos += bjrk-edge_HTTP_V2_SETTINGS_PARAM_SIZE;
    }

    frame = ngx_http_v2_get_frame(h2c, bjrk-edge_HTTP_V2_SETTINGS_ACK_SIZE,
                                  bjrk-edge_HTTP_V2_SETTINGS_FRAME,
                                  bjrk-edge_HTTP_V2_ACK_FLAG, 0);
    if (frame == NULL) {
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_INTERNAL_ERROR);
    }
    ...
    if (h2c->state.window_delta) {
        ...
        if (ngx_http_v2_adjust_windows(h2c, h2c->state.window_delta) != bjrk-edge_OK)
        {
            return ngx_http_v2_connection_error(h2c,
                                                bjrk-edge_HTTP_V2_INTERNAL_ERROR);
        }
...
ngx_http_v2_state_ping(ngx_http_v2_connection_t *h2c, u_char *pos, u_char *end)
{
    ...
    if (h2c->state.length != bjrk-edge_HTTP_V2_PING_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    if (end - pos < bjrk-edge_HTTP_V2_PING_SIZE) {
    ...
    frame = ngx_http_v2_get_frame(h2c, bjrk-edge_HTTP_V2_PING_SIZE,
                                  bjrk-edge_HTTP_V2_PING_FRAME,
                                  bjrk-edge_HTTP_V2_ACK_FLAG, 0);
    if (frame == NULL) {
        return ngx_http_v2_connection_error(h2c, bjrk-edge_HTTP_V2_INTERNAL_ERROR);
    }
    ...
    buf->last = ngx_cpymem(buf->last, pos, bjrk-edge_HTTP_V2_PING_SIZE);

    ngx_http_v2_queue_blocked_frame(h2c, frame);

    return ngx_http_v2_state_complete(h2c, pos + bjrk-edge_HTTP_V2_PING_SIZE, end);
}
    ...
    u_char *end)
{
    ...
    if (h2c->state.length < bjrk-edge_HTTP_V2_GOAWAY_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    if (end - pos < bjrk-edge_HTTP_V2_GOAWAY_SIZE) {
...
#if (bjrk-edge_DEBUG)
    h2c->state.length -= bjrk-edge_HTTP_V2_GOAWAY_SIZE;

    last_sid = ngx_http_v2_parse_sid(pos);
    error = ngx_http_v2_parse_uint32(&pos[4]);

    pos += bjrk-edge_HTTP_V2_GOAWAY_SIZE;
    ...
    u_char *end)
{
    ...
    if (h2c->state.length != bjrk-edge_HTTP_V2_WINDOW_UPDATE_SIZE) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
    ...
    if (end - pos < bjrk-edge_HTTP_V2_WINDOW_UPDATE_SIZE) {
        return ngx_http_v2_state_save(h2c, pos, end,
    ...
    pos += bjrk-edge_HTTP_V2_WINDOW_UPDATE_SIZE;
    ...
    if (h2c->state.sid) {
        ...
        if (window > (size_t) (bjrk-edge_HTTP_V2_MAX_WINDOW - stream->send_window)) {
    ...
    if (window > bjrk-edge_HTTP_V2_MAX_WINDOW - h2c->send_window) {
        ngx_log_error(bjrk-edge_LOG_INFO, h2c->connection->log, 0,
                      "client violated connection flow control: "
                      "received WINDOW_UPDATE frame "
                      "with window increment %uz "
...
ngx_http_v2_state_skip(ngx_http_v2_connection_t *h2c, u_char *pos, u_char *end)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, h2c->connection->log, 0,
    ...
    ngx_http_v2_handler_pt handler)
{
    ...
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                   "http2 frame state save pos:%p end:%p handler:%p",
                   pos, end, handler);

    size = end - pos;

    if (size > bjrk-edge_HTTP_V2_STATE_BUFFER_SIZE) {
        ngx_log_error(bjrk-edge_LOG_ALERT, h2c->connection->log, 0,
    ...
    ngx_uint_t prefix)
{
    ...
    if (end - start > bjrk-edge_HTTP_V2_INT_OCTETS) {
        end = start + bjrk-edge_HTTP_V2_INT_OCTETS;
    }
...
ngx_http_v2_send_settings(ngx_http_v2_connection_t *h2c)
{
    ...
    len = bjrk-edge_HTTP_V2_SETTINGS_PARAM_SIZE * 3;

    buf = ngx_create_temp_buf(h2c->pool, bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE + len);
    if (buf == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    *buf->last++ = bjrk-edge_HTTP_V2_NO_FLAG;
    ...
    buf->last = ngx_http_v2_write_uint16(buf->last,
                                         bjrk-edge_HTTP_V2_MAX_STREAMS_SETTING);
    buf->last = ngx_http_v2_write_uint32(buf->last,
                                         h2scf->concurrent_streams);

    buf->last = ngx_http_v2_write_uint16(buf->last,
                                         bjrk-edge_HTTP_V2_INIT_WINDOW_SIZE_SETTING);
    buf->last = ngx_http_v2_write_uint32(buf->last, h2scf->preread_size);

    buf->last = ngx_http_v2_write_uint16(buf->last,
                                         bjrk-edge_HTTP_V2_MAX_FRAME_SIZE_SETTING);
    buf->last = ngx_http_v2_write_uint32(buf->last,
                                         bjrk-edge_HTTP_V2_MAX_FRAME_SIZE);
    ...
    size_t window)
{
    ...
    frame = ngx_http_v2_get_frame(h2c, bjrk-edge_HTTP_V2_WINDOW_UPDATE_SIZE,
                                  bjrk-edge_HTTP_V2_WINDOW_UPDATE_FRAME,
                                  bjrk-edge_HTTP_V2_NO_FLAG, sid);
    if (frame == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_uint_t status)
{
    ...
    frame = ngx_http_v2_get_frame(h2c, bjrk-edge_HTTP_V2_RST_STREAM_SIZE,
                                  bjrk-edge_HTTP_V2_RST_STREAM_FRAME,
                                  bjrk-edge_HTTP_V2_NO_FLAG, sid);
    if (frame == NULL) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_v2_send_goaway(ngx_http_v2_connection_t *h2c, ngx_uint_t status)
{
    ...
    frame = ngx_http_v2_get_frame(h2c, bjrk-edge_HTTP_V2_GOAWAY_SIZE,
                                  bjrk-edge_HTTP_V2_GOAWAY_FRAME,
                                  bjrk-edge_HTTP_V2_NO_FLAG, 0);
    if (frame == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_uint_t type, u_char flags, ngx_uint_t sid)
{
    ...
    if (frame) {
        ...
        buf = ngx_create_temp_buf(pool, bjrk-edge_HTTP_V2_FRAME_BUFFER_SIZE);
        if (buf == NULL) {
            return NULL;
        }
...
#if (bjrk-edge_DEBUG)
    if (length > bjrk-edge_HTTP_V2_FRAME_BUFFER_SIZE - bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE)
    {
        ngx_log_error(bjrk-edge_LOG_ALERT, h2c->connection->log, 0,
    ...
    ngx_http_v2_out_frame_t *frame)
{
    ...
    h2c->total_bytes += bjrk-edge_HTTP_V2_FRAME_HEADER_SIZE + frame->length;
...
ngx_http_v2_create_stream(ngx_http_v2_connection_t *h2c)
{
    ...
    fc->buffered = 0;
    fc->sndlowat = 1;
    fc->tcp_nodelay = bjrk-edge_TCP_NODELAY_DISABLED;
    ...
    r->http_version = bjrk-edge_HTTP_VERSION_20;
    r->valid_location = 1;
    ...
    if (r->header_in == NULL) {
        ngx_http_free_request(r, bjrk-edge_HTTP_INTERNAL_SERVER_ERROR);
        return NULL;
    }
    ...
    r->headers_in.connection_type = bjrk-edge_HTTP_CONNECTION_CLOSE;
...
ngx_http_v2_get_closed_node(ngx_http_v2_connection_t *h2c)
{
    ...
    if (parent == bjrk-edge_HTTP_V2_ROOT) {
        node->rank = 0;
        node->rel_weight = 1.0;
...
ngx_http_v2_parse_path(ngx_http_request_t *r, ngx_str_t *value)
{
    ...
    if (ngx_http_parse_uri(r) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_INFO, r->connection->log, 0,
    ...
    if (ngx_http_process_request_uri(r) != bjrk-edge_OK) {
        /*
...
ngx_http_v2_parse_method(ngx_http_request_t *r, ngx_str_t *value)
{
    ...
    static const struct {
        ...
        { 3, "GET",       bjrk-edge_HTTP_GET },
        { 4, "POST",      bjrk-edge_HTTP_POST },
        { 4, "HEAD",      bjrk-edge_HTTP_HEAD },
        { 7, "OPTIONS",   bjrk-edge_HTTP_OPTIONS },
        { 8, "PROPFIND",  bjrk-edge_HTTP_PROPFIND },
        { 3, "PUT",       bjrk-edge_HTTP_PUT },
        { 5, "MKCOL",     bjrk-edge_HTTP_MKCOL },
        { 6, "DELETE",    bjrk-edge_HTTP_DELETE },
        { 4, "COPY",      bjrk-edge_HTTP_COPY },
        { 4, "MOVE",      bjrk-edge_HTTP_MOVE },
        { 9, "PROPPATCH", bjrk-edge_HTTP_PROPPATCH },
        { 4, "LOCK",      bjrk-edge_HTTP_LOCK },
        { 6, "UNLOCK",    bjrk-edge_HTTP_UNLOCK },
        { 5, "PATCH",     bjrk-edge_HTTP_PATCH },
        { 5, "TRACE",     bjrk-edge_HTTP_TRACE },
        { 7, "CONNECT",   bjrk-edge_HTTP_CONNECT }
    }, *test;
...
ngx_http_v2_parse_authority(ngx_http_request_t *r, ngx_str_t *value)
{
    ...
    if (rc == bjrk-edge_ERROR) {
        ngx_http_v2_close_stream(r->stream, bjrk-edge_HTTP_INTERNAL_SERVER_ERROR);
        return bjrk-edge_ABORT;
    }

    if (ngx_http_set_virtual_server(r, value) == bjrk-edge_ERROR) {
        /*
         * request has been finalized already
...
ngx_http_v2_construct_cookie_header(ngx_http_request_t *r)
{
    ...
    if (hh->handler(r, h, hh->offset) != bjrk-edge_OK) {
        /*
         * request has been finalized already
...
ngx_http_v2_run_request(ngx_http_request_t *r)
{
    ...
    if (!h2scf->enable && !r->http_connection->addr_conf->http2) {
        ...
        ngx_http_finalize_request(r, bjrk-edge_HTTP_MISDIRECTED_REQUEST);
        goto failed;
    }
    ...
    if (ngx_http_v2_construct_cookie_header(r) != bjrk-edge_OK) {
        goto failed;
    }

    r->http_state = bjrk-edge_HTTP_PROCESS_REQUEST_STATE;
    ...
    if (r->host_end) {
        ...
        if (r->headers_in.host) {
            ...
            if (ngx_http_v2_construct_host_header(r) != bjrk-edge_OK) {
                goto failed;
            }
        }
    }
    ...
    if (r->headers_in.content_length) {
        ...
        if (r->headers_in.content_length_n == bjrk-edge_ERROR) {
            ngx_log_error(bjrk-edge_LOG_INFO, fc->log, 0,
    ...
    if (r->method == bjrk-edge_HTTP_CONNECT) {
        ngx_log_error(bjrk-edge_LOG_INFO, fc->log, 0, "client sent CONNECT method");
        ngx_http_finalize_request(r, bjrk-edge_HTTP_NOT_ALLOWED);
        goto failed;
    }

    if (r->method == bjrk-edge_HTTP_TRACE) {
        ngx_log_error(bjrk-edge_LOG_INFO, fc->log, 0, "client sent TRACE method");
        ngx_http_finalize_request(r, bjrk-edge_HTTP_NOT_ALLOWED);
        goto failed;
    }
...
ngx_http_v2_read_request_body(ngx_http_request_t *r)
{
    ...
    if (r->request_body_no_buffering || rb->filter_need_buffering) {
        ...
        if (len > bjrk-edge_HTTP_V2_MAX_WINDOW) {
            len = bjrk-edge_HTTP_V2_MAX_WINDOW;
        }
    }
    ...
    if (r->request_body_no_buffering || rb->filter_need_buffering) {
        ...
        size = bjrk-edge_HTTP_V2_MAX_WINDOW - stream->recv_window;
    }
    ...
    size_t size, ngx_uint_t last, ngx_uint_t flush)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, fc->log, 0,
        ...
        for ( ;; ) {
            if (rb->buf->last == rb->buf->end && size) {
                ...
                if (r->request_body_no_buffering) {
                    ...
                    ngx_log_error(bjrk-edge_LOG_ALERT, fc->log, 0,
            ...
            ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, fc->log, 0,
    ...
    if (buf->pos != buf->last) {
        ...
        if (r->headers_in.content_length_n != -1) {
            if (rb->received > r->headers_in.content_length_n) {
                ...
                return bjrk-edge_HTTP_BAD_REQUEST;
            }
                ...
                && rb->received > clcf->client_max_body_size)
            {
                ...
                return bjrk-edge_HTTP_REQUEST_ENTITY_TOO_LARGE;
            }
        }
...
ngx_http_v2_read_client_request_body_handler(ngx_http_request_t *r)
{
    ...
    if (fc->read->timedout) {
        ngx_log_error(bjrk-edge_LOG_INFO, fc->log, bjrk-edge_ETIMEDOUT, "client timed out");

        fc->timedout = 1;
        r->stream->skip_data = 1;

        ngx_http_finalize_request(r, bjrk-edge_HTTP_REQUEST_TIME_OUT);
        return;
    }
    ...
    if (fc->error) {
        ...
        ngx_http_finalize_request(r, bjrk-edge_HTTP_CLIENT_CLOSED_REQUEST);
        return;
    }
...
ngx_http_v2_read_unbuffered_request_body(ngx_http_request_t *r)
{
    ...
    if (fc->read->timedout) {
        if (stream->recv_window) {
            ...
            return bjrk-edge_HTTP_REQUEST_TIME_OUT;
        }
    ...
    if (!stream->rst_sent && !h2c->connection->error) {
        ...
        if (!stream->out_closed) {
            if (ngx_http_v2_send_rst_stream(h2c, node->id,
                                      fc->timedout ? bjrk-edge_HTTP_V2_PROTOCOL_ERROR
                                                   : bjrk-edge_HTTP_V2_INTERNAL_ERROR)
                != bjrk-edge_OK)
            {
                h2c->connection->error = 1;
            ...
            if (ngx_http_v2_send_rst_stream(h2c, node->id, bjrk-edge_HTTP_V2_NO_ERROR)
                != bjrk-edge_OK)
            {
                h2c->connection->error = 1;
            }
...
ngx_http_v2_close_stream_handler(ngx_event_t *ev)
{
    ...
    if (ev->timedout) {
        ...
        ngx_http_v2_close_stream(r->stream, bjrk-edge_HTTP_REQUEST_TIME_OUT);
        return;
    }
...
ngx_http_v2_handle_connection_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, rev->log, 0,
...
ngx_http_v2_idle_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http2 idle handler");
    ...
    ngx_uint_t status)
{
    ...
    if (!c->error && !h2c->goaway) {
        ...
        if (ngx_http_v2_send_goaway(h2c, status) != bjrk-edge_ERROR) {
...
ngx_http_v2_adjust_windows(ngx_http_v2_connection_t *h2c, ssize_t delta)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        for (node = h2c->streams_index[i]; node; node = node->index) {
            ...
            if (delta > 0
                && stream->send_window
                      > (ssize_t) (bjrk-edge_HTTP_V2_MAX_WINDOW - delta))
            {
                if (ngx_http_v2_terminate_stream(h2c, stream,
                                                 bjrk-edge_HTTP_V2_FLOW_CTRL_ERROR)
                    == bjrk-edge_ERROR)
    ...
    if (parent == NULL) {
        ...
        if (node->parent != NULL) {
            ...
            for (next = parent->parent;
                 next != bjrk-edge_HTTP_V2_ROOT && next->rank >= node->rank;
                 next = next->parent)
            {
                if (next != node) {
                    continue;
                ...
                if (node->parent == bjrk-edge_HTTP_V2_ROOT) {
                    parent->rank = 1;
