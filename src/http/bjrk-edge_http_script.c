    ...
    ngx_http_complex_value_t *val, size_t default_value)
{
    ...
    if (ngx_http_complex_value(r, val, &value) != bjrk-edge_OK) {
        return default_value;
    }

    size = ngx_parse_size(&value);

    if (size == (size_t) bjrk-edge_ERROR) {
...
ngx_http_compile_complex_value(ngx_http_compile_complex_value_t *ccv)
{
    ...
    {
        if (ngx_conf_full_name(ccv->cf->cycle, v, ccv->conf_prefix) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    ...
    if (ngx_array_init(&lengths, ccv->cf->pool, n, 1) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_array_init(&values, ccv->cf->pool, n, 1) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_set_complex_value_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (*cv != bjrk-edge_CONF_UNSET_PTR && *cv != NULL) {
        return "is duplicate";
    }
    ...
    void *conf)
{
    ...
    if (*cv != bjrk-edge_CONF_UNSET_PTR) {
        return "is duplicate";
    }
    ...
    void *conf)
{
    ...
    if (cv->u.size == (size_t) bjrk-edge_ERROR) {
...
ngx_http_test_predicates(ngx_http_request_t *r, ngx_array_t *predicates)
{
    ...
    for (i = 0; i < predicates->nelts; i++) {
        if (ngx_http_complex_value(r, &cv[i], &val) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
...
ngx_http_script_compile(ngx_http_script_compile_t *sc)
{
    ...
    if (ngx_http_script_init_arrays(sc) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    for (i = 0; i < sc->source->len; /* void */ ) {
        ...
        if (sc->source->data[i] == '$') {
            ...
            if (sc->source->data[i] >= '1' && sc->source->data[i] <= '9') {
                ...
                if (ngx_http_script_add_capture_code(sc, n) != bjrk-edge_OK) {
                    return bjrk-edge_ERROR;
                }
                ...
                continue;
#else
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, sc->cf, 0,
            ...
            if (ngx_http_script_add_var_code(sc, &name) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        ...
        if (sc->source->data[i] == '?' && sc->compile_args) {
            ...
            if (ngx_http_script_add_args_code(sc) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
...
invalid_variable:
    ...
    ngx_conf_log_error(bjrk-edge_LOG_EMERG, sc->cf, 0, "invalid variable name");
...
ngx_http_script_done(ngx_http_script_compile_t *sc)
{
    ...
    if (sc->zero) {
        ...
        if (ngx_http_script_add_copy_code(sc, &zero, 0) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    if (sc->conf_prefix || sc->root_prefix) {
        if (ngx_http_script_add_full_name_code(sc) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_http_script_check_length(ngx_http_script_engine_t *e, size_t len)
{
    ...
    if (e->end - e->pos < (ssize_t) len) {
        ngx_log_error(bjrk-edge_LOG_ALERT, e->request->connection->log, 0,
                      "no buffer space in script copy");
        e->ip = ngx_http_script_exit;
        e->status = bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        return bjrk-edge_ERROR;
    }
...
ngx_http_script_copy_code(ngx_http_script_engine_t *e)
{
    ...
    if (!e->skip) {
        ...
        if (ngx_http_script_check_length(e, code->len) != bjrk-edge_OK) {
            return;
        }
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP, e->request->connection->log, 0,
...
ngx_http_script_copy_var_code(ngx_http_script_engine_t *e)
{
    ...
    if (!e->skip) {
        ...
        if (value && !value->not_found) {
            ...
            if (ngx_http_script_check_length(e, value->len) != bjrk-edge_OK) {
                return;
            }
            ...
            ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP,
...
{
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, e->request->connection->log, 0,
                   "http script args");
...
ngx_http_script_regex_start_code(ngx_http_script_engine_t *e)
{
    ...
    if (rc == bjrk-edge_DECLINED) {
        if (e->log || (r->connection->log->log_level & bjrk-edge_LOG_DEBUG_HTTP)) {
...
ngx_http_script_regex_end_code(ngx_http_script_engine_t *e)
{
    ...
    if (code->redirect) {
        ...
        if (code->add_args && r->args.len) {
            ...
            if (ngx_http_script_check_length(e, r->args.len + 1) != bjrk-edge_OK) {
                return;
            }
...
ngx_http_script_copy_capture_len_code(ngx_http_script_engine_t *e)
{
    ...
    if (n < r->ncaptures) {
            ...
            && (e->request->quoted_uri || e->request->plus_in_uri))
        {
            ...
            return len + 2 * ngx_escape_uri(NULL, p, len, bjrk-edge_ESCAPE_ARGS);
...
ngx_http_script_copy_capture_code(ngx_http_script_engine_t *e)
{
    ...
    if (n < r->ncaptures) {
        ...
        if ((e->is_args || e->quote)
            && (e->request->quoted_uri || e->request->plus_in_uri))
        {
            escape = 2 * ngx_escape_uri(NULL, p, len, bjrk-edge_ESCAPE_ARGS);

            if (ngx_http_script_check_length(e, len + escape) != bjrk-edge_OK) {
                return;
            }

            e->pos = (u_char *) ngx_escape_uri(pos, p, len, bjrk-edge_ESCAPE_ARGS);

        } else {

            if (ngx_http_script_check_length(e, len) != bjrk-edge_OK) {
                return;
            }
...
ngx_http_script_full_name_code(ngx_http_script_engine_t *e)
{
    ...
    if (ngx_get_full_name(e->request->pool, prefix, &value) != bjrk-edge_OK) {
        e->ip = ngx_http_script_exit;
        e->status = bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        return;
    }
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, e->request->connection->log, 0,
...
ngx_http_script_return_code(ngx_http_script_engine_t *e)
{
    ...
    if (code->status < bjrk-edge_HTTP_BAD_REQUEST
        || code->text.value.len
        || code->text.lengths)
    {
...
ngx_http_script_file_code(ngx_http_script_engine_t *e)
{
        ...
        != bjrk-edge_OK)
    {
        ...
        if (of.err != bjrk-edge_ENOENT
            && of.err != bjrk-edge_ENOTDIR
            && of.err != bjrk-edge_ENAMETOOLONG)
        {
