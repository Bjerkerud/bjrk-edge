...
ngx_http_file_cache_init(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    if (ocache) {
        if (ngx_strcmp(cache->path->name.data, ocache->path->name.data) != 0) {
            ngx_log_error(bjrk-edge_LOG_EMERG, shm_zone->shm.log, 0,
        ...
        for (n = 0; n < bjrk-edge_MAX_PATH_LEVEL; n++) {
...
ngx_http_file_cache_new(ngx_http_request_t *r)
{
    ...
    if (ngx_array_init(&c->keys, r->pool, 4, sizeof(ngx_str_t)) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }

    r->cache = c;
    c->file.log = r->connection->log;
    c->file.fd = bjrk-edge_INVALID_FILE;
...
ngx_http_file_cache_create(ngx_http_request_t *r)
{
    ...
    if (ngx_http_file_cache_exists(cache, c) == bjrk-edge_ERROR) {
        return bjrk-edge_ERROR;
    }

    if (ngx_http_file_cache_name(r, cache->path) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_file_cache_create_key(ngx_http_request_t *r)
{
    ...
    ngx_memcpy(c->main, c->key, bjrk-edge_HTTP_CACHE_KEY_LEN);
}
...
ngx_http_file_cache_open(ngx_http_request_t *r)
{
    ...
    if (rc == bjrk-edge_AGAIN) {
        return bjrk-edge_HTTP_CACHE_SCARCE;
    }
    ...
    if (rc == bjrk-edge_OK) {
        ...
        if (c->min_uses > 1) {
            ...
            rv = bjrk-edge_HTTP_CACHE_SCARCE;
    ...
    of.directio = bjrk-edge_OPEN_FILE_DIRECTIO_OFF;
        ...
        != bjrk-edge_OK)
    {
        ...
        default:
            ngx_log_error(bjrk-edge_LOG_CRIT, r->connection->log, of.err,
...
ngx_http_file_cache_read(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if (h->version != bjrk-edge_HTTP_CACHE_VERSION) {
        ngx_log_error(bjrk-edge_LOG_INFO, r->connection->log, 0,
    ...
    if (h->vary_len > bjrk-edge_HTTP_CACHE_VARY_LEN) {
        ngx_log_error(bjrk-edge_LOG_CRIT, r->connection->log, 0,
    ...
    if (h->vary_len) {
        ...
        if (ngx_memcmp(c->variant, h->variant, bjrk-edge_HTTP_CACHE_KEY_LEN) != 0) {
    ...
    if (c->valid_sec < now) {
        ...
        if (c->node->updating) {
            rc = bjrk-edge_HTTP_CACHE_UPDATING;
...
{
#if (bjrk-edge_HAVE_FILE_AIO || bjrk-edge_THREADS)
    ssize_t                    n;
    ngx_http_core_loc_conf_t  *clcf;
    ...
    if (clcf->aio == bjrk-edge_HTTP_AIO_ON && ngx_file_aio) {
    ...
    if (clcf->aio == bjrk-edge_HTTP_AIO_THREADS) {
...
ngx_http_file_cache_exists(ngx_http_file_cache_t *cache, ngx_http_cache_t *c)
{
    ...
    if (fcn == NULL) {
        ...
        fcn = ngx_slab_calloc_locked(cache->shpool,
                                     sizeof(ngx_http_file_cache_node_t));
        if (fcn == NULL) {
            ngx_log_error(bjrk-edge_LOG_ALERT, ngx_cycle->log, 0,
    ...
    ngx_memcpy(fcn->key, &c->key[sizeof(ngx_rbtree_key_t)],
               bjrk-edge_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t));
...
ngx_http_file_cache_name(ngx_http_request_t *r, ngx_path_t *path)
{
    ...
    c->file.name.len = path->name.len + 1 + path->len
                       + 2 * bjrk-edge_HTTP_CACHE_KEY_LEN;
    ...
    p = ngx_hex_dump(p, c->key, bjrk-edge_HTTP_CACHE_KEY_LEN);
    *p = '\0';
    ...
    u_char *hash)
{
    ...
    ngx_str_t   name;
    ngx_md5_t   md5;
    u_char      buf[bjrk-edge_HTTP_CACHE_VARY_LEN];

    ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http file cache vary: \"%*s\"", len, vary);

    ngx_md5_init(&md5);
    ngx_md5_update(&md5, r->cache->main, bjrk-edge_HTTP_CACHE_KEY_LEN);
...
ngx_http_file_cache_reopen(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->file.log, 0,
    ...
    ngx_memcpy(c->key, c->variant, bjrk-edge_HTTP_CACHE_KEY_LEN);
...
ngx_http_file_cache_set_header(ngx_http_request_t *r, u_char *buf)
{
    ...
    h->version = bjrk-edge_HTTP_CACHE_VERSION;
    ...
    if (c->etag.len <= bjrk-edge_HTTP_CACHE_ETAG_LEN) {
        h->etag_len = (u_char) c->etag.len;
        ngx_memcpy(h->etag, c->etag.data, c->etag.len);
    }

    if (c->vary.len) {
        if (c->vary.len > bjrk-edge_HTTP_CACHE_VARY_LEN) {
            /* should not happen */
            c->vary.len = bjrk-edge_HTTP_CACHE_VARY_LEN;
        }
        ...
        ngx_memcpy(h->variant, c->variant, bjrk-edge_HTTP_CACHE_KEY_LEN);
    }

    if (ngx_http_file_cache_update_variant(r, c) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_file_cache_update_variant(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if (c->vary.len
        && ngx_memcmp(c->variant, c->key, bjrk-edge_HTTP_CACHE_KEY_LEN) == 0)
    {
        return bjrk-edge_OK;
    }
    ...
    ngx_memcpy(c->key, c->main, bjrk-edge_HTTP_CACHE_KEY_LEN);
...
ngx_http_file_cache_update(ngx_http_request_t *r, ngx_temp_file_t *tf)
{
    ...
    ext.access = bjrk-edge_FILE_OWNER_ACCESS;
    ext.path_access = bjrk-edge_FILE_OWNER_ACCESS;
    ext.time = -1;
    ext.create_path = 1;
    ext.delete_file = 1;
    ...
    if (rc == bjrk-edge_OK) {
        ...
        if (ngx_fd_info(tf->file.fd, &fi) == bjrk-edge_FILE_ERROR) {
...
ngx_http_file_cache_update_header(ngx_http_request_t *r)
{
    ...
    file.fd = ngx_open_file(file.name.data, bjrk-edge_FILE_RDWR, bjrk-edge_FILE_OPEN, 0);
    ...
    if (file.fd == bjrk-edge_INVALID_FILE) {
        ...
        ngx_log_error(bjrk-edge_LOG_CRIT, r->connection->log, err,
    ...
    if (ngx_close_file(file.fd) == bjrk-edge_FILE_ERROR) {
        ngx_log_error(bjrk-edge_LOG_ALERT, r->connection->log, ngx_errno,
...
ngx_http_file_cache_free(ngx_http_cache_t *c, ngx_temp_file_t *tf)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->file.log, 0,
    ...
    if (c->temp_file) {
        if (tf && tf->file.fd != bjrk-edge_INVALID_FILE) {
            ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->file.log, 0,
                           "http file cache incomplete: \"%s\"",
                           tf->file.name.data);

            if (ngx_delete_file(tf->file.name.data) == bjrk-edge_FILE_ERROR) {
                ngx_log_error(bjrk-edge_LOG_CRIT, c->file.log, ngx_errno,
...
ngx_http_file_cache_cleanup(void *data)
{
    ...
    if (c->updating && !c->background) {
        ngx_log_error(bjrk-edge_LOG_ALERT, c->file.log, 0,
...
ngx_http_file_cache_forced_expire(ngx_http_file_cache_t *cache)
{
    ...
    ngx_http_file_cache_node_t  *fcn;
    u_char                       key[2 * bjrk-edge_HTTP_CACHE_KEY_LEN];

    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "http file cache forced expire");

    path = cache->path;
    len = path->name.len + 1 + path->len + 2 * bjrk-edge_HTTP_CACHE_KEY_LEN;
    ...
    for ( ;; ) {
        ...
        ngx_log_debug6(bjrk-edge_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
        ...
        len = bjrk-edge_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t);
        ...
        ngx_log_error(bjrk-edge_LOG_ALERT, ngx_cycle->log, 0,
                      ...
                      (size_t) 2 * bjrk-edge_HTTP_CACHE_KEY_LEN, key, fcn->count);
    ...
    u_char *name)
{
    ...
    if (fcn->exists) {
        ...
        *p = '\0';
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "http file cache expire: \"%s\"", name);

        if (ngx_delete_file(name) == bjrk-edge_FILE_ERROR) {
            ngx_log_error(bjrk-edge_LOG_CRIT, ngx_cycle->log, ngx_errno,
...
ngx_http_file_cache_manager(void *data)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug3(bjrk-edge_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
...
ngx_http_file_cache_loader(void *data)
{
    ...
    if (ngx_walk_tree(&tree, &cache->path->name) == bjrk-edge_ABORT) {
        cache->sh->loading = 0;
        return;
    }
    ...
    ngx_log_error(bjrk-edge_LOG_NOTICE, ngx_cycle->log, 0,
...
ngx_http_file_cache_manage_file(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ...
    if (ngx_http_file_cache_add_file(ctx, path) != bjrk-edge_OK) {
    ...
    return (ngx_quit || ngx_terminate) ? bjrk-edge_ABORT : bjrk-edge_OK;
}
...
ngx_http_file_cache_add_file(ngx_tree_ctx_t *ctx, ngx_str_t *name)
{
    ...
    if (name->len < 2 * bjrk-edge_HTTP_CACHE_KEY_LEN) {
        return bjrk-edge_ERROR;
    }
    ...
    if (name->len >= 2 * bjrk-edge_HTTP_CACHE_KEY_LEN + 1 + 10
    ...
    if (ctx->size < (off_t) sizeof(ngx_http_file_cache_header_t)) {
        ngx_log_error(bjrk-edge_LOG_CRIT, ctx->log, 0,
    ...
    p = &name->data[name->len - 2 * bjrk-edge_HTTP_CACHE_KEY_LEN];

    for (i = 0; i < bjrk-edge_HTTP_CACHE_KEY_LEN; i++) {
        n = ngx_hextoi(p, 2);
...
{
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, ctx->log, 0,
                   "http file cache delete: \"%s\"", path->data);

    if (ngx_delete_file(path->data) == bjrk-edge_FILE_ERROR) {
        ngx_log_error(bjrk-edge_LOG_CRIT, ctx->log, ngx_errno,
...
ngx_http_file_cache_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    name.len = 0;
    size = 0;
    max_size = bjrk-edge_MAX_OFF_T_VALUE;
    min_free = 0;
    ...
    if (ngx_conf_full_name(cf->cycle, &cache->path->name, 0) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "levels=", 7) == 0) {
            ...
            for (n = 0; n < bjrk-edge_MAX_PATH_LEVEL && p < last; n++) {
                ...
                if (*p > '0' && *p < '3') {
                    ...
                    if (*p++ == ':' && n < bjrk-edge_MAX_PATH_LEVEL - 1 && p < last) {
                        continue;
                    }
            ...
            if (cache->path->len < 10 + bjrk-edge_MAX_PATH_LEVEL) {
                continue;
            }
...
#if (bjrk-edge_WIN32 || bjrk-edge_HAVE_STATFS || bjrk-edge_HAVE_STATVFS)
        ...
        if (ngx_strncmp(value[i].data, "loader_files=", 13) == 0) {
            ...
            if (loader_files == bjrk-edge_ERROR) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
        ...
        if (ngx_strncmp(value[i].data, "loader_sleep=", 13) == 0) {
            ...
            if (loader_sleep == (ngx_msec_t) bjrk-edge_ERROR) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
        ...
        if (ngx_strncmp(value[i].data, "loader_threshold=", 17) == 0) {
            ...
            if (loader_threshold == (ngx_msec_t) bjrk-edge_ERROR) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
        ...
        if (ngx_strncmp(value[i].data, "manager_files=", 14) == 0) {
            ...
            if (manager_files == bjrk-edge_ERROR) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
        ...
        if (ngx_strncmp(value[i].data, "manager_sleep=", 14) == 0) {
            ...
            if (manager_sleep == (ngx_msec_t) bjrk-edge_ERROR) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
        ...
        if (ngx_strncmp(value[i].data, "manager_threshold=", 18) == 0) {
            ...
            if (manager_threshold == (ngx_msec_t) bjrk-edge_ERROR) {
                ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
    ...
    if (ngx_add_path(cf, &cache->path) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
