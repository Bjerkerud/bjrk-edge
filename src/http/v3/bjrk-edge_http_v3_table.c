    ...
    ngx_uint_t index, ngx_str_t *value)
{
    ...
    if (dynamic) {
        ...
        if (dt->base + dt->nelts <= index) {
            return bjrk-edge_HTTP_V3_ERR_ENCODER_STREAM_ERROR;
        }

        index = dt->base + dt->nelts - 1 - index;

        if (ngx_http_v3_lookup(c, index, &name, NULL) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_V3_ERR_ENCODER_STREAM_ERROR;
        }
        ...
        if (ngx_http_v3_lookup_static(c, index, &name, NULL) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_V3_ERR_ENCODER_STREAM_ERROR;
        }
    }
...
ngx_http_v3_insert(ngx_connection_t *c, ngx_str_t *name, ngx_str_t *value)
{
    ...
    if (ngx_http_v3_evict(c, dt->capacity) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }

    ngx_post_event(&dt->send_insert_count, &ngx_posted_events);

    if (ngx_http_v3_new_entry(c) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_v3_set_capacity(ngx_connection_t *c, ngx_uint_t capacity)
{
    ...
    if (ngx_http_v3_evict(c, capacity) != bjrk-edge_OK) {
        return bjrk-edge_HTTP_V3_ERR_ENCODER_STREAM_ERROR;
    }
...
ngx_http_v3_duplicate(ngx_connection_t *c, ngx_uint_t index)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 duplicate %ui", index);
...
ngx_http_v3_ack_section(ngx_connection_t *c, ngx_uint_t stream_id)
{
    ...
    return bjrk-edge_HTTP_V3_ERR_DECODER_STREAM_ERROR;
}
...
ngx_http_v3_check_insert_count(ngx_connection_t *c, ngx_uint_t insert_count)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 block stream");
    ...
    if (block->queue.prev == NULL) {
        ...
        if (h3c->nblocked == h3scf->max_blocked_streams) {
            ...
            ngx_http_v3_finalize_connection(c,
                                          bjrk-edge_HTTP_V3_ERR_DECOMPRESSION_FAILED,
...
ngx_http_v3_new_entry(ngx_connection_t *c)
{
    ...
    while (!ngx_queue_empty(&h3c->blocked)) {
        ...
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, bc->log, 0, "http3 unblock stream");
...
ngx_http_v3_set_param(ngx_connection_t *c, uint64_t id, uint64_t value)
{
    ...
    case bjrk-edge_HTTP_V3_PARAM_MAX_TABLE_CAPACITY:
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
                       "http3 param QPACK_MAX_TABLE_CAPACITY:%uL", value);
        break;

    case bjrk-edge_HTTP_V3_PARAM_MAX_FIELD_SECTION_SIZE:
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
                       "http3 param SETTINGS_MAX_FIELD_SECTION_SIZE:%uL",
                       value);
        break;

    case bjrk-edge_HTTP_V3_PARAM_BLOCKED_STREAMS:
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
