...
ngx_http_v3_init_uni_stream(ngx_connection_t *c)
{
    ...
    if (h3c->hq) {
        ngx_http_v3_finalize_connection(c,
                                        bjrk-edge_HTTP_V3_ERR_STREAM_CREATION_ERROR,
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 init uni stream");

    n = c->quic->id >> 2;

    if (n >= bjrk-edge_HTTP_V3_MAX_UNI_STREAMS) {
        ngx_http_v3_finalize_connection(c,
                                      bjrk-edge_HTTP_V3_ERR_STREAM_CREATION_ERROR,
    ...
    if (us == NULL) {
        ngx_http_v3_finalize_connection(c,
                                        bjrk-edge_HTTP_V3_ERR_INTERNAL_ERROR,
...
ngx_http_v3_close_uni_stream(ngx_connection_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 close stream");
...
ngx_http_v3_register_uni_stream(ngx_connection_t *c, uint64_t type)
{
    ...
    case bjrk-edge_HTTP_V3_STREAM_ENCODER:

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
                       "http3 encoder stream");
        index = bjrk-edge_HTTP_V3_STREAM_CLIENT_ENCODER;
        break;

    case bjrk-edge_HTTP_V3_STREAM_DECODER:

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
                       "http3 decoder stream");
        index = bjrk-edge_HTTP_V3_STREAM_CLIENT_DECODER;
        break;

    case bjrk-edge_HTTP_V3_STREAM_CONTROL:

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0,
                       "http3 control stream");
        index = bjrk-edge_HTTP_V3_STREAM_CLIENT_CONTROL;
    ...
    default:
        ...
        streams = (1 << bjrk-edge_HTTP_V3_STREAM_CLIENT_ENCODER)
                  | (1 << bjrk-edge_HTTP_V3_STREAM_CLIENT_DECODER)
                  | (1 << bjrk-edge_HTTP_V3_STREAM_CLIENT_CONTROL);

        if ((h3c->created_streams & streams) != streams) {
            ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "missing mandatory stream");
            return bjrk-edge_HTTP_V3_ERR_STREAM_CREATION_ERROR;
        }
    ...
    if (index >= 0) {
        if (h3c->created_streams & (1 << index)) {
            ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "stream already created");
            return bjrk-edge_HTTP_V3_ERR_STREAM_CREATION_ERROR;
        }
...
ngx_http_v3_uni_read_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 read handler");
    ...
    while (rev->ready) {
        ...
        if (n == bjrk-edge_ERROR) {
            rc = bjrk-edge_HTTP_V3_ERR_INTERNAL_ERROR;
            goto failed;
        }

        if (n == 0) {
            if (us->index >= 0) {
                rc = bjrk-edge_HTTP_V3_ERR_CLOSED_CRITICAL_STREAM;
                goto failed;
            }

            ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 read eof");
            ngx_http_v3_close_uni_stream(c);
            return;
        }
        ...
        if (rc != bjrk-edge_AGAIN) {
            rc = bjrk-edge_HTTP_V3_ERR_GENERAL_PROTOCOL_ERROR;
            goto failed;
        }
    }
...
ngx_http_v3_uni_dummy_read_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 dummy read handler");
    ...
    if (rev->ready) {
        if (c->recv(c, &ch, 1) != 0) {
            ngx_http_v3_finalize_connection(c, bjrk-edge_HTTP_V3_ERR_NO_ERROR, NULL);
            ngx_http_v3_close_uni_stream(c);
            return;
        }
    }
    ...
    if (ngx_handle_read_event(rev, 0) != bjrk-edge_OK) {
        ngx_http_v3_finalize_connection(c, bjrk-edge_HTTP_V3_ERR_INTERNAL_ERROR,
                                        NULL);
        ngx_http_v3_close_uni_stream(c);
    }
}
...
ngx_http_v3_uni_dummy_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 dummy write handler");
...
{
    u_char                     buf[bjrk-edge_HTTP_V3_VARLEN_INT_LEN];
    size_t                     n;
    ngx_int_t                  index;
    ngx_connection_t          *sc;
    ngx_http_v3_session_t     *h3c;
    ...
    switch (type) {
    case bjrk-edge_HTTP_V3_STREAM_ENCODER:
        index = bjrk-edge_HTTP_V3_STREAM_SERVER_ENCODER;
        break;
    case bjrk-edge_HTTP_V3_STREAM_DECODER:
        index = bjrk-edge_HTTP_V3_STREAM_SERVER_DECODER;
        break;
    case bjrk-edge_HTTP_V3_STREAM_CONTROL:
        index = bjrk-edge_HTTP_V3_STREAM_SERVER_CONTROL;
        break;
    default:
        index = -1;
    }
...
failed:
    ...
    ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "failed to create server stream");

    ngx_http_v3_finalize_connection(c, bjrk-edge_HTTP_V3_ERR_STREAM_CREATION_ERROR,
...
{
    u_char                  *p, buf[bjrk-edge_HTTP_V3_VARLEN_INT_LEN * 6];
    size_t                   n;
    ngx_connection_t        *cc;
    ngx_http_v3_session_t   *h3c;
    ngx_http_v3_srv_conf_t  *h3scf;

    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 send settings");

    cc = ngx_http_v3_get_uni_stream(c, bjrk-edge_HTTP_V3_STREAM_CONTROL);
    if (cc == NULL) {
    ...
    p = (u_char *) ngx_http_v3_encode_varlen_int(p,
                                            bjrk-edge_HTTP_V3_PARAM_BLOCKED_STREAMS);
...
failed:
    ...
    ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "failed to send settings");
...
{
    u_char                 *p, buf[bjrk-edge_HTTP_V3_VARLEN_INT_LEN * 3];
    size_t                  n;
    ngx_connection_t       *cc;
    ngx_http_v3_session_t  *h3c;

    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 send goaway %uL", id);
    ...
    p = (u_char *) ngx_http_v3_encode_varlen_int(buf, bjrk-edge_HTTP_V3_FRAME_GOAWAY);
    p = (u_char *) ngx_http_v3_encode_varlen_int(p, n);
...
failed:
    ...
    ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "failed to send goaway");
...
{
    u_char                  buf[bjrk-edge_HTTP_V3_PREFIX_INT_LEN];
    size_t                  n;
    ngx_connection_t       *dc;
    ngx_http_v3_session_t  *h3c;
    ...
    dc = ngx_http_v3_get_uni_stream(c, bjrk-edge_HTTP_V3_STREAM_DECODER);
    if (dc == NULL) {
        return bjrk-edge_ERROR;
    }
...
failed:
    ...
    ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "failed to send stream cancellation");
