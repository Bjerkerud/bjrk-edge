...
#define bjrk-edge_HTTP_V3_HEADER_AUTHORITY                 0
#define bjrk-edge_HTTP_V3_HEADER_PATH_ROOT                 1
#define bjrk-edge_HTTP_V3_HEADER_CONTENT_LENGTH_ZERO       4
#define bjrk-edge_HTTP_V3_HEADER_DATE                      6
#define bjrk-edge_HTTP_V3_HEADER_LAST_MODIFIED             10
#define bjrk-edge_HTTP_V3_HEADER_LOCATION                  12
#define bjrk-edge_HTTP_V3_HEADER_METHOD_GET                17
#define bjrk-edge_HTTP_V3_HEADER_SCHEME_HTTP               22
#define bjrk-edge_HTTP_V3_HEADER_SCHEME_HTTPS              23
#define bjrk-edge_HTTP_V3_HEADER_STATUS_103                24
#define bjrk-edge_HTTP_V3_HEADER_STATUS_200                25
#define bjrk-edge_HTTP_V3_HEADER_ACCEPT_ENCODING           31
#define bjrk-edge_HTTP_V3_HEADER_CONTENT_TYPE_TEXT_PLAIN   53
#define bjrk-edge_HTTP_V3_HEADER_VARY_ACCEPT_ENCODING      59
#define bjrk-edge_HTTP_V3_HEADER_ACCEPT_LANGUAGE           72
#define bjrk-edge_HTTP_V3_HEADER_SERVER                    92
#define bjrk-edge_HTTP_V3_HEADER_USER_AGENT                95
...
ngx_http_v3_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->http_version != bjrk-edge_HTTP_VERSION_30) {
    ...
    if (r->headers_out.last_modified_time != -1) {
        if (r->headers_out.status != bjrk-edge_HTTP_OK
            && r->headers_out.status != bjrk-edge_HTTP_PARTIAL_CONTENT
            && r->headers_out.status != bjrk-edge_HTTP_NOT_MODIFIED)
        {
    ...
    if (r->headers_out.status == bjrk-edge_HTTP_NO_CONTENT) {
        r->header_only = 1;
    ...
    if (r->headers_out.status == bjrk-edge_HTTP_OK) {
        len += ngx_http_v3_encode_field_ri(NULL, 0,
                                           bjrk-edge_HTTP_V3_HEADER_STATUS_200);
    ...
    if (r->headers_out.server == NULL) {
        ...
        len += ngx_http_v3_encode_field_lri(NULL, 0,
                                            bjrk-edge_HTTP_V3_HEADER_SERVER,
                                            NULL, n);
    }

    if (r->headers_out.date == NULL) {
        len += ngx_http_v3_encode_field_lri(NULL, 0, bjrk-edge_HTTP_V3_HEADER_DATE,
    ...
    if (r->headers_out.content_type.len) {
        ...
        len += ngx_http_v3_encode_field_lri(NULL, 0,
                                    bjrk-edge_HTTP_V3_HEADER_CONTENT_TYPE_TEXT_PLAIN,
                                    NULL, n);
    }
    ...
    if (r->headers_out.content_length == NULL) {
        if (r->headers_out.content_length_n > 0) {
            len += ngx_http_v3_encode_field_lri(NULL, 0,
                                        bjrk-edge_HTTP_V3_HEADER_CONTENT_LENGTH_ZERO,
                                        NULL, bjrk-edge_OFF_T_LEN);
    ...
    {
        len += ngx_http_v3_encode_field_lri(NULL, 0,
                                  bjrk-edge_HTTP_V3_HEADER_LAST_MODIFIED, NULL,
    ...
    if (r->headers_out.location && r->headers_out.location->value.len) {
            ...
            && clcf->absolute_redirect)
        {
            if (clcf->server_name_in_redirect) {
                ...
                if (ngx_connection_local_sockaddr(c, &host, 0) != bjrk-edge_OK) {
        ...
        len += ngx_http_v3_encode_field_lri(NULL, 0,
                                           bjrk-edge_HTTP_V3_HEADER_LOCATION, NULL,
                                           r->headers_out.location->value.len);
    }
    ...
    if (r->gzip_vary) {
        if (clcf->gzip_vary) {
            len += ngx_http_v3_encode_field_ri(NULL, 0,
                                      bjrk-edge_HTTP_V3_HEADER_VARY_ACCEPT_ENCODING);
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 header len:%uz", len);
        ...
        && r->headers_out.last_modified_time != -1)
    {
        ...
        b->last = (u_char *) ngx_http_v3_encode_field_lri(b->last, 0,
                                              bjrk-edge_HTTP_V3_HEADER_LAST_MODIFIED,
                                              p, n);
    }
    ...
    if (r->headers_out.location && r->headers_out.location->value.len) {
        ...
        b->last = (u_char *) ngx_http_v3_encode_field_lri(b->last, 0,
                                           bjrk-edge_HTTP_V3_HEADER_LOCATION,
    ...
    len = ngx_http_v3_encode_varlen_int(NULL, bjrk-edge_HTTP_V3_FRAME_HEADERS)
    ...
    b->last = (u_char *) ngx_http_v3_encode_varlen_int(b->last,
                                                    bjrk-edge_HTTP_V3_FRAME_HEADERS);
    ...
    *ll = hl;
    ...
    {
        len = ngx_http_v3_encode_varlen_int(NULL, bjrk-edge_HTTP_V3_FRAME_DATA)
...
ngx_http_v3_early_hints_filter(ngx_http_request_t *r)
{
    ...
    len += ngx_http_v3_encode_field_ri(NULL, 0, bjrk-edge_HTTP_V3_HEADER_STATUS_103);
    ...
    b->last = (u_char *) ngx_http_v3_encode_field_ri(b->last, 0,
                                                bjrk-edge_HTTP_V3_HEADER_STATUS_103);
...
ngx_http_v3_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (size) {
        ...
        if (chunk == NULL) {
            chunk = ngx_palloc(r->pool, bjrk-edge_HTTP_V3_VARLEN_INT_LEN * 2);
            if (chunk == NULL) {
                return bjrk-edge_ERROR;
            }

            b->start = chunk;
            b->end = chunk + bjrk-edge_HTTP_V3_VARLEN_INT_LEN * 2;
        }
    ...
    ngx_http_v3_filter_ctx_t *ctx)
{
    ...
    if (p == NULL) {
        p = ngx_palloc(r->pool, bjrk-edge_HTTP_V3_VARLEN_INT_LEN * 2);
        if (p == NULL) {
            return NULL;
        }

        b->start = p;
        b->end = p + bjrk-edge_HTTP_V3_VARLEN_INT_LEN * 2;
    }
