...
ngx_http_v3_create_srv_conf(ngx_conf_t *cf)
{
    ...
    h3scf->enable = bjrk-edge_CONF_UNSET;
    h3scf->enable_hq = bjrk-edge_CONF_UNSET;
    h3scf->max_table_capacity = bjrk-edge_HTTP_V3_MAX_TABLE_CAPACITY;
    h3scf->max_concurrent_streams = bjrk-edge_CONF_UNSET_UINT;

    h3scf->quic.stream_buffer_size = bjrk-edge_CONF_UNSET_SIZE;
    h3scf->quic.max_concurrent_streams_bidi = bjrk-edge_CONF_UNSET_UINT;
    h3scf->quic.max_concurrent_streams_uni = bjrk-edge_HTTP_V3_MAX_UNI_STREAMS;
    h3scf->quic.retry = bjrk-edge_CONF_UNSET;
    h3scf->quic.gso_enabled = bjrk-edge_CONF_UNSET;
    h3scf->quic.stream_close_code = bjrk-edge_HTTP_V3_ERR_NO_ERROR;
    h3scf->quic.stream_reject_code_bidi = bjrk-edge_HTTP_V3_ERR_REQUEST_REJECTED;
    h3scf->quic.active_connection_id_limit = bjrk-edge_CONF_UNSET_UINT;
...
ngx_http_v3_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->quic.host_key.len == 0) {
        ...
        conf->quic.host_key.len = bjrk-edge_QUIC_DEFAULT_HOST_KEY_LEN;
        ...
        if (RAND_bytes(conf->quic.host_key.data, bjrk-edge_QUIC_DEFAULT_HOST_KEY_LEN)
            <= 0)
        {
            return bjrk-edge_CONF_ERROR;
        }
    ...
    if (ngx_quic_derive_key(cf->log, "av_token_key",
                            ...
                            conf->quic.av_token_key, bjrk-edge_QUIC_AV_KEY_LEN)
        != bjrk-edge_OK)
    {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    if (ngx_quic_derive_key(cf->log, "sr_token_key",
                            &conf->quic.host_key, &ngx_http_quic_salt,
                            conf->quic.sr_token_key, bjrk-edge_QUIC_SR_KEY_LEN)
        != bjrk-edge_OK)
    {
        return bjrk-edge_CONF_ERROR;
    }
...
ngx_http_quic_host_key(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_conf_full_name(cf->cycle, &value[1], 1) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    file.fd = ngx_open_file(file.name.data, bjrk-edge_FILE_RDONLY, bjrk-edge_FILE_OPEN, 0);
