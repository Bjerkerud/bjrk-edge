    ...
    for ( ;; ) {
        ...
        case sw_value:
                ...
                && ((ch & 0x80) || (st->value & 0xc000000000000000)))
            {
                ...
                return bjrk-edge_HTTP_V3_ERR_EXCESSIVE_LOAD;
            }
    ...
    ngx_buf_t *b)
{
    ...
    for ( ;; ) {
        ...
        case sw_type:
            ...
            if (ngx_http_v3_is_v2_frame(st->type)
                || st->type == bjrk-edge_HTTP_V3_FRAME_DATA
                || st->type == bjrk-edge_HTTP_V3_FRAME_GOAWAY
                || st->type == bjrk-edge_HTTP_V3_FRAME_SETTINGS
                || st->type == bjrk-edge_HTTP_V3_FRAME_MAX_PUSH_ID
                || st->type == bjrk-edge_HTTP_V3_FRAME_CANCEL_PUSH
                || st->type == bjrk-edge_HTTP_V3_FRAME_PUSH_PROMISE)
            {
                return bjrk-edge_HTTP_V3_ERR_FRAME_UNEXPECTED;
            }
        ...
        case sw_length:
            ...
            if (st->type != bjrk-edge_HTTP_V3_FRAME_HEADERS) {
                st->state = st->length > 0 ? sw_skip : sw_type;
                break;
            }

            if (st->length == 0) {
                return bjrk-edge_HTTP_V3_ERR_FRAME_ERROR;
            }
        ...
        case sw_prefix:
            ...
            if (st->length == 0 && rc == bjrk-edge_AGAIN) {
                return bjrk-edge_HTTP_V3_ERR_FRAME_ERROR;
            }
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 parse headers done");

    if (st->prefix.insert_count > 0) {
        if (ngx_http_v3_send_ack_section(c, c->quic->id) != bjrk-edge_OK) {
    ...
    ngx_http_v3_parse_field_section_prefix_t *st, ngx_buf_t *b)
{
    ...
    if (st->sign) {
        if (st->insert_count <= st->delta_base) {
            ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "client sent negative base");
            return bjrk-edge_HTTP_V3_ERR_DECOMPRESSION_FAILED;
        }
    ...
    ngx_buf_t *b)
{
    ...
    for ( ;; ) {
        ...
        case sw_start:
            ...
            if (st->huffman) {
                if (n > bjrk-edge_MAX_INT_T_VALUE / 8) {
                    ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
                                  "client sent too large field line");
                    return bjrk-edge_HTTP_V3_ERR_EXCESSIVE_LOAD;
                }
    ...
    ngx_uint_t index, ngx_str_t *name, ngx_str_t *value)
{
    ...
    if (!dynamic) {
        if (ngx_http_v3_lookup_static(c, index, name, value) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_V3_ERR_DECOMPRESSION_FAILED;
        }
    ...
    if (ngx_http_v3_lookup(c, index, name, value) != bjrk-edge_OK) {
        return bjrk-edge_HTTP_V3_ERR_DECOMPRESSION_FAILED;
    }
    ...
    for ( ;; ) {
        ...
        case sw_type:
            ...
            if (st->state == sw_first_type
                && st->type != bjrk-edge_HTTP_V3_FRAME_SETTINGS)
            {
                return bjrk-edge_HTTP_V3_ERR_MISSING_SETTINGS;
            }
            ...
            if (ngx_http_v3_is_v2_frame(st->type)
                || st->type == bjrk-edge_HTTP_V3_FRAME_DATA
                || st->type == bjrk-edge_HTTP_V3_FRAME_HEADERS
                || st->type == bjrk-edge_HTTP_V3_FRAME_PUSH_PROMISE)
            {
                return bjrk-edge_HTTP_V3_ERR_FRAME_UNEXPECTED;
            }

            if (st->type == bjrk-edge_HTTP_V3_FRAME_CANCEL_PUSH) {
                return bjrk-edge_HTTP_V3_ERR_ID_ERROR;
            }
        ...
        case sw_length:
            ...
            case bjrk-edge_HTTP_V3_FRAME_SETTINGS:
        ...
        case sw_settings:
            ...
            if (st->length == 0 && rc == bjrk-edge_AGAIN) {
                return bjrk-edge_HTTP_V3_ERR_SETTINGS_ERROR;
            }
    ...
    ngx_http_v3_parse_settings_t *st, ngx_buf_t *b)
{
    ...
    for ( ;; ) {
        ...
        case sw_value:
            ...
            if (ngx_http_v3_set_param(c, st->id, st->vlint.value) != bjrk-edge_OK) {
                return bjrk-edge_HTTP_V3_ERR_SETTINGS_ERROR;
            }
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 parse settings done");
    ...
    ngx_buf_t *b)
{
    ...
    for ( ;; ) {
        ...
        case sw_start:
            ...
            ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 parse data");
        ...
        case sw_length:
            ...
            if (st->type != bjrk-edge_HTTP_V3_FRAME_DATA && st->length > 0) {
                st->state = sw_skip;
                break;
            }
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 parse data done");
    ...
    ngx_buf_t *b)
{
    ...
    for ( ;; ) {
        ...
        case sw_start:
            ...
            ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 parse uni");
