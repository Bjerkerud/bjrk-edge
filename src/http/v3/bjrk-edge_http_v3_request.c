...
static const struct {
    ...
    { ngx_string("OPTIONS"),   bjrk-edge_HTTP_OPTIONS },
    { ngx_string("PROPFIND"),  bjrk-edge_HTTP_PROPFIND },
    ...
    { ngx_string("PROPPATCH"), bjrk-edge_HTTP_PROPPATCH },
    { ngx_string("LOCK"),      bjrk-edge_HTTP_LOCK },
    { ngx_string("UNLOCK"),    bjrk-edge_HTTP_UNLOCK },
    { ngx_string("PATCH"),     bjrk-edge_HTTP_PATCH },
    { ngx_string("TRACE"),     bjrk-edge_HTTP_TRACE },
    { ngx_string("CONNECT"),   bjrk-edge_HTTP_CONNECT }
};
...
ngx_http_v3_init_stream(ngx_connection_t *c)
{
    ...
    if (c->quic->id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL) {
...
ngx_http_v3_init(ngx_connection_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 init");

    if (ngx_http_v3_init_session(c) != bjrk-edge_OK) {
    ...
    if (h3scf->enable_hq) {
        ...
        if (len == sizeof(bjrk-edge_HTTP_V3_HQ_PROTO) - 1
            && ngx_strncmp(data, bjrk-edge_HTTP_V3_HQ_PROTO, len) == 0)
        {
            h3c->hq = 1;
            return bjrk-edge_OK;
        }
    ...
    if (ngx_http_v3_send_settings(c) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }

    if (h3scf->max_table_capacity > 0) {
        if (ngx_http_v3_get_uni_stream(c, bjrk-edge_HTTP_V3_STREAM_DECODER) == NULL) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_http_v3_shutdown(ngx_connection_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 shutdown");

    h3c = ngx_http_v3_get_session(c);

    if (h3c == NULL) {
        ngx_quic_finalize_connection(c, bjrk-edge_HTTP_V3_ERR_NO_ERROR,
                                     "connection shutdown");
        return;
    }
...
ngx_http_v3_init_request_stream(ngx_connection_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 init request stream");
    ...
    if (n >= clcf->keepalive_requests * 2) {
        ngx_http_v3_finalize_connection(c, bjrk-edge_HTTP_V3_ERR_EXCESSIVE_LOAD,
        ...
        || ngx_current_msec - c->start_time > clcf->keepalive_time)
    {
        ...
        if (!h3c->hq) {
            if (ngx_http_v3_send_goaway(c, h3c->next_request_id) != bjrk-edge_OK) {
                ngx_http_close_connection(c);
                return;
            }
        }
...
ngx_http_v3_wait_request_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, c->log, 0, "http3 wait request handler");
    ...
    r->http_version = bjrk-edge_HTTP_VERSION_30;
...
ngx_http_v3_reset_stream(ngx_connection_t *c)
{
    ...
    if (!c->read->eof && !h3c->hq
        && h3c->known_streams[bjrk-edge_HTTP_V3_STREAM_SERVER_DECODER]
        && (c->quic->id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL) == 0)
    {
        (void) ngx_http_v3_send_cancel_stream(c, c->quic->id);
    }

    if (c->timedout) {
        ngx_quic_reset_stream(c, bjrk-edge_HTTP_V3_ERR_GENERAL_PROTOCOL_ERROR);

    } else if (c->close) {
        ngx_quic_reset_stream(c, bjrk-edge_HTTP_V3_ERR_REQUEST_REJECTED);

    } else if (c->requests == 0 || c->error) {
        ngx_quic_reset_stream(c, bjrk-edge_HTTP_V3_ERR_INTERNAL_ERROR);
    }
}
...
ngx_http_v3_process_request(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, rev->log, 0, "http3 process request");
    ...
    for ( ;; ) {
        ...
        if (ngx_http_v3_check_flood(c) != bjrk-edge_OK) {
            ngx_http_close_request(r, bjrk-edge_HTTP_CLOSE);
            break;
        }
        ...
        /* rc == bjrk-edge_OK || rc == bjrk-edge_DONE */
        ...
        if (rc == bjrk-edge_DONE) {
            if (ngx_http_v3_process_request_header(r) != bjrk-edge_OK) {
                break;
            }
    ...
    ngx_str_t *value)
{
    ...
    if (ngx_http_v3_validate_header(r, name, value) != bjrk-edge_OK) {
        ngx_http_finalize_request(r, bjrk-edge_HTTP_BAD_REQUEST);
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_http_v3_init_pseudo_headers(r) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    {
        if (ngx_http_v3_cookie(r, value) != bjrk-edge_OK) {
            ngx_http_close_request(r, bjrk-edge_HTTP_INTERNAL_SERVER_ERROR);
            return bjrk-edge_ERROR;
        }
...
ngx_http_v3_process_request_header(ngx_http_request_t *r)
{
    ...
    if (ngx_http_v3_construct_cookie_header(r) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    if (r->method == bjrk-edge_HTTP_CONNECT) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "client sent CONNECT method");
        ngx_http_finalize_request(r, bjrk-edge_HTTP_NOT_ALLOWED);
        return bjrk-edge_ERROR;
    }

    if (r->method == bjrk-edge_HTTP_TRACE) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "client sent TRACE method");
        ngx_http_finalize_request(r, bjrk-edge_HTTP_NOT_ALLOWED);
        return bjrk-edge_ERROR;
    }
    ...
    for (cl = in; cl; cl = cl->next) {
        ...
        while (cl->buf->pos < cl->buf->last) {
            ...
            if (st->length == 0) {
                ...
                if (ngx_http_v3_check_flood(r->connection) != bjrk-edge_OK) {
                    return bjrk-edge_HTTP_CLOSE;
                }
