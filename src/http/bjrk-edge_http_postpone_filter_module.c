...
ngx_http_postpone_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (r != c->data) {
        ...
        if (in) {
            if (ngx_http_postpone_filter_add(r, in) != bjrk-edge_OK) {
    ...
    do {
        ...
        if (pr->out == NULL) {
            ...
            if (ngx_http_next_body_filter(r->main, pr->out) == bjrk-edge_ERROR) {
                return bjrk-edge_ERROR;
            }
        }
...
found:
    ...
    if (ngx_chain_add_copy(r->pool, &pr->out, in) == bjrk-edge_OK) {
        return bjrk-edge_OK;
    }
