...
static ngx_http_variable_t  ngx_http_core_variables[] = {
    ...
    { ngx_string("document_root"), NULL,
      ngx_http_variable_document_root, 0, bjrk-edge_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("realpath_root"), NULL,
      ngx_http_variable_realpath_root, 0, bjrk-edge_HTTP_VAR_NOCACHEABLE, 0 },
    ...
    { ngx_string("is_args"), NULL, ngx_http_variable_is_args,
      0, bjrk-edge_HTTP_VAR_NOCACHEABLE, 0 },
...
ngx_http_add_variable(ngx_conf_t *cf, ngx_str_t *name, ngx_uint_t flags)
{
    ...
    if (flags & bjrk-edge_HTTP_VAR_PREFIX) {
    ...
    for (i = 0; i < cmcf->variables_keys->keys.nelts; i++) {
        ...
        if (!(v->flags & bjrk-edge_HTTP_VAR_CHANGEABLE)) {
            ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
                               "the duplicate \"%V\" variable", name);
            return NULL;
        }

        if (!(flags & bjrk-edge_HTTP_VAR_WEAK)) {
            v->flags &= ~bjrk-edge_HTTP_VAR_WEAK;
        }
...
ngx_http_get_indexed_variable(ngx_http_request_t *r, ngx_uint_t index)
{
        ...
        == bjrk-edge_OK)
    {
        ...
        if (v[index].flags & bjrk-edge_HTTP_VAR_NOCACHEABLE) {
...
ngx_http_get_variable(ngx_http_request_t *r, ngx_str_t *name, ngx_uint_t key)
{
    ...
    if (v) {
        if (v->flags & bjrk-edge_HTTP_VAR_INDEXED) {
        ...
        if (vv && v->get_handler(r, vv, v->data) == bjrk-edge_OK) {
            ngx_http_variable_depth++;
            return vv;
        }
    ...
    if (n != cmcf->prefix_variables.nelts) {
        if (v[n].get_handler(r, vv, (uintptr_t) name) == bjrk-edge_OK) {
            return vv;
        }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(r->pool, bjrk-edge_SIZE_T_LEN);
    if (v->data == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    uintptr_t data)
{
    ...
    if (len == 0 || ngx_http_arg(r, arg, len, &value) != bjrk-edge_OK) {
        v->not_found = 1;
        return bjrk-edge_OK;
    }
    ...
    uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(r->pool, bjrk-edge_INT32_LEN);
    if (v->data == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (r->headers_in.content_length) {
        ...
        p = ngx_pnalloc(r->pool, bjrk-edge_OFF_T_LEN);
        if (p == NULL) {
            return bjrk-edge_ERROR;
        }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    s.len = bjrk-edge_SOCKADDR_STRLEN;
    s.data = addr;

    if (ngx_connection_local_sockaddr(r->connection, &s, 0) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_connection_local_sockaddr(r->connection, NULL, 0) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    ngx_str_t                  path;
    ngx_http_core_loc_conf_t  *clcf;
#if (bjrk-edge_HAVE_MAX_PATH)
    u_char                     buffer[bjrk-edge_MAX_PATH];
#endif
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(r->pool, bjrk-edge_INT_T_LEN);
    if (v->data == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (r->headers_out.status == bjrk-edge_HTTP_SWITCHING_PROTOCOLS) {
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (r->keepalive) {
        ...
        if (clcf->keepalive_header) {
            ...
            p = ngx_pnalloc(r->pool, sizeof("timeout=") - 1 + bjrk-edge_TIME_T_LEN);
            if (p == NULL) {
                return bjrk-edge_ERROR;
            }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(r->pool, bjrk-edge_TIME_T_LEN + 4);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(r->pool, bjrk-edge_ATOMIC_T_LEN);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(r->pool, bjrk-edge_INT_T_LEN);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    p = ngx_pnalloc(r->pool, bjrk-edge_INT64_LEN);
    if (p == NULL) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_regex_compile(ngx_conf_t *cf, ngx_regex_compile_t *rc)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        v = ngx_http_add_variable(cf, &name, bjrk-edge_HTTP_VAR_CHANGEABLE);
        if (v == NULL) {
            return NULL;
        }

        rv[i].index = ngx_http_get_variable_index(cf, &name);
        if (rv[i].index == bjrk-edge_ERROR) {
            return NULL;
        }
...
ngx_http_variables_add_core_vars(ngx_conf_t *cf)
{
    ...
    if (ngx_hash_keys_array_init(cmcf->variables_keys, bjrk-edge_HASH_SMALL)
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    }
...
ngx_http_variables_init_vars(ngx_conf_t *cf)
{
    ...
    for (i = 0; i < cmcf->variables.nelts; i++) {
        ...
        for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
                   ...
                   == 0)
            {
                ...
                av->flags |= bjrk-edge_HTTP_VAR_INDEXED;
                v[i].flags = av->flags;

                av->index = i;

                if (av->get_handler == NULL
                    || (av->flags & bjrk-edge_HTTP_VAR_WEAK))
                {
                    break;
                }
    ...
    for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
        ...
        if (av->flags & bjrk-edge_HTTP_VAR_NOHASH) {
            key[n].key.data = NULL;
        }
    }
