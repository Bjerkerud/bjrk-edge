...
ngx_http_auth_request_handler(ngx_http_request_t *r)
{
    ...
    if (ctx != NULL) {
        ...
        if (ngx_http_auth_request_set_variables(r, arcf, ctx) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }

        /* return appropriate status */

        if (ctx->status == bjrk-edge_HTTP_FORBIDDEN) {
            return ctx->status;
        }

        if (ctx->status == bjrk-edge_HTTP_UNAUTHORIZED) {
            sr = ctx->subrequest;
        ...
        if (ctx->status >= bjrk-edge_HTTP_OK
            && ctx->status < bjrk-edge_HTTP_SPECIAL_RESPONSE)
        {
            return bjrk-edge_OK;
        }
    ...
    if (ngx_http_subrequest(r, &arcf->uri, NULL, &sr, ps,
                            bjrk-edge_HTTP_SUBREQUEST_WAITED)
        != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    }
...
ngx_http_auth_request_create_conf(ngx_conf_t *cf)
{
    ...
    conf->vars = bjrk-edge_CONF_UNSET_PTR;
...
ngx_http_auth_request_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (arcf->vars == bjrk-edge_CONF_UNSET_PTR) {
