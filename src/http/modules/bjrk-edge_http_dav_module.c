...
#define bjrk-edge_HTTP_DAV_OFF             2


#define bjrk-edge_HTTP_DAV_NO_DEPTH        -3
#define bjrk-edge_HTTP_DAV_INVALID_DEPTH   -2
#define bjrk-edge_HTTP_DAV_INFINITY_DEPTH  -1
...
static ngx_conf_bitmask_t  ngx_http_dav_methods_mask[] = {
    { ngx_string("off"), bjrk-edge_HTTP_DAV_OFF },
    { ngx_string("put"), bjrk-edge_HTTP_PUT },
    { ngx_string("delete"), bjrk-edge_HTTP_DELETE },
    { ngx_string("mkcol"), bjrk-edge_HTTP_MKCOL },
    { ngx_string("copy"), bjrk-edge_HTTP_COPY },
    { ngx_string("move"), bjrk-edge_HTTP_MOVE },
    { ngx_null_string, 0 }
};
...
ngx_http_dav_put_handler(ngx_http_request_t *r)
{
    ...
    if (ngx_file_info(path.data, &fi) == bjrk-edge_FILE_ERROR) {
        status = bjrk-edge_HTTP_CREATED;

    } else {
        status = bjrk-edge_HTTP_NO_CONTENT;

        if (ngx_is_dir(&fi)) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, bjrk-edge_EISDIR,
                          "\"%s\" could not be created", path.data);

            if (ngx_delete_file(temp->data) == bjrk-edge_FILE_ERROR) {
                ngx_log_error(bjrk-edge_LOG_CRIT, r->connection->log, ngx_errno,
                              ngx_delete_file_n " \"%s\" failed",
                              temp->data);
            }

            ngx_http_finalize_request(r, bjrk-edge_HTTP_CONFLICT);
            return;
        }
    }
    ...
    if (ngx_ext_rename_file(temp, &path, &ext) != bjrk-edge_OK) {
        ngx_http_finalize_request(r, bjrk-edge_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }

    if (status == bjrk-edge_HTTP_CREATED) {
        if (ngx_http_dav_location(r) != bjrk-edge_OK) {
            ngx_http_finalize_request(r, bjrk-edge_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
...
ngx_http_dav_delete_handler(ngx_http_request_t *r)
{
    ...
    if (r->headers_in.content_length_n > 0 || r->headers_in.chunked) {
        ...
        return bjrk-edge_HTTP_UNSUPPORTED_MEDIA_TYPE;
    }
...
ok:
    ...
    if (ngx_link_info(path.data, &fi) == bjrk-edge_FILE_ERROR) {
        err = ngx_errno;

        rc = (err == bjrk-edge_ENOTDIR) ? bjrk-edge_HTTP_CONFLICT : bjrk-edge_HTTP_NOT_FOUND;
    ...
    if (ngx_is_dir(&fi)) {
        ...
        depth = ngx_http_dav_depth(r, bjrk-edge_HTTP_DAV_INFINITY_DEPTH);

        if (depth != bjrk-edge_HTTP_DAV_INFINITY_DEPTH) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
         ...
         * because ngx_link_info("/file/") returned bjrk-edge_ENOTDIR above
         */

        depth = ngx_http_dav_depth(r, 0);

        if (depth != 0 && depth != bjrk-edge_HTTP_DAV_INFINITY_DEPTH) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
    ...
    if (rc == bjrk-edge_OK) {
        return bjrk-edge_HTTP_NO_CONTENT;
    }
...
ngx_http_dav_delete_path(ngx_http_request_t *r, ngx_str_t *path, ngx_uint_t dir)
{
    ...
    if (dir) {
        ...
        if (ngx_walk_tree(&tree, path) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        }

        if (ngx_delete_dir(path->data) != bjrk-edge_FILE_ERROR) {
            return bjrk-edge_OK;
        }
    ...
    return ngx_http_dav_error(r->connection->log, ngx_errno,
                              bjrk-edge_HTTP_NOT_FOUND, failed, path->data);
}
...
ngx_http_dav_mkcol_handler(ngx_http_request_t *r, ngx_http_dav_loc_conf_t *dlcf)
{
    ...
    *(p - 1) = '\0';
        ...
        != bjrk-edge_FILE_ERROR)
    {
        ...
        return bjrk-edge_HTTP_CREATED;
    }

    return ngx_http_dav_error(r->connection->log, ngx_errno,
                              bjrk-edge_HTTP_CONFLICT, ngx_create_dir_n, path.data);
}
...
destination_done:
    ...
    if (ngx_http_parse_unsafe_uri(r, &duri, &args, &flags) != bjrk-edge_OK) {
        goto invalid_destination;
    }
    ...
    if (depth != bjrk-edge_HTTP_DAV_INFINITY_DEPTH) {
        ...
        if (r->method == bjrk-edge_HTTP_COPY) {
            if (depth != 0) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                              "\"Depth\" header must be 0 or infinity");
                return bjrk-edge_HTTP_BAD_REQUEST;
...
overwrite_done:
    ...
    if (ngx_http_dav_validate_paths(r, &path, &copy.path, slash, dest)
        != bjrk-edge_OK)
    {
        return bjrk-edge_HTTP_FORBIDDEN;
    }

    if (ngx_link_info(copy.path.data, &fi) == bjrk-edge_FILE_ERROR) {
        err = ngx_errno;

        if (err != bjrk-edge_ENOENT) {
            return ngx_http_dav_error(r->connection->log, err,
                                      bjrk-edge_HTTP_NOT_FOUND, ngx_link_info_n,
                                      copy.path.data);
        }
        ...
        if (!overwrite) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, bjrk-edge_EEXIST,
                          "\"%s\" could not be created", copy.path.data);
            return bjrk-edge_HTTP_PRECONDITION_FAILED;
        }
    ...
    if (ngx_is_dir(&fi)) {
        ...
        if (r->method == bjrk-edge_HTTP_MOVE) {
            if (ngx_rename_file(path.data, copy.path.data) != bjrk-edge_FILE_ERROR) {
                return bjrk-edge_HTTP_CREATED;
            }
        }
        ...
        if (r->method == bjrk-edge_HTTP_MOVE) {
            ...
            if (ngx_ext_rename_file(&path, &copy.path, &ext) == bjrk-edge_OK) {
                return bjrk-edge_HTTP_NO_CONTENT;
            }
        ...
        if (ngx_copy_file(path.data, copy.path.data, &cf) == bjrk-edge_OK) {
            return bjrk-edge_HTTP_NO_CONTENT;
        }
    }
...
ngx_http_dav_copy_dir(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ...
    if (ngx_create_dir(dir, ngx_dir_access(ctx->access)) == bjrk-edge_FILE_ERROR) {
...
ngx_http_dav_copy_dir_time(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ...
    fd = ngx_open_file(dir, bjrk-edge_FILE_RDWR, bjrk-edge_FILE_OPEN, 0);
    ...
    if (ngx_set_file_time(NULL, fd, ctx->mtime) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_ALERT, ctx->log, ngx_errno,
...
failed:
    ...
    if (ngx_set_file_time(dir, 0, ctx->mtime) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_ALERT, ctx->log, ngx_errno,
                      ngx_set_file_time_n " \"%s\" failed", dir);
    }
...
ngx_http_dav_depth(ngx_http_request_t *r, ngx_int_t dflt)
{
    ...
    if (depth->value.len == 1) {
        ...
        {
            return bjrk-edge_HTTP_DAV_INFINITY_DEPTH;
        }
    }
    ...
    return bjrk-edge_HTTP_DAV_INVALID_DEPTH;
}
    ...
    char *failed, u_char *path)
{
    ...
    if (err == bjrk-edge_ENOENT || err == bjrk-edge_ENOTDIR || err == bjrk-edge_ENAMETOOLONG) {
        level = bjrk-edge_LOG_ERR;
        rc = not_found;

    } else if (err == bjrk-edge_EACCES || err == bjrk-edge_EPERM) {
        level = bjrk-edge_LOG_ERR;
        rc = bjrk-edge_HTTP_FORBIDDEN;

    } else if (err == bjrk-edge_EEXIST) {
        level = bjrk-edge_LOG_ERR;
        rc = bjrk-edge_HTTP_NOT_ALLOWED;

    } else if (err == bjrk-edge_ENOSPC) {
        level = bjrk-edge_LOG_CRIT;
        rc = bjrk-edge_HTTP_INSUFFICIENT_STORAGE;
...
ngx_http_dav_location(ngx_http_request_t *r)
{
    ...
    escape = 2 * ngx_escape_uri(NULL, r->uri.data, r->uri.len, bjrk-edge_ESCAPE_URI);
    ...
    if (escape) {
        ...
        ngx_escape_uri(p, r->uri.data, r->uri.len, bjrk-edge_ESCAPE_URI);
...
ngx_http_dav_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->min_delete_depth = bjrk-edge_CONF_UNSET_UINT;
    conf->access = bjrk-edge_CONF_UNSET_UINT;
    conf->create_full_put_path = bjrk-edge_CONF_UNSET;
...
ngx_http_dav_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_bitmask_value(conf->methods, prev->methods,
                         (bjrk-edge_CONF_BITMASK_SET|bjrk-edge_HTTP_DAV_OFF));
