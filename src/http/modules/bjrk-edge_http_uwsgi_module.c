...
static ngx_path_init_t ngx_http_uwsgi_temp_path = {
    ngx_string(bjrk-edge_HTTP_UWSGI_TEMP_PATH), { 1, 2, 0 }
};
...
ngx_http_uwsgi_handler(ngx_http_request_t *r)
{
    ...
    if (uwcf->uwsgi_lengths == NULL) {
        ...
        if (ngx_http_uwsgi_eval(r, uwcf) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
...
ngx_http_uwsgi_create_key(ngx_http_request_t *r)
{
    ...
    if (ngx_http_complex_value(r, &uwcf->cache_key, key) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_uwsgi_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->modifier1 = bjrk-edge_CONF_UNSET_UINT;
    conf->modifier2 = bjrk-edge_CONF_UNSET_UINT;
...
ngx_http_uwsgi_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (ngx_http_uwsgi_merge_ssl(cf, conf, prev) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    if (conf->ssl && ngx_http_uwsgi_set_ssl(cf, conf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
...
ngx_http_uwsgi_store(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (uwcf->upstream.store != bjrk-edge_CONF_UNSET) {
        return "is duplicate";
    }
...
ngx_http_uwsgi_cache(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (uwcf->upstream.cache != bjrk-edge_CONF_UNSET) {
        return "is duplicate";
    }
...
ngx_http_uwsgi_ssl_password_file(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (uwcf->upstream.ssl_passwords != bjrk-edge_CONF_UNSET_PTR) {
        return "is duplicate";
    }
...
ngx_http_uwsgi_set_ssl(ngx_conf_t *cf, ngx_http_uwsgi_loc_conf_t *uwcf)
{
    ...
    if (uwcf->upstream.ssl_verify) {
        ...
        if (ngx_ssl_crl(cf, uwcf->upstream.ssl, &uwcf->ssl_crl) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
