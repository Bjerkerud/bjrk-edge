...
static ngx_command_t  ngx_http_grpc_commands[] = {
    ...
    { ngx_string("grpc_pass"),
      bjrk-edge_HTTP_LOC_CONF|bjrk-edge_HTTP_LIF_CONF|bjrk-edge_CONF_TAKE1,
      ngx_http_grpc_pass,
...
ngx_http_grpc_handler(ngx_http_request_t *r)
{
    ...
    if (glcf->grpc_lengths == NULL) {
        ...
        if (ngx_http_grpc_eval(r, ctx, glcf) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
...
ngx_http_grpc_create_request(ngx_http_request_t *r)
{
    ...
    if (r->method == bjrk-edge_HTTP_GET || r->method == bjrk-edge_HTTP_POST) {
        len += 1;
        tmp_len = 0;

    } else {
        if (r->method_name.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                          "too long http2 method: \"%V\"", &r->method_name);
            return bjrk-edge_ERROR;
        }

        len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + r->method_name.len;
    ...
    if (r->valid_unparsed_uri) {
        ...
        escape = 2 * ngx_escape_uri(NULL, r->uri.data, r->uri.len,
                                    bjrk-edge_ESCAPE_URI);
        uri_len = r->uri.len + escape + sizeof("?") - 1 + r->args.len;
    }

    if (uri_len > bjrk-edge_HTTP_V2_MAX_FIELD) {
        ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                      "too long http2 URI");
        return bjrk-edge_ERROR;
    }

    len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + uri_len;
    ...
    if (glcf->host_value
        && ngx_http_complex_value(r, glcf->host_value, &host) != bjrk-edge_OK)
    {
        return bjrk-edge_ERROR;
    }
    ...
    if (host.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
        ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                      "too long http2 host: \"%V\"", &host);
        return bjrk-edge_ERROR;
    }

    len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + host.len;
    ...
    while (*(uintptr_t *) le.ip) {
        ...
        if (key_len > bjrk-edge_HTTP_V2_MAX_FIELD) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                          "too long http2 header name");
            return bjrk-edge_ERROR;
        }

        if (val_len > bjrk-edge_HTTP_V2_MAX_FIELD) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                          "too long http2 header value");
            return bjrk-edge_ERROR;
        }

        headers_len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + key_len
                         + bjrk-edge_HTTP_V2_INT_OCTETS + val_len;
    ...
    if (glcf->upstream.pass_request_headers) {
        ...
        for (i = 0; /* void */; i++) {
            ...
            if (header[i].key.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
            ...
            if (header[i].value.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
            ...
            len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + header[i].key.len
                     + bjrk-edge_HTTP_V2_INT_OCTETS + header[i].value.len;
    ...
    len += sizeof(ngx_http_grpc_frame_t)
           * (len / bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE);
    ...
    f->length_1 = 0;
    f->length_2 = 0;
    f->type = bjrk-edge_HTTP_V2_HEADERS_FRAME;
    f->flags = 0;
    f->stream_id_0 = 0;
    f->stream_id_1 = 0;
    f->stream_id_2 = 0;
    ...
    if (r->method == bjrk-edge_HTTP_GET) {
        *b->last++ = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_METHOD_GET_INDEX);

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "grpc header: \":method: GET\"");

    } else if (r->method == bjrk-edge_HTTP_POST) {
        *b->last++ = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_METHOD_POST_INDEX);

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "grpc header: \":method: POST\"");

    } else {
        *b->last++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_METHOD_INDEX);
...
#if (bjrk-edge_HTTP_SSL)
    if (u->ssl) {
        *b->last++ = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_SCHEME_HTTPS_INDEX);
...
#endif
    {
        *b->last++ = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_SCHEME_HTTP_INDEX);
    ...
    if (r->valid_unparsed_uri) {
        ...
        if (r->unparsed_uri.len == 1 && r->unparsed_uri.data[0] == '/') {
            *b->last++ = ngx_http_v2_indexed(bjrk-edge_HTTP_V2_PATH_ROOT_INDEX);

        } else {
            *b->last++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_PATH_INDEX);
    ...
    *b->last++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_AUTHORITY_INDEX);
    ...
    while (*(uintptr_t *) le.ip) {
        ...
        *b->last++ = 0;
        ...
        if (headers_end - b->last
            < (ssize_t) (bjrk-edge_HTTP_V2_INT_OCTETS + key_len))
        {
            ngx_log_error(bjrk-edge_LOG_ALERT, r->connection->log, 0,
                          "no buffer space in grpc create request");
            return bjrk-edge_ERROR;
        ...
        if (headers_end - b->last
            < (ssize_t) (bjrk-edge_HTTP_V2_INT_OCTETS + val_len))
        {
            ngx_log_error(bjrk-edge_LOG_ALERT, r->connection->log, 0,
                          "no buffer space in grpc create request");
            return bjrk-edge_ERROR;
...
#if (bjrk-edge_DEBUG)
        if (r->connection->log->log_level & bjrk-edge_LOG_DEBUG_HTTP) {
    ...
    if (len > bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE) {
        len = bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE;
        next = 1;
    ...
    while (next) {
        p += sizeof(ngx_http_grpc_frame_t) + bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE;
        len = b->last - p;
        ...
        f->length_1 = (u_char) ((len >> 8) & 0xff);
        f->length_2 = (u_char) (len & 0xff);
        f->type = bjrk-edge_HTTP_V2_CONTINUATION_FRAME;
        f->flags = 0;
        f->stream_id_0 = 0;
        f->stream_id_1 = 0;
        f->stream_id_2 = 0;
    ...
    f->flags |= bjrk-edge_HTTP_V2_END_HEADERS_FLAG;
    ...
    if (r->request_body_no_buffering) {
        ...
        if (body == NULL) {
            f = (ngx_http_grpc_frame_t *) headers_frame;
            f->flags |= bjrk-edge_HTTP_V2_END_STREAM_FLAG;
        }
...
ngx_http_grpc_body_output_filter(void *data, ngx_chain_t *in)
{
    ...
    if (in) {
        if (ngx_chain_add_copy(r->pool, &ctx->in, in) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
    ...
    while (in && limit > 0) {
        ...
        ngx_log_debug7(bjrk-edge_LOG_DEBUG_EVENT, r->connection->log, 0,
        ...
        do {
            ...
            *ll = cl;
            ...
            if (in->buf->in_file) {
                b->file_pos = file_pos;
                file_pos += ngx_min(bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE, limit);
                ...
                b->pos = pos;
                pos += ngx_min(bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE, limit);
            ...
            *ll = cl;
            ...
            f->length_1 = (u_char) ((len >> 8) & 0xff);
            f->length_2 = (u_char) (len & 0xff);
            f->type = bjrk-edge_HTTP_V2_DATA_FRAME;
            f->flags = 0;
            f->stream_id_0 = (u_char) ((ctx->id >> 24) & 0xff);
            f->stream_id_1 = (u_char) ((ctx->id >> 16) & 0xff);
            f->stream_id_2 = (u_char) ((ctx->id >> 8) & 0xff);
...
ngx_http_grpc_process_header(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        ...
        if (ctx->state < ngx_http_grpc_st_payload) {
            ...
            if (ctx->type == bjrk-edge_HTTP_V2_DATA_FRAME
                || (ctx->type == bjrk-edge_HTTP_V2_CONTINUATION_FRAME
                    && !ctx->parsing_headers)
                || (ctx->type != bjrk-edge_HTTP_V2_CONTINUATION_FRAME
                    && ctx->parsing_headers))
            {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_RST_STREAM_FRAME) {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_GOAWAY_FRAME) {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_WINDOW_UPDATE_FRAME) {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_SETTINGS_FRAME) {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_PING_FRAME) {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_PUSH_PROMISE_FRAME) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                          "upstream sent unexpected push promise frame");
            return bjrk-edge_HTTP_UPSTREAM_INVALID_HEADER;
        }

        if (ctx->type != bjrk-edge_HTTP_V2_HEADERS_FRAME
            && ctx->type != bjrk-edge_HTTP_V2_CONTINUATION_FRAME)
        {
        ...
        for ( ;; ) {
            ...
            if (rc == bjrk-edge_OK) {
                ...
                if (ctx->name.len && ctx->name.data[0] == ':') {
                    ...
                    if (status < bjrk-edge_HTTP_OK && status != bjrk-edge_HTTP_EARLY_HINTS)
                    {
                        ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                ...
                if (u->headers_in.status_n == bjrk-edge_HTTP_EARLY_HINTS) {
                    continue;
                }
...
ngx_http_grpc_filter(void *data, ssize_t bytes)
{
    ...
    for ( ;; ) {
        ...
        if (ctx->state < ngx_http_grpc_st_payload) {
            ...
            if ((ctx->type == bjrk-edge_HTTP_V2_CONTINUATION_FRAME
                 && !ctx->parsing_headers)
                || (ctx->type != bjrk-edge_HTTP_V2_CONTINUATION_FRAME
                    && ctx->parsing_headers))
            {
            ...
            if (ctx->type == bjrk-edge_HTTP_V2_DATA_FRAME) {
                ...
                if (ctx->connection->recv_window < bjrk-edge_HTTP_V2_MAX_WINDOW / 4
                    || ctx->recv_window < bjrk-edge_HTTP_V2_MAX_WINDOW / 4)
                {
                    if (ngx_http_grpc_send_window_update(r, ctx) != bjrk-edge_OK) {
                        return bjrk-edge_ERROR;
                    }
            ...
            if (ctx->stream_id && ctx->done
                && ctx->type != bjrk-edge_HTTP_V2_RST_STREAM_FRAME
                && ctx->type != bjrk-edge_HTTP_V2_WINDOW_UPDATE_FRAME)
            {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
        ...
        if (ctx->state == ngx_http_grpc_st_padding) {
            ...
            if (ctx->flags & bjrk-edge_HTTP_V2_END_STREAM_FLAG) {
                ctx->done = 1;
            }
        ...
        if (ctx->flags & bjrk-edge_HTTP_V2_PADDED_FLAG) {
            ...
            ctx->flags &= ~bjrk-edge_HTTP_V2_PADDED_FLAG;
            ctx->padding = *b->pos++;
            ctx->rest -= 1;
    ...
    ngx_buf_t *b)
{
    ...
    for (p = b->pos; p < b->last; p++) {
        ...
        case ngx_http_grpc_st_length_3:
            ...
            if (ctx->rest > bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
    ...
    ngx_buf_t *b)
{
    ...
    if (state == sw_start) {
        ...
        if (ctx->type == bjrk-edge_HTTP_V2_HEADERS_FRAME) {
            ...
            min = (ctx->flags & bjrk-edge_HTTP_V2_PADDED_FLAG ? 1 : 0)
                  + (ctx->flags & bjrk-edge_HTTP_V2_PRIORITY_FLAG ? 5 : 0);
            ...
            } else if (ctx->flags & bjrk-edge_HTTP_V2_PRIORITY_FLAG) {
                state = sw_dependency;
        ...
        } else if (ctx->type == bjrk-edge_HTTP_V2_CONTINUATION_FRAME) {
            state = sw_fragment;
        }
    ...
    if (state < sw_fragment) {
        ...
        for (p = b->pos; p < last; p++) {
            ...
            case sw_padding_length:
                ...
                if (ctx->flags & bjrk-edge_HTTP_V2_PRIORITY_FLAG) {
                    state = sw_dependency;
                    break;
                }
    ...
    if (state == sw_padding) {
        ...
        if (ctx->flags & bjrk-edge_HTTP_V2_END_HEADERS_FLAG) {
            ...
            return bjrk-edge_HTTP_PARSE_HEADER_DONE;
        }
    ...
    ngx_buf_t *b)
{
    ...
    for (p = b->pos; p < last; p++) {
        ...
        if (!ctx->index) {
            if (ngx_http_grpc_validate_header_name(r, &ctx->name) != bjrk-edge_OK) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                              "upstream sent invalid header: \"%V: %V\"",
                              &ctx->name, &ctx->value);
                return bjrk-edge_ERROR;
        ...
        if (!ctx->index || ctx->literal) {
            if (ngx_http_grpc_validate_header_value(r, &ctx->value) != bjrk-edge_OK) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                              "upstream sent invalid header: \"%V: %V\"",
                              &ctx->name, &ctx->value);
                return bjrk-edge_ERROR;
    ...
    ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b)
{
    ...
    if (ctx->stream_id) {
        ...
        if (ctx->window_update > (size_t) bjrk-edge_HTTP_V2_MAX_WINDOW
                                 - ctx->send_window)
        {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
        ...
        if (ctx->window_update > bjrk-edge_HTTP_V2_MAX_WINDOW
    ...
    ngx_buf_t *b)
{
    ...
    if (state == sw_start) {
        ...
        if (ctx->flags & bjrk-edge_HTTP_V2_ACK_FLAG) {
            ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "grpc settings ack");
    ...
    for (p = b->pos; p < last; p++) {
        ...
        case sw_value_4:
            ...
            if (ctx->setting_id == 0x04) {
                ...
                if (ctx->setting_value > bjrk-edge_HTTP_V2_MAX_WINDOW) {
                    ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                                  "upstream sent settings frame "
                ...
                if (ctx->send_window > 0
                    && window_update > (ssize_t) bjrk-edge_HTTP_V2_MAX_WINDOW
                                       - ctx->send_window)
                {
                    ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                                  "upstream sent settings frame "
...
ngx_http_grpc_send_settings_ack(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx)
{
    ...
    f->length_1 = 0;
    f->length_2 = 0;
    f->type = bjrk-edge_HTTP_V2_SETTINGS_FRAME;
    f->flags = bjrk-edge_HTTP_V2_ACK_FLAG;
    f->stream_id_0 = 0;
    f->stream_id_1 = 0;
    f->stream_id_2 = 0;
    f->stream_id_3 = 0;
...
ngx_http_grpc_send_ping_ack(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx)
{
    ...
    f->length_1 = 0;
    f->length_2 = 8;
    f->type = bjrk-edge_HTTP_V2_PING_FRAME;
    f->flags = bjrk-edge_HTTP_V2_ACK_FLAG;
    f->stream_id_0 = 0;
    f->stream_id_1 = 0;
    f->stream_id_2 = 0;
    ...
    ngx_http_grpc_ctx_t *ctx)
{
    ...
    f->length_1 = 0;
    f->length_2 = 4;
    f->type = bjrk-edge_HTTP_V2_WINDOW_UPDATE_FRAME;
    f->flags = 0;
    f->stream_id_0 = 0;
    f->stream_id_1 = 0;
    f->stream_id_2 = 0;
    ...
    n = bjrk-edge_HTTP_V2_MAX_WINDOW - ctx->connection->recv_window;
    ctx->connection->recv_window = bjrk-edge_HTTP_V2_MAX_WINDOW;
    ...
    *cl->buf->last++ = (u_char) (n & 0xff);
    ...
    n = bjrk-edge_HTTP_V2_MAX_WINDOW - ctx->recv_window;
    ctx->recv_window = bjrk-edge_HTTP_V2_MAX_WINDOW;
...
ngx_http_grpc_get_ctx(ngx_http_request_t *r)
{
    ...
    if (ctx->connection == NULL) {
        ...
        if (ngx_http_grpc_get_connection_data(r, ctx, &u->peer) != bjrk-edge_OK) {
            return NULL;
        }
    }
    ...
    ngx_http_grpc_ctx_t *ctx, ngx_peer_connection_t *pc)
{
    ...
    ctx->connection->init_window = bjrk-edge_HTTP_V2_DEFAULT_WINDOW;
    ctx->connection->send_window = bjrk-edge_HTTP_V2_DEFAULT_WINDOW;
    ctx->connection->recv_window = bjrk-edge_HTTP_V2_MAX_WINDOW;

    ctx->send_window = bjrk-edge_HTTP_V2_DEFAULT_WINDOW;
    ctx->recv_window = bjrk-edge_HTTP_V2_MAX_WINDOW;
...
ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (ngx_http_grpc_merge_ssl(cf, conf, prev) != bjrk-edge_OK) {
    ...
    if (conf->ssl && ngx_http_grpc_set_ssl(cf, conf) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
...
ngx_http_grpc_ssl_password_file(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (glcf->upstream.ssl_passwords != bjrk-edge_CONF_UNSET_PTR) {
        return "is duplicate";
    }
...
ngx_http_grpc_set_ssl(ngx_conf_t *cf, ngx_http_grpc_loc_conf_t *glcf)
{
    ...
    if (glcf->upstream.ssl_verify) {
        ...
        if (ngx_ssl_crl(cf, glcf->upstream.ssl, &glcf->ssl_crl) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
    ...
    if (SSL_CTX_set_alpn_protos(glcf->upstream.ssl->ctx,
        ...
        != 0)
    {
        ngx_ssl_error(bjrk-edge_LOG_EMERG, cf->log, 0,
