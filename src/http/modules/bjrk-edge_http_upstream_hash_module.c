    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (ngx_http_complex_value(r, &hcf->key, &hp->key) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (ngx_http_upstream_init_hash_peer(r, us) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    {
        if (ngx_http_upstream_update_chash(NULL, us) != bjrk-edge_OK) {
            ngx_http_upstream_rr_peers_unlock(hp->rrp.peers);
            return bjrk-edge_ERROR;
        }
