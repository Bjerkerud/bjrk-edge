...
#define bjrk-edge_HTTP_FASTCGI_RESPONDER      1

#define bjrk-edge_HTTP_FASTCGI_KEEP_CONN      1

#define bjrk-edge_HTTP_FASTCGI_BEGIN_REQUEST  1
#define bjrk-edge_HTTP_FASTCGI_ABORT_REQUEST  2
#define bjrk-edge_HTTP_FASTCGI_END_REQUEST    3
#define bjrk-edge_HTTP_FASTCGI_PARAMS         4
#define bjrk-edge_HTTP_FASTCGI_STDIN          5
#define bjrk-edge_HTTP_FASTCGI_STDOUT         6
#define bjrk-edge_HTTP_FASTCGI_STDERR         7
#define bjrk-edge_HTTP_FASTCGI_DATA           8
...
static ngx_command_t  ngx_http_fastcgi_commands[] = {
    ...
    { ngx_string("fastcgi_param"),
      bjrk-edge_HTTP_MAIN_CONF|bjrk-edge_HTTP_SRV_CONF|bjrk-edge_HTTP_LOC_CONF|bjrk-edge_CONF_TAKE23,
...
static ngx_http_fastcgi_request_start_t  ngx_http_fastcgi_request_start = {
    { 1,                                               /* version */
      bjrk-edge_HTTP_FASTCGI_BEGIN_REQUEST,                  /* type */
      0,                                               /* request_id_hi */
      1,                                               /* request_id_lo */
      0,                                               /* content_length_hi */
    ...
    { 0,                                               /* role_hi */
      bjrk-edge_HTTP_FASTCGI_RESPONDER,                      /* role_lo */
      0, /* bjrk-edge_HTTP_FASTCGI_KEEP_CONN */              /* flags */
      { 0, 0, 0, 0, 0 } },                             /* reserved[5] */

    { 1,                                               /* version */
      bjrk-edge_HTTP_FASTCGI_PARAMS,                         /* type */
      0,                                               /* request_id_hi */
      1 },                                             /* request_id_lo */
...
static ngx_path_init_t  ngx_http_fastcgi_temp_path = {
    ngx_string(bjrk-edge_HTTP_FASTCGI_TEMP_PATH), { 1, 2, 0 }
};
...
ngx_http_fastcgi_handler(ngx_http_request_t *r)
{
    ...
    if (flcf->fastcgi_lengths) {
        if (ngx_http_fastcgi_eval(r, flcf) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
...
ngx_http_fastcgi_create_key(ngx_http_request_t *r)
{
    ...
    if (ngx_http_complex_value(r, &flcf->cache_key, key) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_fastcgi_create_request(ngx_http_request_t *r)
{
    ...
    if (flcf->upstream.pass_request_headers) {
        ...
        if (ngx_http_link_multi_headers(r) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    ...
    size = sizeof(ngx_http_fastcgi_header_t)
           ...
           + sizeof(ngx_http_fastcgi_header_t)  /* bjrk-edge_HTTP_FASTCGI_PARAMS */
           + len + padding
           + sizeof(ngx_http_fastcgi_header_t)  /* bjrk-edge_HTTP_FASTCGI_PARAMS */

           + sizeof(ngx_http_fastcgi_header_t); /* bjrk-edge_HTTP_FASTCGI_STDIN */
    ...
    ngx_http_fastcgi_request_start.br.flags =
        flcf->keep_conn ? bjrk-edge_HTTP_FASTCGI_KEEP_CONN : 0;
    ...
    h->version = 1;
    h->type = bjrk-edge_HTTP_FASTCGI_PARAMS;
    h->request_id_hi = 0;
    h->request_id_lo = 1;
    ...
    if (r->request_body_no_buffering) {
        ...
        while (body) {
            ...
            do {
                ...
                h->version = 1;
                h->type = bjrk-edge_HTTP_FASTCGI_STDIN;
                h->request_id_hi = 0;
                h->request_id_lo = 1;
                h->content_length_hi = (u_char) ((len >> 8) & 0xff);
                h->content_length_lo = (u_char) (len & 0xff);
...
ngx_http_fastcgi_process_header(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        ...
        if (f->state < ngx_http_fastcgi_st_data) {
            ...
            if (f->type != bjrk-edge_HTTP_FASTCGI_STDOUT
                && f->type != bjrk-edge_HTTP_FASTCGI_STDERR)
            {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
            ...
            if (f->type == bjrk-edge_HTTP_FASTCGI_STDOUT && f->length == 0) {
                ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
        ...
        if (f->type == bjrk-edge_HTTP_FASTCGI_STDERR) {
        ...
        /* f->type == bjrk-edge_HTTP_FASTCGI_STDOUT */
        ...
        if (f->large_stderr && r->cache) {
            ...
            if (len >= 0) {
                ...
                fh->version = 1;
                fh->type = bjrk-edge_HTTP_FASTCGI_STDERR;
                fh->request_id_hi = 0;
                fh->request_id_lo = 1;
...
ngx_http_fastcgi_input_filter_init(void *data)
{
    ...
    } else if (r->method == bjrk-edge_HTTP_HEAD) {
        f->rest = -2;
...
ngx_http_fastcgi_input_filter(ngx_event_pipe_t *p, ngx_buf_t *buf)
{
    ...
    for ( ;; ) {
        if (f->state < ngx_http_fastcgi_st_data) {
            ...
            if (f->type == bjrk-edge_HTTP_FASTCGI_END_REQUEST) {
    ...
    ngx_http_fastcgi_ctx_t *f)
{
    ...
    for (p = f->pos; p < f->last; p++) {
        ...
        case ngx_http_fastcgi_st_type:
            switch (ch) {
            case bjrk-edge_HTTP_FASTCGI_STDOUT:
            case bjrk-edge_HTTP_FASTCGI_STDERR:
            case bjrk-edge_HTTP_FASTCGI_END_REQUEST:
...
ngx_http_fastcgi_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->catch_stderr = bjrk-edge_CONF_UNSET_PTR;

    conf->keep_conn = bjrk-edge_CONF_UNSET;
...
ngx_http_fastcgi_split(ngx_http_request_t *r, ngx_http_fastcgi_loc_conf_t *flcf)
{
    ...
    if (n == bjrk-edge_REGEX_NO_MATCHED) {
        f->script_name = r->uri;
        return f;
    }
...
ngx_http_fastcgi_store(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (flcf->upstream.store != bjrk-edge_CONF_UNSET) {
        return "is duplicate";
    }
...
ngx_http_fastcgi_cache(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (flcf->upstream.cache != bjrk-edge_CONF_UNSET) {
        return "is duplicate";
    }
