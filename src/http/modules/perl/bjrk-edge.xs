...
send_http_header(r, ...)
    ...
    if (rc == bjrk-edge_ERROR || rc > bjrk-edge_OK) {
        ctx->error = 1;
        ctx->status = rc;
...
has_request_body(r, next)
    ...
    if (ngx_http_perl_refcount(aTHX_ r, ctx->next) != bjrk-edge_OK) {
        ctx->error = 1;
        ctx->next = NULL;
...
header_out(r, key, value)
    ...
    if (ngx_http_perl_sv2str(aTHX_ r, &header->key, key) != bjrk-edge_OK) {
        header->hash = 0;
        ctx->error = 1;
        croak("ngx_http_perl_sv2str() failed");
    }

    if (ngx_http_perl_sv2str(aTHX_ r, &header->value, value) != bjrk-edge_OK) {
        header->hash = 0;
        ctx->error = 1;
        croak("ngx_http_perl_sv2str() failed");
    }
...
print(r, ...)
    ...
    if (items == 2) {
        ...
        if (SvREADONLY(sv) && SvPOK(sv)) {
            ...
            if (ngx_http_perl_refcount(aTHX_ r, sv) != bjrk-edge_OK) {
                ctx->error = 1;
                croak("ngx_http_perl_refcount() failed");
            }
...
flush(r)
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0, "$r->flush");
...
internal_redirect(r, uri)
    ...
    if (ngx_http_perl_sv2str(aTHX_ r, &ctx->redirect_uri, uri) != bjrk-edge_OK) {
        ctx->error = 1;
        croak("ngx_http_perl_sv2str() failed");
    }
...
variable(r, name, value = NULL)
    ...
    if (items == 2) {
        ...
        if (ngx_http_perl_sv2str(aTHX_ r, &val, value) != bjrk-edge_OK) {
            ctx->error = 1;
            croak("ngx_http_perl_sv2str() failed");
        }
    }
...
log_error(r, err, msg)
    ...
    ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, e, "perl: %s", p);
