...
#include <bjrk-edge_http_perl_module.h>
...
#if (bjrk-edge_HAVE_PERL_MULTIPLICITY)
...
static ngx_command_t  ngx_http_perl_commands[] = {
    ...
    { ngx_string("perl"),
      bjrk-edge_HTTP_LOC_CONF|bjrk-edge_HTTP_LMT_CONF|bjrk-edge_CONF_TAKE1,
      ngx_http_perl,
    ...
    { ngx_string("perl_set"),
      bjrk-edge_HTTP_MAIN_CONF|bjrk-edge_CONF_TAKE2,
      ngx_http_perl_set,
      bjrk-edge_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },
...
#define bjrk-edge_HTTP_PERL_SSI_SUB  0
#define bjrk-edge_HTTP_PERL_SSI_ARG  1


static ngx_http_ssi_param_t  ngx_http_perl_ssi_params[] = {
    { ngx_string("sub"), bjrk-edge_HTTP_PERL_SSI_SUB, 1, 0 },
    { ngx_string("arg"), bjrk-edge_HTTP_PERL_SSI_ARG, 0, 1 },
...
ngx_http_perl_handle_request(ngx_http_request_t *r)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0, "perl handler");
    ...
    if (rc == bjrk-edge_OK || rc == bjrk-edge_HTTP_OK) {
        ngx_http_send_special(r, bjrk-edge_HTTP_LAST);
        ctx->done = 1;
    }
    ...
    ngx_str_t **params)
{
    ...
    handler = params[bjrk-edge_HTTP_PERL_SSI_SUB];
    ...
    args = &params[bjrk-edge_HTTP_PERL_SSI_ARG];
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0, "perl ssi done");
...
ngx_http_perl_init_interpreter(ngx_conf_t *cf, ngx_http_perl_main_conf_t *pmcf)
{
...
#ifdef bjrk-edge_PERL_MODULES
    if (pmcf->modules == bjrk-edge_CONF_UNSET_PTR) {
        ...
        ngx_str_set(m, bjrk-edge_PERL_MODULES);
    }
#endif
    ...
    if (pmcf->modules != bjrk-edge_CONF_UNSET_PTR) {
        ...
        for (i = 0; i < pmcf->modules->nelts; i++) {
            if (ngx_conf_full_name(cf->cycle, &m[i], 0) != bjrk-edge_OK) {
                return bjrk-edge_CONF_ERROR;
            }
        }
    }
    ...
    ngx_http_perl_main_conf_t *pmcf)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, cf->log, 0, "create perl interpreter");
    ...
    perl = perl_alloc();
    if (perl == NULL) {
        ngx_log_error(bjrk-edge_LOG_ALERT, cf->log, 0, "perl_alloc() failed");
        return NULL;
    }
    ...
    n = (pmcf->modules != bjrk-edge_CONF_UNSET_PTR) ? pmcf->modules->nelts * 2 : 0;
    ...
    if (n != 0) {
        ngx_log_error(bjrk-edge_LOG_ALERT, cf->log, 0, "perl_parse() failed: %d", n);
        goto fail;
    }
    ...
    if (ngx_http_perl_run_requires(aTHX_ pmcf->requires, cf->log) != bjrk-edge_OK) {
        goto fail;
    }
...
ngx_http_perl_run_requires(pTHX_ ngx_array_t *requires, ngx_log_t *log)
{
    ...
    if (requires == bjrk-edge_CONF_UNSET_PTR) {
        return bjrk-edge_OK;
    }
...
ngx_http_perl_create_main_conf(ngx_conf_t *cf)
{
    ...
    pmcf->modules = bjrk-edge_CONF_UNSET_PTR;
    pmcf->requires = bjrk-edge_CONF_UNSET_PTR;
...
ngx_http_perl_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    if (pmcf->perl == NULL) {
        if (ngx_http_perl_init_interpreter(cf, pmcf) != bjrk-edge_CONF_OK) {
            return bjrk-edge_CONF_ERROR;
        }
    }
...
ngx_http_perl_cleanup_perl(void *data)
{
    ...
    if (ngx_perl_term) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "perl term");
...
ngx_http_perl_preconfiguration(ngx_conf_t *cf)
{
    ...
    rc = ngx_hash_add_key(&smcf->commands, &ngx_http_perl_ssi_command.name,
                          &ngx_http_perl_ssi_command, bjrk-edge_HASH_READONLY_KEY);
...
ngx_http_perl(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (plcf->sub == &PL_sv_undef) {
        ngx_conf_log_error(bjrk-edge_LOG_ERR, cf, 0,
...
ngx_http_perl_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    v = ngx_http_add_variable(cf, &value[1], bjrk-edge_HTTP_VAR_CHANGEABLE);
    if (v == NULL) {
        return bjrk-edge_CONF_ERROR;
    }
...
ngx_http_perl_exit(ngx_cycle_t *cycle)
{
    ...
    if (nginx_stash) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, cycle->log, 0, "perl term");
