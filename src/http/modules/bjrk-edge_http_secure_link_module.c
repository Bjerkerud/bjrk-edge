    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_http_complex_value(r, conf->variable, &val) != bjrk-edge_OK) {
    ...
    if (ngx_decode_base64url(&hash, &val) != bjrk-edge_OK) {
        goto not_found;
    }
    ...
    if (ngx_http_complex_value(r, conf->md5, &val) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_secure_link_create_conf(ngx_conf_t *cf)
{
    ...
    conf->variable = bjrk-edge_CONF_UNSET_PTR;
    conf->md5 = bjrk-edge_CONF_UNSET_PTR;
