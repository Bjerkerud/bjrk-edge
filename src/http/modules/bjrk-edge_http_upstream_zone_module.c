    ...
    ngx_http_upstream_rr_peer_t *src)
{
    ...
    dst->name.data = ngx_slab_calloc_locked(pool, bjrk-edge_SOCKADDR_STRLEN);
    if (dst->name.data == NULL) {
        goto failed;
    }

#if (bjrk-edge_HTTP_UPSTREAM_SID)
    dst->sid.data = ngx_slab_calloc_locked(pool, bjrk-edge_HTTP_UPSTREAM_SID_LEN);
...
ngx_http_upstream_zone_resolve_timer(ngx_event_t *event)
{
    ...
    if (ctx == bjrk-edge_NO_RESOLVER) {
        ngx_log_error(bjrk-edge_LOG_ERR, event->log, 0,
...
ngx_http_upstream_zone_resolve_handler(ngx_resolver_ctx_t *ctx)
{
    ...
    for (i = 0; i < ctx->naddrs; i++) {
        len = ngx_sock_ntop(ctx->addrs[i].sockaddr, ctx->addrs[i].socklen,
                            text, bjrk-edge_SOCKADDR_STRLEN, 1);

        ngx_log_debug7(bjrk-edge_LOG_DEBUG_HTTP, event->log, 0,
...
again:
    ...
    for (i = 0; i < ctx->naddrs; i++) {
        ...
        peer->name.len = ngx_sock_ntop(peer->sockaddr, peer->socklen,
                                       peer->name.data, bjrk-edge_SOCKADDR_STRLEN, 1);
