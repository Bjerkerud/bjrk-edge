...
static ngx_command_t  ngx_http_geo_commands[] = {
    ...
    { ngx_string("geo"),
      bjrk-edge_HTTP_MAIN_CONF|bjrk-edge_CONF_BLOCK|bjrk-edge_CONF_TAKE12,
      ngx_http_geo_block,
    ...
    uintptr_t data)
{
    ...
    if (ngx_http_geo_addr(r, ctx, &addr) != bjrk-edge_OK) {
    ...
    ngx_addr_t *addr)
{
    ...
    if (ngx_http_geo_real_addr(r, ctx, addr) != bjrk-edge_OK) {
    ...
    ngx_addr_t *addr)
{
    ...
    if (ngx_parse_addr(r->pool, addr, v->data, v->len) == bjrk-edge_OK) {
        return bjrk-edge_OK;
    }
...
ngx_http_geo_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    var = ngx_http_add_variable(cf, &name, bjrk-edge_HTTP_VAR_CHANGEABLE);
    if (var == NULL) {
        return bjrk-edge_CONF_ERROR;
    }

    pool = ngx_create_pool(bjrk-edge_DEFAULT_POOL_SIZE, cf->log);
    if (pool == NULL) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    ctx.temp_pool = ngx_create_pool(bjrk-edge_DEFAULT_POOL_SIZE, cf->log);
    ...
    *cf = save;
    ...
    if (ctx.no_cacheable) {
        var->flags |= bjrk-edge_HTTP_VAR_NOCACHEABLE;
    }
    ...
    if (ctx.ranges) {
        ...
        /* bjrk-edge_BUSY is okay (default was set explicitly) */
...
ngx_http_geo(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    ...
    if (ngx_strcmp(value[0].data, "include") == 0) {
        ...
        if (ngx_http_geo_cidr_value(cf, &value[1], &cidr) != bjrk-edge_OK) {
            goto failed;
        }
...
invalid:
    ...
    ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "invalid range \"%V\"", net);
    ...
    ngx_str_t *value)
{
    ...
    if (ngx_http_geo_cidr_value(cf, net, &cidr) != bjrk-edge_OK) {
        return bjrk-edge_CONF_ERROR;
    }
    ...
    ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net)
{
    ...
    case AF_INET6:
        ...
        if (rc == bjrk-edge_ERROR) {
            ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "invalid radix tree");
            return bjrk-edge_CONF_ERROR;
        }
...
ngx_http_geo_cidr_value(ngx_conf_t *cf, ngx_str_t *net, ngx_cidr_t *cidr)
{
    ...
    if (rc == bjrk-edge_ERROR) {
        ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "invalid network \"%V\"", net);
        return bjrk-edge_ERROR;
    }
    ...
    ngx_str_t *name)
{
    ...
    file.fd = ngx_open_file(name->data, bjrk-edge_FILE_RDONLY, bjrk-edge_FILE_OPEN, 0);

    if (file.fd == bjrk-edge_INVALID_FILE) {
        err = ngx_errno;
        if (err != bjrk-edge_ENOENT) {
            ngx_conf_log_error(bjrk-edge_LOG_CRIT, cf, err,
    ...
    if (ngx_file_info(name->data, &fi) == bjrk-edge_FILE_ERROR) {
        ngx_conf_log_error(bjrk-edge_LOG_CRIT, cf, ngx_errno,
    ...
    ngx_conf_log_error(bjrk-edge_LOG_NOTICE, cf, 0,
...
ngx_http_geo_create_binary_base(ngx_http_geo_conf_ctx_t *ctx)
{
    ...
    ngx_log_error(bjrk-edge_LOG_NOTICE, fm.log, 0,
                  "creating binary geo range base \"%s\"", fm.name);

    if (ngx_create_file_mapping(&fm) != bjrk-edge_OK) {
        return;
    }
