...
#define bjrk-edge_HTTP_STICKY_COOKIE_MAX_EXPIRES  2145916555
...
typedef struct {
    ...
    u_char                                      sid_len;
    u_char                                      sid[bjrk-edge_HTTP_UPSTREAM_SID_LEN];
} ngx_http_upstream_sticky_sess_node_t;
...
static ngx_command_t  ngx_http_upstream_sticky_commands[] = {
    ...
    { ngx_string("sticky"),
      bjrk-edge_HTTP_UPS_CONF|bjrk-edge_CONF_2MORE,
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (stcf->original_init_upstream(cf, us) != bjrk-edge_OK) {
...
ngx_http_upstream_sticky_get_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    u_char                                 sid_data[bjrk-edge_HTTP_UPSTREAM_SID_LEN];
    ...
    if (pc->hint == NULL && stp->conf->shm_zone && stp->id.len) {
        ...
        if (sn == NULL) {
            ...
            ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP, pc->log, 0,
    ...
    if (ngx_http_upstream_sticky_cookie_insert(pc, stp) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_uint_t state)
{
    ...
    if (state & (bjrk-edge_PEER_FAILED|bjrk-edge_PEER_NEXT)) {
        goto done;
    }
    ...
    ngx_peer_connection_t *pc)
{
    ...
    if (pc->sid == NULL) {
        ngx_log_error(bjrk-edge_LOG_WARN, pc->log, 0,
    ...
    ngx_uint_t type)
{
    ...
    if (type == bjrk-edge_HTTP_UPSTREAM_NOTIFY_HEADER
    ...
    ngx_http_upstream_sticky_peer_data_t *stp)
{
    ...
    if (stcf->cookie_expires != (time_t) bjrk-edge_CONF_UNSET) {
        len += sizeof(expires) - 1 + bjrk-edge_TIME_T_LEN;
    }
    ...
    if (stcf->cookie_samesite) {
        ...
        if (stcf->cookie_samesite->lengths && samesite.len
            && ngx_http_upstream_sticky_samesite(&samesite) != bjrk-edge_OK)
        {
    ...
    *p++ = '=';
    ...
    if (stcf->cookie_expires != (time_t) bjrk-edge_CONF_UNSET) {
        ...
        if (stcf->cookie_expires == bjrk-edge_HTTP_STICKY_COOKIE_MAX_EXPIRES) {
    ...
    ngx_http_upstream_sticky_sess_key_t *key, ngx_str_t *sid)
{
    ...
    if (sn == NULL) {
        ...
        ngx_log_error(bjrk-edge_LOG_WARN, ngx_cycle->log, 0,
...
ngx_http_upstream_sticky_create_conf(ngx_conf_t *cf)
{
    ...
    stcf->cookie_expires = bjrk-edge_CONF_UNSET;
...
ngx_http_upstream_sticky(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "unknown parameter \"%V\"",
                       &value[1]);
    return bjrk-edge_CONF_ERROR;
}
    ...
    for (i = 3; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "domain=", 7) == 0) {
            ...
            if (ngx_strcmp(value[i].data, "max") == 0) {
                stcf->cookie_expires = bjrk-edge_HTTP_STICKY_COOKIE_MAX_EXPIRES;

            } else {
                stcf->cookie_expires = ngx_parse_time(&value[i], 1);
                if (stcf->cookie_expires == (time_t) bjrk-edge_ERROR) {
            ...
            if (stcf->cookie_samesite->lengths == NULL
                && ngx_http_upstream_sticky_samesite(&value[i]) != bjrk-edge_OK)
            {
    ...
    ngx_http_upstream_sticky_srv_conf_t *stcf, ngx_http_upstream_srv_conf_t *us)
{
    ...
    zone_size = 0;
    timeout = bjrk-edge_CONF_UNSET_MSEC;
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "zone=", 5) == 0) {
            ...
            if (timeout != bjrk-edge_CONF_UNSET_MSEC) {
            ...
            if (timeout == (ngx_msec_t) bjrk-edge_ERROR || timeout == 0) {
...
ngx_http_upstream_sticky_init_worker(ngx_cycle_t *cycle)
{
    ...
    if ((ngx_process != bjrk-edge_PROCESS_WORKER || ngx_worker != 0)
        && ngx_process != bjrk-edge_PROCESS_SINGLE)
    {
        return bjrk-edge_OK;
    }
