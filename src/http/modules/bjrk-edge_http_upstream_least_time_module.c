...
#define bjrk-edge_HTTP_UPSTREAM_LT_HEADER     0
#define bjrk-edge_HTTP_UPSTREAM_LT_LAST_BYTE  1
...
static ngx_conf_enum_t  ngx_http_upstream_least_time_mode[] = {
    { ngx_string("header"), bjrk-edge_HTTP_UPSTREAM_LT_HEADER },
    { ngx_string("last_byte"), bjrk-edge_HTTP_UPSTREAM_LT_LAST_BYTE },
    { ngx_null_string, 0 }
};
...
static ngx_command_t  ngx_http_upstream_least_time_commands[] = {
    ...
    { ngx_string("least_time"),
      bjrk-edge_HTTP_UPS_CONF|bjrk-edge_CONF_TAKE12,
    ...
    ngx_http_upstream_rr_peer_t *peer)
{
    ...
    case bjrk-edge_HTTP_UPSTREAM_LT_HEADER:
        rt = peer->header_time;
        break;

    default: /* bjrk-edge_HTTP_UPSTREAM_LT_LAST_BYTE */
    ...
    ngx_uint_t type)
{
    ...
    case bjrk-edge_HTTP_UPSTREAM_NOTIFY_HEADER:

        if (ltp->conf->mode != bjrk-edge_HTTP_UPSTREAM_LT_HEADER) {
            return;
        }
    ...
    void *data, ngx_uint_t state)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, pc->log, 0, "free least time peer");
    ...
    if (ltp->conf->use_inflight
        && ltp->conf->mode == bjrk-edge_HTTP_UPSTREAM_LT_HEADER)
    {
        goto done;
    }
