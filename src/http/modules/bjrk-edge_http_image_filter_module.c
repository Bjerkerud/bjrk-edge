...
#define bjrk-edge_HTTP_IMAGE_OFF       0
#define bjrk-edge_HTTP_IMAGE_TEST      1
#define bjrk-edge_HTTP_IMAGE_SIZE      2
#define bjrk-edge_HTTP_IMAGE_RESIZE    3
#define bjrk-edge_HTTP_IMAGE_CROP      4
#define bjrk-edge_HTTP_IMAGE_ROTATE    5


#define bjrk-edge_HTTP_IMAGE_START     0
#define bjrk-edge_HTTP_IMAGE_READ      1
#define bjrk-edge_HTTP_IMAGE_PROCESS   2
#define bjrk-edge_HTTP_IMAGE_PASS      3
#define bjrk-edge_HTTP_IMAGE_DONE      4


#define bjrk-edge_HTTP_IMAGE_NONE      0
#define bjrk-edge_HTTP_IMAGE_JPEG      1
#define bjrk-edge_HTTP_IMAGE_GIF       2
#define bjrk-edge_HTTP_IMAGE_PNG       3
#define bjrk-edge_HTTP_IMAGE_WEBP      4


#define bjrk-edge_HTTP_IMAGE_BUFFERED  0x08
...
static ngx_command_t  ngx_http_image_filter_commands[] = {
    ...
    { ngx_string("image_filter"),
      bjrk-edge_HTTP_LOC_CONF|bjrk-edge_CONF_TAKE123,
...
ngx_http_image_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->headers_out.status == bjrk-edge_HTTP_NOT_MODIFIED) {
    ...
    if (conf->filter == bjrk-edge_HTTP_IMAGE_OFF) {
        return ngx_http_next_header_filter(r);
    }
...
ngx_http_image_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, r->connection->log, 0, "image filter");
    ...
    case bjrk-edge_HTTP_IMAGE_START:

        ctx->type = ngx_http_image_test(r, in);

        conf = ngx_http_get_module_loc_conf(r, ngx_http_image_filter_module);

        if (ctx->type == bjrk-edge_HTTP_IMAGE_NONE) {

            if (conf->filter == bjrk-edge_HTTP_IMAGE_SIZE) {
                out.buf = ngx_http_image_json(r, NULL);

                if (out.buf) {
                    out.next = NULL;
                    ctx->phase = bjrk-edge_HTTP_IMAGE_DONE;
            ...
            return ngx_http_filter_finalize_request(r,
                                              ...
                                              bjrk-edge_HTTP_UNSUPPORTED_MEDIA_TYPE);
        }
        ...
        if (conf->filter == bjrk-edge_HTTP_IMAGE_TEST) {
            ctx->phase = bjrk-edge_HTTP_IMAGE_PASS;

            return ngx_http_image_send(r, ctx, in);
        }

        ctx->phase = bjrk-edge_HTTP_IMAGE_READ;

        /* fall through */

    case bjrk-edge_HTTP_IMAGE_READ:
    ...
    case bjrk-edge_HTTP_IMAGE_PROCESS:
    ...
    case bjrk-edge_HTTP_IMAGE_PASS:

        return ngx_http_next_body_filter(r, in);

    default: /* bjrk-edge_HTTP_IMAGE_DONE */

        rc = ngx_http_next_body_filter(r, NULL);

        /* bjrk-edge_ERROR resets any pending data */
        return (rc == bjrk-edge_OK) ? bjrk-edge_ERROR : rc;
    }
}
    ...
    ngx_chain_t *in)
{
    ...
    if (ctx->phase == bjrk-edge_HTTP_IMAGE_DONE) {
        /* bjrk-edge_ERROR resets any pending data */
        return (rc == bjrk-edge_OK) ? bjrk-edge_ERROR : rc;
    }
...
ngx_http_image_test(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (in->buf->last - p < 16) {
        return bjrk-edge_HTTP_IMAGE_NONE;
    }
    ...
    if (p[0] == 0xff && p[1] == 0xd8) {
        ...
        return bjrk-edge_HTTP_IMAGE_JPEG;
               ...
               && p[5] == 'a')
    {
        if (p[4] == '9' || p[4] == '7') {
            /* GIF */
            return bjrk-edge_HTTP_IMAGE_GIF;
        }
               ...
               && p[4] == 0x0d && p[5] == 0x0a && p[6] == 0x1a && p[7] == 0x0a)
    {
        ...
        return bjrk-edge_HTTP_IMAGE_PNG;
               ...
               && p[8] == 'W' && p[9] == 'E' && p[10] == 'B' && p[11] == 'P')
    {
        ...
        return bjrk-edge_HTTP_IMAGE_WEBP;
    }
...
ngx_http_image_read(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ctx->last = p;
    r->connection->buffered |= bjrk-edge_HTTP_IMAGE_BUFFERED;
...
ngx_http_image_process(ngx_http_request_t *r)
{
    ...
    if (conf->filter == bjrk-edge_HTTP_IMAGE_SIZE) {
        return ngx_http_image_json(r, rc == bjrk-edge_OK ? ctx : NULL);
    }

    ctx->angle = ngx_http_image_filter_get_value(r, conf->acv, conf->angle);

    if (conf->filter == bjrk-edge_HTTP_IMAGE_ROTATE) {
...
ngx_http_image_size(ngx_http_request_t *r, ngx_http_image_filter_ctx_t *ctx)
{
    ...
    case bjrk-edge_HTTP_IMAGE_JPEG:
    ...
    case bjrk-edge_HTTP_IMAGE_WEBP:
...
transparent:
    ...
    if (conf->filter == bjrk-edge_HTTP_IMAGE_RESIZE) {
    ...
    } else if (conf->filter == bjrk-edge_HTTP_IMAGE_ROTATE) {

        resize = 0;

    } else { /* bjrk-edge_HTTP_IMAGE_CROP */
    ...
    if (conf->filter == bjrk-edge_HTTP_IMAGE_CROP) {
...
ngx_http_image_source(ngx_http_request_t *r, ngx_http_image_filter_ctx_t *ctx)
{
    ...
    if (img == NULL) {
        ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0, failed);
    }
...
ngx_http_image_filter_create_conf(ngx_conf_t *cf)
{
    ...
    conf->filter = bjrk-edge_CONF_UNSET_UINT;
    conf->jpeg_quality = bjrk-edge_CONF_UNSET_UINT;
    conf->webp_quality = bjrk-edge_CONF_UNSET_UINT;
    conf->sharpen = bjrk-edge_CONF_UNSET_UINT;
    conf->transparency = bjrk-edge_CONF_UNSET;
    conf->interlace = bjrk-edge_CONF_UNSET;
...
ngx_http_image_filter_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->filter == bjrk-edge_CONF_UNSET_UINT) {

        if (prev->filter == bjrk-edge_CONF_UNSET_UINT) {
            conf->filter = bjrk-edge_HTTP_IMAGE_OFF;
    ...
    if (conf->jpeg_quality == bjrk-edge_CONF_UNSET_UINT) {
    ...
    if (conf->webp_quality == bjrk-edge_CONF_UNSET_UINT) {
    ...
    if (conf->sharpen == bjrk-edge_CONF_UNSET_UINT) {
...
ngx_http_image_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cf->args->nelts == 2) {
        if (ngx_strcmp(value[i].data, "off") == 0) {
            imcf->filter = bjrk-edge_HTTP_IMAGE_OFF;

        } else if (ngx_strcmp(value[i].data, "test") == 0) {
            imcf->filter = bjrk-edge_HTTP_IMAGE_TEST;

        } else if (ngx_strcmp(value[i].data, "size") == 0) {
            imcf->filter = bjrk-edge_HTTP_IMAGE_SIZE;
        ...
        if (ngx_strcmp(value[i].data, "rotate") == 0) {
            if (imcf->filter != bjrk-edge_HTTP_IMAGE_RESIZE
                && imcf->filter != bjrk-edge_HTTP_IMAGE_CROP)
            {
                imcf->filter = bjrk-edge_HTTP_IMAGE_ROTATE;
            }
    ...
    if (ngx_strcmp(value[i].data, "resize") == 0) {
        imcf->filter = bjrk-edge_HTTP_IMAGE_RESIZE;
...
failed:
    ...
    ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "invalid parameter \"%V\"",
                       &value[i]);
