...
#define bjrk-edge_HTTP_AUTOINDEX_HTML         0
#define bjrk-edge_HTTP_AUTOINDEX_JSON         1
#define bjrk-edge_HTTP_AUTOINDEX_JSONP        2
#define bjrk-edge_HTTP_AUTOINDEX_XML          3

#define bjrk-edge_HTTP_AUTOINDEX_PREALLOCATE  50

#define bjrk-edge_HTTP_AUTOINDEX_NAME_LEN     50
...
static ngx_conf_enum_t  ngx_http_autoindex_format[] = {
    { ngx_string("html"), bjrk-edge_HTTP_AUTOINDEX_HTML },
    { ngx_string("json"), bjrk-edge_HTTP_AUTOINDEX_JSON },
    { ngx_string("jsonp"), bjrk-edge_HTTP_AUTOINDEX_JSONP },
    { ngx_string("xml"), bjrk-edge_HTTP_AUTOINDEX_XML },
    { ngx_null_string, 0 }
};
...
ngx_http_autoindex_handler(ngx_http_request_t *r)
{
    ...
    last = ngx_http_map_uri_to_path(r, &path, &root,
                                    bjrk-edge_HTTP_AUTOINDEX_PREALLOCATE);
    if (last == NULL) {
    ...
    if (format == bjrk-edge_HTTP_AUTOINDEX_JSONP) {
        if (ngx_http_autoindex_jsonp_callback(r, &callback) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_BAD_REQUEST;
        }

        if (callback.len == 0) {
            format = bjrk-edge_HTTP_AUTOINDEX_JSON;
        }
    }

    if (ngx_open_dir(&path, &dir) == bjrk-edge_ERROR) {
        err = ngx_errno;

        if (err == bjrk-edge_ENOENT
            || err == bjrk-edge_ENOTDIR
            || err == bjrk-edge_ENAMETOOLONG)
        {
            level = bjrk-edge_LOG_ERR;
            rc = bjrk-edge_HTTP_NOT_FOUND;

        } else if (err == bjrk-edge_EACCES) {
            level = bjrk-edge_LOG_ERR;
    ...
    case bjrk-edge_HTTP_AUTOINDEX_JSON:
        ngx_str_set(&r->headers_out.content_type, "application/json");
        break;

    case bjrk-edge_HTTP_AUTOINDEX_JSONP:
        ngx_str_set(&r->headers_out.content_type, "application/javascript");
        break;

    case bjrk-edge_HTTP_AUTOINDEX_XML:
        ngx_str_set(&r->headers_out.content_type, "text/xml");
        ngx_str_set(&r->headers_out.charset, "utf-8");
        break;

    default: /* bjrk-edge_HTTP_AUTOINDEX_HTML */
    ...
    for ( ;; ) {
        ...
        if (!dir.valid_info) {
            ...
            if (ngx_de_info(filename, &dir) == bjrk-edge_FILE_ERROR) {
                err = ngx_errno;

                if (err != bjrk-edge_ENOENT && err != bjrk-edge_ELOOP) {
                    ngx_log_error(bjrk-edge_LOG_CRIT, r->connection->log, err,
                                  ngx_de_info_n " \"%s\" failed", filename);

                    if (err == bjrk-edge_EACCES) {
                        continue;
                    }
                ...
                if (ngx_de_link_info(filename, &dir) == bjrk-edge_FILE_ERROR) {
...
ngx_http_autoindex_html(ngx_http_request_t *r, ngx_array_t *entries)
{
    ...
    for (i = 0; i < entries->nelts; i++) {
        entry[i].escape = 2 * ngx_escape_uri(NULL, entry[i].name.data,
                                             entry[i].name.len,
                                             bjrk-edge_ESCAPE_URI_COMPONENT);
        ...
        entry_len = sizeof("<a href=\"") - 1
                  ...
                  + entry[i].escape_html
                  + bjrk-edge_HTTP_AUTOINDEX_NAME_LEN + sizeof("&gt;") - 2
                  + sizeof("</a>") - 1
                  + sizeof(" 28-Sep-1970 12:00 ") - 1
                  + 20                                   /* the file size */
                  + 2;

        if (len > bjrk-edge_MAX_SIZE_T_VALUE - entry_len) {
            return NULL;
        }
    ...
    for (i = 0; i < entries->nelts; i++) {
        ...
        *b->last++ = '>';
        ...
        if (entry[i].name.len != len) {
            if (len > bjrk-edge_HTTP_AUTOINDEX_NAME_LEN) {
                char_len = bjrk-edge_HTTP_AUTOINDEX_NAME_LEN - 3 + 1;

            } else {
                char_len = bjrk-edge_HTTP_AUTOINDEX_NAME_LEN + 1;
            }
            ...
            if (entry[i].escape_html) {
                if (len > bjrk-edge_HTTP_AUTOINDEX_NAME_LEN) {
                    char_len = bjrk-edge_HTTP_AUTOINDEX_NAME_LEN - 3;
                ...
                b->last = ngx_cpystrn(b->last, entry[i].name.data,
                                      bjrk-edge_HTTP_AUTOINDEX_NAME_LEN + 1);
                last = b->last - 3;
            }
        }
        ...
        if (len > bjrk-edge_HTTP_AUTOINDEX_NAME_LEN) {
            ...
            if (entry[i].dir && bjrk-edge_HTTP_AUTOINDEX_NAME_LEN - len > 0) {
                *b->last++ = '/';
                len++;
            }
            ...
            if (bjrk-edge_HTTP_AUTOINDEX_NAME_LEN - len > 0) {
                ngx_memset(b->last, ' ', bjrk-edge_HTTP_AUTOINDEX_NAME_LEN - len);
                b->last += bjrk-edge_HTTP_AUTOINDEX_NAME_LEN - len;
            }
        }
    ...
    ngx_str_t *callback)
{
    ...
    for (i = 0; i < entries->nelts; i++) {
        ...
        if (entry[i].file) {
            entry_len += sizeof(", \"size\":") - 1 + bjrk-edge_OFF_T_LEN;
        }
...
ngx_http_autoindex_jsonp_callback(ngx_http_request_t *r, ngx_str_t *callback)
{
    ...
    if (ngx_http_arg(r, (u_char *) "callback", 8, callback) != bjrk-edge_OK) {
        callback->len = 0;
        return bjrk-edge_OK;
    }
...
ngx_http_autoindex_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->enable = bjrk-edge_CONF_UNSET;
    conf->format = bjrk-edge_CONF_UNSET_UINT;
    conf->localtime = bjrk-edge_CONF_UNSET;
    conf->exact_size = bjrk-edge_CONF_UNSET;
