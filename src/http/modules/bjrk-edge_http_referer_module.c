...
#define bjrk-edge_HTTP_REFERER_NO_URI_PART  ((void *) 4)
...
static ngx_command_t  ngx_http_referer_commands[] = {
    ...
    { ngx_string("valid_referers"),
      bjrk-edge_HTTP_SRV_CONF|bjrk-edge_HTTP_LOC_CONF|bjrk-edge_CONF_1MORE,
    ...
    { ngx_string("referer_hash_max_size"),
      bjrk-edge_HTTP_SRV_CONF|bjrk-edge_HTTP_LOC_CONF|bjrk-edge_CONF_TAKE1,
...
uri:
    ...
    if (uri == bjrk-edge_HTTP_REFERER_NO_URI_PART) {
        goto valid;
    }
...
#if (bjrk-edge_PCRE)
    conf->regex = bjrk-edge_CONF_UNSET_PTR;
    conf->server_name_regex = bjrk-edge_CONF_UNSET_PTR;
#endif

    conf->no_referer = bjrk-edge_CONF_UNSET;
    conf->blocked_referer = bjrk-edge_CONF_UNSET;
    conf->referer_hash_max_size = bjrk-edge_CONF_UNSET_UINT;
    conf->referer_hash_bucket_size = bjrk-edge_CONF_UNSET_UINT;
...
ngx_http_referer_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->no_referer == bjrk-edge_CONF_UNSET) {
        conf->no_referer = 0;
    }

    if (conf->blocked_referer == bjrk-edge_CONF_UNSET) {
...
ngx_http_valid_referers(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (rlcf->keys == NULL) {
        ...
        if (ngx_hash_keys_array_init(rlcf->keys, bjrk-edge_HASH_SMALL) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }
    }
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (value[i].data[0] == '~') {
            if (ngx_http_add_regex_referer(cf, rlcf, &value[i]) != bjrk-edge_OK) {
                return bjrk-edge_CONF_ERROR;
            }
        ...
        if (ngx_http_add_referer(cf, rlcf->keys, &value[i], &uri) != bjrk-edge_OK) {
            return bjrk-edge_CONF_ERROR;
        }
    }
    ...
    ngx_str_t *value, ngx_str_t *uri)
{
    ...
    if (uri == NULL || uri->len == 0) {
        u = bjrk-edge_HTTP_REFERER_NO_URI_PART;
    ...
    rc = ngx_hash_add_key(keys, value, u, bjrk-edge_HASH_WILDCARD_KEY);
    ...
    ngx_str_t *name)
{
    ...
    if (name->len == 1) {
        ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "empty regex in \"%V\"", name);
        return bjrk-edge_ERROR;
    }

    if (rlcf->regex == bjrk-edge_CONF_UNSET_PTR) {
    ...
    ngx_http_regex_t *regex)
{
    ...
    if (rlcf->server_name_regex == bjrk-edge_CONF_UNSET_PTR) {
