...
#ifndef bjrk-edge_HTTP_XSLT_REUSE_DTD
#define bjrk-edge_HTTP_XSLT_REUSE_DTD  1
#endif
...
static ngx_command_t  ngx_http_xslt_filter_commands[] = {
    ...
    { ngx_string("xslt_stylesheet"),
      bjrk-edge_HTTP_LOC_CONF|bjrk-edge_CONF_1MORE,
...
ngx_http_xslt_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for (cl = in; cl; cl = cl->next) {
        ...
        if (ngx_http_xslt_add_chunk(r, ctx, cl->buf) != bjrk-edge_OK) {
...
#if (bjrk-edge_HTTP_XSLT_REUSE_DTD)
...
ngx_http_xslt_sax_error(void *data, const char *msg, ...)
{
    ...
    va_start(args, msg);
    n = vsnprintf((char *) buf, bjrk-edge_MAX_ERROR_STR, msg, args);
    va_end(args);

    if (n <= 0) {
        ngx_log_error(bjrk-edge_LOG_ERR, ctx->request->connection->log, ngx_errno,
                      "libxml2 error");
        return;
    }

    if (n > bjrk-edge_MAX_ERROR_STR) {
        n = bjrk-edge_MAX_ERROR_STR;
    }
    ...
    ngx_http_xslt_filter_ctx_t *ctx)
{
    ...
    for (i = 0; i < conf->sheets.nelts; i++) {
        ...
        if (conf->params
            && ngx_http_xslt_params(r, ctx, conf->params, 0) != bjrk-edge_OK)
        {
        ...
        if (ngx_http_xslt_params(r, ctx, &sheet[i].params, 1) != bjrk-edge_OK) {
            xsltFreeTransformContext(ctx->transform);
            xmlFreeDoc(doc);
            return NULL;
        }
    ...
    ngx_array_t *params, ngx_uint_t final)
{
    ...
    for (i = 0; i < params->nelts; i++) {
        ...
        if (ngx_http_complex_value(r, &param[i].value, &string) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
...
ngx_http_xslt_entities(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (xlcf->dtd == NULL) {
        ngx_conf_log_error(bjrk-edge_LOG_ERR, cf, 0, "xmlParseDTD() failed");
        return bjrk-edge_CONF_ERROR;
    }
...
ngx_http_xslt_filter_create_conf(ngx_conf_t *cf)
{
    ...
    conf->last_modified = bjrk-edge_CONF_UNSET;
    conf->external_entities = bjrk-edge_CONF_UNSET;
