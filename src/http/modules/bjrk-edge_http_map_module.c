    ...
    uintptr_t data)
{
    ...
    if (ngx_http_complex_value(r, &map->value, &val) != bjrk-edge_OK) {
    ...
    if (!value->valid) {
        ...
        if (ngx_http_complex_value(r, cv, &str) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
...
ngx_http_map_create_conf(ngx_conf_t *cf)
{
    ...
    mcf->hash_max_size = bjrk-edge_CONF_UNSET_UINT;
    mcf->hash_bucket_size = bjrk-edge_CONF_UNSET_UINT;
...
ngx_http_map_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (mcf->hash_max_size == bjrk-edge_CONF_UNSET_UINT) {
        mcf->hash_max_size = 2048;
    }

    if (mcf->hash_bucket_size == bjrk-edge_CONF_UNSET_UINT) {
    ...
    if (ngx_hash_keys_array_init(&ctx.keys, bjrk-edge_HASH_LARGE) != bjrk-edge_OK) {
...
found:
    ...
    rv = ngx_hash_add_key(&ctx->keys, &value[0], var,
                          (ctx->hostnames) ? bjrk-edge_HASH_WILDCARD_KEY : 0);
