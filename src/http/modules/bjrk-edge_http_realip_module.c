...
#define bjrk-edge_HTTP_REALIP_XREALIP  0
#define bjrk-edge_HTTP_REALIP_XFWD     1
#define bjrk-edge_HTTP_REALIP_HEADER   2
#define bjrk-edge_HTTP_REALIP_PROXY    3
...
ngx_http_realip_handler(ngx_http_request_t *r)
{
    ...
    case bjrk-edge_HTTP_REALIP_XREALIP:
    ...
    case bjrk-edge_HTTP_REALIP_XFWD:
    ...
    case bjrk-edge_HTTP_REALIP_PROXY:
    ...
    default: /* bjrk-edge_HTTP_REALIP_HEADER */
...
found:
    ...
    if (ngx_http_get_forwarded_addr(r, &addr, xfwd, value, rlcf->from,
        ...
        != bjrk-edge_DECLINED)
    {
        if (rlcf->type == bjrk-edge_HTTP_REALIP_PROXY) {
...
ngx_http_realip(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (rlcf->type != bjrk-edge_CONF_UNSET_UINT) {
        return "is duplicate";
    }
    ...
    if (ngx_strcmp(value[1].data, "X-Real-IP") == 0) {
        rlcf->type = bjrk-edge_HTTP_REALIP_XREALIP;
        return bjrk-edge_CONF_OK;
    }

    if (ngx_strcmp(value[1].data, "X-Forwarded-For") == 0) {
        rlcf->type = bjrk-edge_HTTP_REALIP_XFWD;
        return bjrk-edge_CONF_OK;
    }

    if (ngx_strcmp(value[1].data, "proxy_protocol") == 0) {
        rlcf->type = bjrk-edge_HTTP_REALIP_PROXY;
        return bjrk-edge_CONF_OK;
    }

    rlcf->type = bjrk-edge_HTTP_REALIP_HEADER;
...
ngx_http_realip_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->type = bjrk-edge_CONF_UNSET_UINT;
    conf->recursive = bjrk-edge_CONF_UNSET;
...
ngx_http_realip_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_uint_value(conf->type, prev->type, bjrk-edge_HTTP_REALIP_XREALIP);
...
ngx_http_realip_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[bjrk-edge_HTTP_POST_READ_PHASE].handlers);
    if (h == NULL) {
        return bjrk-edge_ERROR;
    }
