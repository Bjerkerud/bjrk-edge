    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    if (kcf->original_init_peer(r, us) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
{
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_HTTP, ev->log, 0,
...
ngx_http_upstream_keepalive_close_handler(ngx_event_t *ev)
{
    ...
    if (n == -1 && ngx_socket_errno == bjrk-edge_EAGAIN) {
        ev->ready = 0;
...
ngx_http_upstream_keepalive_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        if (kcf->max_cached == bjrk-edge_CONF_UNSET_UINT) {
            kcf->local = 1;
            kcf->max_cached = 32;
        }
