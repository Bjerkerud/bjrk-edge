...
#define bjrk-edge_HTTP_HEADERS_INHERIT_OFF    0
#define bjrk-edge_HTTP_HEADERS_INHERIT_ON     1
#define bjrk-edge_HTTP_HEADERS_INHERIT_MERGE  2
...
typedef enum {
    bjrk-edge_HTTP_EXPIRES_OFF,
    bjrk-edge_HTTP_EXPIRES_EPOCH,
    bjrk-edge_HTTP_EXPIRES_MAX,
    bjrk-edge_HTTP_EXPIRES_ACCESS,
    bjrk-edge_HTTP_EXPIRES_MODIFIED,
    bjrk-edge_HTTP_EXPIRES_DAILY,
    bjrk-edge_HTTP_EXPIRES_UNSET
} ngx_http_expires_t;
...
static ngx_conf_enum_t  ngx_http_headers_inherit[] = {
    { ngx_string("off"),   bjrk-edge_HTTP_HEADERS_INHERIT_OFF },
    { ngx_string("on"),    bjrk-edge_HTTP_HEADERS_INHERIT_ON },
    { ngx_string("merge"), bjrk-edge_HTTP_HEADERS_INHERIT_MERGE },
    { ngx_null_string, 0 }
};
...
ngx_http_headers_filter(ngx_http_request_t *r)
{
    ...
    if (conf->expires == bjrk-edge_HTTP_EXPIRES_OFF
        && conf->headers == NULL
        && conf->trailers == NULL)
    {
        return ngx_http_next_header_filter(r);
    ...
    case bjrk-edge_HTTP_CREATED:
    case bjrk-edge_HTTP_NO_CONTENT:
    case bjrk-edge_HTTP_PARTIAL_CONTENT:
    case bjrk-edge_HTTP_MOVED_PERMANENTLY:
    case bjrk-edge_HTTP_MOVED_TEMPORARILY:
    case bjrk-edge_HTTP_SEE_OTHER:
    case bjrk-edge_HTTP_NOT_MODIFIED:
    case bjrk-edge_HTTP_TEMPORARY_REDIRECT:
    case bjrk-edge_HTTP_PERMANENT_REDIRECT:
        safe_status = 1;
        break;
    ...
    if (conf->expires != bjrk-edge_HTTP_EXPIRES_OFF && safe_status) {
        if (ngx_http_set_expires(r, conf) != bjrk-edge_OK) {
    ...
    if (conf->headers) {
        ...
        for (i = 0; i < conf->headers->nelts; i++) {
            ...
            if (ngx_http_complex_value(r, &h[i].value, &value) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }

            if (h[i].handler(r, &h[i], &value) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        }
    }
...
ngx_http_set_expires(ngx_http_request_t *r, ngx_http_headers_conf_t *conf)
{
    ...
    if (conf->expires_value != NULL) {
        ...
        if (ngx_http_complex_value(r, conf->expires_value, &value) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
        ...
        if (expires == bjrk-edge_HTTP_EXPIRES_OFF) {
            return bjrk-edge_OK;
        }
    }
    ...
    if (expires == bjrk-edge_HTTP_EXPIRES_EPOCH) {
        e->value.data = (u_char *) "Thu, 01 Jan 1970 00:00:01 GMT";
        ngx_str_set(&cc->value, "no-cache");
        return bjrk-edge_OK;
    }

    if (expires == bjrk-edge_HTTP_EXPIRES_MAX) {
    ...
    if (expires_time == 0 && expires != bjrk-edge_HTTP_EXPIRES_DAILY) {
    ...
    if (expires == bjrk-edge_HTTP_EXPIRES_DAILY) {
        expires_time = ngx_next_time(expires_time);
        max_age = expires_time - now;

    } else if (expires == bjrk-edge_HTTP_EXPIRES_ACCESS
    ...
    cc->value.data = ngx_pnalloc(r->pool,
                                 sizeof("max-age=") + bjrk-edge_TIME_T_LEN + 1);
    ...
    time_t *expires_time, char **err)
{
    ...
    if (*expires != bjrk-edge_HTTP_EXPIRES_MODIFIED) {

        if (value->len == 5 && ngx_strncmp(value->data, "epoch", 5) == 0) {
            *expires = bjrk-edge_HTTP_EXPIRES_EPOCH;
            return bjrk-edge_OK;
        }

        if (value->len == 3 && ngx_strncmp(value->data, "max", 3) == 0) {
            *expires = bjrk-edge_HTTP_EXPIRES_MAX;
            return bjrk-edge_OK;
        }

        if (value->len == 3 && ngx_strncmp(value->data, "off", 3) == 0) {
            *expires = bjrk-edge_HTTP_EXPIRES_OFF;
            return bjrk-edge_OK;
        }
    }
    ...
    if (value->len && value->data[0] == '@') {
        ...
        *expires = bjrk-edge_HTTP_EXPIRES_DAILY;
    ...
    *expires_time = ngx_parse_time(value, 1);
    ...
    if (*expires_time == (time_t) bjrk-edge_ERROR) {
        *err = "invalid value";
        return bjrk-edge_ERROR;
    }
...
ngx_http_set_last_modified(ngx_http_request_t *r, ngx_http_header_val_t *hv,
    ngx_str_t *value)
{
    if (ngx_http_set_response_header(r, hv, value) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_headers_create_conf(ngx_conf_t *cf)
{
    ...
    conf->expires = bjrk-edge_HTTP_EXPIRES_UNSET;
    conf->headers_inherit = bjrk-edge_CONF_UNSET_UINT;
    conf->trailers_inherit = bjrk-edge_CONF_UNSET_UINT;
...
ngx_http_headers_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->expires == bjrk-edge_HTTP_EXPIRES_UNSET) {
        conf->expires = prev->expires;
        conf->expires_time = prev->expires_time;
        conf->expires_value = prev->expires_value;

        if (conf->expires == bjrk-edge_HTTP_EXPIRES_UNSET) {
            conf->expires = bjrk-edge_HTTP_EXPIRES_OFF;
        }
    }

    ngx_conf_merge_uint_value(conf->headers_inherit, prev->headers_inherit,
                              bjrk-edge_HTTP_HEADERS_INHERIT_ON);
    ngx_conf_merge_uint_value(conf->trailers_inherit, prev->trailers_inherit,
                              bjrk-edge_HTTP_HEADERS_INHERIT_ON);

    if (conf->headers_inherit != bjrk-edge_HTTP_HEADERS_INHERIT_OFF
        && prev->headers)
    {
        if (conf->headers == NULL) {
            conf->headers = prev->headers;

        } else if (conf->headers_inherit == bjrk-edge_HTTP_HEADERS_INHERIT_MERGE) {
    ...
    if (conf->trailers_inherit != bjrk-edge_HTTP_HEADERS_INHERIT_OFF
        && prev->trailers)
    {
        if (conf->trailers == NULL) {
            conf->trailers = prev->trailers;

        } else if (conf->trailers_inherit == bjrk-edge_HTTP_HEADERS_INHERIT_MERGE) {
...
ngx_http_headers_expires(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (hcf->expires != bjrk-edge_HTTP_EXPIRES_UNSET) {
        return "is duplicate";
    }
    ...
    if (cf->args->nelts == 2) {
        ...
        hcf->expires = bjrk-edge_HTTP_EXPIRES_ACCESS;
        ...
        hcf->expires = bjrk-edge_HTTP_EXPIRES_MODIFIED;
