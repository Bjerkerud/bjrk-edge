...
ngx_http_proxy_v2_handler(ngx_http_request_t *r)
{
    ...
    if (plcf->proxy_lengths == NULL) {
        ...
        if (ngx_http_proxy_eval(r, &ctx->ctx, plcf) != bjrk-edge_OK) {
            return bjrk-edge_HTTP_INTERNAL_SERVER_ERROR;
        }
    }

#if (bjrk-edge_HTTP_SSL)
    ngx_str_set(&u->ssl_alpn_protocol, bjrk-edge_HTTP_V2_ALPN_PROTO);
#endif
...
ngx_http_proxy_v2_create_request(ngx_http_request_t *r)
{
        ...
        || (method.len == 4 && ngx_strncmp(method.data, "POST", 4) == 0))
    {
        ...
        if (method.len > bjrk-edge_HTTP_V2_MAX_FIELD) {
            ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
                          "too long http2 method: \"%V\"", &method);
            return bjrk-edge_ERROR;
        }

        len += 1 + bjrk-edge_HTTP_V2_INT_OCTETS + method.len;
        tmp_len = method.len;
    }
    ...
    *b->last++ = ngx_http_v2_inc_indexed(bjrk-edge_HTTP_V2_AUTHORITY_INDEX);
    ...
    while (next) {
        p += sizeof(ngx_http_proxy_v2_frame_t) + bjrk-edge_HTTP_V2_DEFAULT_FRAME_SIZE;
        len = b->last - p;
...
ngx_http_proxy_v2_body_filter(ngx_event_pipe_t *p, ngx_buf_t *b)
{
    ...
    if (ngx_event_pipe_add_free_buf(p, b) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_proxy_v2_get_ctx(ngx_http_request_t *r)
{
    ...
    if (ctx->connection == NULL) {
        ...
        if (ngx_http_proxy_v2_get_connection_data(r, ctx, &u->peer) != bjrk-edge_OK) {
            return NULL;
        }
    }
