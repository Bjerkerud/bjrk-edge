    ...
    u_char *p)
{
    ...
    for (i = 0; i < alcf->rules6->nelts; i++) {
        ...
        {
        size_t  cl, ml, al;
        u_char  ct[bjrk-edge_INET6_ADDRSTRLEN];
        u_char  mt[bjrk-edge_INET6_ADDRSTRLEN];
        u_char  at[bjrk-edge_INET6_ADDRSTRLEN];

        cl = ngx_inet6_ntop(p, ct, bjrk-edge_INET6_ADDRSTRLEN);
        ml = ngx_inet6_ntop(rule6[i].mask.s6_addr, mt, bjrk-edge_INET6_ADDRSTRLEN);
        al = ngx_inet6_ntop(rule6[i].addr.s6_addr, at, bjrk-edge_INET6_ADDRSTRLEN);
