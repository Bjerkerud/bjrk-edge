...
ngx_http_slice_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->headers_out.status != bjrk-edge_HTTP_PARTIAL_CONTENT) {
        if (r == r->main) {
    ...
    if (ngx_http_slice_parse_content_range(r, &cr) != bjrk-edge_OK) {
        ngx_log_error(bjrk-edge_LOG_ERR, r->connection->log, 0,
...
ngx_http_slice_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (rc == bjrk-edge_ERROR || !ctx->last) {
        return rc;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (ctx == NULL) {
        ...
        p = ngx_pnalloc(r->pool, sizeof("bytes=-") - 1 + 2 * bjrk-edge_OFF_T_LEN);
        if (p == NULL) {
            return bjrk-edge_ERROR;
        }
...
ngx_http_slice_create_loc_conf(ngx_conf_t *cf)
{
    ...
    slcf->size = bjrk-edge_CONF_UNSET_SIZE;
