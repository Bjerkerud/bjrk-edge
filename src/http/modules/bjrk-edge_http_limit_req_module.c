...
#define bjrk-edge_HTTP_LIMIT_REQ_PASSED            1
#define bjrk-edge_HTTP_LIMIT_REQ_DELAYED           2
#define bjrk-edge_HTTP_LIMIT_REQ_REJECTED          3
#define bjrk-edge_HTTP_LIMIT_REQ_DELAYED_DRY_RUN   4
#define bjrk-edge_HTTP_LIMIT_REQ_REJECTED_DRY_RUN  5
...
static ngx_conf_enum_t  ngx_http_limit_req_log_levels[] = {
    { ngx_string("info"), bjrk-edge_LOG_INFO },
    { ngx_string("notice"), bjrk-edge_LOG_NOTICE },
    { ngx_string("warn"), bjrk-edge_LOG_WARN },
    { ngx_string("error"), bjrk-edge_LOG_ERR },
    { ngx_null_string, 0 }
};
...
static ngx_command_t  ngx_http_limit_req_commands[] = {
    ...
    { ngx_string("limit_req_zone"),
      bjrk-edge_HTTP_MAIN_CONF|bjrk-edge_CONF_TAKE3,
...
static ngx_http_variable_t  ngx_http_limit_req_vars[] = {
    ...
    { ngx_string("limit_req_status"), NULL,
      ngx_http_limit_req_status_variable, 0, bjrk-edge_HTTP_VAR_NOCACHEABLE, 0 },
...
ngx_http_limit_req_handler(ngx_http_request_t *r)
{
    ...
    for (n = 0; n < lrcf->limits.nelts; n++) {
        ...
        if (ngx_http_complex_value(r, &ctx->key, &key) != bjrk-edge_OK) {
    ...
    if (rc == bjrk-edge_BUSY || rc == bjrk-edge_ERROR) {
        ...
        if (lrcf->dry_run) {
            r->main->limit_req_status = bjrk-edge_HTTP_LIMIT_REQ_REJECTED_DRY_RUN;
            return bjrk-edge_DECLINED;
        }

        r->main->limit_req_status = bjrk-edge_HTTP_LIMIT_REQ_REJECTED;

        return lrcf->status_code;
    }

    /* rc == bjrk-edge_AGAIN || rc == bjrk-edge_OK */
    ...
    if (!delay) {
        r->main->limit_req_status = bjrk-edge_HTTP_LIMIT_REQ_PASSED;
        return bjrk-edge_DECLINED;
    }
    ...
    if (lrcf->dry_run) {
        r->main->limit_req_status = bjrk-edge_HTTP_LIMIT_REQ_DELAYED_DRY_RUN;
        return bjrk-edge_DECLINED;
    }

    r->main->limit_req_status = bjrk-edge_HTTP_LIMIT_REQ_DELAYED;
...
ngx_http_limit_req_create_conf(ngx_conf_t *cf)
{
    ...
    conf->limit_log_level = bjrk-edge_CONF_UNSET_UINT;
    conf->status_code = bjrk-edge_CONF_UNSET_UINT;
    conf->dry_run = bjrk-edge_CONF_UNSET;
...
ngx_http_limit_req_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    conf->delay_log_level = (conf->limit_log_level == bjrk-edge_LOG_INFO) ?
                                bjrk-edge_LOG_INFO : conf->limit_log_level + 1;

    ngx_conf_merge_uint_value(conf->status_code, prev->status_code,
                              bjrk-edge_HTTP_SERVICE_UNAVAILABLE);
...
ngx_http_limit_req(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (ngx_strcmp(value[i].data, "nodelay") == 0) {
            delay = bjrk-edge_MAX_INT_T_VALUE / 1000;
            continue;
        }
...
ngx_http_limit_req_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[bjrk-edge_HTTP_PREACCESS_PHASE].handlers);
    if (h == NULL) {
        return bjrk-edge_ERROR;
    }
