...
ngx_http_gunzip_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (!ctx->started) {
        if (ngx_http_gunzip_filter_inflate_start(r, ctx) != bjrk-edge_OK) {
            goto failed;
        }
    }
    ...
    ngx_http_gunzip_ctx_t *ctx)
{
    ...
    if (ctx->flush == Z_FINISH && ctx->zstream.avail_in == 0) {
        ...
        if (ngx_http_gunzip_filter_inflate_end(r, ctx) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
...
ngx_http_gunzip_filter_alloc(void *opaque, u_int items, u_int size)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_HTTP, ctx->request->connection->log, 0,
