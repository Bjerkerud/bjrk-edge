...
#include <bjrk-edge_crypt.h>


#define bjrk-edge_HTTP_AUTH_BUF_SIZE  2048
...
static ngx_command_t  ngx_http_auth_basic_commands[] = {
    ...
    { ngx_string("auth_basic"),
      bjrk-edge_HTTP_MAIN_CONF|bjrk-edge_HTTP_SRV_CONF|bjrk-edge_HTTP_LOC_CONF|bjrk-edge_HTTP_LMT_CONF
...
ngx_http_auth_basic_handler(ngx_http_request_t *r)
{
    ...
    u_char                           buf[bjrk-edge_HTTP_AUTH_BUF_SIZE];
    enum {
        sw_login,
        sw_passwd,
        sw_skip
    ...
    if (ngx_http_complex_value(r, alcf->realm, &realm) != bjrk-edge_OK) {
    ...
    if (ngx_http_complex_value(r, alcf->user_file, &user_file) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }

    fd = ngx_open_file(user_file.data, bjrk-edge_FILE_RDONLY, bjrk-edge_FILE_OPEN, 0);
    ...
    for ( ;; ) {
        ...
        n = ngx_read_file(&file, buf + left, bjrk-edge_HTTP_AUTH_BUF_SIZE - left,
                          offset);
...
cleanup:
    ...
    ngx_explicit_memzero(buf, bjrk-edge_HTTP_AUTH_BUF_SIZE);
...
ngx_http_auth_basic_set_realm(ngx_http_request_t *r, ngx_str_t *realm)
{
    ...
    *p = '"';
    ...
    if (ngx_http_proxy_auth(r)) {
        ...
        return bjrk-edge_HTTP_PROXY_AUTH_REQUIRED;

    } else {
        r->headers_out.www_authenticate = h;
        ngx_str_set(&h->key, "WWW-Authenticate");
        return bjrk-edge_HTTP_UNAUTHORIZED;
    }
}
...
ngx_http_auth_basic_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->realm = bjrk-edge_CONF_UNSET_PTR;
    conf->user_file = bjrk-edge_CONF_UNSET_PTR;
...
ngx_http_auth_basic_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[bjrk-edge_HTTP_ACCESS_PHASE].handlers);
    if (h == NULL) {
        return bjrk-edge_ERROR;
    }
...
ngx_http_auth_basic_user_file(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (alcf->user_file != bjrk-edge_CONF_UNSET_PTR) {
        return "is duplicate";
    }
