...
parse:
    ...
    case bjrk-edge_HTTP_RANGE_NOT_SATISFIABLE:
        return ngx_http_range_not_satisfiable(r);

    case bjrk-edge_ERROR:
        return bjrk-edge_ERROR;

    default: /* bjrk-edge_DECLINED */
        break;
    }
    ...
    ngx_uint_t ranges)
{
    ...
    cutoff = bjrk-edge_MAX_OFF_T_VALUE / 10;
    cutlim = bjrk-edge_MAX_OFF_T_VALUE % 10;
    ...
    for ( ;; ) {
        ...
        if (*p != '-') {
            if (*p < '0' || *p > '9') {
                return bjrk-edge_HTTP_RANGE_NOT_SATISFIABLE;
            }
    ...
    found:
        ...
        if (start < end) {
            ...
            if (size > bjrk-edge_MAX_OFF_T_VALUE - (end - start)) {
                return bjrk-edge_HTTP_RANGE_NOT_SATISFIABLE;
            }
    ...
    if (max_ranges == bjrk-edge_MAX_INT32_VALUE) {
    ...
    ngx_http_range_filter_ctx_t *ctx)
{
    ...
    content_range->value.data = ngx_pnalloc(r->pool,
                                    sizeof("bytes -/") - 1 + 3 * bjrk-edge_OFF_T_LEN);
    ...
    ngx_http_range_filter_ctx_t *ctx)
{
    ...
    size = sizeof(CRLF "--") - 1 + bjrk-edge_ATOMIC_T_LEN
    ...
    len = sizeof(CRLF "--") - 1 + bjrk-edge_ATOMIC_T_LEN + sizeof("--" CRLF) - 1;
    ...
    for (i = 0; i < ctx->ranges.nelts; i++) {
        ...
        range[i].content_range.data =
                               ngx_pnalloc(r->pool, 3 * bjrk-edge_OFF_T_LEN + 2 + 4);
...
ngx_http_range_not_satisfiable(ngx_http_request_t *r)
{
    ...
    r->headers_out.status = bjrk-edge_HTTP_RANGE_NOT_SATISFIABLE;
    ...
    content_range->value.data = ngx_pnalloc(r->pool,
                                       sizeof("bytes */") - 1 + bjrk-edge_OFF_T_LEN);
    if (content_range->value.data == NULL) {
        content_range->hash = 0;
        r->headers_out.content_range = NULL;
        return bjrk-edge_ERROR;
...
ngx_http_range_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ngx_http_range_test_overlapped(r, ctx, in) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_http_range_filter_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    b->pos = ngx_pnalloc(r->pool, sizeof(CRLF "--") - 1 + bjrk-edge_ATOMIC_T_LEN
                                  + sizeof("--" CRLF) - 1);
    if (b->pos == NULL) {
        return bjrk-edge_ERROR;
    }
    ...
    b->last = ngx_cpymem(b->pos, ctx->boundary_header.data,
                         sizeof(CRLF "--") - 1 + bjrk-edge_ATOMIC_T_LEN);
    *b->last++ = '-'; *b->last++ = '-';
    *b->last++ = CR; *b->last++ = LF;
