...
ngx_http_write_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for (cl = r->out; cl; cl = cl->next) {
        ...
        ngx_log_debug7(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
    ...
    *ll = NULL;
    ...
    if (c->write->delayed) {
        c->buffered |= bjrk-edge_HTTP_WRITE_BUFFERED;
        return bjrk-edge_AGAIN;
    }

    if (size == 0
        && !(c->buffered & bjrk-edge_LOWLEVEL_BUFFERED)
    ...
    if ((c->buffered & bjrk-edge_LOWLEVEL_BUFFERED) && r->postponed == NULL) {
        return bjrk-edge_AGAIN;
    }
