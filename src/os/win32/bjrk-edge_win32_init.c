...
char        ngx_unique[bjrk-edge_INT32_LEN + 1];
...
#pragma warning(default:4996)
    ...
    if (ngx_win32_version < bjrk-edge_WIN_NT) {
        ngx_max_wsabufs = 16;
        return bjrk-edge_OK;
    }
    ...
    if (WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &ax_guid, sizeof(GUID),
        ...
        == -1)
    {
        ngx_log_error(bjrk-edge_LOG_NOTICE, log, ngx_socket_errno,
    ...
    if (hmod == NULL) {
        ngx_log_error(bjrk-edge_LOG_NOTICE, log, ngx_errno,
...
nopoll:
    ...
    if (GetEnvironmentVariable("ngx_unique", ngx_unique, bjrk-edge_INT32_LEN + 1)
        != 0)
    {
...
ngx_os_status(ngx_log_t *log)
{
    ...
    if (osviex) {
        ...
        if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
            ...
            ngx_log_error(bjrk-edge_LOG_INFO, log, 0, "OS: %ui build:%ud, \"%s\"",
