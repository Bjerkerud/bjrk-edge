...
#define bjrk-edge_UTF16_BUFLEN  256
#define bjrk-edge_UTF8_BUFLEN   512
...
ngx_open_file(u_char *name, u_long mode, u_long create, u_long access)
{
    ...
    ngx_fd_t    fd;
    ngx_err_t   err;
    u_short     utf16[bjrk-edge_UTF16_BUFLEN];

    len = bjrk-edge_UTF16_BUFLEN;
    ...
    if (create == bjrk-edge_FILE_OPEN
        && ngx_win32_check_filename(u, len, 0) != bjrk-edge_OK)
    {
        goto failed;
    }
...
ngx_read_file(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
    ...
    if (ReadFile(file->fd, buf, size, &n, povlp) == 0) {
        ...
        ngx_log_error(bjrk-edge_LOG_ERR, file->log, err,
...
ngx_write_file(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
    ...
    if (WriteFile(file->fd, buf, size, &n, povlp) == 0) {
        ngx_log_error(bjrk-edge_LOG_ERR, file->log, ngx_errno,
                      "WriteFile() \"%s\" failed", file->name.data);
        return bjrk-edge_ERROR;
    }

    if (n != size) {
        ngx_log_error(bjrk-edge_LOG_CRIT, file->log, 0,
...
ngx_delete_file(u_char *name)
{
    ...
    rc = bjrk-edge_FILE_ERROR;

    if (ngx_win32_check_filename(u, len, 0) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_rename_file(u_char *from, u_char *to)
{
    ...
    u_short    *fu, *tu;
    ngx_err_t   err;
    u_short     utf16f[bjrk-edge_UTF16_BUFLEN];
    u_short     utf16t[bjrk-edge_UTF16_BUFLEN];
    ...
    if (ngx_win32_check_filename(fu, len, 0) != bjrk-edge_OK) {
        goto failed;
    }
    ...
    if (ngx_win32_check_filename(tu, len, 1) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_win32_rename_file(ngx_str_t *from, ngx_str_t *to, ngx_log_t *log)
{
    ...
    name = ngx_alloc(to->len + 1 + bjrk-edge_ATOMIC_T_LEN + 1 + sizeof("DELETE"),
                     log);
    if (name == NULL) {
        return bjrk-edge_ENOMEM;
    }
    ...
    for ( ;; ) {
        ...
        if (ngx_rename_file(to->data, name) != bjrk-edge_FILE_ERROR) {
            break;
        }
        ...
        ngx_log_error(bjrk-edge_LOG_CRIT, log, err,
                      "MoveFile() \"%s\" to \"%s\" failed", to->data, name);
        goto failed;
    }

    if (ngx_rename_file(from->data, to->data) == bjrk-edge_FILE_ERROR) {
        err = ngx_errno;
...
ngx_set_file_time(u_char *name, ngx_fd_t fd, time_t s)
{
    ...
    /* 116444736000000000 is commented in src/os/win32/bjrk-edge_time.c */
...
ngx_create_file_mapping(ngx_file_mapping_t *fm)
{
    ...
    fm->fd = ngx_open_file(fm->name, bjrk-edge_FILE_RDWR, bjrk-edge_FILE_TRUNCATE,
                           bjrk-edge_FILE_DEFAULT_ACCESS);

    if (fm->fd == bjrk-edge_INVALID_FILE) {
        ngx_log_error(bjrk-edge_LOG_CRIT, fm->log, ngx_errno,
...
failed:
    ...
    if (fm->handle) {
        if (CloseHandle(fm->handle) == 0) {
            ngx_log_error(bjrk-edge_LOG_ALERT, fm->log, ngx_errno,
                          "CloseHandle() of file mapping \"%s\" failed",
                          fm->name);
        }
    }

    if (ngx_close_file(fm->fd) == bjrk-edge_FILE_ERROR) {
        ngx_log_error(bjrk-edge_LOG_ALERT, fm->log, ngx_errno,
...
ngx_getcwd(u_char *buf, size_t size)
{
    u_char   *p;
    size_t    n;
    u_short   utf16[bjrk-edge_MAX_PATH];

    n = GetCurrentDirectoryW(bjrk-edge_MAX_PATH, utf16);

    if (n == 0) {
        return 0;
    }

    if (n > bjrk-edge_MAX_PATH) {
...
ngx_create_dir(u_char *name, ngx_uint_t access)
{
    ...
    if (ngx_win32_check_filename(u, len, 1) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_read_glob(ngx_glob_t *gl, ngx_str_t *name)
{
    ...
    size_t      len;
    ngx_err_t   err;
    u_char      utf8[bjrk-edge_UTF8_BUFLEN];
    ...
    ngx_log_error(bjrk-edge_LOG_ALERT, gl->log, err,
...
ngx_close_glob(ngx_glob_t *gl)
{
    ...
    if (FindClose(gl->dir) == 0) {
        ngx_log_error(bjrk-edge_LOG_ALERT, gl->log, ngx_errno,
...
ngx_fs_available(u_char *name)
{
    ...
    if (u == NULL) {
        return bjrk-edge_MAX_OFF_T_VALUE;
    }
...
ngx_win32_check_filename(u_short *u, size_t len, ngx_uint_t dirname)
{
    ...
    if (n == 0) {
        ...
        if (dirname && slash && ngx_errno == bjrk-edge_ENOENT) {
            ngx_set_errno(bjrk-edge_ENOPATH);
        }
...
invalid:
    ...
    ngx_set_errno(bjrk-edge_ENOENT);
...
ngx_utf8_to_utf16(u_short *utf16, u_char *utf8, size_t *len, size_t reserved)
{
    ...
    while (u < last) {
        ...
        if (n > 0x10ffff) {
            ngx_set_errno(bjrk-edge_EILSEQ);
            return NULL;
        }
