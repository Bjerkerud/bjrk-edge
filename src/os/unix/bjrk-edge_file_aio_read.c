    ...
    ngx_pool_t *pool)
{
    ...
    if (file->aio == NULL && ngx_file_aio_init(file, pool) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    if (n == -1) {
        ...
        if (n == bjrk-edge_EAGAIN) {
            return ngx_read_file(file, buf, size, offset);
        }

        ngx_log_error(bjrk-edge_LOG_CRIT, file->log, n,
                      "aio_read(\"%V\") failed", &file->name);

        if (n == bjrk-edge_ENOSYS) {
            ngx_file_aio = 0;
            return ngx_read_file(file, buf, size, offset);
        }
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_CORE, file->log, 0,
...
ngx_file_aio_result(ngx_file_t *file, ngx_event_aio_t *aio, ngx_event_t *ev)
{
    ...
    if (n == -1) {
        ...
        ngx_log_error(bjrk-edge_LOG_ALERT, file->log, err,
                      "aio_error(\"%V\") failed", &file->name);
        return bjrk-edge_ERROR;
    }

    if (n == bjrk-edge_EINPROGRESS) {
        if (ev->ready) {
            ev->ready = 0;
            ngx_log_error(bjrk-edge_LOG_ALERT, file->log, n,
...
ngx_file_aio_event_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_CORE, ev->log, 0,
                   "aio event handler fd:%d %V", aio->fd, &aio->file->name);

    if (ngx_file_aio_result(aio->file, aio, ev) != bjrk-edge_AGAIN) {
        aio->handler(ev);
    }
}
