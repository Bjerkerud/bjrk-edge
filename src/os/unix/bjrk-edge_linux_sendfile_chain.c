 ...
 * see src/os/unix/bjrk-edge_linux_config.h
 *
...
#define bjrk-edge_SENDFILE_MAXSIZE  2147483647L
...
ngx_linux_sendfile_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    ngx_chain_t   *cl;
    ngx_iovec_t    header;
    struct iovec   headers[bjrk-edge_IOVS_PREALLOCATE];
    ...
    if (limit == 0 || limit > (off_t) (bjrk-edge_SENDFILE_MAXSIZE - ngx_pagesize)) {
        limit = bjrk-edge_SENDFILE_MAXSIZE - ngx_pagesize;
    }
    ...
    header.iovs = headers;
    header.nalloc = bjrk-edge_IOVS_PREALLOCATE;
    ...
    for ( ;; ) {
        ...
        if (c->tcp_nopush == bjrk-edge_TCP_NOPUSH_UNSET
            && header.count != 0
            && cl
            && cl->buf->in_file)
        {
            ...
            if (c->tcp_nodelay == bjrk-edge_TCP_NODELAY_SET) {
                               ...
                               (const void *) &tcp_nodelay, sizeof(int)) == -1)
                {
                    ...
                    c->tcp_nodelay = bjrk-edge_TCP_NODELAY_UNSET;
            ...
            if (c->tcp_nodelay == bjrk-edge_TCP_NODELAY_UNSET) {
                ...
                if (ngx_tcp_nopush(c->fd) == -1) {
                    ...
                    c->tcp_nopush = bjrk-edge_TCP_NOPUSH_SET;
...
eintr:
    ...
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "sendfile: %z of %uz @%O",
...
ngx_linux_sendfile_thread(ngx_connection_t *c, ngx_buf_t *file, size_t size)
{
    ...
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_CORE, c->log, 0,
    ...
    if (file->file->thread_handler(task, file->file) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_linux_sendfile_thread_handler(void *data, ngx_log_t *log)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, log, 0, "linux sendfile thread handler");
...
again:
    ...
    ngx_log_debug4(bjrk-edge_LOG_DEBUG_EVENT, log, 0,
                   "sendfile: %z (err: %d) of %uz @%O",
                   n, ctx->err, ctx->size, file->file_pos);

    if (ctx->err == bjrk-edge_EINTR) {
        goto again;
    }
}
