...
#if (bjrk-edge_TEST_BUILD_SOLARIS_SENDFILEV)
...
#define bjrk-edge_SENDFILEVECS  bjrk-edge_IOVS_PREALLOCATE
...
ngx_solaris_sendfilev_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    ngx_buf_t      *file;
    ngx_uint_t      nsfv;
    sendfilevec_t  *sfv, sfvs[bjrk-edge_SENDFILEVECS];
    ngx_event_t    *wev;
    ngx_chain_t    *cl;
        ...
        for (cl = in; cl && send < limit; cl = cl->next) {
            ...
            if (ngx_buf_in_memory_only(cl->buf)) {
                ...
                if (prev == cl->buf->pos) {
                    ...
                    if (nsfv == bjrk-edge_SENDFILEVECS) {
                        break;
                    }
