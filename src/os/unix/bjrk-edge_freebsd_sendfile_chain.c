...
ngx_freebsd_sendfile_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    struct iovec     trailers[bjrk-edge_IOVS_PREALLOCATE];
    ...
    if ((ngx_event_flags & bjrk-edge_USE_KQUEUE_EVENT) && wev->pending_eof) {
    ...
    if (limit == 0 || limit > (off_t) (bjrk-edge_MAX_SIZE_T_VALUE - ngx_pagesize)) {
        limit = bjrk-edge_MAX_SIZE_T_VALUE - ngx_pagesize;
    }
    ...
    trailer.nalloc = bjrk-edge_IOVS_PREALLOCATE;
    ...
    for ( ;; ) {
        ...
        if (cl && cl->buf->in_file && send < limit) {
            ...
            if (rc == -1) {
                ...
                ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, err,
