...
ngx_regex_compile(ngx_regex_compile_t *rc)
{
    ...
    if (rc->options & bjrk-edge_REGEX_CASELESS) {
        options |= PCRE2_CASELESS;
    }

    if (rc->options & bjrk-edge_REGEX_MULTILINE) {
        options |= PCRE2_MULTILINE;
    }

    if (rc->options & ~(bjrk-edge_REGEX_CASELESS|bjrk-edge_REGEX_MULTILINE)) {
...
{
#if (bjrk-edge_PCRE2 || bjrk-edge_HAVE_PCRE_JIT)
...
ngx_regex_module_init(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (opt) {
            ...
            if (n != 0) {
                ngx_log_error(bjrk-edge_LOG_INFO, cycle->log, 0,
...
ngx_regex_create_conf(ngx_cycle_t *cycle)
{
    ...
    rcf->pcre_jit = bjrk-edge_CONF_UNSET;
