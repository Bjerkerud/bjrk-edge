...
#define bjrk-edge_MIN_READ_AHEAD  (128 * 1024)
...
ngx_open_file_cache_cleanup(void *data)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, ngx_cycle->log, 0,
    ...
    for ( ;; ) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, ngx_cycle->log, 0,
    ...
    ngx_open_file_info_t *of, ngx_pool_t *pool)
{
    ...
    if (file) {
        ...
        if (file->fd == bjrk-edge_INVALID_FILE && file->err == 0 && !file->is_dir) {

            /* file was not used often enough to keep open */

            rc = ngx_open_and_stat_file(name, of, pool->log);

            if (rc != bjrk-edge_OK && (of->err == 0 || !of->errors)) {
                goto failed;
            }
        ...
        ngx_log_debug4(bjrk-edge_LOG_DEBUG_CORE, pool->log, 0,
        ...
        if (file->count == 0) {
            ...
            if (ngx_close_file(file->fd) == bjrk-edge_FILE_ERROR) {
                ngx_log_error(bjrk-edge_LOG_ALERT, pool->log, ngx_errno,
...
found:
    ...
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_CORE, pool->log, 0,
...
failed:
    ...
    if (of->fd != bjrk-edge_INVALID_FILE) {
        if (ngx_close_file(of->fd) == bjrk-edge_FILE_ERROR) {
            ngx_log_error(bjrk-edge_LOG_ALERT, pool->log, ngx_errno,
                          ngx_close_file_n " \"%V\" failed", name);
        }
    }
...
#if (bjrk-edge_HAVE_O_PATH)
    if (ngx_file_o_path_info(fd, &fi, log) == bjrk-edge_ERROR) {
        err = ngx_errno;
        goto failed;
    }
#else
...
ngx_file_o_path_info(ngx_fd_t fd, ngx_file_info_t *fi, ngx_log_t *log)
{
    ...
    if (ngx_file_at_info(fd, "", fi, AT_EMPTY_PATH) != bjrk-edge_FILE_ERROR) {
        return bjrk-edge_OK;
    }
    ...
    ngx_int_t mode, ngx_int_t create, ngx_int_t access, ngx_log_t *log)
{
    ...
    if (of->disable_symlinks == bjrk-edge_DISABLE_SYMLINKS_OFF) {
        fd = ngx_open_file(name->data, mode, create, access);
    ...
    if (of->disable_symlinks_from) {
        ...
        *cp = '\0';
        ...
        at_fd = ngx_open_file(p, bjrk-edge_FILE_SEARCH|bjrk-edge_FILE_NONBLOCK,
                              bjrk-edge_FILE_OPEN, 0);

        *cp = '/';

        if (at_fd == bjrk-edge_INVALID_FILE) {
            of->err = ngx_errno;
            of->failed = ngx_open_file_n;
            return bjrk-edge_INVALID_FILE;
        }
        ...
        at_fd = ngx_open_file("/",
                              bjrk-edge_FILE_SEARCH|bjrk-edge_FILE_NONBLOCK,
                              bjrk-edge_FILE_OPEN, 0);
    ...
    for ( ;; ) {
        ...
        *cp = '\0';
        ...
        if (of->disable_symlinks == bjrk-edge_DISABLE_SYMLINKS_NOTOWNER) {
            fd = ngx_openat_file_owner(at_fd, p,
                                       bjrk-edge_FILE_SEARCH|bjrk-edge_FILE_NONBLOCK,
                                       bjrk-edge_FILE_OPEN, 0, log);

        } else {
            fd = ngx_openat_file(at_fd, p,
                           bjrk-edge_FILE_SEARCH|bjrk-edge_FILE_NONBLOCK|bjrk-edge_FILE_NOFOLLOW,
                           bjrk-edge_FILE_OPEN, 0);
        }
        ...
        *cp = '/';
        ...
        if (at_fd != bjrk-edge_AT_FDCWD && ngx_close_file(at_fd) == bjrk-edge_FILE_ERROR) {
            ngx_log_error(bjrk-edge_LOG_ALERT, log, ngx_errno,
    ...
    if (of->disable_symlinks == bjrk-edge_DISABLE_SYMLINKS_NOTOWNER
        && !(create & (bjrk-edge_FILE_CREATE_OR_OPEN|bjrk-edge_FILE_TRUNCATE)))
    {
        fd = ngx_openat_file_owner(at_fd, p, mode, create, access, log);

    } else {
        fd = ngx_openat_file(at_fd, p, mode|bjrk-edge_FILE_NOFOLLOW, create, access);
    }
    ...
    ngx_file_info_t *fi, ngx_log_t *log)
{
    ...
    if (rc == bjrk-edge_FILE_ERROR) {
        of->err = ngx_errno;
        of->failed = ngx_file_info_n;
        return bjrk-edge_FILE_ERROR;
    }
    ...
    fd = ngx_open_file_wrapper(name, of, bjrk-edge_FILE_RDONLY|bjrk-edge_FILE_NONBLOCK,
                               bjrk-edge_FILE_OPEN, 0, log);
    ...
    ngx_log_t *log)
{
    ...
    if (of->fd != bjrk-edge_INVALID_FILE) {
        ...
        if (ngx_file_info_wrapper(name, of, &fi, log) == bjrk-edge_FILE_ERROR) {
            of->fd = bjrk-edge_INVALID_FILE;
            return bjrk-edge_ERROR;
        }
    ...
    if (!of->log) {
        ...
        fd = ngx_open_file_wrapper(name, of, bjrk-edge_FILE_APPEND,
                                   bjrk-edge_FILE_CREATE_OR_OPEN,
                                   bjrk-edge_FILE_DEFAULT_ACCESS, log);
    }
    ...
    if (ngx_fd_info(fd, &fi) == bjrk-edge_FILE_ERROR) {
        ngx_log_error(bjrk-edge_LOG_CRIT, log, ngx_errno,
    ...
    if (ngx_is_dir(&fi)) {
        ...
        if (of->read_ahead && ngx_file_size(&fi) > bjrk-edge_MIN_READ_AHEAD) {
            if (ngx_read_ahead(fd, of->read_ahead) == bjrk-edge_ERROR) {
                ngx_log_error(bjrk-edge_LOG_ALERT, log, ngx_errno,
        ...
        if (of->directio <= ngx_file_size(&fi)) {
            if (ngx_directio_on(fd) == bjrk-edge_FILE_ERROR) {
                ngx_log_error(bjrk-edge_LOG_ALERT, log, ngx_errno,
    ...
    ngx_cached_open_file_t *file, ngx_open_file_info_t *of, ngx_log_t *log)
{
    ...
    if (!(ngx_event_flags & bjrk-edge_USE_VNODE_EVENT)
        || !of->events
        || file->event
        || of->fd == bjrk-edge_INVALID_FILE
    ...
    if (ngx_add_event(file->event, bjrk-edge_VNODE_EVENT, bjrk-edge_ONESHOT_EVENT)
        != bjrk-edge_OK)
    {
...
ngx_close_cached_file(ngx_open_file_cache_t *cache,
    ngx_cached_open_file_t *file, ngx_uint_t min_uses, ngx_log_t *log)
{
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_CORE, log, 0,
...
ngx_open_file_del_event(ngx_cached_open_file_t *file)
{
    ...
    (void) ngx_del_event(file->event, bjrk-edge_VNODE_EVENT,
                         file->count ? bjrk-edge_FLUSH_EVENT : bjrk-edge_CLOSE_EVENT);
    ...
    ngx_log_t *log)
{
    ...
    while (n < 3) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, log, 0,
...
ngx_open_file_cache_remove(ngx_event_t *ev)
{
    ...
    /* bjrk-edge_ONESHOT_EVENT was already deleted */
    file->event = NULL;
    file->use_event = 0;
