...
#define bjrk-edge_PROXY_PROTOCOL_V2_MAX_HEADER    52

#define bjrk-edge_PROXY_PROTOCOL_CMD_LOCAL        0
#define bjrk-edge_PROXY_PROTOCOL_CMD_PROXY        1

#define bjrk-edge_PROXY_PROTOCOL_AF_UNSPEC        0
#define bjrk-edge_PROXY_PROTOCOL_AF_INET          1
#define bjrk-edge_PROXY_PROTOCOL_AF_INET6         2

#define bjrk-edge_PROXY_PROTOCOL_TYPE_UNSPEC      0
#define bjrk-edge_PROXY_PROTOCOL_TYPE_STREAM      1
#define bjrk-edge_PROXY_PROTOCOL_TYPE_DGRAM       2

#define bjrk-edge_PROXY_PROTOCOL_TLV_ALPN         0x01
#define bjrk-edge_PROXY_PROTOCOL_TLV_AUTHORITY    0x02
#define bjrk-edge_PROXY_PROTOCOL_TLV_CRC32C       0x03
#define bjrk-edge_PROXY_PROTOCOL_TLV_UNIQUE_ID    0x05
#define bjrk-edge_PROXY_PROTOCOL_TLV_SSL          0x20
#define bjrk-edge_PROXY_PROTOCOL_TLV_SSL_VERSION  0x21
#define bjrk-edge_PROXY_PROTOCOL_TLV_SSL_CN       0x22
#define bjrk-edge_PROXY_PROTOCOL_TLV_SSL_CIPHER   0x23
#define bjrk-edge_PROXY_PROTOCOL_TLV_SSL_SIG_ALG  0x24
#define bjrk-edge_PROXY_PROTOCOL_TLV_SSL_KEY_ALG  0x25
#define bjrk-edge_PROXY_PROTOCOL_TLV_NETNS        0x30

#define bjrk-edge_PROXY_PROTOCOL_V2_CLIENT_SSL          0x01
#define bjrk-edge_PROXY_PROTOCOL_V2_CLIENT_CERT_CONN    0x02
#define bjrk-edge_PROXY_PROTOCOL_V2_CLIENT_CERT_SESS    0x04
...
static ngx_proxy_protocol_tlv_entry_t  ngx_proxy_protocol_tlv_entries[] = {
    { ngx_string("alpn"),       bjrk-edge_PROXY_PROTOCOL_TLV_ALPN },
    { ngx_string("authority"),  bjrk-edge_PROXY_PROTOCOL_TLV_AUTHORITY },
    { ngx_string("unique_id"),  bjrk-edge_PROXY_PROTOCOL_TLV_UNIQUE_ID },
    { ngx_string("ssl"),        bjrk-edge_PROXY_PROTOCOL_TLV_SSL },
    { ngx_string("netns"),      bjrk-edge_PROXY_PROTOCOL_TLV_NETNS },
    { ngx_null_string,          0x00 }
};


static ngx_proxy_protocol_tlv_entry_t  ngx_proxy_protocol_tlv_ssl_entries[] = {
    { ngx_string("version"),    bjrk-edge_PROXY_PROTOCOL_TLV_SSL_VERSION },
    { ngx_string("cn"),         bjrk-edge_PROXY_PROTOCOL_TLV_SSL_CN },
    { ngx_string("cipher"),     bjrk-edge_PROXY_PROTOCOL_TLV_SSL_CIPHER },
    { ngx_string("sig_alg"),    bjrk-edge_PROXY_PROTOCOL_TLV_SSL_SIG_ALG },
    { ngx_string("key_alg"),    bjrk-edge_PROXY_PROTOCOL_TLV_SSL_KEY_ALG },
    { ngx_null_string,          0x00 }
};
...
ngx_proxy_protocol_read(ngx_connection_t *c, u_char *buf, u_char *last)
{
    ...
    if (len >= 7 && ngx_strncmp(p, "UNKNOWN", 7) == 0) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, c->log, 0,
    ...
    ngx_log_debug4(bjrk-edge_LOG_DEBUG_CORE, c->log, 0,
...
ngx_proxy_protocol_write(ngx_connection_t *c, u_char *buf, u_char *last)
{
    ...
    if (last - buf < bjrk-edge_PROXY_PROTOCOL_V1_MAX_HEADER) {
...
ngx_proxy_protocol_v2_read(ngx_connection_t *c, u_char *buf, u_char *last)
{
    ...
    if ((size_t) (last - buf) < len) {
        ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "header is too large");
        return NULL;
    }
    ...
    if (command != bjrk-edge_PROXY_PROTOCOL_CMD_PROXY) {
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, c->log, 0,
    ...
    if (transport != bjrk-edge_PROXY_PROTOCOL_TYPE_STREAM) {
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, c->log, 0,
    ...
    case bjrk-edge_PROXY_PROTOCOL_AF_INET:
    ...
    case bjrk-edge_PROXY_PROTOCOL_AF_INET6:
    ...
    pp->src_addr.data = ngx_pnalloc(c->pool, bjrk-edge_SOCKADDR_STRLEN);
    if (pp->src_addr.data == NULL) {
        return NULL;
    }

    pp->src_addr.len = ngx_sock_ntop(&src_sockaddr.sockaddr, socklen,
                                     pp->src_addr.data, bjrk-edge_SOCKADDR_STRLEN, 0);

    pp->dst_addr.data = ngx_pnalloc(c->pool, bjrk-edge_SOCKADDR_STRLEN);
    if (pp->dst_addr.data == NULL) {
        return NULL;
    }

    pp->dst_addr.len = ngx_sock_ntop(&dst_sockaddr.sockaddr, socklen,
                                     pp->dst_addr.data, bjrk-edge_SOCKADDR_STRLEN, 0);
    ...
    ngx_str_t *value)
{
    ...
    if (n >= 4 && p[0] == 's' && p[1] == 's' && p[2] == 'l' && p[3] == '_') {
        ...
        rc = ngx_proxy_protocol_lookup_tlv(c, tlvs,
                                           bjrk-edge_PROXY_PROTOCOL_TLV_SSL, &ssl);
        if (rc != bjrk-edge_OK) {
            return rc;
        }
        ...
        if (n == 6 && ngx_strncmp(p, "verify", 6) == 0) {
            ...
            value->data = ngx_pnalloc(c->pool, bjrk-edge_INT32_LEN);
            if (value->data == NULL) {
                return bjrk-edge_ERROR;
            }
    ...
    ngx_uint_t type, ngx_str_t *value)
{
    ...
    while (n) {
        if (n < sizeof(ngx_proxy_protocol_tlv_t)) {
            ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "broken PROXY protocol TLV");
            return bjrk-edge_ERROR;
        }
    ...
    ngx_array_t *ssl_tlvs, ngx_uint_t *client, uint32_t *verify)
{
    ...
    if (ngx_proxy_protocol_v2_authority(c, &value) == bjrk-edge_OK) {
        tlv = ngx_array_push(tlvs);
        if (tlv == NULL) {
            return bjrk-edge_ERROR;
        }

        tlv->type = bjrk-edge_PROXY_PROTOCOL_TLV_AUTHORITY;
        tlv->value = value;
    }

    if (ngx_proxy_protocol_v2_alpn(c, &value) == bjrk-edge_OK) {
        tlv = ngx_array_push(tlvs);
        if (tlv == NULL) {
            return bjrk-edge_ERROR;
        }

        tlv->type = bjrk-edge_PROXY_PROTOCOL_TLV_ALPN;
        tlv->value = value;
    }

    for (type = bjrk-edge_PROXY_PROTOCOL_TLV_SSL_VERSION;
         type <= bjrk-edge_PROXY_PROTOCOL_TLV_SSL_KEY_ALG;
         type++)
    {
    ...
    *client = bjrk-edge_PROXY_PROTOCOL_V2_CLIENT_SSL;
    *verify = 1;  /* X509_V_ERR_UNSPECIFIED */
    ...
    if (cert != NULL) {
        ...
        *client |= bjrk-edge_PROXY_PROTOCOL_V2_CLIENT_CERT_SESS;

        if (!SSL_session_reused(c->ssl->connection)) {
            *client |= bjrk-edge_PROXY_PROTOCOL_V2_CLIENT_CERT_CONN;
        }
    ...
    ngx_str_t *out)
{
    ...
    case bjrk-edge_PROXY_PROTOCOL_TLV_SSL_VERSION:
        out->data = (u_char *) SSL_get_version(c->ssl->connection);
        out->len = ngx_strlen(out->data);
        return bjrk-edge_OK;

    case bjrk-edge_PROXY_PROTOCOL_TLV_SSL_CIPHER:
        out->data = (u_char *) SSL_get_cipher_name(c->ssl->connection);
        out->len = ngx_strlen(out->data);
        return bjrk-edge_OK;

    case bjrk-edge_PROXY_PROTOCOL_TLV_SSL_CN:
        cert = SSL_get_peer_certificate(c->ssl->connection);
        if (cert == NULL) {
            return bjrk-edge_DECLINED;
        }
    ...
    case bjrk-edge_PROXY_PROTOCOL_TLV_SSL_SIG_ALG:
    ...
    case bjrk-edge_PROXY_PROTOCOL_TLV_SSL_KEY_ALG:
        cert = SSL_get_certificate(c->ssl->connection);
    ...
    u_char *last)
{
    ...
    if (last - buf < bjrk-edge_PROXY_PROTOCOL_V2_MAX_HEADER) {
        ngx_log_error(bjrk-edge_LOG_ALERT, c->log, 0,
    ...
    if (src_af == bjrk-edge_PROXY_PROTOCOL_AF_UNSPEC
        || dst_af == bjrk-edge_PROXY_PROTOCOL_AF_UNSPEC)
    {
        command = bjrk-edge_PROXY_PROTOCOL_CMD_LOCAL;
        family = bjrk-edge_PROXY_PROTOCOL_AF_UNSPEC;
        transport = bjrk-edge_PROXY_PROTOCOL_TYPE_UNSPEC;

    } else {
        command = bjrk-edge_PROXY_PROTOCOL_CMD_PROXY;
        family = ngx_max(src_af, dst_af);

        switch (c->type) {
        case SOCK_STREAM:
            transport = bjrk-edge_PROXY_PROTOCOL_TYPE_STREAM;
            break;
        case SOCK_DGRAM:
            transport = bjrk-edge_PROXY_PROTOCOL_TYPE_DGRAM;
            break;
        default:
            transport = bjrk-edge_PROXY_PROTOCOL_TYPE_UNSPEC;
            break;
    ...
    case bjrk-edge_PROXY_PROTOCOL_AF_INET6:
        ...
        if (src_fam == AF_INET6 && dst_fam == AF_INET6) {
            ...
            if (IN6_IS_ADDR_V4MAPPED(src6) && IN6_IS_ADDR_V4MAPPED(dst6)) {
                ...
                header->family_transport = (bjrk-edge_PROXY_PROTOCOL_AF_INET << 4)
                                           | transport;
    ...
    default:  /* bjrk-edge_PROXY_PROTOCOL_AF_UNSPEC */
        break;
    }
...
ngx_proxy_protocol_v2_family(ngx_uint_t family)
{
    ...
    case AF_INET:
        return bjrk-edge_PROXY_PROTOCOL_AF_INET;

#if (bjrk-edge_HAVE_INET6)
    case AF_INET6:
        return bjrk-edge_PROXY_PROTOCOL_AF_INET6;
#endif

    default:
        return bjrk-edge_PROXY_PROTOCOL_AF_UNSPEC;
    }
}
    ...
    ngx_array_t *ssl_tlvs, ngx_uint_t client, uint32_t verify)
{
    ...
    tlv->type = bjrk-edge_PROXY_PROTOCOL_TLV_SSL;
    ...
    u_char *p, u_char *last)
{
    ...
    tlv = (ngx_proxy_protocol_tlv_t *) p;
    tlv->type = bjrk-edge_PROXY_PROTOCOL_TLV_CRC32C;
    tlv->len[0] = 0;
    tlv->len[1] = 4;
