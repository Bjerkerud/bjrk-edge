...
{
    u_char               *p;
#if (bjrk-edge_HAVE_INET6 || bjrk-edge_HAVE_UNIX_DOMAIN)
    size_t                n;
#endif
...
ngx_inet6_ntop(u_char *p, u_char *text, size_t len)
{
    ...
    if (len < bjrk-edge_INET6_ADDRSTRLEN) {
        return 0;
    }
...
#if (bjrk-edge_HAVE_INET6)
    } else if (ngx_inet6_addr(addr, len, cidr->u.in6.addr.s6_addr) == bjrk-edge_OK) {
...
#if (bjrk-edge_HAVE_INET6)
    } else if (ngx_inet6_addr(text, len, inaddr6.s6_addr) == bjrk-edge_OK) {
        family = AF_INET6;
...
no_port:
    ...
    if (ngx_inet_resolve_host(pool, u) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_parse_inet6_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (ngx_inet6_addr(host, len, sin6->sin6_addr.s6_addr) != bjrk-edge_OK) {
...
#if (bjrk-edge_HAVE_GETADDRINFO && bjrk-edge_HAVE_INET6)
...
#else /* !bjrk-edge_HAVE_GETADDRINFO || !bjrk-edge_HAVE_INET6 */
...
#endif /* bjrk-edge_HAVE_GETADDRINFO && bjrk-edge_HAVE_INET6 */
...
#if (bjrk-edge_HAVE_INET6)
        case AF_INET6:
            len = bjrk-edge_INET6_ADDRSTRLEN + sizeof("[]:65536") - 1;
            break;
#endif
