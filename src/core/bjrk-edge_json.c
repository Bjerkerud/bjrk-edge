...
ngx_json_render(ngx_pool_t *pool, ngx_data_item_t *item)
{
    ...
    if (length == (size_t) bjrk-edge_ERROR) {
        return NULL;
    }
...
ngx_json_obj_length(ngx_data_item_t *item)
{
    ...
    if (i) {
        for ( ;; ) {
            ...
            if (total > bjrk-edge_MAX_SIZE_T_VALUE - len) {
            ...
            if (total > bjrk-edge_MAX_SIZE_T_VALUE - (sizeof(",") - 1)) {
                return bjrk-edge_ERROR;
            }
