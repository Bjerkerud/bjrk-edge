...
#if 0
#define bjrk-edge_SENDFILE_LIMIT  4096
#endif
...
ngx_output_chain(ngx_output_chain_ctx_t *ctx, ngx_chain_t *in)
{
       ...
       )
    {
        ...
        if (in->next == NULL
#if (bjrk-edge_SENDFILE_LIMIT)
            && !(in->buf->in_file && in->buf->file_last > bjrk-edge_SENDFILE_LIMIT)
#endif
    ...
    if (in) {
        if (ngx_output_chain_add_copy(ctx->pool, &ctx->in, in) == bjrk-edge_ERROR) {
    ...
    for ( ;; ) {
        ...
        while (ctx->in) {
            ...
            if (bsize == 0 && !ngx_buf_special(ctx->in->buf)) {
                ...
                ngx_log_error(bjrk-edge_LOG_ALERT, ctx->pool->log, 0,
            ...
            if (ctx->buf == NULL) {
                ...
                if (rc != bjrk-edge_OK) {
                    ...
                    } else if (ngx_output_chain_get_buf(ctx, bsize) != bjrk-edge_OK) {
                        return bjrk-edge_ERROR;
                    }
                }
            }
        ...
        if (out == NULL && last != bjrk-edge_NONE) {
        ...
        if (last == bjrk-edge_ERROR || last == bjrk-edge_DONE) {
            return last;
        }
...
ngx_output_chain_as_is(ngx_output_chain_ctx_t *ctx, ngx_buf_t *buf)
{
    ...
    if (buf->in_file && buf->file_pos >= bjrk-edge_SENDFILE_LIMIT) {
        sendfile = 0;
    }
    ...
    ngx_chain_t *in)
{
    ...
    while (in) {
        ...
        if (buf->in_file
            && buf->file_pos < bjrk-edge_SENDFILE_LIMIT
            && buf->file_last > bjrk-edge_SENDFILE_LIMIT)
        {
            ...
            if (ngx_buf_in_memory(buf)) {
                buf->pos += (ssize_t) (bjrk-edge_SENDFILE_LIMIT - buf->file_pos);
                b->last = buf->pos;
            }

            buf->file_pos = bjrk-edge_SENDFILE_LIMIT;
            b->file_last = bjrk-edge_SENDFILE_LIMIT;
...
#if (bjrk-edge_HAVE_ALIGNED_DIRECTIO)
    ctx->unaligned = 1;
#endif
...
ngx_output_chain_copy_buf(ngx_output_chain_ctx_t *ctx)
{
    ...
    if (src->in_file && src->file_pos >= bjrk-edge_SENDFILE_LIMIT) {
        sendfile = 0;
    }
    ...
    if (ngx_buf_in_memory(src)) {
        ...
        if (ctx->unaligned) {
            if (ngx_directio_off(src->file->fd) == bjrk-edge_FILE_ERROR) {
                ngx_log_error(bjrk-edge_LOG_ALERT, ctx->pool->log, ngx_errno,
        ...
        if (ctx->unaligned) {
            ...
            if (ngx_directio_on(src->file->fd) == bjrk-edge_FILE_ERROR) {
                ngx_log_error(bjrk-edge_LOG_ALERT, ctx->pool->log, ngx_errno,
...
ngx_chain_writer(void *data, ngx_chain_t *in)
{
    ...
    for (size = 0; in; in = in->next) {
        ...
        ngx_log_debug2(bjrk-edge_LOG_DEBUG_CORE, c->log, 0,
    ...
    if (chain == bjrk-edge_CHAIN_ERROR) {
        return bjrk-edge_ERROR;
    }
