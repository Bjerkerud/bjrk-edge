...
ngx_radix_tree_create(ngx_pool_t *pool, ngx_int_t preallocate)
{
    ...
    tree->root->value = bjrk-edge_RADIX_NO_VALUE;
    ...
    while (preallocate--) {
        ...
        do {
            if (ngx_radix32tree_insert(tree, key, mask, bjrk-edge_RADIX_NO_VALUE)
                != bjrk-edge_OK)
            {
                return NULL;
            }
    ...
    uintptr_t value)
{
    ...
    if (next) {
        if (node->value != bjrk-edge_RADIX_NO_VALUE) {
    ...
    while (bit & mask) {
        ...
        next->left = NULL;
        next->parent = node;
        next->value = bjrk-edge_RADIX_NO_VALUE;
...
ngx_radix32tree_delete(ngx_radix_tree_t *tree, uint32_t key, uint32_t mask)
{
    ...
    if (node->right || node->left) {
        if (node->value != bjrk-edge_RADIX_NO_VALUE) {
            node->value = bjrk-edge_RADIX_NO_VALUE;
            return bjrk-edge_OK;
        }
...
ngx_radix32tree_find(ngx_radix_tree_t *tree, uint32_t key)
{
    ...
    bit = 0x80000000;
    value = bjrk-edge_RADIX_NO_VALUE;
    node = tree->root;
