...
#define bjrk-edge_SYSLOG_MAX_STR                                                    \
    bjrk-edge_MAX_ERROR_STR + sizeof("<255>Jan 01 00:00:00 ") - 1                   \
    + (bjrk-edge_MAXHOSTNAMELEN - 1) + 1 /* space */                                \
...
ngx_syslog_process_conf(ngx_conf_t *cf, ngx_syslog_peer_t *peer)
{
    ...
    peer->facility = bjrk-edge_CONF_UNSET_UINT;
    peer->severity = bjrk-edge_CONF_UNSET_UINT;

    if (ngx_syslog_parse_args(cf, peer) != bjrk-edge_CONF_OK) {
    ...
    if (peer->facility == bjrk-edge_CONF_UNSET_UINT) {
        peer->facility = 23; /* local7 */
    }

    if (peer->severity == bjrk-edge_CONF_UNSET_UINT) {
...
ngx_syslog_writer(ngx_log_t *log, ngx_uint_t level, u_char *buf,
    size_t len)
{
    u_char             *p, msg[bjrk-edge_SYSLOG_MAX_STR];
    ngx_uint_t          head_len;
    ...
    if (len > bjrk-edge_SYSLOG_MAX_STR - head_len) {
        len = bjrk-edge_SYSLOG_MAX_STR - head_len;
    }
...
ngx_syslog_send(ngx_syslog_peer_t *peer, u_char *buf, size_t len)
{
    ...
    if (peer->conn.fd == (ngx_socket_t) -1) {
        if (ngx_syslog_init_peer(peer) != bjrk-edge_OK) {
    ...
    if (n == bjrk-edge_ERROR) {
        ...
        if (ngx_close_socket(peer->conn.fd) == -1) {
            ngx_log_error(bjrk-edge_LOG_ALERT, &peer->log, ngx_socket_errno,
