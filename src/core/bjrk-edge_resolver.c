...
#define bjrk-edge_RESOLVER_UDP_SIZE   4096

#define bjrk-edge_RESOLVER_TCP_RSIZE  (2 + 65535)
#define bjrk-edge_RESOLVER_TCP_WSIZE  8192
...
ngx_resolver_create(ngx_conf_t *cf, ngx_str_t *names, ngx_uint_t n)
{
    ...
    for (i = 0; i < n; i++) {
        if (ngx_strncmp(names[i].data, "valid=", 6) == 0) {
            ...
            if (r->valid == (time_t) bjrk-edge_ERROR) {
    ...
    if (n && r->connections.nelts == 0) {
        ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0, "no name servers defined");
        return NULL;
    }
...
ngx_resolver_cleanup(void *data)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, ngx_cycle->log, 0, "cleanup resolver");
...
ngx_resolve_name(ngx_resolver_ctx_t *ctx)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, r->log, 0,
                   "resolve: \"%V\"", &ctx->name);
...
ngx_resolve_name_done(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->state == bjrk-edge_AGAIN || ctx->state == bjrk-edge_RESOLVE_TIMEDOUT) {
        ...
        if (rn) {
            ...
            ngx_log_error(bjrk-edge_LOG_ALERT, r->log, 0,
    ...
    ngx_str_t *name)
{
    ...
    if (rn) {
        ...
        /* ctx can be a list after bjrk-edge_RESOLVE_CNAME */
        for (last = ctx; last->next; last = last->next);

        if (rn->valid >= ngx_time()) {

            ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, r->log, 0, "resolve cached");
            ...
            /* bjrk-edge_RESOLVE_CNAME */

            if (ctx->recursion++ < bjrk-edge_RESOLVER_MAX_RECURSION) {
            ...
            do {
                ctx->state = bjrk-edge_RESOLVE_NXDOMAIN;
        ...
        if (rn->waiting) {
            if (ngx_resolver_set_timeout(r, ctx) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }

            last->next = rn->waiting;
            rn->waiting = ctx;
            ctx->state = bjrk-edge_AGAIN;
            ctx->async = 1;
    ...
    if (ngx_resolver_send_query(r, rn) != bjrk-edge_OK) {
    ...
    rn->cnlen = 0;
    rn->valid = 0;
    rn->ttl = bjrk-edge_MAX_UINT32_VALUE;
    rn->waiting = ctx;
...
ngx_resolve_addr(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ngx_resolver_create_addr_query(r, rn, &ctx->addr) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_resolver_expire(ngx_resolver_t *r, ngx_rbtree_t *tree, ngx_queue_t *queue)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, r->log, 0, "resolver expire");
    ...
    for (i = 0; i < 2; i++) {
        ...
        ngx_log_debug2(bjrk-edge_LOG_DEBUG_CORE, r->log, 0,
    ...
    u_char *query, u_short qlen)
{
    ...
    if (rec->udp == NULL) {
        if (ngx_udp_connect(rec) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    ...
    if ((size_t) n != (size_t) qlen) {
        ngx_log_error(bjrk-edge_LOG_CRIT, &rec->log, 0, "send() incomplete");
        goto failed;
    }
    ...
    u_char *query, u_short qlen)
{
    ...
    if (rec->tcp == NULL) {
        ...
        if (b == NULL) {
            ...
            b->start = ngx_resolver_alloc(r, bjrk-edge_RESOLVER_TCP_RSIZE);
            if (b->start == NULL) {
                ngx_resolver_free(r, b);
                return bjrk-edge_ERROR;
            }

            b->end = b->start + bjrk-edge_RESOLVER_TCP_RSIZE;
        ...
        if (b == NULL) {
            ...
            b->start = ngx_resolver_alloc(r, bjrk-edge_RESOLVER_TCP_WSIZE);
            if (b->start == NULL) {
                ngx_resolver_free(r, b);
                return bjrk-edge_ERROR;
            }

            b->end = b->start + bjrk-edge_RESOLVER_TCP_WSIZE;
    ...
    if (b->end - b->last <  2 + qlen) {
        ngx_log_error(bjrk-edge_LOG_CRIT, &rec->log, 0, "buffer overflow");
        return bjrk-edge_ERROR;
    }
...
ngx_resolver_resend_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_CORE, r->log, 0,
...
ngx_resolver_resend(ngx_resolver_t *r, ngx_rbtree_t *tree, ngx_queue_t *queue)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug3(bjrk-edge_LOG_DEBUG_CORE, r->log, 0,
                       "resolver resend \"%*s\" %p",
...
ngx_resolver_udp_read(ngx_event_t *rev)
{
    ...
    ngx_connection_t           *c;
    ngx_resolver_connection_t  *rec;
    u_char                      buf[bjrk-edge_RESOLVER_UDP_SIZE];

    c = rev->data;
    rec = c->data;

    do {
        n = ngx_udp_recv(c, buf, bjrk-edge_RESOLVER_UDP_SIZE);
    ...
    ngx_uint_t tcp)
{
    ...
    ngx_log_debug6(bjrk-edge_LOG_DEBUG_CORE, r->log, 0,
    ...
    if (code == bjrk-edge_RESOLVE_FORMERR) {
    ...
    if (code > bjrk-edge_RESOLVE_REFUSED) {
        goto dns_error;
    }
...
found:
    ...
    case bjrk-edge_RESOLVE_A:
#if (bjrk-edge_HAVE_INET6)
    case bjrk-edge_RESOLVE_AAAA:
#endif
    ...
    ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, r->log, 0, "resolver qs:%V", &name);
    ...
    if (code == 0 && nan == 0) {
        ...
        code = bjrk-edge_RESOLVE_NXDOMAIN;
    }
    ...
    found:
        ...
        case bjrk-edge_RESOLVE_AAAA:
            ...
            if (qtype != bjrk-edge_RESOLVE_AAAA) {
        ...
        case bjrk-edge_RESOLVE_CNAME:

            cname = &buf[i];

            break;

        case bjrk-edge_RESOLVE_DNAME:
...
#if (bjrk-edge_HAVE_INET6 && bjrk-edge_SUPPRESS_WARN)
            addr6 = NULL;
#endif
        }
...
#if (bjrk-edge_HAVE_INET6)
            else if (type == bjrk-edge_RESOLVE_AAAA) {
    ...
    if (cname) {
        ...
        if (ngx_resolver_copy(r, &name, buf, cname, buf + n) != bjrk-edge_OK) {
            goto failed;
        }
...
ngx_resolver_resolve_srv_names(ngx_resolver_ctx_t *ctx, ngx_resolver_node_t *rn)
{
    ...
    for (i = 0; i < (ngx_uint_t) rn->nsrvs; i++) {
        ...
        if (ngx_resolve_name(cctx) == bjrk-edge_ERROR) {
            srvs[i].ctx = NULL;
            goto failed;
        }
    }
    ...
    ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan)
{
    ...
    for (mask = 0; mask < 32; mask += 8) {
        ...
        if (octet == bjrk-edge_ERROR || octet > 255) {
...
ptr:
    ...
    if (ngx_resolver_copy(r, &name, buf, buf + i, buf + n) != bjrk-edge_OK) {
        goto failed;
    }

    ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, r->log, 0, "resolver an:%V", &name);
    ...
    ngx_str_t *name)
{
    ...
    /* query type */
    qs->type_hi = 0; qs->type_lo = bjrk-edge_RESOLVE_A;
    ...
    *p = (u_char) len;
    ...
    qs->type_lo = bjrk-edge_RESOLVE_AAAA;
#endif
    ...
    ngx_str_t *name)
{
    ...
    /* query type */
    qs->type_hi = 0; qs->type_lo = bjrk-edge_RESOLVE_SRV;
...
ngx_resolver_timeout_handler(ngx_event_t *ev)
{
    ...
    ctx->state = bjrk-edge_RESOLVE_TIMEDOUT;
...
ngx_resolver_report_srv(ngx_resolver_t *r, ngx_resolver_ctx_t *ctx)
{
    ...
    for (i = 0; i < nsrvs; i++) {
        if (srvs[i].state == bjrk-edge_ERROR) {
            ctx->state = bjrk-edge_ERROR;
            ctx->valid = ngx_time() + (r->valid ? r->valid : 10);
    ...
    if (naddrs == 0) {
        ...
        for (i = 0; i < nsrvs; i++) {
            if (srvs[i].state == bjrk-edge_RESOLVE_NXDOMAIN) {
                ctx->state = bjrk-edge_RESOLVE_NXDOMAIN;
                break;
            }
        }
...
ngx_resolver_strerror(ngx_int_t err)
{
    ...
    if (err == bjrk-edge_RESOLVE_TIMEDOUT) {
...
ngx_udp_connect(ngx_resolver_connection_t *rec)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, &rec->log, 0, "UDP socket %d", s);

    if (s == (ngx_socket_t) -1) {
        ngx_log_error(bjrk-edge_LOG_ALERT, &rec->log, ngx_socket_errno,
                      ngx_socket_n " failed");
        return bjrk-edge_ERROR;
    }
    ...
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, &rec->log, 0,
    ...
    if (rc == -1) {
        ngx_log_error(bjrk-edge_LOG_CRIT, &rec->log, ngx_socket_errno,
                      "connect() failed");
    ...
    event = (ngx_event_flags & bjrk-edge_USE_CLEAR_EVENT) ?
                /* kqueue, epoll */                 bjrk-edge_CLEAR_EVENT:
                /* select, poll, /dev/poll */       bjrk-edge_LEVEL_EVENT;
                /* eventport event type has no meaning: oneshot only */

    if (ngx_add_event(rev, bjrk-edge_READ_EVENT, event) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_tcp_connect(ngx_resolver_connection_t *rec)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, &rec->log, 0, "TCP socket %d", s);
    ...
    if (ngx_add_conn) {
        if (ngx_add_conn(c) == bjrk-edge_ERROR) {
            goto failed;
        }
    }
    ...
    if (rc == -1) {
        ...
        if (err != bjrk-edge_EINPROGRESS
#if (bjrk-edge_WIN32)
            /* Winsock returns WSAEWOULDBLOCK (bjrk-edge_EAGAIN) */
            && err != bjrk-edge_EAGAIN
#endif
            )
        {
            if (err == bjrk-edge_ECONNREFUSED
#if (bjrk-edge_LINUX)
                /*
    ...
    if (ngx_add_conn) {
        if (rc == -1) {
            ...
            /* bjrk-edge_EINPROGRESS */

            return bjrk-edge_AGAIN;
        }

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, &rec->log, 0, "connected");
    ...
    if (ngx_event_flags & bjrk-edge_USE_IOCP_EVENT) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, &rec->log, ngx_socket_errno,
                       "connect(): %d", rc);
         ...
         *
         * TODO: check in Win32, etc. As workaround we can use bjrk-edge_ONESHOT_EVENT
         */
    ...
    if (ngx_event_flags & bjrk-edge_USE_CLEAR_EVENT) {
        ...
        event = bjrk-edge_CLEAR_EVENT;

    } else {

        /* select, poll, /dev/poll */

        event = bjrk-edge_LEVEL_EVENT;
    }
    ...
    if (rc == -1) {
        ...
        if (ngx_add_event(wev, bjrk-edge_WRITE_EVENT, event) != bjrk-edge_OK) {
            goto failed;
        }
