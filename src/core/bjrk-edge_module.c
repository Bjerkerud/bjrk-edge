...
#define bjrk-edge_MAX_DYNAMIC_MODULES  128
...
ngx_preinit_modules(void)
{
    ...
    ngx_modules_n = i;
    ngx_max_module = ngx_modules_n + bjrk-edge_MAX_DYNAMIC_MODULES;
...
ngx_init_modules(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; cycle->modules[i]; i++) {
        if (cycle->modules[i]->init_module) {
            if (cycle->modules[i]->init_module(cycle) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        }
    }
...
ngx_count_modules(ngx_cycle_t *cycle, ngx_uint_t type)
{
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        if (module->ctx_index != bjrk-edge_MODULE_UNSET_INDEX) {
    ...
    char **order)
{
    ...
    if (ngx_strcmp(module->signature, bjrk-edge_MODULE_SIGNATURE) != 0) {
        ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, 0,
    ...
    if (module->index == bjrk-edge_MODULE_UNSET_INDEX) {
        ...
        for ( /* void */ ; order[i]; i++) {
...
#if 0
            ngx_log_debug2(bjrk-edge_LOG_DEBUG_CORE, cf->log, 0,
                           "module: %s before %s",
                           module->name, order[i]);
#endif
            ...
            for (m = 0; m < before; m++) {
                if (ngx_strcmp(cf->cycle->modules[m]->name, order[i]) == 0) {
                    ...
                    ngx_log_debug3(bjrk-edge_LOG_DEBUG_CORE, cf->log, 0,
                                   "module: %s before %s:%i",
    ...
    if (module->type == bjrk-edge_CORE_MODULE) {
