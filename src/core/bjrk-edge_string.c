...
{
    u_char         *p, temp[bjrk-edge_INT64_LEN + 1];
                       /*
                        * we need temp[bjrk-edge_INT64_LEN] only,
    ...
    p = temp + bjrk-edge_INT64_LEN;

    if (hexadecimal == 0) {

        if (ui64 <= (uint64_t) bjrk-edge_MAX_UINT32_VALUE) {
    ...
    len = (temp + bjrk-edge_INT64_LEN) - p;
...
ngx_atosz(u_char *line, size_t n)
{
    ...
    cutoff = bjrk-edge_MAX_SIZE_T_VALUE / 10;
    cutlim = bjrk-edge_MAX_SIZE_T_VALUE % 10;
...
ngx_atotm(u_char *line, size_t n)
{
    ...
    cutoff = bjrk-edge_MAX_TIME_T_VALUE / 10;
    cutlim = bjrk-edge_MAX_TIME_T_VALUE % 10;
...
ngx_hextoi(u_char *line, size_t n)
{
    ...
    cutoff = bjrk-edge_MAX_INT_T_VALUE / 16;
...
ngx_unescape_uri(u_char **dst, u_char **src, size_t size, ngx_uint_t type)
{
    ...
    while (size--) {
        ...
        case sw_usual:
            if (ch == '?'
                && (type & (bjrk-edge_UNESCAPE_URI|bjrk-edge_UNESCAPE_REDIRECT)))
            {
                *d++ = ch;
                goto done;
            }
        ...
        case sw_quoted_second:
            ...
            if (ch >= '0' && ch <= '9') {
                ...
                if (type & bjrk-edge_UNESCAPE_REDIRECT) {
                    if (ch > '%' && ch < 0x7f) {
                        *d++ = ch;
                        break;
                    }
            ...
            if (c >= 'a' && c <= 'f') {
                ...
                if (type & bjrk-edge_UNESCAPE_URI) {
                    if (ch == '?') {
                        *d++ = ch;
                        goto done;
                    }
...
#if (bjrk-edge_MEMCPY_LIMIT)

void *
ngx_memcpy(void *dst, const void *src, size_t n)
{
    if (n > bjrk-edge_MEMCPY_LIMIT) {
        ngx_log_error(bjrk-edge_LOG_ALERT, ngx_cycle->log, 0, "memcpy %uz bytes", n);
        ngx_debug_point();
    }
