...
ngx_event_pipe(ngx_event_pipe_t *p, ngx_int_t do_write)
{
    ...
    for ( ;; ) {
        ...
        if (ngx_event_pipe_read_upstream(p) == bjrk-edge_ABORT) {
            return bjrk-edge_ABORT;
        }
        ...
        && p->upstream->fd != (ngx_socket_t) -1)
    {
        ...
        flags = (rev->eof || rev->error) ? bjrk-edge_CLOSE_EVENT : 0;

        if (ngx_handle_read_event(rev, flags) != bjrk-edge_OK) {
            return bjrk-edge_ABORT;
        }
...
ngx_event_pipe_read_upstream(ngx_event_pipe_t *p)
{
    ...
    if (p->aio) {
        ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, p->log, 0,
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, p->log, 0,
    ...
    for ( ;; ) {
        ...
        if (p->preread_bufs) {
                ...
                )
            {
                ...
                if (p->upstream->read->kq_errno) {
                    ...
                    ngx_log_error(bjrk-edge_LOG_ERR, p->log,
        ...
        while (cl && n > 0) {
            ...
            if (n >= size) {
                ...
                if (p->input_filter(p, cl->buf) == bjrk-edge_ERROR) {
                    return bjrk-edge_ABORT;
                }
    ...
    for (cl = p->busy; cl; cl = cl->next) {
        ngx_log_debug8(bjrk-edge_LOG_DEBUG_EVENT, p->log, 0,
    ...
    if ((p->upstream_eof || p->upstream_error) && p->free_raw_bufs) {
        ...
        if (p->input_filter(p, p->free_raw_bufs->buf) == bjrk-edge_ERROR) {
            return bjrk-edge_ABORT;
        }
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {
            if (p->out) {
                ...
                if (cl->buf->recycled) {
                    ngx_log_error(bjrk-edge_LOG_ALERT, p->log, 0,
                ...
                ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, p->log, 0,
    ...
    flush:
        ...
        for (cl = p->free; cl; cl = cl->next) {
            ...
            if (cl->buf->last_shadow) {
                if (ngx_event_pipe_add_free_buf(p, cl->buf->shadow) != bjrk-edge_OK) {
                    return bjrk-edge_ABORT;
                }
...
ngx_event_pipe_write_chain_to_temp_file(ngx_event_pipe_t *p)
{
    ...
    if (!p->cacheable) {
        ...
        do {
            ...
            ngx_log_debug4(bjrk-edge_LOG_DEBUG_EVENT, p->log, 0,
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, p->log, 0, "size: %z", size);
