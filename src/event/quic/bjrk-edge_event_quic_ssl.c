 ...
 */
#define bjrk-edge_QUIC_MAX_BUFFERED    65535


#if (bjrk-edge_QUIC_OPENSSL_API)
...
static int ngx_quic_set_write_secret(ngx_ssl_conn_t *ssl_conn,
    enum ssl_encryption_level_t ssl_level, const SSL_CIPHER *cipher,
    const uint8_t *secret, size_t secret_len);
#else /* bjrk-edge_QUIC_QUICTLS_API */
    ...
    const unsigned char *data, size_t len, size_t *consumed, void *arg)
{
    ...
    *consumed = 0;
    ...
    if (alpn_len == 0) {
        qc->error = bjrk-edge_QUIC_ERR_CRYPTO(SSL_AD_NO_APPLICATION_PROTOCOL);
    ...
    if (!qc->client_tp_done) {
        ...
        qc->error = bjrk-edge_QUIC_ERR_CRYPTO(SSL_AD_MISSING_EXTENSION);
    ...
    frame->type = bjrk-edge_QUIC_FT_CRYPTO;
    ...
    int direction, const unsigned char *secret, size_t secret_len, void *arg)
{
    ...
    case OSSL_RECORD_PROTECTION_LEVEL_NONE:
        level = bjrk-edge_QUIC_ENCRYPTION_INITIAL;
        break;
    case OSSL_RECORD_PROTECTION_LEVEL_EARLY:
        level = bjrk-edge_QUIC_ENCRYPTION_EARLY_DATA;
        break;
    case OSSL_RECORD_PROTECTION_LEVEL_HANDSHAKE:
        level = bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE;
        break;
    default: /* OSSL_RECORD_PROTECTION_LEVEL_APPLICATION */
        level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
        break;
    }
    ...
    const unsigned char *params, size_t params_len, void *arg)
{
    ...
    if (ngx_quic_parse_transport_params(p, end, &ctp, c->log) != bjrk-edge_OK) {
    ...
    if (ngx_quic_apply_transport_params(c, &ctp) != bjrk-edge_OK) {
        return 1;
    }
...
ngx_quic_cbs_alert(ngx_ssl_conn_t *ssl_conn, unsigned char alert, void *arg)
{
    ...
    qc->error = bjrk-edge_QUIC_ERR_CRYPTO(alert);
...
ngx_quic_map_encryption_level(enum ssl_encryption_level_t ssl_level)
{
    ...
    case ssl_encryption_initial:
        return bjrk-edge_QUIC_ENCRYPTION_INITIAL;
    case ssl_encryption_early_data:
        return bjrk-edge_QUIC_ENCRYPTION_EARLY_DATA;
    case ssl_encryption_handshake:
        return bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE;
    default: /* ssl_encryption_application */
        return bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    }
}
    ...
    const uint8_t *wsecret, size_t secret_len)
{
    ...
    if (level == bjrk-edge_QUIC_ENCRYPTION_EARLY_DATA) {
        return 1;
    }
    ...
    ngx_quic_frame_t *frame)
{
    ...
    if (last > ctx->crypto.offset + bjrk-edge_QUIC_MAX_BUFFERED) {
        qc->error = bjrk-edge_QUIC_ERR_CRYPTO_BUFFER_EXCEEDED;
    ...
    if (ngx_quic_crypto_provide(c, pkt->level) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_handshake(ngx_connection_t *c)
{
    ...
    if (n <= 0) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_get_error: %d",
                       sslerr);
    ...
    if (!SSL_is_init_finished(ssl_conn)) {
        if (ngx_quic_keys_available(qc->keys, bjrk-edge_QUIC_ENCRYPTION_EARLY_DATA, 0)
            && qc->client_tp_done)
        {
            if (ngx_quic_init_streams(c) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        }
    ...
    frame->type = bjrk-edge_QUIC_FT_HANDSHAKE_DONE;
    ngx_quic_queue_frame(qc, frame);

    if (qc->conf->retry) {
        if (ngx_quic_send_new_token(c, qc->path) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
     ...
     */
    ngx_quic_discard_ctx(c, bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE);

    ngx_quic_discover_path_mtu(c, qc->path);

    /* start accepting clients on negotiated number of server ids */
    if (ngx_quic_create_sockets(c) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
{
#if (bjrk-edge_QUIC_BORINGSSL_API || bjrk-edge_QUIC_QUICTLS_API)
    ...
    switch (level) {
    case bjrk-edge_QUIC_ENCRYPTION_INITIAL:
        ssl_level = ssl_encryption_initial;
        break;
    case bjrk-edge_QUIC_ENCRYPTION_EARLY_DATA:
        ssl_level = ssl_encryption_early_data;
        break;
    case bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE:
        ssl_level = ssl_encryption_handshake;
        break;
    default: /* bjrk-edge_QUIC_ENCRYPTION_APPLICATION */
...
ngx_quic_init_connection(ngx_connection_t *c)
{
    ...
    if (ngx_ssl_create_connection(qc->conf->ssl, c, 0) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
