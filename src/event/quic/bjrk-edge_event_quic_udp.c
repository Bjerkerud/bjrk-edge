...
ngx_quic_recvmsg(ngx_event_t *ev)
{
    ...
    static u_char       buffer[bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE];
    ...
    do {
        ...
        if (n == -1) {
            ...
            ngx_log_error(bjrk-edge_LOG_ALERT, ev->log, err, "quic recvmsg() failed");
        ...
        if (sockaddr->sa_family == AF_UNIX) {
            ...
            if (socklen <= (socklen_t) offsetof(struct sockaddr_un, sun_path)
                || saun->sun_path[0] == '\0')
            {
                ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ngx_cycle->log, 0,
                               "unbound unix socket");
                goto next;
            }
        }
        ...
        if (ls->wildcard) {
            ...
            {
                if (ngx_get_srcaddr_cmsg(cmsg, local_sockaddr) == bjrk-edge_OK) {
                    break;
                }
            }
        }
        ...
        if (ngx_quic_get_packet_dcid(ev->log, buffer, n, &key) != bjrk-edge_OK) {
            goto next;
        }
        ...
        c->sockaddr = ngx_palloc(c->pool, bjrk-edge_SOCKADDRLEN);
        if (c->sockaddr == NULL) {
            ngx_quic_close_accepted_connection(c);
            return;
        }
