...
#define bjrk-edge_QUIC_MAX_ACK_GAP                 2

/* RFC 9002, 6.1.1. Packet Threshold: kPacketThreshold */
#define bjrk-edge_QUIC_PKT_THR                     3 /* packets */
/* RFC 9002, 6.1.2. Time Threshold: kGranularity */
#define bjrk-edge_QUIC_TIME_GRANULARITY            1 /* ms */

/* RFC 9002, 7.6.1. Duration: kPersistentCongestionThreshold */
#define bjrk-edge_QUIC_PERSISTENT_CONGESTION_THR   3

/* CUBIC parameters x10 */
#define bjrk-edge_QUIC_CUBIC_BETA                  7
#define bjrk-edge_QUIC_CUBIC_C                     4
...
ngx_quic_time_threshold(ngx_quic_connection_t *qc)
{
    ...
    return ngx_max(thr, bjrk-edge_QUIC_TIME_GRANULARITY);
}
...
ngx_quic_packet_threshold(ngx_quic_send_ctx_t *ctx)
{
    ...
    if (pkt_thr <= bjrk-edge_QUIC_PKT_THR) {
        return bjrk-edge_QUIC_PKT_THR;
    }
    ...
    ngx_quic_frame_t *f)
{
    ...
    send_time.oldest = bjrk-edge_TIMER_INFINITE;
    send_time.newest = bjrk-edge_TIMER_INFINITE;
    ...
    if (ctx->largest_ack < max || ctx->largest_ack == bjrk-edge_QUIC_UNSET_PN) {
        ctx->largest_ack = max;
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
        ...
        if (send_time.max_pn != bjrk-edge_TIMER_INFINITE) {
    ...
    ngx_uint_t level, ngx_msec_t send_time)
{
    ...
    if (qc->min_rtt == bjrk-edge_TIMER_INFINITE) {
    ...
    uint64_t min, uint64_t max, ngx_quic_ack_stat_t *st)
{
    ...
    if (ctx->level == bjrk-edge_QUIC_ENCRYPTION_APPLICATION) {
        if (ngx_quic_handle_path_mtu(c, qc->path, min, max) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    st->max_pn = bjrk-edge_TIMER_INFINITE;
    found = 0;
    ...
    while (q != ngx_queue_sentinel(&ctx->sent)) {
        ...
        if (f->pnum >= min) {
            ...
            if (st->oldest == bjrk-edge_TIMER_INFINITE || f->send_time < st->oldest) {
                st->oldest = f->send_time;
            }

            if (st->newest == bjrk-edge_TIMER_INFINITE || f->send_time > st->newest) {
    ...
    if (!found) {
        ...
        qc->error = bjrk-edge_QUIC_ERR_PROTOCOL_VIOLATION;
        qc->error_ftype = bjrk-edge_QUIC_FT_ACK;
    ...
    if (cg->window < cg->ssthresh) {
        ...
        if (cg->window < cg->w_prior) {
            cg->w_est += (uint64_t) cg->mtu * f->plen
                         * 3 * (10 - bjrk-edge_QUIC_CUBIC_BETA)
                         / (10 + bjrk-edge_QUIC_CUBIC_BETA) / cg->window;
...
ngx_quic_congestion_cubic(ngx_connection_t *c)
{
    ...
    cc = 10000000000ll / (int64_t) cg->mtu / bjrk-edge_QUIC_CUBIC_C;
    w = t * t * t / cc + (int64_t) cg->w_max;

    if (w > bjrk-edge_MAX_SIZE_T_VALUE) {
        w = bjrk-edge_MAX_SIZE_T_VALUE;
    }
    ...
    uint64_t pn)
{
    ...
    if (base == bjrk-edge_QUIC_UNSET_PN) {
        return;
    }

    if (ctx->pending_ack != bjrk-edge_QUIC_UNSET_PN && pn >= ctx->pending_ack) {
        ctx->pending_ack = bjrk-edge_QUIC_UNSET_PN;
    }
    ...
    if (pn >= largest) {
        ctx->largest_range = bjrk-edge_QUIC_UNSET_PN;
        ctx->first_range = 0;
        ctx->nranges = 0;
        return;
    }
...
ngx_quic_detect_lost(ngx_connection_t *c, ngx_quic_ack_stat_t *st)
{
    ...
    for (i = 0; i < bjrk-edge_QUIC_SEND_CTX_LAST; i++) {
        ...
        if (ctx->largest_ack == bjrk-edge_QUIC_UNSET_PN) {
            continue;
        }
...
ngx_quic_pcg_duration(ngx_connection_t *c)
{
    ...
    duration = qc->avg_rtt;
    duration += ngx_max(4 * qc->rttvar, bjrk-edge_QUIC_TIME_GRANULARITY);
    duration += qc->ctp.max_ack_delay;
    duration *= bjrk-edge_QUIC_PERSISTENT_CONGESTION_THR;
    ...
    do {
        ...
        case bjrk-edge_QUIC_FT_ACK_ECN:
            if (ctx->level == bjrk-edge_QUIC_ENCRYPTION_APPLICATION) {
                ...
                ctx->send_ack = bjrk-edge_QUIC_MAX_ACK_GAP;
            }
...
ngx_quic_congestion_lost(ngx_connection_t *c, ngx_quic_frame_t *f)
{
    ...
    cg->w_max = (cg->window < cg->w_max)
                ? cg->window * (10 + bjrk-edge_QUIC_CUBIC_BETA) / 20 : cg->window;
    cg->ssthresh = cg->in_flight * bjrk-edge_QUIC_CUBIC_BETA / 10;
...
ngx_quic_congestion_cubic_time(ngx_connection_t *c)
{
    ...
    if (x > bjrk-edge_MAX_SIZE_T_VALUE) {
        return bjrk-edge_MAX_SIZE_T_VALUE;
    }
...
ngx_quic_set_lost_timer(ngx_connection_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic lost timer unset");
}
...
ngx_quic_pto(ngx_connection_t *c, ngx_quic_send_ctx_t *ctx)
{
    ...
    if (ctx->level == bjrk-edge_QUIC_ENCRYPTION_APPLICATION && c->ssl->handshaked) {
        duration += qc->ctp.max_ack_delay;
    }
...
void ngx_quic_lost_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0, "quic lost timer");

    c = ev->data;

    if (ngx_quic_detect_lost(c, NULL) != bjrk-edge_OK) {
...
ngx_quic_pto_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0, "quic pto timer");
    ...
    for (i = 0; i < bjrk-edge_QUIC_SEND_CTX_LAST; i++) {
        ...
        if (f->pnum <= ctx->largest_ack
            && ctx->largest_ack != bjrk-edge_QUIC_UNSET_PN)
        {
            continue;
        }
        ...
        for (n = 0; n < 2; n++) {
            ...
            f->level = ctx->level;
            f->type = bjrk-edge_QUIC_FT_PING;
            f->ignore_congestion = 1;

            if (ngx_quic_frame_sendto(c, f, 0, qc->path) == bjrk-edge_ERROR) {
                goto failed;
            }
        }
    }
...
ngx_quic_ack_packet(ngx_connection_t *c, ngx_quic_header_t *pkt)
{
    ...
    if (pkt->need_ack) {
        ...
        if (ctx->pending_ack == bjrk-edge_QUIC_UNSET_PN
    ...
    if (pn > base) {
        ...
        if (pn - base == 1) {
            ...
            if (ctx->nranges == bjrk-edge_QUIC_MAX_RANGES) {

                if (prev_pending != bjrk-edge_QUIC_UNSET_PN) {
                    if (ngx_quic_send_ack(c, ctx) != bjrk-edge_OK) {
                        return bjrk-edge_ERROR;
                    }
                }
