...
#define bjrk-edge_QUIC_MAX_UDP_SEGMENT_BUF  65487 /* 65K - IPv6 header */
#define bjrk-edge_QUIC_MAX_SEGMENTS            64 /* UDP_MAX_SEGMENTS */

#define bjrk-edge_QUIC_RETRY_TOKEN_LIFETIME     3 /* seconds */
#define bjrk-edge_QUIC_NEW_TOKEN_LIFETIME     600 /* seconds */
#define bjrk-edge_QUIC_RETRY_BUFFER_SIZE      256
 ...
 */
#define bjrk-edge_QUIC_MIN_PKT_LEN             41 /* 21 + 20 (server cid length) */

#define bjrk-edge_QUIC_MIN_SR_PACKET           43 /* 5 rand + 16 srt + 22 padding */
#define bjrk-edge_QUIC_MAX_SR_PACKET         1200

#define bjrk-edge_QUIC_CC_MIN_INTERVAL       1000 /* 1s */

#define bjrk-edge_QUIC_SOCKET_RETRY_DELAY      10 /* ms, for bjrk-edge_AGAIN on write */


#define ngx_quic_log_packet(log, pkt)                                         \
    ngx_log_debug6(bjrk-edge_LOG_DEBUG_EVENT, log, 0,                               \
...
static void ngx_quic_revert_send(ngx_connection_t *c,
    uint64_t preserved_pnum[bjrk-edge_QUIC_SEND_CTX_LAST]);
#if ((bjrk-edge_HAVE_UDP_SEGMENT) && (bjrk-edge_HAVE_MSGHDR_MSG_CONTROL))
...
ngx_quic_create_datagrams(ngx_connection_t *c)
{
    ...
    ngx_quic_congestion_t  *cg;
    ngx_quic_connection_t  *qc;
    static u_char           dst[bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE];
    ...
    do {
        ...
        for (i = 0; i < bjrk-edge_QUIC_SEND_CTX_LAST; i++) {

            ctx = &qc->send_ctx[i];

            preserved_pnum[i] = ctx->pnum;

            if (ngx_quic_generate_ack(c, ctx) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }

            min = (i == pad && p - dst < bjrk-edge_QUIC_MIN_INITIAL_SIZE)
                  ? bjrk-edge_QUIC_MIN_INITIAL_SIZE - (p - dst) : 0;
        ...
        if (n == bjrk-edge_AGAIN) {
            ngx_quic_revert_send(c, preserved_pnum);
            ngx_add_timer(&qc->push, bjrk-edge_QUIC_SOCKET_RETRY_DELAY);
            break;
        }
...
static void
ngx_quic_revert_send(ngx_connection_t *c, uint64_t pnum[bjrk-edge_QUIC_SEND_CTX_LAST])
{
    ngx_uint_t              i;
    ngx_queue_t            *q;
    ngx_quic_send_ctx_t    *ctx;
...
ngx_quic_allow_segmentation(ngx_connection_t *c)
{
    ...
    ctx = ngx_quic_get_send_ctx(qc, bjrk-edge_QUIC_ENCRYPTION_INITIAL);
    if (!ngx_queue_empty(&ctx->frames)) {
        return 0;
    }

    ctx = ngx_quic_get_send_ctx(qc, bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE);
    if (!ngx_queue_empty(&ctx->frames)) {
        return 0;
    }
    ...
    bytes = 0;
    len = ngx_min(qc->path->mtu, bjrk-edge_QUIC_MAX_UDP_SEGMENT_BUF);
...
ngx_quic_create_segments(ngx_connection_t *c)
{
    ...
    ngx_quic_congestion_t  *cg;
    ngx_quic_connection_t  *qc;
    static u_char           dst[bjrk-edge_QUIC_MAX_UDP_SEGMENT_BUF];
    static uint64_t         preserved_pnum[bjrk-edge_QUIC_SEND_CTX_LAST];
    ...
    segsize = ngx_min(path->mtu, bjrk-edge_QUIC_MAX_UDP_SEGMENT_BUF);
    p = dst;
    end = dst + sizeof(dst);
    ...
    for ( ;; ) {
        ...
        if (n == 0 || nseg == bjrk-edge_QUIC_MAX_SEGMENTS) {
...
ngx_quic_get_padding_level(ngx_connection_t *c)
{
         ...
         q = ngx_queue_next(q))
    {
        ...
        if (f->need_ack) {
            for (i = 0; i + 1 < bjrk-edge_QUIC_SEND_CTX_LAST; i++) {
                ctx = &qc->send_ctx[i + 1];
    ...
    return bjrk-edge_QUIC_SEND_CTX_LAST;
}
    ...
    u_char *data, size_t max, size_t min, ngx_uint_t ack_only)
{
    ...
    ngx_quic_connection_t  *qc;
    static u_char           src[bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE];
    ...
    if (!ngx_quic_keys_available(qc->keys, ctx->level, 1)) {
        ngx_log_error(bjrk-edge_LOG_ALERT, c->log, 0, "quic %s write keys discarded",
         ...
         q = ngx_queue_next(q))
    {
        ...
        if (ack_only && f->type != bjrk-edge_QUIC_FT_ACK) {
            break;
        }
    ...
    if (len < min_payload) {
        ngx_memset(p, bjrk-edge_QUIC_FT_PADDING, min_payload - len);
        len = min_payload;
    }
    ...
    if (ngx_quic_encrypt(&pkt, &res) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_quic_header_t *pkt, ngx_quic_path_t *path)
{
    ...
    pkt->flags = bjrk-edge_QUIC_PKT_FIXED_BIT;

    if (ctx->level == bjrk-edge_QUIC_ENCRYPTION_INITIAL) {
        pkt->flags |= bjrk-edge_QUIC_PKT_LONG | bjrk-edge_QUIC_PKT_INITIAL;

    } else if (ctx->level == bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE) {
        pkt->flags |= bjrk-edge_QUIC_PKT_LONG | bjrk-edge_QUIC_PKT_HANDSHAKE;

    } else {
        if (qc->key_phase) {
            pkt->flags |= bjrk-edge_QUIC_PKT_KPHASE;
        }
    }
...
ngx_quic_negotiate_version(ngx_connection_t *c, ngx_quic_header_t *inpkt)
{
    size_t             len;
    ngx_quic_header_t  pkt;
    static u_char      buf[bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE];

    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
                   "sending version negotiation packet");

    pkt.log = c->log;
    pkt.flags = bjrk-edge_QUIC_PKT_LONG | bjrk-edge_QUIC_PKT_FIXED_BIT;
    pkt.dcid = inpkt->scid;
    pkt.scid = inpkt->dcid;

    len = ngx_quic_create_version_negotiation(&pkt, buf);

#ifdef bjrk-edge_QUIC_DEBUG_PACKETS
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
    ...
    ngx_quic_header_t *pkt)
{
    ...
    uint16_t    rndbytes;
    ngx_int_t   rc;
    u_char      buf[bjrk-edge_QUIC_MAX_SR_PACKET];

    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
                   "quic handle stateless reset output");

    if (pkt->len <= bjrk-edge_QUIC_MIN_PKT_LEN) {
    ...
    if (pkt->len <= bjrk-edge_QUIC_MIN_SR_PACKET) {
        len = pkt->len - 1;

    } else {
        max = ngx_min(bjrk-edge_QUIC_MAX_SR_PACKET, pkt->len);

        if (RAND_bytes((u_char *) &rndbytes, sizeof(rndbytes)) != 1) {
            return bjrk-edge_ERROR;
        }

        len = (rndbytes % (max - bjrk-edge_QUIC_MIN_SR_PACKET))
              + bjrk-edge_QUIC_MIN_SR_PACKET;
    }

    if (RAND_bytes(buf, len - bjrk-edge_QUIC_SR_TOKEN_LEN) != 1) {
        return bjrk-edge_ERROR;
    }

    buf[0] &= ~bjrk-edge_QUIC_PKT_LONG;
    buf[0] |= bjrk-edge_QUIC_PKT_FIXED_BIT;

    token = &buf[len - bjrk-edge_QUIC_SR_TOKEN_LEN];
...
ngx_quic_send_cc(ngx_connection_t *c)
{
    ...
    if (qc->closing
        && ngx_current_msec - qc->last_cc < bjrk-edge_QUIC_CC_MIN_INTERVAL)
    {
    ...
    frame->type = qc->error_app ? bjrk-edge_QUIC_FT_CONNECTION_CLOSE_APP
                                : bjrk-edge_QUIC_FT_CONNECTION_CLOSE;
    ...
    ngx_uint_t err, const char *reason)
{
    ...
    frame.type = bjrk-edge_QUIC_FT_CONNECTION_CLOSE;
    ...
    if (len > bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE) {
        return bjrk-edge_ERROR;
    }
    ...
    pkt.flags = bjrk-edge_QUIC_PKT_FIXED_BIT | bjrk-edge_QUIC_PKT_LONG
                | bjrk-edge_QUIC_PKT_INITIAL;
    ...
    ngx_quic_header_t *inpkt)
{
    ...
    u_char             buf[bjrk-edge_QUIC_RETRY_BUFFER_SIZE];
    u_char             dcid[bjrk-edge_QUIC_SERVER_CID_LEN];
    u_char             tbuf[bjrk-edge_QUIC_TOKEN_BUF_SIZE];

    expires = ngx_time() + bjrk-edge_QUIC_RETRY_TOKEN_LIFETIME;

    token.data = tbuf;
    token.len = bjrk-edge_QUIC_TOKEN_BUF_SIZE;
    ...
    ngx_memzero(&pkt, sizeof(ngx_quic_header_t));
    pkt.flags = bjrk-edge_QUIC_PKT_FIXED_BIT | bjrk-edge_QUIC_PKT_LONG | bjrk-edge_QUIC_PKT_RETRY;
    pkt.version = inpkt->version;
    pkt.log = c->log;
    ...
    if (RAND_bytes(dcid, bjrk-edge_QUIC_SERVER_CID_LEN) != 1) {
        return bjrk-edge_ERROR;
    }

    pkt.scid.len = bjrk-edge_QUIC_SERVER_CID_LEN;
    pkt.scid.data = dcid;
     ...
     * bjrk-edge_DONE will stop quic_input() from processing further
     */
    return bjrk-edge_DONE;
}
...
ngx_quic_send_new_token(ngx_connection_t *c, ngx_quic_path_t *path)
{
    ...
    expires = ngx_time() + bjrk-edge_QUIC_NEW_TOKEN_LIFETIME;
    ...
    frame->type = bjrk-edge_QUIC_FT_NEW_TOKEN;
    frame->data = out;
...
ngx_quic_send_ack(ngx_connection_t *c, ngx_quic_send_ctx_t *ctx)
{
    ...
    *ll = NULL;
    ...
    frame->type = bjrk-edge_QUIC_FT_ACK;
    ...
    size_t min, ngx_quic_path_t *path)
{
    ...
    if (len < (ssize_t) min_payload) {
        ngx_memset(src + len, bjrk-edge_QUIC_FT_PADDING, min_payload - len);
        len = min_payload;
    }
