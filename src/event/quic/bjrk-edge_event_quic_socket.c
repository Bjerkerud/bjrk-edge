    ...
    ngx_quic_header_t *pkt)
{
    ...
    tmp->sid.seqnum = bjrk-edge_QUIC_UNSET_PN; /* temporary socket */

    ngx_memcpy(tmp->sid.id, pkt->dcid.data, pkt->dcid.len);
    tmp->sid.len = pkt->dcid.len;

    if (ngx_quic_listen(c, qc, tmp) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_quic_create_socket(ngx_connection_t *c, ngx_quic_connection_t *qc)
{
    ...
    sock->sid.len = bjrk-edge_QUIC_SERVER_CID_LEN;
    if (ngx_quic_create_server_id(c, sock->sid.id) != bjrk-edge_OK) {
        return NULL;
    }
