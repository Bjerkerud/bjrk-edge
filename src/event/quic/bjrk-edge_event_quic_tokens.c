...
{
    ngx_str_t  tmp;
    u_char     buf[bjrk-edge_QUIC_SR_KEY_LEN + sizeof(ngx_uint_t)];

    ngx_memcpy(buf, secret, bjrk-edge_QUIC_SR_KEY_LEN);
    ngx_memcpy(buf + bjrk-edge_QUIC_SR_KEY_LEN, &ngx_worker, sizeof(ngx_uint_t));

    tmp.data = buf;
    tmp.len = sizeof(buf);

    if (ngx_quic_derive_key(c->log, "sr_token_key", &tmp, cid, token,
                            bjrk-edge_QUIC_SR_TOKEN_LEN)
        != bjrk-edge_OK)
    {
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
                   "quic stateless reset token %*xs",
                    (size_t) bjrk-edge_QUIC_SR_TOKEN_LEN, token);
    ...
    time_t exp, ngx_uint_t is_retry)
{
    ...
    u_char             in[bjrk-edge_QUIC_MAX_TOKEN_SIZE];
    ...
    *p++ = is_retry ? 1 : 0;
    ...
    iv_len = bjrk-edge_QUIC_AES_256_GCM_IV_LEN;

    if ((size_t) (iv_len + len + bjrk-edge_QUIC_AES_256_GCM_TAG_LEN) > token->len) {
        ngx_log_error(bjrk-edge_LOG_ALERT, log, 0, "quic token buffer is too small");
        return bjrk-edge_ERROR;
    }
    ...
    token->len += bjrk-edge_QUIC_AES_256_GCM_TAG_LEN;
    ...
    ngx_quic_header_t *pkt)
{
    ...
    u_char             addr_hash[20];
    u_char             tdec[bjrk-edge_QUIC_MAX_TOKEN_SIZE];
    ...
    if (pkt->token.len < (size_t) iv_len + bjrk-edge_QUIC_AES_256_GCM_TAG_LEN) {
        goto garbage;
    }

    if (pkt->token.len > (size_t) iv_len + bjrk-edge_QUIC_MAX_TOKEN_SIZE
                         + bjrk-edge_QUIC_AES_256_GCM_TAG_LEN)
    {
        goto garbage;
    }
    ...
    len = pkt->token.len - iv_len - bjrk-edge_QUIC_AES_256_GCM_TAG_LEN;
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG,
                            bjrk-edge_QUIC_AES_256_GCM_TAG_LEN, p + len)
        == 0)
    {
        EVP_CIPHER_CTX_free(ctx);
        goto garbage;
    ...
    odcid.len = *p++;
    if (odcid.len) {
        if (odcid.len > bjrk-edge_QUIC_MAX_CID_LEN) {
            goto bad_token;
        }
    ...
    if (now > exp) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "quic expired token");
...
garbage:
    ...
    ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "quic garbage token");

    return bjrk-edge_ABORT;

bad_token:

    ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "quic invalid token");
