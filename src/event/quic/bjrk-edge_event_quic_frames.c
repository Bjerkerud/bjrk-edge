...
#define bjrk-edge_QUIC_BUFFER_SIZE  4096
...
ngx_quic_alloc_buf(ngx_connection_t *c)
{
    ...
    if (b) {
        ...
        p = ngx_pnalloc(c->pool, bjrk-edge_QUIC_BUFFER_SIZE);
        if (p == NULL) {
            return NULL;
        }
    }
...
#ifdef bjrk-edge_QUIC_DEBUG_ALLOC
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic alloc buffer %p", b);
#endif
    ...
    b->pos = p;
    b->last = p;
    b->end = p + bjrk-edge_QUIC_BUFFER_SIZE;
...
ngx_quic_split_frame(ngx_connection_t *c, ngx_quic_frame_t *f, size_t len)
{
    ...
    if ((size_t) f->len > len) {
        ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0, "could not split QUIC frame");
        return bjrk-edge_ERROR;
    }
    ...
    *nf = *f;
    ...
    if (f->type == bjrk-edge_QUIC_FT_STREAM) {
        f->u.stream.fin = 0;
    }
...
ngx_quic_copy_buffer(ngx_connection_t *c, u_char *data, size_t len)
{
    ...
    if (ngx_quic_write_buffer(c, &qb, &cl, len, 0) == bjrk-edge_CHAIN_ERROR) {
...
ngx_quic_read_buffer(ngx_connection_t *c, ngx_quic_buffer_t *qb, uint64_t limit)
{
    ...
    for (ll = &out; *ll; ll = &(*ll)->next) {
        ...
        if (n > limit) {
            if (ngx_quic_split_chain(c, *ll, limit) != bjrk-edge_OK) {
                return bjrk-edge_CHAIN_ERROR;
            }
    ...
    ngx_chain_t *in, uint64_t limit, uint64_t offset)
{
    ...
    while (in && limit) {
        ...
        if (b->sync && offset > base) {
            if (ngx_quic_split_chain(c, cl, offset - base) != bjrk-edge_OK) {
                return bjrk-edge_CHAIN_ERROR;
            }
        ...
        if (b->sync && p != b->pos) {
            if (ngx_quic_split_chain(c, cl, p - b->pos) != bjrk-edge_OK) {
                return bjrk-edge_CHAIN_ERROR;
            }
...
ngx_quic_log_frame(ngx_log_t *log, ngx_quic_frame_t *f, ngx_uint_t tx)
{
...
#ifdef bjrk-edge_QUIC_DEBUG_FRAMES
        {
            ngx_chain_t  *cl;
    ...
    case bjrk-edge_QUIC_FT_CONNECTION_CLOSE_APP:
        p = ngx_slprintf(p, last, "CONNECTION_CLOSE%s err:%uL",
                         f->type == bjrk-edge_QUIC_FT_CONNECTION_CLOSE ? "" : "_APP",
    ...
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_EVENT, log, 0, "quic frame %s %s:%uL %*s",
