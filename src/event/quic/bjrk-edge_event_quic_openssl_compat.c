...
#define bjrk-edge_QUIC_COMPAT_RECORD_SIZE          1024

#define bjrk-edge_QUIC_COMPAT_SSL_TP_EXT           0x39

#define bjrk-edge_QUIC_COMPAT_CLIENT_HANDSHAKE     "CLIENT_HANDSHAKE_TRAFFIC_SECRET"
#define bjrk-edge_QUIC_COMPAT_SERVER_HANDSHAKE     "SERVER_HANDSHAKE_TRAFFIC_SECRET"
#define bjrk-edge_QUIC_COMPAT_CLIENT_APPLICATION   "CLIENT_TRAFFIC_SECRET_0"
#define bjrk-edge_QUIC_COMPAT_SERVER_APPLICATION   "SERVER_TRAFFIC_SECRET_0"
...
ngx_quic_compat_init(ngx_conf_t *cf, SSL_CTX *ctx)
{
    ...
    if (SSL_CTX_has_client_custom_ext(ctx, bjrk-edge_QUIC_COMPAT_SSL_TP_EXT)) {
        return bjrk-edge_OK;
    }

    if (SSL_CTX_add_custom_ext(ctx, bjrk-edge_QUIC_COMPAT_SSL_TP_EXT,
...
ngx_quic_compat_keylog_callback(const SSL *ssl, const char *line)
{
    ...
    if (n == sizeof(bjrk-edge_QUIC_COMPAT_CLIENT_HANDSHAKE) - 1
        && ngx_strncmp(start, bjrk-edge_QUIC_COMPAT_CLIENT_HANDSHAKE, n) == 0)
    {
        level = ssl_encryption_handshake;
        write = 0;

    } else if (n == sizeof(bjrk-edge_QUIC_COMPAT_SERVER_HANDSHAKE) - 1
               && ngx_strncmp(start, bjrk-edge_QUIC_COMPAT_SERVER_HANDSHAKE, n) == 0)
    {
        level = ssl_encryption_handshake;
        write = 1;

    } else if (n == sizeof(bjrk-edge_QUIC_COMPAT_CLIENT_APPLICATION) - 1
               && ngx_strncmp(start, bjrk-edge_QUIC_COMPAT_CLIENT_APPLICATION, n)
                  == 0)
    {
        level = ssl_encryption_application;
        write = 0;

    } else if (n == sizeof(bjrk-edge_QUIC_COMPAT_SERVER_APPLICATION) - 1
               && ngx_strncmp(start, bjrk-edge_QUIC_COMPAT_SERVER_APPLICATION, n)
                   == 0)
    {
        level = ssl_encryption_application;
        write = 1;
    ...
    for (n = 0, start = p; *p; p++) {
        ...
        ngx_log_error(bjrk-edge_LOG_EMERG, c->log, 0,
    ...
    const SSL_CIPHER *cipher, const uint8_t *secret, size_t secret_len)
{
    ...
    if (key_len == bjrk-edge_ERROR) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, c->log, 0, "unexpected cipher");
        return bjrk-edge_ERROR;
    }
...
SSL_set_quic_method(SSL *ssl, const SSL_QUIC_METHOD *quic_method)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic compat set method");
    ...
    const uint8_t *data, size_t len)
{
    ...
    u_char                     in[bjrk-edge_QUIC_COMPAT_RECORD_SIZE + 1];
    u_char                     out[bjrk-edge_QUIC_COMPAT_RECORD_SIZE + 1
                                   + SSL3_RT_HEADER_LENGTH
                                   + bjrk-edge_QUIC_TAG_LEN];
    ...
    while (len) {
        ...
        if (level == ssl_encryption_initial) {
            ...
            n = ngx_min(len, bjrk-edge_QUIC_COMPAT_RECORD_SIZE);
            ...
            *p++ = SSL3_RT_HANDSHAKE;
            ...
            if (ngx_quic_compat_create_record(&rec, &res) != bjrk-edge_OK) {
                return 0;
            }
...
ngx_quic_compat_create_record(ngx_quic_compat_record_t *rec, ngx_str_t *res)
{
    ngx_str_t           ad, out;
    ngx_quic_secret_t  *secret;
    u_char              nonce[bjrk-edge_QUIC_IV_LEN];

    ad.data = res->data;
    ad.len = ngx_quic_compat_create_header(rec, ad.data, 0);

    out.len = rec->payload.len + bjrk-edge_QUIC_TAG_LEN;
    out.data = res->data + ad.len;

#ifdef bjrk-edge_QUIC_DEBUG_CRYPTO
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, rec->log, 0,
...
#endif /* bjrk-edge_QUIC_OPENSSL_COMPAT */
