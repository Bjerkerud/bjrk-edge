...
#if (bjrk-edge_QUIC_BORINGSSL_EVP_API)
...
#define bjrk-edge_QUIC_HP_LEN               5

#define bjrk-edge_QUIC_AES_128_KEY_LEN      16

#define bjrk-edge_QUIC_INITIAL_CIPHER       TLS1_3_CK_AES_128_GCM_SHA256
    ...
    ngx_log_t *log)
{
    ...
    client = &keys->secrets[bjrk-edge_QUIC_ENCRYPTION_INITIAL].client;
    server = &keys->secrets[bjrk-edge_QUIC_ENCRYPTION_INITIAL].server;
...
#ifdef bjrk-edge_QUIC_DEBUG_CRYPTO
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, log, 0,
    ...
    client_key.len = bjrk-edge_QUIC_AES_128_KEY_LEN;
    server_key.len = bjrk-edge_QUIC_AES_128_KEY_LEN;

    client->hp.len = bjrk-edge_QUIC_AES_128_KEY_LEN;
    server->hp.len = bjrk-edge_QUIC_AES_128_KEY_LEN;

    client->iv.len = bjrk-edge_QUIC_IV_LEN;
    server->iv.len = bjrk-edge_QUIC_IV_LEN;
    ...
    for (i = 0; i < (sizeof(seq) / sizeof(seq[0])); i++) {
        if (ngx_quic_hkdf_expand(&seq[i], digest, log) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    if (ngx_quic_ciphers(bjrk-edge_QUIC_INITIAL_CIPHER, &ciphers) == bjrk-edge_ERROR) {
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_quic_crypto_hp_init(ciphers.hp, client, log) == bjrk-edge_ERROR) {
        goto failed;
    }

    if (ngx_quic_crypto_hp_init(ciphers.hp, server, log) == bjrk-edge_ERROR) {
        goto failed;
    }
...
ngx_quic_hkdf_expand(ngx_quic_hkdf_t *h, const EVP_MD *digest, ngx_log_t *log)
{
    ...
    *p = '\0';
    ...
    if (ngx_hkdf_expand(h->out, h->out_len, digest,
        ...
        != bjrk-edge_OK)
    {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0,
                      "ngx_hkdf_expand(%*s) failed", h->label_len, h->label);
        return bjrk-edge_ERROR;
    }

#ifdef bjrk-edge_QUIC_DEBUG_CRYPTO
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_EVENT, log, 0,
    ...
    ngx_quic_md_t *key, ngx_int_t enc, ngx_log_t *log)
{
    ...
    if (ctx == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_AEAD_CTX_new() failed");
        return bjrk-edge_ERROR;
    }
#else
    EVP_CIPHER_CTX  *ctx;
    ...
    if (ctx == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_CIPHER_CTX_new() failed");
        return bjrk-edge_ERROR;
    }

    if (EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, enc) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_CipherInit_ex() failed");
        return bjrk-edge_ERROR;
    }

    if (EVP_CIPHER_mode(cipher) == EVP_CIPH_CCM_MODE
        && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, bjrk-edge_QUIC_TAG_LEN,
                               NULL)
           == 0)
    {
        EVP_CIPHER_CTX_free(ctx);
    ...
    ngx_str_t *in, ngx_str_t *ad, ngx_log_t *log)
{
    ...
    if (EVP_AEAD_CTX_open(s->ctx, out->data, &out->len, out->len, nonce,
        ...
        != 1)
    {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_AEAD_CTX_open() failed");
        return bjrk-edge_ERROR;
    }
    ...
    ngx_str_t *in, ngx_str_t *ad, ngx_log_t *log)
{
    ...
    if (EVP_AEAD_CTX_seal(s->ctx, out->data, &out->len, out->len, nonce,
        ...
        != 1)
    {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_AEAD_CTX_seal() failed");
        return bjrk-edge_ERROR;
    }
    ...
    const u_char *nonce, ngx_str_t *in, ngx_str_t *ad, ngx_log_t *log)
{
    ...
    if (enc == 0) {
        in->len -= bjrk-edge_QUIC_TAG_LEN;

        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, bjrk-edge_QUIC_TAG_LEN,
                                in->data + in->len)
            == 0)
        {
            ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0,
    ...
    {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_CipherUpdate() failed");
        return bjrk-edge_ERROR;
    }
    ...
    if (EVP_CipherFinal_ex(ctx, out->data + out->len, &len) <= 0) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_CipherFinal_ex failed");
        return bjrk-edge_ERROR;
    }
    ...
    if (enc == 1) {
        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, bjrk-edge_QUIC_TAG_LEN,
                                out->data + out->len)
            == 0)
        {
            ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0,
        ...
        out->len += bjrk-edge_QUIC_TAG_LEN;
    }
    ...
    ngx_log_t *log)
{
    ...
    if (EVP_EncryptInit_ex(ctx, cipher, NULL, s->hp.data, NULL) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_EncryptInit_ex() failed");
        return bjrk-edge_ERROR;
    }
    ...
    ngx_log_t *log)
{
    ...
    static const u_char zero[bjrk-edge_QUIC_HP_LEN];
    ...
    if (ctx == NULL) {
        ...
        CRYPTO_chacha_20(out, zero, bjrk-edge_QUIC_HP_LEN, s->hp.data, &in[4], cnt);
        return bjrk-edge_OK;
    }
#endif
    ...
    if (!EVP_EncryptUpdate(ctx, out, &outlen, zero, bjrk-edge_QUIC_HP_LEN)) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_EncryptUpdate() failed");
        return bjrk-edge_ERROR;
    }

    if (!EVP_EncryptFinal_ex(ctx, out + bjrk-edge_QUIC_HP_LEN, &outlen)) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "EVP_EncryptFinal_Ex() failed");
        return bjrk-edge_ERROR;
    }
    ...
    const uint8_t *secret, size_t secret_len)
{
    ...
    if (key_len == bjrk-edge_ERROR) {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0, "unexpected cipher");
        return bjrk-edge_ERROR;
    }
    ...
    key.len = key_len;
    peer_secret->iv.len = bjrk-edge_QUIC_IV_LEN;
    ...
    for (i = 0; i < (sizeof(seq) / sizeof(seq[0])); i++) {
        if (ngx_quic_hkdf_expand(&seq[i], ciphers.d, log) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
    ...
    if (ngx_quic_crypto_hp_init(ciphers.hp, peer_secret, log) == bjrk-edge_ERROR) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_keys_switch(ngx_connection_t *c, ngx_quic_keys_t *keys)
{
    ...
    current = &keys->secrets[bjrk-edge_QUIC_ENCRYPTION_APPLICATION];
    next = &keys->next_key;
...
ngx_quic_keys_update(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic key update");
    ...
    for (i = 0; i < (sizeof(seq) / sizeof(seq[0])); i++) {
        if (ngx_quic_hkdf_expand(&seq[i], ciphers.d, c->log) != bjrk-edge_OK) {
            goto failed;
        }
    }
...
ngx_quic_keys_cleanup(ngx_quic_keys_t *keys)
{
    ...
    for (i = 0; i < bjrk-edge_QUIC_ENCRYPTION_LAST; i++) {
...
ngx_quic_create_packet(ngx_quic_header_t *pkt, ngx_str_t *res)
{
    ...
    u_char              nonce[bjrk-edge_QUIC_IV_LEN], mask[bjrk-edge_QUIC_HP_LEN];

    ad.data = res->data;
    ad.len = ngx_quic_create_header(pkt, ad.data, &pnp);

    out.len = pkt->payload.len + bjrk-edge_QUIC_TAG_LEN;
    ...
    if (ngx_quic_crypto_hp(secret, mask, sample, pkt->log) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_create_retry_packet(ngx_quic_header_t *pkt, ngx_str_t *res)
{
    ...
    static const u_char   nonce[bjrk-edge_QUIC_IV_LEN] = {
    ...
    itag.len = bjrk-edge_QUIC_TAG_LEN;
    ...
    secret.iv.len = bjrk-edge_QUIC_IV_LEN;
    ...
    ngx_str_t *salt, u_char *out, size_t len)
{
    ...
    *p = '\0';
    ...
    if (ngx_hkdf_expand(out, len, digest, is, is_len, info, info_len) != bjrk-edge_OK)
    {
        ngx_ssl_error(bjrk-edge_LOG_INFO, log, 0,
...
ngx_quic_decrypt(ngx_quic_header_t *pkt, uint64_t *largest_pn)
{
    ...
    ngx_quic_secret_t  *secret;
    uint8_t             nonce[bjrk-edge_QUIC_IV_LEN], mask[bjrk-edge_QUIC_HP_LEN];
    ...
    if (len < bjrk-edge_QUIC_TAG_LEN + 4) {
    ...
    if (ngx_quic_short_pkt(pkt->flags)) {
        key_phase = (pkt->flags & bjrk-edge_QUIC_PKT_KPHASE) != 0;
        ...
        if (key_phase != pkt->key_phase) {
            if (pkt->keys->next_key.client.ctx != NULL) {
                 ...
                 */
                ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, pkt->log, 0,
    ...
    pkt->payload.len = in.len - bjrk-edge_QUIC_TAG_LEN;
    ...
    if (pkt->payload.len == 0) {
         ...
         */
        ngx_log_error(bjrk-edge_LOG_INFO, pkt->log, 0, "quic zero-length packet");
...
#if defined(bjrk-edge_QUIC_DEBUG_CRYPTO) && defined(bjrk-edge_QUIC_DEBUG_PACKETS)
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, pkt->log, 0,
