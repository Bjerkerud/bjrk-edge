...
#define bjrk-edge_QUIC_MAX_SERVER_IDS   8


#if (bjrk-edge_QUIC_BPF)
...
{
    if (RAND_bytes(id, bjrk-edge_QUIC_SERVER_CID_LEN) != 1) {
        return bjrk-edge_ERROR;
    }

#if (bjrk-edge_QUIC_BPF)
    if (ngx_quic_bpf_attach_id(c, id) != bjrk-edge_OK) {
...
ngx_quic_bpf_attach_id(ngx_connection_t *c, u_char *id)
{
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_COOKIE, &cookie, &optlen) == -1) {
        ngx_log_error(bjrk-edge_LOG_ERR, c->log, ngx_socket_errno,
    ...
    ngx_quic_new_conn_id_frame_t *f)
{
    ...
    if (f->seqnum < qc->max_retired_seqnum) {
        ...
        frame->type = bjrk-edge_QUIC_FT_RETIRE_CONNECTION_ID;
    ...
    if (cid) {
        ...
        if (cid->len != f->len
            ...
            || ngx_strncmp(cid->sr_token, f->srt, bjrk-edge_QUIC_SR_TOKEN_LEN) != 0)
        {
            /*
...
retire:
    ...
    while (q != ngx_queue_sentinel(&qc->client_ids)) {
        ...
        if (ngx_quic_retire_client_id(c, cid) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
    ...
    if (qc->nclient_ids > qc->tp.active_connection_id_limit) {
         ...
         */
        qc->error = bjrk-edge_QUIC_ERR_CONNECTION_ID_LIMIT_ERROR;
    ...
    uint64_t seqnum, u_char *token)
{
    ...
    if (token) {
        ngx_memcpy(cid->sr_token, token, bjrk-edge_QUIC_SR_TOKEN_LEN);
    }
    ...
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
                   "quic cid seq:%uL received id:%uz:%xV:%*xs",
                    ...
                    (size_t) bjrk-edge_QUIC_SR_TOKEN_LEN, cid->sr_token);
...
ngx_quic_create_sockets(ngx_connection_t *c)
{
    ...
    n = ngx_min(bjrk-edge_QUIC_MAX_SERVER_IDS, qc->ctp.active_connection_id_limit);
    ...
    while (qc->nsockets < n) {
        ...
        if (ngx_quic_listen(c, qc, qsock) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }

        if (ngx_quic_send_server_id(c, &qsock->sid) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_quic_send_server_id(ngx_connection_t *c, ngx_quic_server_id_t *sid)
{
    ...
    frame->level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    frame->type = bjrk-edge_QUIC_FT_NEW_CONNECTION_ID;
    frame->u.ncid.seqnum = sid->seqnum;
    frame->u.ncid.retire = 0;
    frame->u.ncid.len = bjrk-edge_QUIC_SERVER_CID_LEN;
    ngx_memcpy(frame->u.ncid.cid, sid->id, bjrk-edge_QUIC_SERVER_CID_LEN);
