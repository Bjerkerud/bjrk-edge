...
#define bjrk-edge_QUIC_STREAM_GONE     (void *) -1
...
ngx_quic_open_stream(ngx_connection_t *c, ngx_uint_t bidi)
{
    ...
    if (bidi) {
        ...
        id = (qc->streams.server_streams_bidi << 2)
             | bjrk-edge_QUIC_STREAM_SERVER_INITIATED;
        ...
        id = (qc->streams.server_streams_uni << 2)
             | bjrk-edge_QUIC_STREAM_SERVER_INITIATED
             | bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL;
...
ngx_quic_close_streams(ngx_connection_t *c, ngx_quic_connection_t *qc)
{
    ...
    while (node) {
        ...
        qs->recv_state = bjrk-edge_QUIC_STREAM_RECV_RESET_RECVD;
        qs->send_state = bjrk-edge_QUIC_STREAM_SEND_RESET_SENT;
...
ngx_quic_do_reset_stream(ngx_quic_stream_t *qs, ngx_uint_t err)
{
    ...
    if (qs->send_state == bjrk-edge_QUIC_STREAM_SEND_DATA_RECVD
        || qs->send_state == bjrk-edge_QUIC_STREAM_SEND_RESET_SENT
        || qs->send_state == bjrk-edge_QUIC_STREAM_SEND_RESET_RECVD)
    {
        return bjrk-edge_OK;
    }
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, pc->log, 0,
    ...
    frame->type = bjrk-edge_QUIC_FT_RESET_STREAM;
...
{
    if (how == bjrk-edge_RDWR_SHUTDOWN || how == bjrk-edge_WRITE_SHUTDOWN) {
        if (ngx_quic_shutdown_stream_send(c) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    if (how == bjrk-edge_RDWR_SHUTDOWN || how == bjrk-edge_READ_SHUTDOWN) {
        if (ngx_quic_shutdown_stream_recv(c) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_quic_shutdown_stream_send(ngx_connection_t *c)
{
    ...
    if (qs->send_state != bjrk-edge_QUIC_STREAM_SEND_READY
        && qs->send_state != bjrk-edge_QUIC_STREAM_SEND_SEND)
    {
        return bjrk-edge_OK;
    }
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, qs->parent->log, 0,
...
ngx_quic_shutdown_stream_recv(ngx_connection_t *c)
{
    ...
    if (qs->recv_state != bjrk-edge_QUIC_STREAM_RECV_RECV
        && qs->recv_state != bjrk-edge_QUIC_STREAM_RECV_SIZE_KNOWN)
    {
        return bjrk-edge_OK;
    }
    ...
    frame->level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    frame->type = bjrk-edge_QUIC_FT_STOP_SENDING;
...
ngx_quic_get_stream(ngx_connection_t *c, uint64_t id)
{
    ...
    if (qc->shutdown || qc->closing) {
        return bjrk-edge_QUIC_STREAM_GONE;
    }

    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
                   "quic stream id:0x%xL is missing", id);

    if (id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL) {

        if (id & bjrk-edge_QUIC_STREAM_SERVER_INITIATED) {
            if ((id >> 2) < qc->streams.server_streams_uni) {
                return bjrk-edge_QUIC_STREAM_GONE;
            }

            qc->error = bjrk-edge_QUIC_ERR_STREAM_STATE_ERROR;
            return NULL;
        }
        ...
        if ((id >> 2) >= qc->streams.client_max_streams_uni) {
            qc->error = bjrk-edge_QUIC_ERR_STREAM_LIMIT_ERROR;
            return NULL;
        }
    ...
    for ( /* void */ ; min_id <= id; min_id += 0x04) {
        ...
        if (qs == NULL) {
            if (ngx_quic_reject_stream(c, min_id) != bjrk-edge_OK) {
                return NULL;
            }
...
ngx_quic_reject_stream(ngx_connection_t *c, uint64_t id)
{
    ...
    code = (id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL)
...
ngx_quic_init_stream_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic init stream");

    if ((qs->id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL) == 0) {
        c->write->active = 1;
        c->write->ready = 1;
    }
...
{
    if (ngx_quic_do_init_streams(c) != bjrk-edge_OK) {
...
ngx_quic_do_init_streams(ngx_connection_t *c)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic init streams");

    qc = ngx_quic_get_connection(c);

    if (qc->conf->init) {
        if (qc->conf->init(c) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_quic_create_stream(ngx_connection_t *c, uint64_t id)
{
    ...
    pool = ngx_create_pool(bjrk-edge_DEFAULT_POOL_SIZE, c->log);
    if (pool == NULL) {
    ...
    *log = *c->log;
    ...
    sc->tcp_nodelay = bjrk-edge_TCP_NODELAY_DISABLED;
    ...
    if (id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL) {
        if (id & bjrk-edge_QUIC_STREAM_SERVER_INITIATED) {
            ...
            qs->recv_state = bjrk-edge_QUIC_STREAM_RECV_DATA_READ;
            qs->send_state = bjrk-edge_QUIC_STREAM_SEND_READY;

        } else {
            qs->recv_max_data = qc->tp.initial_max_stream_data_uni;
            qs->recv_state = bjrk-edge_QUIC_STREAM_RECV_RECV;
            qs->send_state = bjrk-edge_QUIC_STREAM_SEND_DATA_RECVD;
        }
...
ngx_quic_cancelable_stream(ngx_connection_t *c)
{
    ...
    if (!qs->cancelable) {
        ...
        if (ngx_quic_can_shutdown(pc) == bjrk-edge_OK) {
...
ngx_quic_stream_recv(ngx_connection_t *c, u_char *buf, size_t size)
{
    ...
    if (qs->recv_state == bjrk-edge_QUIC_STREAM_RECV_RESET_RECVD
        || qs->recv_state == bjrk-edge_QUIC_STREAM_RECV_RESET_READ)
    {
        qs->recv_state = bjrk-edge_QUIC_STREAM_RECV_RESET_READ;
        return bjrk-edge_ERROR;
    }

    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, pc->log, 0,
    ...
    if (len == 0) {
        ...
        if (qs->recv_state == bjrk-edge_QUIC_STREAM_RECV_DATA_RECVD
            && qs->recv_offset == qs->recv_final_size)
        {
            qs->recv_state = bjrk-edge_QUIC_STREAM_RECV_DATA_READ;
        }

        if (qs->recv_state == bjrk-edge_QUIC_STREAM_RECV_DATA_READ) {
            rev->eof = 1;
            return 0;
        }
    ...
    if (ngx_quic_update_flow(qs, qs->recv_offset + len) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_stream_send(ngx_connection_t *c, u_char *buf, size_t size)
{
    ...
    if (ngx_quic_stream_send_chain(c, &cl, 0) == bjrk-edge_CHAIN_ERROR) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_stream_send_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    if (ngx_quic_stream_flush(qs) != bjrk-edge_OK) {
        return bjrk-edge_CHAIN_ERROR;
    }
...
ngx_quic_stream_flush(ngx_quic_stream_t *qs)
{
    ...
    if (qs->send_state != bjrk-edge_QUIC_STREAM_SEND_SEND) {
        return bjrk-edge_OK;
    }
    ...
    {
        qs->send_state = bjrk-edge_QUIC_STREAM_SEND_DATA_SENT;
        last = 1;
    }
    ...
    frame->level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    frame->type = bjrk-edge_QUIC_FT_STREAM;
    frame->data = out;
    ...
    ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, pc->log, 0,
...
ngx_quic_stream_cleanup_handler(void *data)
{
    ...
    if (ngx_quic_shutdown_stream(c, bjrk-edge_RDWR_SHUTDOWN) != bjrk-edge_OK) {
        qs->connection = NULL;
        goto failed;
    }
    ...
    if (ngx_quic_close_stream(qs) != bjrk-edge_OK) {
        goto failed;
    }
...
failed:
    ...
    qc->error = bjrk-edge_QUIC_ERR_INTERNAL_ERROR;
...
ngx_quic_close_stream(ngx_quic_stream_t *qs)
{
    ...
    if (!pc->reusable && ngx_quic_can_shutdown(pc) == bjrk-edge_OK) {
        ngx_reusable_connection(pc, 1);
    }
    ...
    if ((qs->id & bjrk-edge_QUIC_STREAM_SERVER_INITIATED) == 0) {
        frame = ngx_quic_alloc_frame(pc);
        if (frame == NULL) {
            return bjrk-edge_ERROR;
        }
        ...
        frame->level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
        frame->type = bjrk-edge_QUIC_FT_MAX_STREAMS;

        if (qs->id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL) {
    ...
    ngx_quic_frame_t *frame)
{
    ...
    if ((f->stream_id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL)
        && (f->stream_id & bjrk-edge_QUIC_STREAM_SERVER_INITIATED))
    {
        qc->error = bjrk-edge_QUIC_ERR_STREAM_STATE_ERROR;
        return bjrk-edge_ERROR;
    }
    ...
    if (qs == bjrk-edge_QUIC_STREAM_GONE) {
        return bjrk-edge_OK;
    }
    ...
    {
        qc->error = bjrk-edge_QUIC_ERR_FINAL_SIZE_ERROR;
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_quic_control_flow(qs, last) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_quic_write_buffer(c, &qs->recv, frame->data, f->length, f->offset)
        == bjrk-edge_CHAIN_ERROR)
    {
        return bjrk-edge_ERROR;
    }

    if (qs->recv_state == bjrk-edge_QUIC_STREAM_RECV_SIZE_KNOWN
        && qs->recv.size == qs->recv_final_size)
    {
        qs->recv_state = bjrk-edge_QUIC_STREAM_RECV_DATA_RECVD;
    }
    ...
    ngx_quic_header_t *pkt, ngx_quic_stream_data_blocked_frame_t *f)
{
    ...
    if ((f->id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL)
        && (f->id & bjrk-edge_QUIC_STREAM_SERVER_INITIATED))
    {
        qc->error = bjrk-edge_QUIC_ERR_STREAM_STATE_ERROR;
        return bjrk-edge_ERROR;
    }
    ...
    ngx_quic_header_t *pkt, ngx_quic_max_stream_data_frame_t *f)
{
    ...
    if ((f->id & bjrk-edge_QUIC_STREAM_UNIDIRECTIONAL)
        && (f->id & bjrk-edge_QUIC_STREAM_SERVER_INITIATED) == 0)
    {
        qc->error = bjrk-edge_QUIC_ERR_STREAM_STATE_ERROR;
        return bjrk-edge_ERROR;
    }
    ...
    ngx_quic_header_t *pkt, ngx_quic_reset_stream_frame_t *f)
{
    ...
    if (ngx_quic_control_flow(qs, f->final_size) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_quic_update_flow(qs, qs->recv_final_size) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_quic_header_t *pkt, ngx_quic_stop_sending_frame_t *f)
{
    ...
    if (ngx_quic_do_reset_stream(qs, f->error_code) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_handle_stream_ack(ngx_connection_t *c, ngx_quic_frame_t *f)
{
    ...
    case bjrk-edge_QUIC_FT_STREAM:
        ...
        if (qs->send_state == bjrk-edge_QUIC_STREAM_SEND_DATA_SENT
...
ngx_quic_control_flow(ngx_quic_stream_t *qs, uint64_t last)
{
    ...
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_EVENT, pc->log, 0,
    ...
    {
        qc->error = bjrk-edge_QUIC_ERR_FLOW_CONTROL_ERROR;
        return bjrk-edge_ERROR;
    }
...
ngx_quic_update_flow(ngx_quic_stream_t *qs, uint64_t last)
{
    ...
    if (qs->recv_max_data <= qs->recv_offset + qs->recv_window / 2) {
        if (ngx_quic_update_max_stream_data(qs) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
    ...
    {
        if (ngx_quic_update_max_data(pc) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }
...
ngx_quic_update_max_stream_data(ngx_quic_stream_t *qs)
{
    ...
    frame->level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    frame->type = bjrk-edge_QUIC_FT_MAX_STREAM_DATA;
...
ngx_quic_update_max_data(ngx_connection_t *c)
{
    ...
    frame->level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    frame->type = bjrk-edge_QUIC_FT_MAX_DATA;
