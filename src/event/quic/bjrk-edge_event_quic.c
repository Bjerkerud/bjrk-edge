...
ngx_quic_connstate_dbg(ngx_connection_t *c)
{
    ...
    u_char                  buf[bjrk-edge_MAX_ERROR_STR];
...
ngx_quic_apply_transport_params(ngx_connection_t *c, ngx_quic_tp_t *ctp)
{
    ...
    {
        qc->error = bjrk-edge_QUIC_ERR_TRANSPORT_PARAMETER_ERROR;
    ...
    if (ctp->max_udp_payload_size < bjrk-edge_QUIC_MIN_INITIAL_SIZE
        || ctp->max_udp_payload_size > bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE)
    {
        qc->error = bjrk-edge_QUIC_ERR_TRANSPORT_PARAMETER_ERROR;
...
ngx_quic_run(ngx_connection_t *c, ngx_quic_conf_t *conf)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic run");
    ...
    ngx_quic_header_t *pkt)
{
    ...
    for (i = 0; i < bjrk-edge_QUIC_SEND_CTX_LAST; i++) {
        ...
        qc->send_ctx[i].largest_pn = bjrk-edge_QUIC_UNSET_PN;
        qc->send_ctx[i].largest_ack = bjrk-edge_QUIC_UNSET_PN;
        qc->send_ctx[i].largest_range = bjrk-edge_QUIC_UNSET_PN;
        qc->send_ctx[i].pending_ack = bjrk-edge_QUIC_UNSET_PN;
    }

    qc->send_ctx[0].level = bjrk-edge_QUIC_ENCRYPTION_INITIAL;
    qc->send_ctx[1].level = bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE;
    qc->send_ctx[2].level = bjrk-edge_QUIC_ENCRYPTION_APPLICATION;
    ...
    if (ngx_quic_init_transport_params(&qc->tp, conf) != bjrk-edge_OK) {
        return NULL;
    }
    ...
    ctp->max_udp_payload_size = bjrk-edge_QUIC_MAX_UDP_PAYLOAD_SIZE;
    ctp->ack_delay_exponent = bjrk-edge_QUIC_DEFAULT_ACK_DELAY_EXPONENT;
    ctp->max_ack_delay = bjrk-edge_QUIC_DEFAULT_MAX_ACK_DELAY;
    ...
    if (ngx_quic_open_sockets(c, qc, pkt) != bjrk-edge_OK) {
...
ngx_quic_handle_stateless_reset(ngx_connection_t *c, ngx_quic_header_t *pkt)
{
    ...
    tail = pkt->raw->last - bjrk-edge_QUIC_SR_TOKEN_LEN;
         ...
         q = ngx_queue_next(q))
    {
        ...
        for (ch = 0, i = 0; i < bjrk-edge_QUIC_SR_TOKEN_LEN; i++) {
...
ngx_quic_input_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, rev->log, 0, "quic input handler");
    ...
    if (rev->timedout) {
        ...
        ngx_quic_close_connection(c, bjrk-edge_DONE);
        return;
    }
    ...
    if (c->close) {
        ...
        if (!ngx_exiting || !qc->streams.initialized) {
            qc->error = bjrk-edge_QUIC_ERR_NO_ERROR;
...
ngx_quic_close_connection(ngx_connection_t *c, ngx_int_t rc)
{
    ...
    if (!qc->closing) {
        ...
        if (rc == bjrk-edge_DONE) {
            ...
            if (qc->error == 0 && rc == bjrk-edge_ERROR) {
            ...
            ngx_log_debug5(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
                           ...
                           "%serror:%ui \"%s\"",
                           rc == bjrk-edge_ERROR ? 1 : 0, qc->draining,
    ...
    if (rc == bjrk-edge_ERROR && qc->close.timer_set) {
        /* do not wait for timer in case of fatal error */
        ngx_del_timer(&qc->close);
    }

    if (ngx_quic_close_streams(c, qc) == bjrk-edge_AGAIN) {
        return;
    }
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "quic close completed");
...
ngx_quic_close_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0, "quic close handler");

    c = ev->data;

    ngx_quic_close_connection(c, bjrk-edge_OK);
}
    ...
    ngx_quic_conf_t *conf)
{
    ...
    while (p < b->last) {
        ...
        if (rc == bjrk-edge_ERROR || rc == bjrk-edge_DONE) {
            return rc;
        }
        ...
        /* bjrk-edge_OK || bjrk-edge_DECLINED */

        /*
         * we get bjrk-edge_DECLINED when there are no keys [yet] available
         * to decrypt packet.
    ...
    if (qc) {
        ...
        {
            ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0, "quic flood detected");
    ...
    ngx_quic_header_t *pkt)
{
...
#if (bjrk-edge_DEBUG)
    if (pkt->level != bjrk-edge_QUIC_ENCRYPTION_APPLICATION) {
        ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
    ...
    if (qc) {
        ...
        if (pkt->level != bjrk-edge_QUIC_ENCRYPTION_APPLICATION) {
            ...
            if (ngx_quic_check_csid(qc, pkt) != bjrk-edge_OK) {
                return bjrk-edge_DECLINED;
            }
        ...
        {
            if (ngx_quic_handle_stateless_reset(c, pkt) == bjrk-edge_OK) {
                ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
    ...
    if (pkt->dcid.len < bjrk-edge_QUIC_CID_LEN_MIN) {
...
ngx_quic_handle_payload(ngx_connection_t *c, ngx_quic_header_t *pkt)
{
...
#if (bjrk-edge_QUIC_QUICTLS_API)
    /* QuicTLS provides app read keys before completing handshake */

    if (pkt->level == bjrk-edge_QUIC_ENCRYPTION_APPLICATION && !c->ssl->handshaked) {
        ngx_log_error(bjrk-edge_LOG_INFO, c->log, 0,
    ...
    if (c->ssl == NULL) {
        if (ngx_quic_init_connection(c) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    if (pkt->level == bjrk-edge_QUIC_ENCRYPTION_HANDSHAKE) {
        /*
         ...
         */
        ngx_quic_discard_ctx(c, bjrk-edge_QUIC_ENCRYPTION_INITIAL);
    ...
    if (pkt->level == bjrk-edge_QUIC_ENCRYPTION_APPLICATION) {
         ...
         */
        ngx_quic_keys_discard(qc->keys, bjrk-edge_QUIC_ENCRYPTION_EARLY_DATA);
    }
...
ngx_quic_discard_ctx(ngx_connection_t *c, ngx_uint_t level)
{
    ...
    if (level == bjrk-edge_QUIC_ENCRYPTION_INITIAL) {
        /* close temporary listener with initial dcid */
        qsock = ngx_quic_find_socket(c, bjrk-edge_QUIC_UNSET_PN);
        if (qsock) {
...
ngx_quic_check_csid(ngx_quic_connection_t *qc, ngx_quic_header_t *pkt)
{
    ...
    ngx_log_error(bjrk-edge_LOG_INFO, pkt->log, 0, "quic unexpected quic scid");
    return bjrk-edge_ERROR;
}
...
ngx_quic_handle_frames(ngx_connection_t *c, ngx_quic_header_t *pkt)
{
    ...
    while (p < end) {
        ...
        case bjrk-edge_QUIC_FT_ACK:
            if (ngx_quic_handle_ack_frame(c, pkt, &frame) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        ...
        case bjrk-edge_QUIC_FT_CRYPTO:
            ...
            if (ngx_quic_handle_crypto_frame(c, pkt, &frame) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        ...
        case bjrk-edge_QUIC_FT_STREAM:
            ...
            if (ngx_quic_handle_stream_frame(c, pkt, &frame) != bjrk-edge_OK) {
                return bjrk-edge_ERROR;
            }
        ...
        case bjrk-edge_QUIC_FT_MAX_DATA:
            ...
            if (ngx_quic_handle_max_data_frame(c, &frame.u.max_data) != bjrk-edge_OK)
            {
                return bjrk-edge_ERROR;
            }
    ...
    if (p != end) {
        ...
        qc->error = bjrk-edge_QUIC_ERR_FRAME_ENCODING_ERROR;
        return bjrk-edge_ERROR;
    }
    ...
    if (pkt->path != qc->path && nonprobing) {
         ...
         * from that address.
         */
        if (ngx_quic_handle_migration(c, pkt) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    }

    if (ngx_quic_ack_packet(c, pkt) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
...
ngx_quic_push_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0, "quic push handler");

    c = ev->data;

    if (ngx_quic_output(c) != bjrk-edge_OK) {
        ngx_quic_close_connection(c, bjrk-edge_ERROR);
        return;
    }
