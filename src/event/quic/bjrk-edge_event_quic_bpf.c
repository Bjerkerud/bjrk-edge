...
#define bjrk-edge_QUIC_BPF_VARNAME  "NGINX_BPF_MAPS"
#define bjrk-edge_QUIC_BPF_VARSEP    ';'
#define bjrk-edge_QUIC_BPF_ADDRSEP   '#'
...
ngx_quic_bpf_create_conf(ngx_cycle_t *cycle)
{
    ...
    bcf->enabled = bjrk-edge_CONF_UNSET;
    bcf->map_size = bjrk-edge_CONF_UNSET_UINT;
...
ngx_quic_bpf_module_init(ngx_cycle_t *cycle)
{
    ...
    if (ngx_inherited && ngx_is_init_cycle(cycle->old_cycle)) {
        if (ngx_quic_bpf_import_maps(cycle) != bjrk-edge_OK) {
            goto failed;
        }
    }
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        if (ls[i].quic && ls[i].reuseport) {
            if (ngx_quic_bpf_group_add_socket(cycle, &ls[i]) != bjrk-edge_OK) {
                goto failed;
            }
        }
    }

    if (ngx_quic_bpf_export_maps(cycle) != bjrk-edge_OK) {
        goto failed;
    }
...
ngx_quic_bpf_create_group(ngx_cycle_t *cycle, ngx_listening_t *ls)
{
    ...
    if (flags == -1) {
        ngx_log_error(bjrk-edge_LOG_EMERG, cycle->log, errno,
...
ngx_quic_bpf_group_add_socket(ngx_cycle_t *cycle,  ngx_listening_t *ls)
{
    ...
    if (cookie == (uint64_t) bjrk-edge_ERROR) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_log_debug4(bjrk-edge_LOG_DEBUG_EVENT, cycle->log, 0,
...
ngx_quic_bpf_socket_key(ngx_fd_t fd, ngx_log_t *log)
{
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_COOKIE, &cookie, &optlen) == -1) {
        ...
        return (ngx_uint_t) bjrk-edge_ERROR;
    }
...
ngx_quic_bpf_export_maps(ngx_cycle_t *cycle)
{
    ...
    len = sizeof(bjrk-edge_QUIC_BPF_VARNAME) + 1;
    ...
    while (q != ngx_queue_sentinel(&bcf->groups)) {
        ...
        len += bjrk-edge_INT32_LEN + 1 + bjrk-edge_SOCKADDR_STRLEN + 1;
    }
    ...
    p = ngx_cpymem(buf, bjrk-edge_QUIC_BPF_VARNAME "=",
                   sizeof(bjrk-edge_QUIC_BPF_VARNAME));
    ...
    *p = '\0';
    ...
    var->data = buf;
    var->len = sizeof(bjrk-edge_QUIC_BPF_VARNAME) - 1;
...
ngx_quic_bpf_import_maps(ngx_cycle_t *cycle)
{
    ...
    inherited = (u_char *) getenv(bjrk-edge_QUIC_BPF_VARNAME);
    ...
    for (p = inherited, v = p; *p; p++) {
        ...
        case bjrk-edge_QUIC_BPF_ADDRSEP:
        ...
        case bjrk-edge_QUIC_BPF_VARSEP:
