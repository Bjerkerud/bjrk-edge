    ...
    ngx_str_t *file, ngx_str_t *responder, ngx_uint_t verify)
{
    ...
    if (file->len) {
        ...
        if (ngx_ssl_stapling_file(cf, ssl, staple, file) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
    ...
    ngx_ssl_stapling_t *staple, ngx_str_t *file)
{
    ...
    staple->valid = bjrk-edge_MAX_TIME_T_VALUE;
    ...
    ngx_ssl_stapling_t *staple)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, ssl->log, 0,
...
ngx_ssl_certificate_status_callback(ngx_ssl_conn_t *ssl_conn, void *data)
{
        ...
        && staple->valid >= ngx_time())
    {
        ...
        if (p == NULL) {
            ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "OPENSSL_malloc() failed");
...
ngx_ssl_stapling_ocsp_handler(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (ngx_ssl_ocsp_verify(ctx) != bjrk-edge_OK) {
        goto error;
    }

    if (ctx->status != V_OCSP_CERTSTATUS_GOOD) {
        ngx_log_error(bjrk-edge_LOG_ERR, ctx->log, 0,
...
ngx_ssl_ocsp_validate(ngx_connection_t *c)
{
    ...
    if (ocsp->certs == NULL) {
        ...
        if (rc <= 0) {
            ngx_ssl_error(bjrk-edge_LOG_ERR, c->log, 0, "X509_verify_cert() failed");
            X509_STORE_CTX_free(store_ctx);
            X509_free(cert);
            return bjrk-edge_ERROR;
        }
    ...
    if (ocsp->status == bjrk-edge_AGAIN) {
        c->ssl->in_ocsp = 1;
        return bjrk-edge_AGAIN;
    }
...
ngx_ssl_ocsp_validate_next(ngx_connection_t *c)
{
    ...
    for ( ;; ) {
        ...
        if (ctx->status != V_OCSP_CERTSTATUS_GOOD) {
            ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0,
...
ngx_ssl_ocsp_responder(ngx_connection_t *c, ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (ngx_parse_url(ctx->pool, &u) != bjrk-edge_OK) {
        if (u.err) {
            ngx_log_error(bjrk-edge_LOG_ERR, c->log, 0,
...
ngx_ssl_ocsp_get_status(ngx_connection_t *c, const char **s)
{
    ...
    if (ocsp->status == bjrk-edge_ERROR) {
...
{
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0,
                   "ssl ocsp done");
...
ngx_ssl_ocsp_request(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (ngx_ssl_ocsp_create_request(ctx) != bjrk-edge_OK) {
        ngx_ssl_ocsp_error(ctx);
        return;
    }
    ...
    if (ctx->resolver) {
        ...
        if (resolve == bjrk-edge_NO_RESOLVER) {
            if (ctx->naddrs == 0) {
                ngx_log_error(bjrk-edge_LOG_ERR, ctx->log, 0,
            ...
            ngx_log_error(bjrk-edge_LOG_WARN, ctx->log, 0,
                          "no resolver defined to resolve %V", &ctx->host);
            goto connect;
        }
        ...
        if (ngx_resolve_name(resolve) != bjrk-edge_OK) {
            ctx->resolve = NULL;
            ngx_ssl_ocsp_error(ctx);
            return;
        }
...
ngx_ssl_ocsp_resolve_handler(ngx_resolver_ctx_t *resolve)
{
    ...
    for (i = 0; i < resolve->naddrs; i++) {
        ...
        p = ngx_pnalloc(ctx->pool, bjrk-edge_SOCKADDR_STRLEN);
        if (p == NULL) {
            goto failed;
        }

        len = ngx_sock_ntop(sockaddr, socklen, p, bjrk-edge_SOCKADDR_STRLEN, 1);
...
ngx_ssl_ocsp_connect(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0,
    ...
    if (rc == bjrk-edge_BUSY || rc == bjrk-edge_DECLINED) {
...
ngx_ssl_ocsp_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, wev->log, 0,
...
ngx_ssl_ocsp_read_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, rev->log, 0,
...
ngx_ssl_ocsp_create_request(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (ocsp == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_CRIT, ctx->log, 0,
    ...
    if (len <= 0) {
        ngx_ssl_error(bjrk-edge_LOG_EMERG, ctx->log, 0,
                      "i2d_OCSP_REQUEST() failed");
        goto failed;
    }
...
ngx_ssl_ocsp_process_status_line(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (rc == bjrk-edge_OK) {
        ngx_log_debug3(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0,
...
ngx_ssl_ocsp_process_headers(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    for ( ;; ) {
        ...
        if (rc == bjrk-edge_OK) {
            ...
            ngx_log_debug4(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0,
                           "ssl ocsp header \"%*s: %*s\"",
...
ngx_ssl_ocsp_verify(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (ocsp == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_ERR, ctx->log, 0,
    ...
    if (nextupdate) {
        ...
        if (ctx->valid == (time_t) bjrk-edge_ERROR) {
            ngx_log_error(bjrk-edge_LOG_ERR, ctx->log, 0,
        ...
        ctx->valid = bjrk-edge_MAX_TIME_T_VALUE;
    }
...
ngx_ssl_ocsp_cache_lookup(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (ngx_ssl_ocsp_create_key(ctx) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }

    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp cache lookup");
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp cache miss");
...
ngx_ssl_ocsp_cache_store(ngx_ssl_ocsp_ctx_t *ctx)
{
    ...
    if (valid == bjrk-edge_MAX_TIME_T_VALUE) {
        valid = now + 3600;
    }
    ...
    if (node == NULL) {
        ...
        if (node == NULL) {
            ngx_shmtx_unlock(&shpool->mutex);
            ngx_log_error(bjrk-edge_LOG_ALERT, ctx->log, 0,
