...
ngx_select_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if ((event == bjrk-edge_READ_EVENT && max_read >= FD_SETSIZE)
        || (event == bjrk-edge_WRITE_EVENT && max_write >= FD_SETSIZE))
    {
    ...
    ngx_uint_t flags)
{
    ...
    if (err) {
        ngx_log_error(bjrk-edge_LOG_ALERT, cycle->log, err, "select() failed");
