...
ngx_poll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (ev->index != bjrk-edge_INVALID_INDEX) {
    ...
    if (event == bjrk-edge_READ_EVENT) {
        e = c->write;
#if (bjrk-edge_READ_EVENT != POLLIN)
        event = POLLIN;
#endif

    } else {
        e = c->read;
#if (bjrk-edge_WRITE_EVENT != POLLOUT)
        event = POLLOUT;
#endif
    }
    ...
    if (e == NULL || e->index == bjrk-edge_INVALID_INDEX) {
...
ngx_poll_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    ev->index = bjrk-edge_INVALID_INDEX;
...
ngx_poll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
...
#if (bjrk-edge_DEBUG0)
    if (cycle->log->log_level & bjrk-edge_LOG_DEBUG_ALL) {
        for (i = 0; i < nevents; i++) {
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, cycle->log, 0, "poll timer: %M", timer);
    ...
    if (err) {
        ngx_log_error(bjrk-edge_LOG_ALERT, cycle->log, err, "WSAPoll() failed");
        return bjrk-edge_ERROR;
    }
    ...
    for (i = 0; i < nevents && ready; i++) {
        ...
        if (c->fd == (ngx_socket_t) -1) {
            ngx_log_error(bjrk-edge_LOG_ALERT, cycle->log, 0, "unexpected event");
    ...
    if (ready != 0) {
        ngx_log_error(bjrk-edge_LOG_ALERT, cycle->log, 0, "poll ready != events");
    }
