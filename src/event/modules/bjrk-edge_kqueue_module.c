...
#define bjrk-edge_KQUEUE_UDATA_T
#else
#define bjrk-edge_KQUEUE_UDATA_T  (void *)
#endif
...
ngx_kqueue_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
...
#ifdef EVFILT_USER
        if (ngx_kqueue_notify_init(cycle->log) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
#endif
    }
    ...
    ngx_event_flags = bjrk-edge_USE_ONESHOT_EVENT
                      |bjrk-edge_USE_KQUEUE_EVENT
                      |bjrk-edge_USE_VNODE_EVENT;
    ...
    if (timer) {
        ...
        kev.fflags = 0;
        kev.data = timer;
        kev.udata = bjrk-edge_KQUEUE_UDATA_T (uintptr_t) 0;
        ...
        ngx_event_flags |= bjrk-edge_USE_TIMER_EVENT;
    }
    ...
    ngx_event_flags |= bjrk-edge_USE_LOWAT_EVENT;
#endif
...
ngx_kqueue_notify_init(ngx_log_t *log)
{
    ...
    notify_kev.fflags = 0;
    notify_kev.udata = bjrk-edge_KQUEUE_UDATA_T (uintptr_t) 0;
    ...
    notify_kev.udata = bjrk-edge_KQUEUE_UDATA_T ((uintptr_t) &notify_event);
...
ngx_kqueue_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
            ...
            == (uintptr_t) ev)
    {
        if (change_list[ev->index].flags == EV_DISABLE) {
            ...
            ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0,
...
ngx_kqueue_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (flags & bjrk-edge_DISABLE_EVENT) {
        ev->disabled = 1;
...
ngx_kqueue_set_event(ngx_event_t *ev, ngx_int_t filter, ngx_uint_t flags)
{
    ...
    if (nchanges >= max_changes) {
        ngx_log_error(bjrk-edge_LOG_WARN, ev->log, 0,
        ...
        if (kevent(ngx_kqueue, change_list, (int) nchanges, NULL, 0, &ts)
            == -1)
        {
            ngx_log_error(bjrk-edge_LOG_ALERT, ev->log, ngx_errno, "kevent() failed");
            return bjrk-edge_ERROR;
        }
    ...
    kev->udata = bjrk-edge_KQUEUE_UDATA_T ((uintptr_t) ev | ev->instance);
...
#if (bjrk-edge_HAVE_LOWAT_EVENT)
        if (flags & bjrk-edge_LOWAT_EVENT) {
    ...
    if (flags & bjrk-edge_FLUSH_EVENT) {
        ts.tv_sec = 0;
        ts.tv_nsec = 0;

        ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0, "kevent flush");
    ...
    ngx_uint_t flags)
{
...
#if (bjrk-edge_DARWIN_KEVENT_BUG)
        ts.tv_nsec <<= 32;
#endif
    ...
    for (i = 0; i < events; i++) {
        ...
        if (event_list[i].flags & EV_ERROR) {
            ngx_log_error(bjrk-edge_LOG_ALERT, cycle->log, event_list[i].data,
        ...
        case EVFILT_WRITE:
            ...
            if (ev->log && (ev->log->log_level & bjrk-edge_LOG_DEBUG_CONNECTION)) {
...
ngx_kqueue_create_conf(ngx_cycle_t *cycle)
{
    ...
    kcf->changes = bjrk-edge_CONF_UNSET;
    kcf->events = bjrk-edge_CONF_UNSET;
