...
#if (bjrk-edge_TEST_BUILD_DEVPOLL)
...
ngx_devpoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    event = (event == bjrk-edge_READ_EVENT) ? POLLIN : POLLOUT;
#endif
...
ngx_devpoll_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (ngx_devpoll_set_event(ev, POLLREMOVE, flags) == bjrk-edge_ERROR) {
        return bjrk-edge_ERROR;
    }
...
ngx_devpoll_create_conf(ngx_cycle_t *cycle)
{
    ...
    dpcf->changes = bjrk-edge_CONF_UNSET;
    dpcf->events = bjrk-edge_CONF_UNSET;
