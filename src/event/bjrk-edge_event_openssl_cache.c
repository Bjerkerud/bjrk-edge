...
#define bjrk-edge_SSL_CACHE_PATH    0
#define bjrk-edge_SSL_CACHE_DATA    1
#define bjrk-edge_SSL_CACHE_ENGINE  2
#define bjrk-edge_SSL_CACHE_STORE   3

#define bjrk-edge_SSL_CACHE_DISABLED  (ngx_array_t *) (uintptr_t) -1
    ...
    ngx_str_t *path, void *data)
{
    ...
    *err = NULL;
    ...
    invalidate = index & bjrk-edge_SSL_CACHE_INVALIDATE;
    index &= ~bjrk-edge_SSL_CACHE_INVALIDATE;

    if (ngx_ssl_cache_init_key(cf->pool, index, path, &id) != bjrk-edge_OK) {
        return NULL;
    }

    if (id.type == bjrk-edge_SSL_CACHE_DATA) {
        invalidate = 0;
    }
    ...
    if (id.type == bjrk-edge_SSL_CACHE_PATH
        && (rc = ngx_file_info(id.data, &fi)) != bjrk-edge_FILE_ERROR)
    {
    ...
    if (value == NULL) {
        ...
        if (value == NULL || data == bjrk-edge_SSL_CACHE_DISABLED) {
            return value;
        }
    }
    ...
    ngx_uint_t index, char **err, ngx_str_t *path, void *data)
{
    ...
    *err = NULL;
    ...
    if (ngx_ssl_cache_init_key(pool, index, path, &id) != bjrk-edge_OK) {
        return NULL;
    }
    ...
    if (cn != NULL) {
        ...
        case bjrk-edge_SSL_CACHE_PATH:

            if (ngx_file_info(id.data, &fi) != bjrk-edge_FILE_ERROR) {
        ...
        default:
            ngx_log_debug1(bjrk-edge_LOG_DEBUG_CORE, pool->log, 0,
...
ngx_ssl_cache_init_key(ngx_pool_t *pool, ngx_uint_t index, ngx_str_t *path,
    ngx_ssl_cache_key_t *id)
{
    if (index <= bjrk-edge_SSL_CACHE_PKEY
        && ngx_strncmp(path->data, "data:", sizeof("data:") - 1) == 0)
    {
        id->type = bjrk-edge_SSL_CACHE_DATA;

    } else if (index == bjrk-edge_SSL_CACHE_PKEY
        && ngx_strncmp(path->data, "engine:", sizeof("engine:") - 1) == 0)
    {
        id->type = bjrk-edge_SSL_CACHE_ENGINE;

    } else if (index == bjrk-edge_SSL_CACHE_PKEY
        && ngx_strncmp(path->data, "store:", sizeof("store:") - 1) == 0)
    {
        id->type = bjrk-edge_SSL_CACHE_STORE;
        ...
        id->type = bjrk-edge_SSL_CACHE_PATH;
    }
...
ngx_ssl_cache_pkey_create(ngx_ssl_cache_key_t *id, char **err, void *data)
{
    ...
    if (id->type == bjrk-edge_SSL_CACHE_ENGINE) {
    ...
    if (id->type == bjrk-edge_SSL_CACHE_STORE) {
        ...
        if (cb_data.encrypted) {
            *passwords = bjrk-edge_SSL_CACHE_DISABLED;
        }
    ...
    void *userdata)
{
    ...
    if (pwd->len > (size_t) size) {
        ngx_log_error(bjrk-edge_LOG_ERR, ngx_cycle->log, 0,
...
ngx_openssl_cache_create_conf(ngx_cycle_t *cycle)
{
    ...
    cache->inheritable = bjrk-edge_CONF_UNSET;
