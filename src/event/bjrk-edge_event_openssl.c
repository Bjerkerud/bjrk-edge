...
#if (bjrk-edge_ZLIB && defined TLSEXT_cert_compression_zlib)
#include <zlib.h>
#endif


#define bjrk-edge_SSL_PASSWORD_BUFFER_SIZE  4096
...
static ngx_command_t  ngx_openssl_commands[] = {
    ...
    { ngx_string("ssl_engine"),
      bjrk-edge_MAIN_CONF|bjrk-edge_DIRECT_CONF|bjrk-edge_CONF_TAKE1,
      ngx_openssl_engine,
      0,
      0,
      NULL },
...
ngx_module_t  ngx_openssl_module = {
    ...
    bjrk-edge_CORE_MODULE,                       /* module type */
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
...
u_char  ngx_ssl_session_buffer[bjrk-edge_SSL_MAX_SESSION_SIZE];
...
ngx_ssl_init(ngx_log_t *log)
{
...
#if (bjrk-edge_OPENSSL_NO_CONFIG)
    ...
    if (init == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "OPENSSL_INIT_new() failed");
        return bjrk-edge_ERROR;
    }

#ifndef OPENSSL_NO_STDIO
    if (OPENSSL_INIT_set_config_appname(init, "nginx") == 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0,
    ...
    if (OPENSSL_init_ssl(opts, init) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "OPENSSL_init_ssl() failed");
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_ssl_connection_index == -1) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "SSL_get_ex_new_index() failed");
        return bjrk-edge_ERROR;
    }
    ...
    if (ngx_ssl_certificate_name_index == -1) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "X509_get_ex_new_index() failed");
        return bjrk-edge_ERROR;
    }
...
ngx_ssl_create(ngx_ssl_t *ssl, ngx_uint_t protocols, void *data)
{
    ...
    if (ssl->ctx == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0, "SSL_CTX_new() failed");
        return bjrk-edge_ERROR;
    }

    if (SSL_CTX_set_ex_data(ssl->ctx, ngx_ssl_server_conf_index, data) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0,
    ...
    ssl->buffer_size = bjrk-edge_SSL_BUFSIZE;
    ...
    if (!(protocols & bjrk-edge_SSL_SSLv2)) {
        SSL_CTX_set_options(ssl->ctx, SSL_OP_NO_SSLv2);
    }
    if (!(protocols & bjrk-edge_SSL_SSLv3)) {
        SSL_CTX_set_options(ssl->ctx, SSL_OP_NO_SSLv3);
    }
    if (!(protocols & bjrk-edge_SSL_TLSv1)) {
        SSL_CTX_set_options(ssl->ctx, SSL_OP_NO_TLSv1);
    }
#ifdef SSL_OP_NO_TLSv1_1
    SSL_CTX_clear_options(ssl->ctx, SSL_OP_NO_TLSv1_1);
    if (!(protocols & bjrk-edge_SSL_TLSv1_1)) {
        SSL_CTX_set_options(ssl->ctx, SSL_OP_NO_TLSv1_1);
    }
#endif
#ifdef SSL_OP_NO_TLSv1_2
    SSL_CTX_clear_options(ssl->ctx, SSL_OP_NO_TLSv1_2);
    if (!(protocols & bjrk-edge_SSL_TLSv1_2)) {
        SSL_CTX_set_options(ssl->ctx, SSL_OP_NO_TLSv1_2);
    }
#endif
#ifdef SSL_OP_NO_TLSv1_3
    SSL_CTX_clear_options(ssl->ctx, SSL_OP_NO_TLSv1_3);
    if (!(protocols & bjrk-edge_SSL_TLSv1_3)) {
...
retry:
    ...
    chain = ngx_ssl_cache_fetch(cf, bjrk-edge_SSL_CACHE_CERT | mask,
                                &err, cert, NULL);
    if (chain == NULL) {
        if (err != NULL) {
            ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0,
    ...
    if (X509_set_ex_data(x509, ngx_ssl_certificate_name_index, cert->data)
        == 0)
    {
        ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0, "X509_set_ex_data() failed");
        X509_free(x509);
        sk_X509_pop_free(chain, X509_free);
        return bjrk-edge_ERROR;
    }
    ...
    *elm = x509;
    ...
    pkey = ngx_ssl_cache_fetch(cf, bjrk-edge_SSL_CACHE_PKEY | mask,
                               &err, key, passwords);
    if (pkey == NULL) {
        if (err != NULL) {
            ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0,
    ...
    if (SSL_CTX_use_PrivateKey(ssl->ctx, pkey) == 0) {
        ...
        {
            ERR_clear_error();
            mask = bjrk-edge_SSL_CACHE_INVALIDATE;
            goto retry;
        }
    ...
    ngx_uint_t enable)
{
    ...
    if (SSL_CTX_compress_certs(ssl->ctx, 0) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_WARN, ssl->log, 0,
...
#elif (bjrk-edge_ZLIB && defined TLSEXT_cert_compression_zlib)
    ...
    ngx_log_error(bjrk-edge_LOG_WARN, ssl->log, 0,
    ...
    const uint8_t *in, size_t in_len)
{
    ...
    if (rc != Z_OK) {
        ngx_log_error(bjrk-edge_LOG_ALERT, c->log, 0, "deflateInit() failed: %d", rc);
        goto error;
    }
    ...
    if (rc != Z_OK) {
        ngx_log_error(bjrk-edge_LOG_ALERT, c->log, 0, "deflateEnd() failed: %d", rc);
        goto error;
    }
    ...
    if (X509_set_ex_data(cert, ngx_ssl_certificate_comp_index, p) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "X509_set_ex_data() failed");
        ngx_free(p);
    }
...
ngx_ssl_cert_compression_alloc(void *opaque, u_int items, u_int size)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, pool->log, 0,
...
ngx_ssl_cert_compression_free(void *opaque, void *address)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, pool->log, 0,
    ...
    ngx_int_t depth)
{
    ...
    chain = ngx_ssl_cache_fetch(cf, bjrk-edge_SSL_CACHE_CA, &err, cert, NULL);
    if (chain == NULL) {
        if (err != NULL) {
            ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0,
                          "cannot load certificate \"%s\": %s",
...
ngx_ssl_crl(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *crl)
{
    ...
    chain = ngx_ssl_cache_fetch(cf, bjrk-edge_SSL_CACHE_CRL, &err, crl, NULL);
    if (chain == NULL) {
        if (err != NULL) {
            ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0,
...
ngx_ssl_verify_callback(int ok, X509_STORE_CTX *x509_store)
{
    ...
    if (!(c->log->log_level & bjrk-edge_LOG_DEBUG_EVENT)) {
        return 1;
    }
    ...
    if (sname) {
        ...
        if (subject == NULL) {
            ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0,
    ...
    ngx_log_debug5(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
...
ngx_ssl_info_callback(const ngx_ssl_conn_t *ssl_conn, int where, int ret)
{
        ...
        && SSL_is_server((ngx_ssl_conn_t *) ssl_conn))
    {
        ...
        if (c->ssl->handshaked) {
            ...
            ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL renegotiation");
        }
    }
    ...
    if ((where & SSL_CB_ACCEPT_LOOP) == SSL_CB_ACCEPT_LOOP) {
        ...
        if (!c->ssl->handshake_buffer_set) {
            ...
            if (rbio != wbio) {
                (void) BIO_set_write_buffer_size(wbio, bjrk-edge_SSL_BUFSIZE);
...
ngx_ssl_read_password_file(ngx_conf_t *cf, ngx_str_t *file)
{
    ...
    u_char               buf[bjrk-edge_SSL_PASSWORD_BUFFER_SIZE];

    if (ngx_conf_full_name(cf->cycle, file, 1) != bjrk-edge_OK) {
        return NULL;
    }
    ...
    fd = ngx_open_file(file->data, bjrk-edge_FILE_RDONLY, bjrk-edge_FILE_OPEN, 0);

    if (fd == bjrk-edge_INVALID_FILE) {
        ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, ngx_errno,
    ...
    do {
        n = ngx_read_fd(fd, last, bjrk-edge_SSL_PASSWORD_BUFFER_SIZE - len);
        ...
        if (len == bjrk-edge_SSL_PASSWORD_BUFFER_SIZE) {
...
cleanup:
    ...
    if (ngx_close_file(fd) == bjrk-edge_FILE_ERROR) {
        ngx_conf_log_error(bjrk-edge_LOG_ALERT, cf, ngx_errno,
                           ngx_close_file_n " \"%s\" failed", file->data);
    }

    ngx_explicit_memzero(buf, bjrk-edge_SSL_PASSWORD_BUFFER_SIZE);
...
ngx_ssl_ech_files(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_array_t *filenames)
{
    ...
    if (es == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_EMERG, ssl->log, 0, "OSSL_ECHSTORE_new() failed");
        return bjrk-edge_ERROR;
    }
    ...
    for (i = 0; i < filenames->nelts; i++) {
        ...
        if (ngx_conf_full_name(cf->cycle, &filename[i], 1) != bjrk-edge_OK) {
            goto cleanup;
        }
...
ngx_ssl_early_data(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_uint_t enable)
{
    ...
    SSL_CTX_set_max_early_data(ssl->ctx, bjrk-edge_SSL_BUFSIZE);
...
ngx_ssl_conf_commands(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_array_t *commands)
{
    ...
    for (i = 0; i < commands->nelts; i++) {
        ...
        if (type == SSL_CONF_TYPE_FILE || type == SSL_CONF_TYPE_DIR) {
            if (ngx_conf_full_name(cf->cycle, &cmd[i].value, 1) != bjrk-edge_OK) {
...
#else
    ngx_log_error(bjrk-edge_LOG_EMERG, ssl->log, 0,
...
ngx_ssl_create_connection(ngx_ssl_t *ssl, ngx_connection_t *c, ngx_uint_t flags)
{
    ...
    sc->buffer = ((flags & bjrk-edge_SSL_BUFFER) != 0);
    ...
    if (sc->connection == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "SSL_new() failed");
        return bjrk-edge_ERROR;
    }

    if (SSL_set_fd(sc->connection, c->fd) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "SSL_set_fd() failed");
        return bjrk-edge_ERROR;
    }

    if (flags & bjrk-edge_SSL_CLIENT) {
    ...
    if (SSL_set_ex_data(sc->connection, ngx_ssl_connection_index, c) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "SSL_set_ex_data() failed");
        return bjrk-edge_ERROR;
    }
...
ngx_ssl_set_session(ngx_connection_t *c, ngx_ssl_session_t *session)
{
    if (session) {
        if (SSL_set_session(c->ssl->connection, session) == 0) {
            ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "SSL_set_session() failed");
            return bjrk-edge_ERROR;
        }
    }
...
ngx_ssl_handshake(ngx_connection_t *c)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_do_handshake: %d", n);
...
#if (defined BIO_get_ktls_send && !bjrk-edge_WIN32)
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_get_error: %d", sslerr);
...
ngx_ssl_handshake_handler(ngx_event_t *ev)
{
    ...
    if (ngx_ssl_handshake(c) == bjrk-edge_AGAIN) {
        return;
    }
...
ngx_ssl_recv(ngx_connection_t *c, u_char *buf, size_t size)
{
    ...
    if (c->ssl->last == bjrk-edge_ERROR) {
        c->read->ready = 0;
        c->read->error = 1;
        return bjrk-edge_ERROR;
    }

    if (c->ssl->last == bjrk-edge_DONE) {
        c->read->ready = 0;
        c->read->eof = 1;
        return 0;
    }
    ...
    for ( ;; ) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_read: %d", n);
...
ngx_ssl_handle_recv(ngx_connection_t *c, int n)
{
    ...
    if (c->ssl->renegotiation) {
        ...
        ngx_log_error(bjrk-edge_LOG_NOTICE, c->log, 0, "SSL renegotiation disabled");

        while (ERR_peek_error()) {
            ngx_ssl_error(bjrk-edge_LOG_DEBUG, c->log, 0,
...
ngx_ssl_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL write handler");
...
ngx_ssl_send_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    if (limit == 0 || limit > (off_t) (bjrk-edge_MAX_INT32_VALUE - ngx_pagesize)) {
        limit = bjrk-edge_MAX_INT32_VALUE - ngx_pagesize;
    }
    ...
    for ( ;; ) {
        ...
        if (size == 0) {
            ...
            buf->flush = 0;
            c->buffered &= ~bjrk-edge_SSL_BUFFERED;
...
ngx_ssl_write(ngx_connection_t *c, u_char *data, size_t size)
{
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL to write: %uz", size);

    n = SSL_write(c->ssl->connection, data, size);

    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_write: %d", n);
...
ngx_ssl_sendfile(ngx_connection_t *c, ngx_buf_t *file, size_t size)
{
...
#if (bjrk-edge_HAVE_SENDFILE_NODISKIO)
    ...
    ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_sendfile: %z", n);
...
ngx_ssl_read_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL read handler");
...
ngx_ssl_free_buffer(ngx_connection_t *c)
{
    if (c->ssl->buf && c->ssl->buf->start) {
        if (ngx_pfree(c->pool, c->ssl->buf->start) == bjrk-edge_OK) {
...
ngx_ssl_shutdown(ngx_connection_t *c)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0, "SSL_shutdown: %d", n);
...
ngx_ssl_shutdown_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, ev->log, 0, "SSL shutdown handler");
    ...
    char *text)
{
    ...
    if (sslerr == SSL_ERROR_SYSCALL) {
        ...
        if (err == bjrk-edge_ECONNRESET
#if (bjrk-edge_WIN32)
            || err == bjrk-edge_ECONNABORTED
#endif
            || err == bjrk-edge_EPIPE
            || err == bjrk-edge_ENOTCONN
            || err == bjrk-edge_ETIMEDOUT
            || err == bjrk-edge_ECONNREFUSED
            || err == bjrk-edge_ENETDOWN
            || err == bjrk-edge_ENETUNREACH
            || err == bjrk-edge_EHOSTDOWN
            || err == bjrk-edge_EHOSTUNREACH)
        {
            switch (c->log_error) {

            case bjrk-edge_ERROR_IGNORE_ECONNRESET:
            case bjrk-edge_ERROR_INFO:
                level = bjrk-edge_LOG_INFO;
                break;

            case bjrk-edge_ERROR_ERR:
...
{
    while (ERR_peek_error()) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "ignoring stale global SSL error");
    }
...
ngx_ssl_error(ngx_uint_t level, ngx_log_t *log, ngx_err_t err, char *fmt, ...)
{
    ...
    last = errstr + bjrk-edge_MAX_CONF_ERRSTR;
    ...
    ngx_shm_zone_t *shm_zone, time_t timeout)
{
    ...
    if (ngx_ssl_session_id_context(ssl, sess_ctx, certificates) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }

    if (builtin_session_cache == bjrk-edge_SSL_NO_SCACHE) {
        SSL_CTX_set_session_cache_mode(ssl->ctx, SSL_SESS_CACHE_OFF);
        return bjrk-edge_OK;
    }

    if (builtin_session_cache == bjrk-edge_SSL_NONE_SCACHE) {
    ...
    if (shm_zone && builtin_session_cache == bjrk-edge_SSL_NO_BUILTIN_SCACHE) {
        cache_mode |= SSL_SESS_CACHE_NO_INTERNAL;
    }

    SSL_CTX_set_session_cache_mode(ssl->ctx, cache_mode);

    if (builtin_session_cache != bjrk-edge_SSL_NO_BUILTIN_SCACHE) {

        if (builtin_session_cache != bjrk-edge_SSL_DFLT_BUILTIN_SCACHE) {
...
ngx_ssl_new_session(ngx_ssl_conn_t *ssl_conn, ngx_ssl_session_t *sess)
{
    ...
    if (len > bjrk-edge_SSL_MAX_SESSION_SIZE) {
        return 0;
    }
...
failed:
    ...
    if (cache->fail_time != ngx_time()) {
        ...
        ngx_log_error(bjrk-edge_LOG_WARN, c->log, 0,
...
ngx_ssl_remove_session(SSL_CTX *ssl, ngx_ssl_session_t *sess)
{
    ...
    ngx_log_debug2(bjrk-edge_LOG_DEBUG_EVENT, ngx_cycle->log, 0,
    ...
    ngx_slab_pool_t *shpool, ngx_uint_t n)
{
    ...
    while (n < 3) {
        ...
        ngx_log_debug1(bjrk-edge_LOG_DEBUG_EVENT, ngx_cycle->log, 0,
...
ngx_ssl_session_ticket_keys(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_array_t *paths)
{
    ...
    if (SSL_CTX_set_tlsext_ticket_key_cb(ssl->ctx, ngx_ssl_ticket_key_callback)
        == 0)
    {
        ngx_log_error(bjrk-edge_LOG_WARN, cf->log, 0,
    ...
    for (i = 0; i < paths->nelts; i++) {
        ...
        if (ngx_conf_full_name(cf->cycle, &path[i], 1) != bjrk-edge_OK) {
            return bjrk-edge_ERROR;
        }
        ...
        file.fd = ngx_open_file(file.name.data, bjrk-edge_FILE_RDONLY,
                                bjrk-edge_FILE_OPEN, 0);

        if (file.fd == bjrk-edge_INVALID_FILE) {
            ngx_conf_log_error(bjrk-edge_LOG_EMERG, cf, ngx_errno,
                               ngx_open_file_n " \"%V\" failed", &file.name);
            return bjrk-edge_ERROR;
        }

        if (ngx_fd_info(file.fd, &fi) == bjrk-edge_FILE_ERROR) {
            ngx_conf_log_error(bjrk-edge_LOG_CRIT, cf, ngx_errno,
        ...
        if ((size_t) n != size) {
            ngx_conf_log_error(bjrk-edge_LOG_CRIT, cf, 0,
        ...
        if (ngx_close_file(file.fd) == bjrk-edge_FILE_ERROR) {
            ngx_log_error(bjrk-edge_LOG_ALERT, cf->log, ngx_errno,
    ...
    HMAC_CTX *hctx, int enc)
{
    ...
    if (ngx_ssl_rotate_ticket_keys(ssl_ctx, c->log) != bjrk-edge_OK) {
        return -1;
    }
    ...
    if (enc == 1) {
        ...
        if (RAND_bytes(iv, EVP_CIPHER_iv_length(cipher)) != 1) {
            ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "RAND_bytes() failed");
            return -1;
        }
        ...
        if (HMAC_Init_ex(hctx, key[0].hmac_key, size, digest, NULL) != 1) {
            ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "HMAC_Init_ex() failed");
            return -1;
        }
#else
...
ngx_ssl_rotate_ticket_keys(SSL_CTX *ssl_ctx, ngx_log_t *log)
{
    ...
    if (key[0].expire == 0) {
        ...
        if (RAND_bytes(buf, 80) != 1) {
            ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "RAND_bytes() failed");
            ngx_shmtx_unlock(&shpool->mutex);
            return bjrk-edge_ERROR;
        }
...
ngx_ssl_check_host(ngx_connection_t *c, ngx_str_t *name)
{
    ...
    if (altnames) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (ngx_ssl_check_name(name, str) == bjrk-edge_OK) {
                ngx_log_debug0(bjrk-edge_LOG_DEBUG_EVENT, c->log, 0,
...
ngx_ssl_get_raw_certificate(ngx_connection_t *c, ngx_pool_t *pool, ngx_str_t *s)
{
    ...
    if (bio == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "BIO_new() failed");
        X509_free(cert);
        return bjrk-edge_ERROR;
    }

    if (PEM_write_bio_X509(bio, cert) == 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "PEM_write_bio_X509() failed");
        goto failed;
    }
...
ngx_ssl_get_certificate(ngx_connection_t *c, ngx_pool_t *pool, ngx_str_t *s)
{
    ...
    if (ngx_ssl_get_raw_certificate(c, pool, &cert) != bjrk-edge_OK) {
        return bjrk-edge_ERROR;
    }
    ...
    ngx_str_t *s)
{
    ...
    n = ngx_escape_uri(NULL, cert.data, cert.len, bjrk-edge_ESCAPE_URI_COMPONENT);
    ...
    ngx_escape_uri(s->data, cert.data, cert.len, bjrk-edge_ESCAPE_URI_COMPONENT);
...
ngx_ssl_get_subject_dn(ngx_connection_t *c, ngx_pool_t *pool, ngx_str_t *s)
{
    ...
    if (X509_NAME_print_ex(bio, name, 0, XN_FLAG_RFC2253) < 0) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "X509_NAME_print_ex() failed");
        goto failed;
    }
    ...
    ngx_str_t *s)
{
    ...
    if (p == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "X509_NAME_oneline() failed");
        X509_free(cert);
        return bjrk-edge_ERROR;
    }
...
ngx_ssl_get_fingerprint(ngx_connection_t *c, ngx_pool_t *pool, ngx_str_t *s)
{
    ...
    if (!X509_digest(cert, EVP_sha1(), buf, &len)) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, c->log, 0, "X509_digest() failed");
        X509_free(cert);
        return bjrk-edge_ERROR;
    }
...
ngx_ssl_get_client_verify(ngx_connection_t *c, ngx_pool_t *pool, ngx_str_t *s)
{
    ...
    if (rc == X509_V_OK) {
        if (ngx_ssl_ocsp_get_status(c, &str) == bjrk-edge_OK) {
            ngx_str_set(s, "SUCCESS");
            return bjrk-edge_OK;
        }
...
ngx_ssl_get_client_v_remain(ngx_connection_t *c, ngx_pool_t *pool, ngx_str_t *s)
{
    ...
    s->data = ngx_pnalloc(pool, bjrk-edge_TIME_T_LEN);
    if (s->data == NULL) {
        X509_free(cert);
        return bjrk-edge_ERROR;
    }
    ...
    ASN1_TIME *asn1time, ngx_log_t *log)
{
    ...
    bio = BIO_new(BIO_s_mem());
    if (bio == NULL) {
        ngx_ssl_error(bjrk-edge_LOG_ALERT, log, 0, "BIO_new() failed");
        return bjrk-edge_ERROR;
    }
